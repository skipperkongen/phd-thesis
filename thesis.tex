...
\begin{enumerate}
\item There are at many ways to score the importance of objects;
\item There are at least three important categories of constraints: unary constraints, binary object-relative constraints and space-relative constraints. Each category contains many possible constraint formulations;
\item There are at least two important models for processing multi-scale selection of geographic data, namely the star and the ladder model~\cite{foerster2010challenges};
\item There are many different ways to define a set of scales for a multi-scale visualization.
\end{enumerate}
...
Consequently, a map visualization systems must be able to scalably process a specification for multi-scale selection over large datasets (millions of records). One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a high-level language such as SQL, which reuses optimization techniques that are built-in to SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Additionally, since data for data visualizations is typically stored and managed in RDBMS~\cite{wu2014case} it makes sense to process the data where it is stored. Despite the obvious opportunity, state-of-the-art map visualization systems do not significantly leverage RDBMS technology to process multi-scale selection of geographic information, other than to extract data for post-processing using simple bounding-box and attribute filtering queries~\cite{wms, }.

The key question is: \emph{how we can scalably process holistic multi-scale selection of geographic data inside an RDBMS?}
