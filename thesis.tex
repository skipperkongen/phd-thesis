...
\item \textbf{In-Database Map Generalization}: \texttt{https://github.com/skipperkongen/declarative-cartography}
\item \textbf{Prediction for Tile Materialization}: \textt{https://github.com/skipperkongen/tileheat}
\end{itemize}
...
The general topic of this dissertation is map visualization. An important point that arises during map creation, is the process of adapting geographical data to scale of a given maps, which is refered to as map generalization~\cite{weibel1999generalising}. This thesis focuses mainly on holistic multi-scale selection, which is a specific type of map generalization operator. Indeed, many other operators exist, for example displacement and amalgamation, that could have been the focus of this thesis, however they are not. This section will cover the most important map generalization operators, and states the case for why they were not considered for a map visualization system for the web, in place of holistic multi-scale selection. Section~\ref{state:of:the:art:map:generalization:operators} covers the discussion of map generalization operators.

There are many ways to think about the map generalization process. A high-level way to think about it is, that map generalization is a search problem: we are trying to find an ``optimal'' map, one that optimizes an objective function and satisfies a set of cartographic constraints~\cite{harrie1999constraint}. In this framework, we can think of the operators as functions that move the current solution from one map to another, and possible better, map. When multiple operators are considered simultaneously, the solution space may be enourmous. To simplify the discussion a bit, the discussion will be limited to methods that involve holistic multi-scale selection and multi-scale projection. Section~\ref{state:of:the:art:programming:multiscale:selection} covers the discussion of high-level generalization methods.


\section{Overview of Map Generalization Operators}
\label{state:of:the:art:map:generalization:operators}

Displacement, amalgamation. The others are supported by our contributions.



% EXPRESSING
\section{Formulations of Multi-scale Selection}
\label{state:of:the:art:programming:multiscale:selection}

State requirements for both: express multi-scale star and ladder, holistic selection (i.e. user-defined constraints and objectives), integrate relational data, encourage code sharing and reuse, intuitive for non-experts (constraints), maximize program clarity/minimize program length (objective).
\emph{maybe state these in Introduction: problem statement, and refer to the section}.

Briefly mention graphics stuff~\cite{jacques1967semiologie, nickerson1994visual, tufte1983visual, wilkinson2006grammar, wickham2009ggplot2}.
