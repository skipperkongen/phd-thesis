...
Thematic mapping applications on the Web are important since they allow map designers -- such as journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} -- to communicate relevant and focused spatial information to their target groups. Thematic maps increase the level of transparency and enlightenment in our society -- they can even save lives. % What are they exactly?
Thematic maps are typically composed of two distinct layers of information. The \emph{thematic map layer} consists of foreground data, such as news-worthy geographic events, which is typically the primary content of the map. The \emph{contextual map layer} consists of background data, such as a topographic map, which provides that spatial context in which the former layer may be understood. Such a composition of thematic and contextual layers is sometimes informally called a ``geographic mash-up'' (see Figure~\ref{fig:introduction:background:mashups}}). % Why is it hard?? 

Contextual map layers have become a commodity that is readily availably from private companies~\cite{googlemaps, bingmaps} and national mapping agencies~\cite{gst2014digitalmapsupply}. In contrast, thematic map layers presents a challenge to map designers. The source mateterial for the thematic layer typically consists of raw and emerging geographic information, such as user-contributed data~\cite{openstreetmap,zooniverse2014oldweather}, which must first be molded by map designers into a presentable visual representation (see Figure~\ref{fig:raw:poi:data}).
...
There are two extremes types of thematic map layers that are being created. At one extreme, choropleth~\cite{guardian2013ananas} and heatmap~\cite{wiki2014heatmap} layers present a statistical and aggregated view of geographic information, which is fairly straight-forward to produce using spatial statistics~\cite{ripley2005spatial}. An example is the left map in Figure~\ref{fig:introduction:background:mashups}. At the other extreme, thematic point maps present an immersive and detailed view of information~\cite{bereuter2013real, sarma2012fusiontables,nutanong2012multiresolution}, which fundamentally challenges map designers due to the need for \emph{map generalization}~\cite{harrie2007modelling,mackaness2011generalisation,weibel1997generalization}. An example is the right map in Figure~\ref{fig:introduction:background:mashups}.  
...
While selection of data for maps can be intuitively understood as filtering objects subject to some criteria, many different selection models have been explored. State-of-the-art models can be categorised broadly into context-independent~\cite{weibel1999generalising} and context-dependent~\cite{schmid2013opensciencemap} selection models. Within context-dependent selection, models with different selection criteria have been proposed separately, such as weighted selection~\cite{rocha2011efficient}, weighted selection with density bounds~\cite{sarma2012fusiontables} and weighted selection with proximity bounds~\cite{nutanong2012multiresolution}. Each of these models require separate selection algorithms.
...
\item \textbf{Declarative Specification}: There are currently two extremes in how state-of-the-art mapping systems let programmers specify the selection of data for multi-scale maps. At one extreme, fully-automated selection systems completely encapsulate the logic inside an algorithm library~\cite{sarma2012fusiontables, nutanong2012multiresolution}. At the other extreme, complex programmable selection frameworks require programmers to write highly advanced and often very geography-specific code~\cite{lamy1999application, ware2003generalization_sa}. While selection of thematic data for geographic mash-ups is simple enough that it does not mandate a highly advanced and time-consuming approach, it is also not so trivial that it justifies a fully-automated approach. Specifically:
