...
Scale change may render information on a map illegible or irrellevant. Consequently, this situation must be remedied, which is the purpose of map genealization. Map generalization changes the information abstraction of the map, such that the relevance and legibility of the map is maximized. Information can be abstracted by one of three fundamental operators: \emph{selection} is an operator that derives a subset of the information objects that appear on a map; \emph{projection} is an operator that modifies the attributes of information objects that appear on a map; \emph{aggregation} is an operator that combines several information objects into a new information object that somehow represents them all. These three fundamental operators can each be implemented by a multitude of concrete algorithms and procedures. In a hierarchical view, map generalization can be mapped to three fundamental operators which in turn can be mapped to many concrete algorithms and procedures. 

Operators can be classified as either \emph{independent} or \emph{holistic} operators. While an independent operator, such as selection in SQL or random sampling, transforms objects independently of each other, a holistic operator transform each object while considering the context other the objects. 

For example:

\begin{itemize}
\item \textbf{Selection procedures}: examples of simple selection procedures are \emph{random sampling}~\cite{wang2014efficient} and \emph{predicate filtering}~\cite{sld}; examples of holistic selection procedures are \emph{select-distinct}~\cite{nutanong2012multiresolution} and \emph{density-based thinning}~\cite{sarma2012fusiontables}.

\item \textbf{Projection procedures}: example of projection procedures are map projection~\cite{snyder1997flattening} and simplification~\cite{douglas1973algorithms}. an example of holistic projection is feature displacement~\cite{bader2001energy}. It should be noted that, in geography, projection is usually used interchangeably with map projection. However, this dissertation adopts the notion of projection as the term is used in databases~\cite{ramakrishnan2000database}.

\item \textbf{Aggregation procedures}: examples of simple aggregation procedures are online aggregation~\cite{hellerstein1997online} and sketching~\cite{cormode2012synopses}, which compute quantities such as averages, counts, minimum and maximum values; an example of holistic aggregation is feature aggregation by mixed-integer programming~\cite{haunert2006landcover}.

\end{itemize}


The classification of generalization operators into three high-level categories (selection, projection, and aggregation), which is presented here, is much more abstract than the typologies usually proposed in the generalization literature. Various authors have attempted to come up with a final and fundamental classification, hitherto without any consensus being reached among cartographers~\cite{rieger1993consensus}. For example, operator typologies have been attempted by Foerster, Morales, and Stoter~\cite{foerster2008classification}, McMaster and Shea~\cite{mcmaster1992generalization}, and Weibel and Dutton~\cite{weibel1999generalising}. While all these typologies provide insight into the concrete aspects of the generalization process, it is hard to argue that either one of them is conclusive. Therefore, this section has covered generalization operators using their most fundamental description, without endorsing any specific classification. However, the main contribution of this work is in a specific type of selection, namely holistic selection.
