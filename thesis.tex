...
For thousands of years, humans have visualized geographic information on maps, most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. During those years, maps have helped humans comprehend a multitude of spatial phenomena, from natural bodies such as oceans, glaciers, and mountains to political artefacts such as borders, demographics and land ownership. An example of the latter is a four-thousand-year-old clay tablet that was discovered near present-day Kirkuk, Iraq in 1930. It shows the boundaries of a land parcel between two rivers together with the owner's name, ``Azala''. Given its age, this cadastral map is probably among the earliest data visualizations in human history. 

As the human capacity to gather data has increased, the creation of maps has become more challenging; finite map mediums can only accommodate a finite number of legible symbols. This challenge has lead to a plethora of cartographic information abstraction techniques that are collectively known as \emph{map generalization}~\cite{brassel1988generalization,mcmaster1992generalization,weibel1999generalising}. The purpose of map generalization is to create legible, accurate, and ultimately useful maps for any map scale and for any amount of underlying geographic data. Naturally, this process involves a compromise between readability and precision for most real-world cases. For example, maps of the physical world must carefully select the subset of the available information that is visualized, or else the map would soon become cluttered and illegible. Similarly, demographic maps must often aggregate information about individuals by area in order to clarify the important trends in local populations. Finally, small scale maps must usually distort the information in order to make it more legible, for example by enlarging symbols that would otherwise be imperceivable or displacing symbols that would otherwise overlap.

Over time, technology and map making has evolved hand-in-hand. For example, cloud computing is one of the latest technologies that has impacted map making and made possible a host of new use cases for maps. However, the visual perception of Homo Sapiens -- the principal user of maps -- has probably remained unchanged since the Neolithic period. Thus, despite the tremendous advances in technology, modern map makers must accommodate the same limitations of visual perception in humans as ancient map makers.  
...
Glossy and CVL are systems for multi-scale abstraction of data for map and data visualizations. For this project, the focus has been on holistic selection. Therefore, other important abstraction operators, such as aggregation, have been ignored. For this project, the systems have been implemented on-top-of the PostgreSQL database engine. While this is a mature transactional database system, especially for spatial data, it it lacks the scalability of newer parallel databases and cluster computing systems. Thus, the scalability results for Glossy and CVL can only hint at the potential scalability if these approaches. Finally, the work with the systems was focused on algorithmic contributions, language semantics, and performance evaluation. Therefore, the usability of the Glossy SQL and CVL languages has only been informally evaluated.
...
The background for this thesis has been the impact that the Web and cloud computing has made on the time-honored fields of multi-scale map and data visualization. We have argued that new tools are needed, which support the design, processing, and distribution of map and data visualizations in this new situation. The new tools should be able to handle massive amounts of data. Furthermore, a new breed of non-expert designers should be able to operate these new tool. Finally, the new tools should ensure low-cost, low-latency, highly-available, and scalable access to maps and data visualizations -- for large numbers of concurrent users.

An important map generalization technique is selection of data (Subject A). This thesis contributes new declarative tools that enable non-expert designers to select data for multi-scale maps and data visualizations over large quantities of data. Batch-oriented prediction is an important technique for pre-materializing sections of a map ahead of peak load (Subject B). This thesis contributes a new algorithm and framework for prediction of map tile requests, which bases prediction on an analysis of an log of previous requests. Below, we review the goals for these contributions and give the results that were obtained in each case.


\subsection{Selection: Goals and Results}

This thesis makes two contributions on the topic of selection (Subject A): The language contribution (Contribution 1) consists of two declarative languages, Glossy SQL and CVL, which allow users to express multi-scale selection for geographic maps and data visualizations. The compiler contribution (Contribution 2) consists of two compilers for these declarative languages. The compilers translate Glossy SQL and CVL into SQL and stored procedures for in-database execution of selection for multi-scale maps and data visualizations.
