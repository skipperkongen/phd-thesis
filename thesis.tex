...
The web has transformed how map visualization should ideally be approached. This has opened up important new problems that must be solved, and which are not adequately addressed by the state of the art. Map visualizations must scale infinitely both in data volume~\cite{Im2013visreduce, battle2013dynamicreduction} and the size of the audience~\cite{viegas2007manyeyes}. Moreover, designers must be able to create compelling and complex map visualizations using inituitive and expressive interfaces~\cite{wu2014case, battle2013scalar, fekete2012dmvis}. This dissertation focuses on how to build map visualization systems that fulfill these requirements. In particular, the following problems are addressed:

\begin{itemize}

% HOLISTIC MULTI-SCALE SELECTION
\item \textbf{Holistic Multi-scale Selection} Geographical information is typically spatially skewed, which leaves some areas of space dense with data while other areas hardly have any~\cite{clauset2009power}. Simple selection, which filters a dataset row-by-row according to simple predicates, can not capture binary or multi-way object relations~\cite{egenhofer1991point, randell1992spatial}. Consequently, simple selection can not be used to ensure that a map is thinned to a legible level. In contrast, a holistic selection process will filter records according to global constraints and objectives~\cite{woodruff1998constant, topfer1966principles, harrie1999constraint}, such as minimum proximity, maximum density, and relative object importance (see Figure~\ref{fig:introduction:holistic:selection}). To enable coherent selection for multi-scale maps, the process of selection should be carried through to multiple scales, using a process model such as star or ladder generalization~\cite{foerster2010challenges}.

A model abstration process based on multi-scale selection is appropriate for non-expert map designers, as it can be intuitively understood and moreover produces maps that are interactive, since the identity of records is preserved. Moreover, users intuitively understand that more data may appear when zooming in, for example on a map displaying points of interest.

While a handful of recents map visualization systems have implemented automatic multi-scale selection to some degree~\cite{sarma2012fusiontables}, there is currently a lack of models that allow expression of flexible user-defined constraints and objectives, based on both binary and multi-way object relations. This dissertation focuses on the development of such a model in the context of map visualizations.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.45]{figs-thesis/holistic-selection.pdf}
\caption{\emph{Holistic selection:} }
...
% DONE 1
\item \textbf{Declarative Specification}: Designers, especially those who are not GIS-experts, must be able to express holistic multi-scale selection over large geographical tables. Moreover, in the terminology of Gr{\"u}nreich, designers must have distinct control over the model and visual abstraction stages of the visualization pipeline~\cite{gruenreich1985cag} (see Figure~\ref{fig:introduction:multiscale:selection:pipeline}). A model abstration process based on multi-scale selection is easy to understand and moreover produces maps that are interactive, since the identity of records is preserved. A productive pipeline for multi-scale selection, one that is programmable by designers, will require the specification only of high-level goals in an intuitive specification language, which leaving the minute selection of data to the system~\cite{meliou2011reverse}. The high-level goals will include constraints, objectives and scales~\cite{woodruff1998constant, topfer1966principles, harrie1999constraint} as well as styling rules that consistitute the visual abstraction of data items. Ideally, a specification language requires only the schema to be known, as opposed to a priori knowledge of the data itself. This would facilitate code reuse across similar datasets.

In summary, a high-level programming langage for multi-scale selection must: (1) must be intuitive and productive for non-experts, (2) capture both the star and ladder paradigm of multi-scale data abstraction~\cite{foerster2010challenges}, (3) allow designers to control constraints and objectives that are enforced during selection, (4) target geographical data that is stored in relational databases, (5) encourage code sharing and reuse through schema-oriented language features, similar to SQL. Subject to these requirements, the language should be designed to minimize the average length of programs.
...
% PROCESSING
% PENDING
\item \textbf{In-database Processing}: A Map visualization systems must be able to scalably process a specification for multi-scale selection of geographical data, since the data may be large. One approach is to express geographical selection in low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a higher-level language such as SQL, which reuses optimization techniques that are built-in to SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Additionally, data for map visualizations is frequently stored and managed in RDBMS~\cite{wu2014case}, and from an I/O perspective it makes sense to process the data where it is stored. Therefore, to scalably compute multi-scale selection over geographical data stored in an RDBMS, it is ideal to translate a specification into a select statement in pure SQL (see Figure~\cite{fig:introduction:indatabase:execution}).

The compiler must: (1) produce SQL that is compatible with an existing relational database with spatial extensions, (2), produce SQL that is scalable and computes multi-scale selection for large datasets, (3) support the full expressiveness of the specification language.
