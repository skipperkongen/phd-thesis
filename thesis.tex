...
\date{\parbox{0.85\linewidth}{\centering%
  ...
  Prof. Martin Zachariaen \hspace*{1cm} Dr. Marcos Vaz Salles\endgraf\bigskip
...
\item Materialized views of the primary data model may become stale.
...
\item We compare the character-count of Glossy SQL to the character-count of an equivalent queries in pure SQL (the output of the compiler). The goal of this evaluation is to demonstrate that Glossy SQL is a concise formulation, which is objectively less complex to write than the equivalent SQL in the PostgreSQL dialect.
...
We evaluated the scalability of Glossy SQL by deriving ten POI datasets with between $1000$ and $512000$ records, while attempting to scale dataset size exponentially. We adopt the same scaling procedure described by Kefaloukos et al.~\cite{kefaloukos2014declarative}. A sweep-line approach is employed to enlarge the dataset along the x-axis in the west to east direction. The main property of this procedure is that the size of the dataset can be gradually increased, while maintaining local density. The scalability results are reported in Figure~\ref{fig:glossy:scalability}. 
...
This section will discuss some of the limitations as well as future work that have been planned for the Tileheat contribution. The discussion will focus on the choice of workload for testing, namely the workload for a popular background map that is provided by the Danish Geodata Agency.
