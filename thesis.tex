...
\emph{What are the challenges?} There are two crucial problems that must be addressed in order to successfully develop a data-driven visualization. The first problem is the design effort required to create a high-quality visual representation of a large and perhaps heterogenous dataset. Very quickly, the representation budget of fixed resources like screens and manageable files are depleted as the data grows in size. At this point, the data must be abstracted to fit the medium, for example by selecting subsets or by aggregating. An important question is: how should data be abstracted and how can we control the process to get the result that we want? The second problem is the cost to operate a data-intensive service with 24 x 7 x 365 availability and low latency. Devices in our pockets can reach any point in the world of open data at the speed of light, but that data may be stuck in the black hole of an overloaded endpoint. While techniques exist that minimize the risk of data being stuck on overloaded servers, using these techniques generally require a monetary investment, e.g. in cloud infrastructure. In many technology companies, the costs allocated to cloud infrastructure rival the cost of hiring expensive engineers. \emph{at this point, we should know that data-driven applications require both availability/performance and visual quality, and that it is not comletely straight forward}.

\emph{What is the state of the art?} To address these crucial problems, the research community have worked on many useful techniques. First, the data abstraction problem has been the subject of research for many years (thousands!) and in many areas, e.g. geography~\cite{weibel}, knowledge discovery in databases~\cite{see:db} and visualization of news stories~\cite{metro:stops}. Data partitioning and data replication are important techniques for making access to data highly available, performant, and scalable~\cite{datacenter:as:a:computer}. The trade-offs involved are well-known, and have been the subject of intensive studies~\cite{danger:of:replication, cap:theorem}. Data partitioning schemes have been specifically developed for data that is visualized, e.g. hierarchical linerization of spatial data~\cite{hilbert:curve, z:curve, xyz:protocol}.  Key-Value stores (K-V stores) inherently support data partitioning, data replication and ``scale out''~\cite{dynamodb, datacenter:as:a:computer}. Furthermore, they feature a schema-less data model, which is appropriate for most data that is visualized online (e.g. objects and multimedia data). For these reasons, K-V stores are a good match for storing data that must be visualized online. However, the advantages of K-V stores are offset by severe limitations: they generally lack support for advanced query languages (e.g. SQL), hence they can not implement the logic needed for data abstraction. In constrast, relational database systems (RDBMS) have the power to express the complex selection and aggregation queries needed for data abstraction. However, they in turn are limited by being harder to scale. It seems that the ideal system to power data-driven visualizations would have the scalability of a K-V store (or better) and the expressibility of a RDBMS (or better). In this thesis we focus on developing such a system.
