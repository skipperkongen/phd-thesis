...
% Background
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}. Map visualization is fundamentally a ``big data'' challenge, since the amount of geographic information typically exceed the representational budget of the medium (see Figure~\ref{fig:introduction:background:raw:poi}). Therefore, the geographic information must be adapted to fit the budget, which is a fundamentally hard process that is known as \emph{map generalization}~\cite{shea1989cartographic,weibel1997generalization}.
...
The web mapping process can be modeled as a four-stage logical pipeline (see Figure~\ref{fig:introduction:requirements:pipeline}). The pipeline captures the life cycle of geographic information, from initial data modeling to final display on a digital map. It is important to study the tasks and requirements of each stage in the logical pipeline, since this allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. 
...
Geographic information must be adapted for display on maps, which generally requires the derivation of an appropriate subset of information, or alternatively aggregation. This thesis focuses on \emph{selection} as a means for adapting information, since aggregation is mostly, while not always, a straight-forward process. Even as the Web has impacted when, why and how maps are created and consumed, maps must (still) be \emph{legible}, \emph{consistent} (with reality), and \emph{relevant} (for the purpose of the map). The six design goals of the iOS6 Apple Maps API concretize this further: 
...
The generalization operator of \emph{selection} can, if modeled appropriately, meet all of these requirements.
