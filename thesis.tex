...
% I have to keep making the problem tighter, but still important.

% Maps are important
\emph{Geographic information represents our understanding of the association of phenomena 
with their location on and near the Earth's surface. When presented in map form, an 
essential aspect of that information is that it may be adapted in semantic abstraction 
and level of geometric detail according to the purpose of the map and the extent of the 
Earth that is being considered at any one time. Representations of small areas in detail 
result in so-called large-scale maps, while representations of large regions in lesser 
detail are referred to as small-scale maps. Traditionally cartographers have performed 
the task of adapting the content and the level of detail of a map to suit its scale and 
purpose and this process is called map generalisation.}

Digital map visualization studies how to visually represent the geography of the Earth at multiple scales on restricted digital mediums, such as computer screens. On the web, geographical mash-up is the process by which designers of maps will layer new and interesting geographical data on top of commonly shared background maps. While the background maps are generally of high-quality, the result of many hours of expert labor, new and interesting data presents a raw challenge for creators of mash-ups. Simply displaying symbols for raw geographical data on top of a background map, without appropriate abstraction, will work only for the tiniest datasets (tens of records). One of the most fundamental problems in digital map creation is the mismatch between data volume and the small representational budget of computer screens. For example, a laptop display (e.g. 2880 x 1800 pixels) can display just a few thousand symbols without overlap, which is entirely indadequate for displaying individual records for most geographical datasets, i.e. datasets of more than a hundred spatially skewed records. 

To ensure uncluttered and relevant maps, geographical data must be holistically transformed according to scale~\cite{jones2005webage, weibel1999generalising}. Examples of data transformations include: reduction in the number of discrete objects that are displayed (geometric selection), altering the geometric detail (geometric simplification) or position (geometric displacement) of objects, and combination of grouped objects into fewer objects (geometric aggregation). In the geographical literature, the field that studies the process of data and visual abstraction for maps is field known as \emph{map generalization}. For an overview of this complex field, you are refered to the work of Weibel and Dutton~\cite{weibel1999generalising} as well as Jones and Ware~\cite{jones2005webage}. An good way to deal with too much data, which retains the opportunity for interaction, is to perform geometric selection~\cite{sarma2012fusiontables}. In contrast, aggregation trades the opportunity to interact with individual objects in order to communicate a statistical message, for example, the number of restaurants in a city. However, statistical information is not useful for a person looking to book a table at a particular restaurant. Selection for maps is not as straight forward as selection subject to simple predicates. Specifically, geographical data must be \emph{holistically selected} according to constraints and objectives that ensure an appropriate balance between inclusion of relevant objects and cartographic legibility for the whole map~\cite{jones2005webage}.

% lots of diverse data
Today, the web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap, zooniverse2014oldweather} datasets that can be visualized on maps. While geographical data is characterized by great diversity, is also reaches great volume. Each day, millions of new objects are added to the OpenStreetMap database that already contains billions of objects~\cite{openstreetmap2014stats}. Still, OpenStreetMap accounts for just a tiny fraction of all spatial data on the web, and perpaps, we have only seen the beginning. In 2014, a company launched a system of micro satellites equipped with multi-band sensors that will capture the entire surface of the Earth in high resolution every day~\cite{planetlabs2014flock1}. Moreover, as mobile devices increasingly bond millions of people to an online services, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and games~\cite{geocaching2014geocaching}, geographical information can be produced and consumed by anybody, anytime, and anywhere.

% lots of in-experiences designers, aggregation is too easy,  GIS is too hard.
Given the availability of so much interesting data, more and more organizations with no strong tradition for geography, such as news agencies, are attempting to communicate spatial narratives using maps~\cite{nytimes2010iraq}. As a consequence, more and more professionals will need to acquire the skills to craft map visualizations. While tools exist that will help a novice designer to style geographical data for a map~\cite{cartodb2014cartodb, google2014fusiontables}, no tools are currently available that will adequately allow novice designer to customize the selection of data. The easy way out is to aggregate the geographical data, for example using heatmaps, choropleth maps or clustering. As a result, users will loose the important opportunities for interaction, such as clicking on individual record symbols to get more information about concrete records. The hard way out, is to employ a geographical information system (GIS) specialist to select data for a map. However, demand for multi-scale maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS). In any event, while many powerful data selection methods have been developed for maps~\cite{gen1, gen2, gen3}, applying them to maps is currently a time consuming process -- even for GIS experts. In contrast, photo editing tools have long enabled novice designers to easily edit images by offering general-purpose plugins. What is needed, is a framework that will allow the development of similar plugins for data selection on multi-scale maps. To be successful, any data selection method must be scalable in data volume, and ideally facilitate shareable and resuable code that works for new and unknown datasets.
...
Simple filtering~\cite{cartodb2014cartodb}.
Indexing~\cite{nutanong2012multiresolution}
