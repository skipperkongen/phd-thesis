...
The rise of the Web, and subordinately cloud computing, has created a new situation for map visualization. In the following sections, both the challenges and opportunities that have been created by these phenomena are covered in more detail.

\subsection{Challenge of the Web}
% The changes and challenges caused by the web.
The Web has structurally impacted \emph{why}, \emph{how}, by \emph{whom}, \emph{when}, and \emph{where} digital maps are produced and consumed. In combination, the Web and mobile devices have resulted in an online eco-system where information is created, accessed, and shared ubiquitously. Today, online maps are used by millions of people every day and maps have become a commodity. As with any online commodity, users expect 24 x 7 availability and acceptable latency for map services.

In its wake, the rise of the Web poses a dual challenge of map creation and distribution, which cannot be adequately addressed by traditional geographic information systems (GIS). GIS was developed before the World Wide Web and primarily as a single-user system~\cite{ucgis2014tomlinson}. Later on, GIS has been retrofitted with web service capabilities, such as the Web Map Service (WMS) and Web Feature Service (WFS). However, these protocols do not fundamentally alter the architecture of GIS, they simply expose the GIS interface over the Web. While this may work well for a few concurrent users, it becomes prohibitively expensive when the number of concurrent users rise. As a consequence, almost all succesful online maps today rely on a new architecture, which uses pre-materialized maps that are distributed to end users.

Below, the challenges posed by the Web to map visualization are investigated in more detail.
...
\minisec{Huge and Constantly Evolving Geospatial Datasets}
The Web has shifted the reasons why maps are created, which is caused by two root factors. On one hand, private companies~\cite{google2014maps,stamen2014tiles,vizzuality2012cartodb} and national mapping agencies~\cite{gst2014digitalmapsupply} supply an abundance of online background maps, which means that there is less need to create these types of maps. On the other hand, there is a steady rise in the supply of thematic geographic datasets online -- both curated~\cite{mst2014miljo}, user-contributed~\cite{twitter2014twitter,zooniverse2014oldweather}, and machine generated~\cite{planetlabs2014flock1} -- which grow by millions of information objects each day~\cite{openstreetmap2014stats}. The combination of these two factors means that demand has shifted from background maps to thematic maps~\cite{bereuter2013real}. Visualizing emerging datasets on thematic maps is potentially of great relevance and importance. For example, journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} have created thematic maps that increased the transparency and knowledge in the World and have potentially even saved human lives. 

In general, the increase in data volume calls for scalable data processing methods for production of thematic maps. However, not all thematic maps are challenging to create. On one hand, statistical maps, such as the choropleth map in Figure~\ref{fig:introduction:background:mashups} (left map), can be efficiently produced by using aggregation techniques from spatial statistics~\cite{ripley2005spatial}. On the other hand, entity maps, such as the spatial event map that is shown in Figure~\ref{fig:introduction:background:mashups} (right map), require holistic selection of information~\cite{sarma2012fusiontables,nutanong2012multiresolution}, a much greater challenge.
...
\subsection{Opportunities of Cloud Computing}
Cloud computing is a model for enabling ubiquitous, convenient, on-demand network access to a shared pool of configurable computing resources~\cite{mell2009nist}. Today, powerful data analytics systems, such as mapreduce~\cite{apache2015hadoop} and column-stores~\cite{bauer2013redshift}, can be provisioned and rented by the hour from cloud providers~\cite{amazon2010aws}. Furthermore, cloud services such as Amazon Web Services, offer practically unlimited and cheap storage of data. In combination, on-demand and scalable processing of spatial data~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} and unlimited storage of data creates an unprecedented and powerful opportunity to compute large-scale geographic maps.

Today, most information distributers, such as video and social networking sites, use cloud-based content delivery networks (CDN) to distribute their product to a global audience~\cite{dilley2002cdn}. Some of the key advantages of CDN are their superiour availability, scalability and performance~\cite{barroso2009datacenter,pilchin2007dynamo}. Any type of data that can be organized as a set of key-value pairs can be distributed in this way via a content delivery network. For example, photos have a filename or identifer (the key) and some bytes that make up the image content (the value)~\cite{beaver2010haystack}. Similarly, spatial data can be partitioned into key-value pairs by using a spatial grid~\cite{decola1993pyramid} or linearization~\cite{hilbert1891ueber,morton1966computer} decomposition. Once partitioned, the spatial data for a map can be distributed to millions of people online as a set of map tiles~\cite{gaffuri12vectortiles}.

Recently, in-memory and fault-tolerant stores, such as RAMCloud~\cite{ousterhout2011ramcloud} have appeared. RAMCloud improves on the volatility of in-memory caches and the latency of disk-based systems, by using fault-tolerance techniques that make them comparable to disk-based stores.
...
\subsection{Glossy and CVL}
% Glossy + CVL discussion
Unsupported data types, e.g. text, trees, and graphs; unsupported operators, e.g. aggregation and holistic projection; limitations in data size, e.g. explain that while results are shown for Postgres, other and more scalable databases or clustersystems could be used. User-studies could be conducted.}
