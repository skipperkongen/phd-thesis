...
Digital map visualization studies how the intricate geography of the Earth can be visually and geometrically represented by the finite pixels on a computer screen. Digital maps allow users to explore geographical information at multiple focal points (panning) and levels of detail (zooming). On the web, a geographical mash-up is the process by which designers of maps will layer new and interesting geographical data on top of commonly shared background maps. While the background maps are generally of high-quality, the result of many hours of expert labor, new datasets present a raw challenge for creators of mash-ups. Simply displaying symbols for raw geographical data on top of a background map will only work for tiny datasets (tens of records)~\cite{topfer1966principles}. 
...
One of the most fundamental problems in digital map creation is the mismatch between data volume and the small representational budget of computer screens. For example, a computer screen can display just a few thousand distinct and legible symbols, which is entirely indadequate for displaying all individual records of most real-life geographical datasets (see Figure~\ref{fig:raw:poi:data}... grrr, why 1.1). 

To address this challenge, generalization is the process of creating a map of a given scale that retains as much as possible of the relevant geographical information, subject to constraints of legibility~\cite{jones2005webage}. Methods of generalization include \emph{selection} of objects~\cite{sarma2012fusiontables}, \emph{simplification} of geometric detail (and other types of projection)~\cite{douglas1973algorithms},  \emph{displacement} of coordinates~\cite{lonergan2001iterative}, and \emph{amalgamation} of many objects into fewer objects~\cite{haunert2006landcover}. Moreover, \emph{statistical aggregation} of geographical information, such as choropleth maps, eliminates the geometric representation of data altogether~\cite{friendly2008milestones}. For a more complete overview of this complex field, you are refered to the works of Weibel and Dutton~\cite{weibel1999generalising} as well as Jones and Ware~\cite{jones2005webage}.  


% lots of diverse data
\vspace{5ex}
Today, the web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap, zooniverse2014oldweather} datasets that can be visualized on maps. While geographical data is characterized by great diversity, is also reaches great volume. Each day, millions of new objects are added to the OpenStreetMap database that already contains billions of objects~\cite{openstreetmap2014stats}. Still, OpenStreetMap accounts for just a tiny fraction of all spatial data on the web, and perpaps, we have only seen the beginning. In 2014, a company launched a system of micro satellites equipped with multi-band sensors that will capture the entire surface of the Earth in high resolution every day~\cite{planetlabs2014flock1}. Moreover, as mobile devices increasingly bond millions of people to an online services, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and games~\cite{geocaching2014geocaching} to name a few. Consequently, geographical information can be produced and consumed by anybody, anytime, and anywhere.

% lots of in-experiences designers, aggregation is too easy,  GIS is too hard.
Given the availability of so much interesting data, more and more organizations with no strong tradition for geography, such as news agencies, are attempting to communicate spatial narratives using maps~\cite{nytimes2010iraq}. As a consequence, more and more professionals will need to acquire the skills to craft map visualizations. While tools exist that will help a novice designer to style geographical data for a map~\cite{vizzuality2012cartodb}, for example to pick appropriate symbol colors and shapes, no tools are currently available that will adequately help novice designers to customize the holistic selection of data. The easy way out is to aggregate the geographical data, for example using heatmaps, choropleth maps or clustering. As a result, users will loose the important opportunities for interaction, such as clicking on individual record symbols to get more information about concrete records. The hard way out, is to employ a geographical information system (GIS) specialist to holistically select data for a map. However, demand for multi-scale maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS). In any event, while many powerful data selection methods have been developed for maps~\cite{gen1, gen2, gen3}, applying them to maps is currently a time consuming process -- even for GIS experts. In contrast, photo editing tools have long enabled novice designers to easily edit images by offering general-purpose plugins, such as gaussian blur. What is needed, is a framework that will allow the development of similar plugins for holistic data selection in multi-scale maps. To be successful, any data selection method must be scalable in data volume, and ideally facilitate shareable and resuable code that also works for new and unknown datasets. 

% lots of users
Mash-ups are potentially interesting to millions of people. Thanks to innovations in mobile mapping applications and ubiquitous web access, a map visualization may suddenly be visited by large numbers of concurrent users. For this reason, maps must scale in the number of active consumers, such that an acceptable latency-level remains constant. Furthermore, maps must be highly available, since this web users expect 24 x 7 availabilty of services on the web.
...
The general topic of this dissertation is map visualization, however an important focus point of this dissertation is map generalization, which is the process of adapting geographical data to scale of a given map~\cite{weibel1999generalising}.
