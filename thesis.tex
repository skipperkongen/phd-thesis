...
The digital mapping process can be modelled as a pipeline that translates between ``reality'' and a configuration of pixels on a computer screen. This mapping pipeline has three intermediate states that correspond to increasingly abstract representations of reality. These intermediate states are: the \emph{data model}, the \emph{logical model} and the \emph{graphical model}~\cite{gruenreich1992atkis} (see Figure~\ref{fig:introduction:requirements:pipeline}).
...
As the scale of a map decreases, geographic information must be logically and graphically adapted for display on maps~\cite{weibel1999generalising}. This section discusses the constraints and objectives that drive logical abstraction. The means of logical abstraction are covered by selection, projection and aggregation of geographic information objects. This thesis focuses on selection as the primary means for logical abstraction of information. In contrast to selection in relational algebra, selection of geographic information is a holistic process, which cannot be computed by simple object-by-object predicate filtering~\cite{harrie2007modelling}. 

Even as the Web has impacted when, why and how maps are created and consumed, information that is displayed on maps must (still) be legible, consistent with reality, and relevant for the purpose of the map. The six design goals of the iOS6 Apple Maps API concretize this further: 
...
While the above example of design goals is illustrative, a goal of this thesis is to derive a more formal classification of constraints and objectives for logical abstraction of geographic information. In doing so, this thesis adopts a combinatorial optimization model of the map generalisation process~\cite{harrie2007modelling}: 
