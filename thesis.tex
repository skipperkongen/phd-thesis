...
%\todo{Introduce the term multi-scale data abstraction (MSDA) and stick with it.}
%\todo{Introduce the data abstraction operators: global selection, global projection and global aggregation, and stick with them. Call them the global operators. Either in the background or the problem statement.}
%\nice{Relational Databases (RDBMS) on the other hand, with their support for indexing and SQL,  are certainly powerful enough to express MSDA. While a full system for MSDA that supports all the global operators could likely be realized on top of a relational database, this thesis concentrates on using RDMBSs for multi-scale global selection.}
...
%\item How does this thesis make progress on the state of the art?
%\item What are the specific contributions? 
%\end{enumerate}

% How has the world changed; what is the new situation?
\emph{How has the world changed; what is the new situation?} The web today is home to an eco-system of data providers and data consumers who create, combine and visualize a huge diversity of open datasets. In combination, open data covers the full spectrum of human interests, e.g. culture~\cite{rapgenius,wikipedia}, economics~\cite{worldbank}, geography~\cite{openstreetmap,danish:geodata:agency}, demography~\cite{unstats} and politics~\cite{data:gov, data:co:uk}. Indeed, the value proposition of open data is clear: open data will increase transparency, efficiency and enlightenment in our societies~\cite{hans:rosling, offentlige:data:i:spil}. Consequently, the availability of open data, both curated and user-contributed, has created a great demand for online data-driven visualizations, e.g. maps~\cite{guardian:sunrise:on:twitter}, bubbles~\cite{nytimes:obama:budget}, and buckets~\cite{largest:vocabulary:in:rap}. To stimulate inquisitive behaviour, data visualizations should allow users to explore a dataset at multiple focal points and levels of detail; data can be explored at the global level or deep down in the detail. Since data visializations are so useful, they can quickly attract an audience of millions, for instance, when they are shared on social media. 
...
There are two crucial problems that must be addressed in order to successfully develop a data-driven visualization in the current situation. 

First, developers must address the crucial problem of data abstraction in order to create a high-quality visual representation of a large and heterogenous dataset. Very quickly, the representational budget of finite resources available to visually represent the data, such as screen pixels or fast-access memory, is depleted as the data grows in size~\cite{topfer:selection, weibel, stonebraker}. At this point, the data must be abstracted to fit the media, e.g. by selecting a representative subset or aggregating the data. However, designers can not manually abstract the data as it is too time consuming. Thus, the data abstraction process must be at least partially automated. Some crucial questions are: what is the ideal data abstraction for a given visualization, and how can designers control a partially automated data abstraction process to achieve a desired result? 

Second, system operators must ensure that data is delivered to applications, e.g. client-side visualization code running in a browser~\cite{d3}, with low latency and high availability. Failing to recognize the importance of these goals will likely make a negative impact on the success of a service provider~\cite{latency:studies:google:and:amazon}. Techniques exist that can be used to architect a system with low latency and high availability~\cite{data:center:as:a:computer}. Nonetheless, a crucial problem is to operate such a system while also minimizing the cost. 

%A high-number of concurrent users can potentially increase the latency in a data delivery system~\cite{tileheat}, which in turn can lead to higher levels of concurrency. In architectures that have a data bottleneck, e.g. the classical 3-tier database application, this problem occurs frequently. 

%While devices in our pockets can reach any point in the world of open data at the speed of light, that data may be stuck in the black hole of an overloaded endpoint. 

\emph{What is the state of the art?} To address the crucial problems, the research community has worked on many useful techniques. First, the data abstraction problem has been studied for many years (in the case of cartography, thousands!) and in many areas, e.g. map generalization~\cite{weibel}, visual analytics~\cite{see:db} and information graphs~\cite{metro:stops}. Second, the performance and availability problems have been studied by the distributed systems and database communities, e.g. data partitioning~\cite{foo} and data replication~\cite{foo}. The trade-offs involved are well-known, and have been the subject of important studies~\cite{danger:of:replication, cap:theorem}. Additionally, data partitioning schemes have been specifically developed for data that is visualized, e.g. hierarchical linerization of spatial data~\cite{hilbert:curve, z:curve, xyz:protocol}.  Third, key-value stores with inherent support for data partitioning, data replication and ``scale out'' have been popularized by Internet giants such as Google, Amazon and Facebook~\cite{dynamodb, bigtable, haystack}. Key-value stores feature a schema-less data model that is appropriate for data that is visualized (e.g. objects and multimedia data). However, the advantages of key-value stores are offset by severe limitations: they generally lack powerful query languages like SQL. Hence, they are not a good fit for computing data abstractions. In constrast, relational database systems (RDBMS) have the power to express the complex selection and aggregation queries needed for data abstraction. However, data abstraction queries are generally not easy to express in pure SQL, as data abstraction is a holistic process that generally considers the whole dataset. It seems that the ideal system to power data-driven visualizations would have the scalability of a K-V store (or better) and the expressibility of a RDBMS (or better). In this thesis we focus on developing such a system.

\note{Two times however in a row does not flow well.}

% How does the situation motivate the topic of the thesis?
\emph{How does this thesis make progress on the state of the art?} K-V stores have a what-you-store-is-what-you-get data model (another form of WYSIWYG). Thus, data abstraction and data partitioning are activities that must be performed either upstream or downstream from the K-V store. In contrast, relational databases have the power to express complex data transformations and business logic. This thesis will explore how to integrate K-V stores with relational databases to power data-driven visualizations in a ``best of both worlds'' approach. The goal is to arrive at a system that can power data-driven visualizations with 24 x 7 availability, high performance and scalability, and high-quality...... hmmmm. \emph{elaborate}
...
In the field of databases, research on data abstraction is still in its infancy. Outside the field of databases, there exists prior and interesting work, e.g. in map generalization and information visualization. There is an opportunity to export useful ideas from these areas to the area of data abstraction in databases. To fullfill the potential of data visualization, this thesis proposes to bring data abstraction techniques to where the data is frequently managed, i.e. relational database systems (RDBMS). There is arguably a need for specialized RDBMS that are geared towards data visualization, e.g. systems that can perform multi-scale data abstraction operations over data tables. This thesis focuses on four problems that must be addressed in order to build such systems.

%\begin{enumerate}
%\item Data Abstraction Operators
%\item Programming Languages
%\item RDBMS Integration
%\item Adaptive Data Serving
%\end{enumerate}

\subsection{Holistic Selection} Today, relational databases do not support data abstraction operators for powering data visualizations. Critically, records must be \emph{holistically selected}, i.e. according to global criteria such as legibility and relative importance~\cite{weibel:controls:of:generalization} (\emph{any other requirements?}). Two important questions that must be answered are: What are the sematics of a holistic selection operator? How should the operator parameterized?

\subsection{SQL extension} Relational databases are powerful thanks to their query language, SQL. To enable data visualization queries, the SQL syntax must be \emph{extended} in order to to express data abstraction queries. This will enable developers to select data for visualizations using a familiar query language and in a concise declarative style. An important question is how the syntax should balance between expressibility and conciseness.

\subsection{RDBMS Integration} SQL with data abstraction extensions must be \emph{implemented} on top of, or inside, relational database engines. This will enable data abstraction queries to harness the power of indexing and battle-proven query optimizers. An important question is how existing database engine functionality can be reused to power data abstraction queries.

\subsection{Predictive Data Serving} Many types of data visualization, e.g. maps, incur heavy materialization costs when data is requested. Being able to provision resources ahead of time is important for workloads that exhibit clear trends. Using a log of previous requests, we can theoretically predict the requests that will be seen tomorrow. How well can we predict the workload for a data visualization? For example, in a 24-hour life cycle of a data visualization, where are experiences peaks and valleys in the number of concurrent users. Furthermore, the probability of a request for a subset of the data will be many orders of magnitude higher than the average probility of data being requested. Being able to predict what data will be requested and when, will allow a system to pre-materialize the most likely requests ahead of time. Some important questions are: on what basis should prediction we performed and what are the prediction models that are most effective?

%Being able to provision resources according to predicted workloads is crucial for enabling timely and performant data visialization experiances. Using a log of previous requests, predict the requests that will be seen tomorrow. Account for trends, seasonal variation and toblers first law.
