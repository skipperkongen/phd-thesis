...
Given the availability of so much interesting data, more and more organizations with no strong tradition for geography, such as news agencies, are attempting to communicate spatial narratives using maps~\cite{nytimes2010iraq}. As a consequence, more and more professionals will need to acquire the skills to craft map visualizations. While tools exist that will help a novice designer to style geographical data for a map~\cite{cartodb2014cartodb, google2014fusiontables}, no tools are currently available that will adequately allow novice designer to customize the selection of data. The easy way out is to aggregate the geographical data, for example using heatmaps, choropleth maps or clustering. As a result, users will loose the important opportunities for interaction, such as clicking on individual record symbols to get more information about concrete records. The hard way out, is to employ a GIS specialist to use advanced tools to select data for each scale offered by the map. However, demand for multi-scale maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS). In any event, this is currently a time consuming process even for experts. In contrast, photo editing tools have long allowed novice designers to modify images using easibly mastered plugins for filtering. What is needed, is a framework that will allow the development of similar plugins for selecting geographical data for multi-scale maps. Such as framework should facilitate resuable and sharable code for holistic data selection.
