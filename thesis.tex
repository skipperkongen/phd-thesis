...
\section{Motivation}
Multiscale visualizations, with support for \emph{pan} and \emph{zoom} operatons, allow users to explore large datasets of discrete information objects at different focal points and levels of abstraction. Further interaction is enabled by responding e.g. to click-events. In recent years, the exponential growth in online data has caused many visualization applications to migrate onto the web, where they can simultaneously reach a much larger audience and integrate against large repositories of data center local data. Professional and private users interact with online multiscale visualizations to gain data insights and support decision making -- e.g. using a plot to understand complex financial data or using a map to find a restaurant to have dinner at. 
