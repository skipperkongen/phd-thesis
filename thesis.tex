...
Map visualization studies how to visually represent the geography of the Earth at multiple scales on restricted mediums, such as computer screens. On the web, geographical mash-ups layer new and interesting geographical data on top of commonly shared background maps. While the background maps are generally of high-quality, the result of many hours of expert labor, new and interesting data presents a raw challenge for creators of mash-ups. Simply displaying raw geographical data on top of a background map, without appropriate abstraction, will only work for tiny datasets (tens of records)~\cite{jones, weibel1999generalising}. One of the most fundamental problems in digital map creation is the mismatch between data volume and the small representational budget of computer screens. For example, a laptop display (e.g. 2880 x 1800 pixels) can display just a few thousand symbols without overlap, which is entirely indadequate for displaying most geographical datasets, i.e. datasets of more than a hundred records that are spatially skewed. Necessarily, geographical data must be abstracted to scale, for example by selection. However, selection of geographical data for maps must be holistic, i.e. data must be selected according to both relevance and aesthetic metrics considered for the whole map.
