...
\subsection{Key-value Stores for Geographic Data}
Geographic data that has been converted to tiles can be uniquely addressed by key in several ways: addressing schemes include space-filling curves~\cite{hilbert1891ueber,morton1966computer} and by the more obvious indexing by x, y, and z (for zoom level). Thus, it is trivial to design an indexing scheme that allows subsets of geographic data to be addressed by key.

Key-value stores -- such as Dynamo~\cite{pilchin2007dynamo} and Keyspace~\cite{trencseni2010keyspace} --  are general purpose and highly scalable distributed storage services that allow opaque data to be stored accessed by key. Thus, given a set of tiles together with a scheme to address them by key, it is straightforward to store geographic data in a key-value store. Recent years have seen an explosion in the number of key-value stores, that have been proposed in academia and commercialized in industry.

Most key-value stores offer superior availability and low latency, since data is replicated and partitioned over a distributed network of storage nodes~\cite{barroso2009datacenter}. A special type of key-value store, the content delivery network (CDN)~\cite{dilley2002cdn}, offers even better availability and performance since data is georeplicated and stored at edge locations in the network -- close to the users.

Recently, in-memory and fault-tolerant stores, such as RAMCloud~\cite{ousterhout2011ramcloud} have appeared. RAMCloud improves on the volatility of in-memory caches, such as memcached, by using fault-tolerance techniques that make them comparable to disk-based stores.

\subsection{Best Fit for Large-scale Web Maps}

While real-time systems present an ``ideal'' for mapping systems (see the advantages above), state-of-the-art real-time mapping systems are inappropriate for large-scale web maps. First, these systems have relatively low availability and high latency compared to precomputed content that is served from a key-value store or CDN. Second, they do not cost-effectively scale with the number of concurrent users. These problems are critical in a web context.

Dispite the drawbacks, state-of-the-art batch-oriented mapping systems are appropriate for large-scale web maps. The main reasons are that (1) batch-oriented systems can exploit highly scalable processing systems (e.g. mapreduce~\cite{aji2012largespatial,aji2013hadoopgis,eldawy2014spatialhadoop}) that can handle any data volume, and highly available, low-latency key-value stores for data distribution. Today, scalability is a crucial requirement for large-scale web map systems. Additionally, the disadvantages are not too severe. First, disk-based storage has become very cheap and pure in-memory storage is bound to become cheaper over time. Second, materialized view can be refreshed as demand dictates, either by leveraging batch-oriented parallel processing~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} or by employing techniques for materialized view maintenance~\cite{gupta1995maintenance}. Finally, user preferences can to some extent be accommodated by materializing views for different user profiles and choose between them at query time. However, this last problem is probably the most fundamental problem of batch-oriented systems.
