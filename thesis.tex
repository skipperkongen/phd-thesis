...
This thesis focuses on generalization of geographic data in relational databases. Map generalization is a big and complex field, which on record has been studied for decades. Off the record, it has probably been studied for thousands of years, and consequently, there is a lot of related work. However, the purpose of this section is not to give a comprehensive overview of map generalization, as this is too big an area. For that, the reader is refered to an older survey by Weibel and Dutton~\cite{weibel1999generalising} and a recent book by Mackaness, Ruas, and Sarjakoski~\cite{mackaness2011generalisation}. Instead, this section positions the contributions of this thesis in relation to selected parts of the related work, both older and recent.

While the geography literature on map generalization tends to makes many distinctions, recent computer science studies of map generalization tend to make fewer distinctions; it is as though the recent literature deliberately departs from the heavy complexity of ``traditional'' map generalization. An interpretation could be that map generalization for complex problems is generally a closed field. This view is supported by the availability of so many good background maps from Google, Microsoft, OpenStreetMap, as well as the national mapping agencies. While arguably map generalization has been solved for the production of topographic, physical and other types of background maps, the web has opened up new challenges for the generalization of emerging foreground data. This view is supported by the appearance of a new wave of map generalization studies that have this focus. In computer science, recent work on map generalization has mainly centered around the generalization of foreground data, and mostly point sets. This section positions the contributions of the thesis in relation to both the recent and the older work in map generalization.

Outside the fields of geography and map generalization, highly similar data abstraction challenges are being addressed in the field of data visualization. It seems that map visualization converges with the broader field of data visualization. While spatial is still special, there is no doubt that opportunities exist for cross pollination across these two fields. \emph{enumerate}.
