...
\title{Declarative Map Visualization in a Relationally Integrated World}
...
Map visualization studies how the geography of the Earth can be represented, most recently on computer screens and historically on pieces of paper. Since ancient times the requirements for this process have been to represent as much as possible of the relevant information on images that are legible, useful, and to some degree aesthetically pleasing. The three dimensions of an image make the visual perception our most powerful perceptive system, but images have only three dimensions and the consequences of this limit are important~\cite{bertin1973semiologie}. As an entire planet is scaled down and projected on a computer screen, invariably, map designers are faced with the need to abstract the information~\cite{weibel1997generalization}.

Web and mobile mapping applications typically present ermerging thematic foreground data in the context of existing background map that provide the spatial context for so-called geographic mash-ups (see Figure~\ref{fig:introduction:background:mashups}}). % Why is it cool? 
Mash-ups are important because they combine news-worthy or simply interesting spatial information with existing high-quality background maps that provide the spatial context in which a geographic narrative unfolds~\cite{bereuter2012algorithms}. % Why is it hard?
High-quality map must display as much as possible of the relevant information while also remaining legible or even beautiful~\cite{jones2005webage}, however, as data grows in size beyond a few thousand records, symbols on the map will begin to become indistinquishable (see Figure~\ref{fig:raw:poi:data}). At this point, there is a need for geographic information abstraction, which is also known as map generalization~\cite{weibel1999generalising}. For example, a map designer may choose to \emph{select}~\cite{topfer1966principles}, \emph{simplify}~\cite{douglas1973algorithms},  \emph{displace}~\cite{lonergan2001iterative}, and \emph{amalgamate}~\cite{haunert2006landcover} the geographic information in order to emphasize the important and suppress the unimportant. % Why is the state-of-the-art not good enough?
Map generalization approaches can be divided into two polar opposites, namely on-the-fly~\cite{bereuter2013real,van1995gap,nutanong2012multiresolution} generalization and batch-oriented~\cite{schmid2013opensciencemap,haunert2006landcover,sarma2012fusiontables} generalization. Initially, on-the-fly generalization seems to have all the benefits, such as only storing the data once and the opportunity to react individually to users' behavior. However, on-the-fly methods do not cost-effectively scale to massive amounts of concurrent users, since they rely on datastructures that while efficient for single queries are difficult to partition and scale with the number of concurrent queries~\cite{}. In contrast, batch-oriented methods  gets an input data set and returns an output data set using algorithms, rules, or constraints without the intervention of humans, which allows effortless scalability to massive amounts of concurrent users. The materialized and abstracted data can be partitioned and replicated inside highly scalable content delivery networks (CDN)~\cite{barroso2009datacenter}, for example as vector tiles~\cite{gaffuri12vectortiles} which partition the data in a multi-scale linearization scheme~\cite{hilbert1891ueber, zcurve}. However, the most crucial drawback of a batch-oriented approach is that it sacrifices consistency in order to gain scalability. When the base data changes, the materialized information abstraction becomes an inconsistent view of the data. To make it consistent again, the abstraction must be computed from scratch. Given a requirement that a map must by highly scalable, for example in a geospatial data infrastructure~\cite{gst2014digitalmapsupply}, batch-oriented approaches are presently the only viable option for the majority of map generalization use cases. A few exceptions exist where scalable on-the-fly methods have been proposed for very specific problems, such as select-distinct~\cite{nutanong2012multiresolution}. While a batch-oriented approach is are generally needed for high-scalability maps on the web, the current state-of-the-art in batch-oriented map generalization is suboptimal for abstraction of thematic foreground data. % Why?
...
While data integration is a fundamentally hard problem, important work in the field has made the problem easier to address than ever~\cite{marcosthesis}. For example, relational databases~\cite{importantdatabasedataintegrationwork}. As a result, bla bla bla~\cite{citeplaceswheretheyareused}. Relational databases combine a flexible data model~\cite{} with a powerful query language~\cite{}. Furthermore, decades of work in the optimization of query processing has yielded highly efficient interpretation of relational algebra queries~\cite{cooldatabases}. It is only logical then that recent data visualization methodologies increasingly propose that visualization pipelines should be tightly integrated with relational database systems. The two main advantages of this approach are foo and bar~\cite{wu2014case} However there is currently a lack of similar database integrated methodologies for map visualization. % why is map visualization different?
