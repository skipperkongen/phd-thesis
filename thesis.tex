...
\minisec{Example: \emph{Vector Tiles for Points of Interest (POI)}}
Tiling of vector data, just as tiling of raster data, is a way to partition and primarily distribute geographic information to mapping clients~\cite{gaffuri12vectortiles}. A vector dataset is partitioned into tiles by intersecting it with a set of multi-scale cells, such that each object falls into one or more cells at each scale. Moreover each tile contains either the identifiers of the intersected geographic objects (index tiles), or the the geographic objects that have been geometrically cropped to the boundaries the cell~\cite{gaffuri12vectortiles}. While the two approaches are functionally equivalent, one may perform better than the other for particular datasets and workloads. Since the tiles contains data as opposed to images, the information that is distributed is a \emph{logical map}, i.e. pertains to \emph{what} the map contains, not \emph{how} is should be graphically expressed.

There are several advantages to tiled vector data and logical maps. First, distribution of logical maps allows many degrees of freedom with regard to the graphic expression of the map. For example, the \emph{size}, \emph{value}, \emph{texture}, \emph{color}, \emph{orientation}, and \emph{shape} of symbols may be chosen such that the efficiency of the graphic expression is maximized~\cite{jacques1967semiologie}. Second, tiled vector data can be scalable stored and distributed on a content delivery network (CDN), which facilitate high availability and acceptable latency for Web maps~\ref{dilley2002cdn}. Third, each mapping client that retrieves data from the CDN, contributes a CPU that is utilized to transform data into graphics.
...
\caption{\textbf{Creation and distribution of tiled vector data}: 1, 2, 3, 4, A, B}
