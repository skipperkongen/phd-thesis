...
\note{Data visualizations today must be scalable, in terms of data~\cite{Im2013visreduce, battle2013dynamicreduction} and size of the audience~\cite{cohen2011journalism, viegas2007manyeyes}. Moreover, it is crucial that we can produce compelling data visualizations effectively in terms of system and human resources~\cite{wu2014case, fekete2012dmvis}. \emph{insert arguments from vision papers... also see if reverse data management fits in somehow.}}
...
In general, data visualization is concerned with the creation and study of (interactive) visual representations of data to reinforce human cognition~\cite{friendly2008milestones}. While data can be both numerical and non-numerical, this thesis will focus on data that is stored in tables, i.e sets of rows composed of columns. At first, this may seem constricted, however tables are versatile and make up the bulk of all data that is visualized today~\cite{wu2014case}. Defined in this way, the distinction between information visualization and scientific visualization (see Munzner~\cite{munzner2008pitfalls}) becomes irrellevant. For example, a table may contain geographical objects where the spatial representation is given, or contain economical data where the spatial representation is chosen. In any event, the tables are assumed to contain abstract objects represented as rows that capture some aspect of ``reality'' -- ideally with a high level of granularity. For example, a table may contain roads with their high-precision geographical geometries~\cite{openstreetmap}, or the minute details of a federal budget~\cite{nytimes2012obamabudget}.
...
Agenda: Overview of different fields that relate to data visualization. Show how they are different. Show how they are similar. What are the goals and problems of each field? What types of data is handled? Where does the work fit into the pipeline: What type of workflows (e.g. operators, controls, stages, etc.).

\todo{Can I find a citation for a ``typical'' pipeline?}

\todo{Read related work section of Glossy}
