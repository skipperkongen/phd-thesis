...
For thousands of years, humans have used their visual senses to comprehend geographic information on maps. Humans learned early on that legible maps enable instant comprehension of relevant geographic information, while cluttered maps inhibit comprehension. Over time, the reasons that maps are created and the context they are created in have constantly evolved. Accordingly, novel methods for drawing maps have been invented, studied, and sometimes discarded. However, there exists no evidence to suggest that the human capacity for visual perception and comprehension has evolved significantly since the stone age. Consequently, modern map makers who wield modern map making tools must accomodate the same human cognitive limitations as ancient map makers.
...
\minisec{Difficulty of Map Visualization}
Map visualization studies the process by which geographic information is displayed on a three-dimensional medium (x, y, color), most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. Since ancient times, the requirements for this process have been to \emph{accurately} and \emph{legibly} represent as much as possible of the \emph{relevant} information on a highly restricted medium~\cite{jones2005webage}. It is a challenging process, since opposing goals must balanced in order to achieve an optimal map. Frequently, there will be too much information to be visualized compared to the representational budget of the map medium. 

If you need convincing that map visualization is a difficult task, try to pick up a pencil and draw a detailed map of the World on a sheet of A4 paper. Almost immidiately, you will need to select which cities, islands, lakes, etc. to include, and which to leave out. Now imagine that you have to draw a map of a different world than the physical Earth; a world you have no prior knowledge of; a world where you do not start with a mental model of what is important and what is irrelevant. A daunting task to be sure. This is the kind of world that data journalists mapped using Wikileaks data, which came about as a pile of unranked messages  that detailed a set of spatial events during the 2003-2011 Iraq War (see the left map in Figure~\ref{fig:introduction:background:mashups}).

\minisec{Geographic Information}
Geographic information is typically represented by discrete and multi-dimensional \emph{geographic objects}. For example, geographic objects can represent roads, buildings, lakes, locations, restaurants and much more. Each object associates a \emph{geometry} (point, line or area), with a \emph{set of attributes} of qualitative, ordered and/or quantitative types~\cite{bertin1967semiologie}: \emph{qualitative attributes}, such as the cousine of a restaurant, allow basic distinction between its possible values. For example, Italian is different from Japanese; \emph{ordered attributes}, such as price category, allows a ranking of its possible values. For example, gourmet restaurants are more expensive than fast-food restaurants; \emph{quantitative attributes}, such as average rating (on a 0-5 scale), allow aritmetic to be performed on their values.

For example, \texttt{Restarant(point, name, cousine, price\_category, average\_rating)} is a six-dimensional geographic object type. Note that the point attribute accounts for two dimensions (longitude and latitude).

\subsection{Effects of Scale Reduction}
A basic challenge in map visualization is how the perceptability of graphics is affected by a reduction in scale. Maps have been produced at many different scales, where the term scale signifies the ratio between distance as it is measured on the map medium and distance as it is measured on the ground. For example, on a 1:500.000 map, one centimeter on the map surface, say the diameter of a city on the map, corresponds to five kilometers on the ground. At the extremes, a map that shows just a single city may be called a large scale map, while a map that depicts the entire world may be called a small scale map. As a graphic is scaled down a number of fundamental effects have a dramatic impact on the visual perceptibility of the graphic.

\minisec{Size and Separation Decreases}
An effect that occurs when a graphic is scaled down is that the \emph{size} of objects in the graphic decreases. Consequently, objects may approach the limit of perceptibility. For example, if an object becomes too small it may become impercievable to the human eye and appear to vanish. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.

As the size of objects decreases, so does the space that \emph{separates} the objects. Consequently, pairs of objects may appear indistinguishable from each other. For example, two symbols that are 0.4 mm apart on a 1:500.000 map will be 0.2 mm apart on a 1:1.000.000 map, which is probably below the limit at which symbols can be distinguished from each other (see Figure~\ref{fig:introduction:background:raw:poi}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.495]{figs-thesis/data-draws-the-world.pdf}
\caption{\textbf{Unfiltered geographic objects}: On a small scale map of Europe, the hundreds of thousands of symbols for unfiltered points of interest from OpenStreetMap are indistinguishable from each other (left). The density of symbols is so high that it entirely covers the physical territories of many national territories. In constrast, the same symbols on a larger scale map are clearly separated from each other (right). It illustrates how for large datasets (thousands of records or more) it generally does not make sense to display unfiltered geographic information on a map.}
\label{fig:introduction:background:raw:poi}
\end{center}
\vspace*{-4ex}
\end{figure}

\minisec{Information Density Increases}
An effect that occurs when a graphic is scaled down is that the \emph{information density} of a graphic increases. Several metrics for information density have been studied, such as Tufte's density metric~\cite{tufte1983visual} along the other density metrics surveyed by Nickerson~\cite{nickerson1994visual}. The \emph{principle of constant information density} states that information density should remain constant across changes in scale, which was formally established by the cartographers T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that maps in published atlasses contain a roughly fixed average number of settlements per a unit area independently of the map scale. The rationalized their observation by observing that a linear reduction in scale implies a reduction in area according to geometrical progression, which must also lead to a reduction in information per unit area according to geometrical progression, if the information is to remain legible.

\subsection{Map Generalization}
A basic challenge in map design, known as map generalization~\cite{shea1989cartographic,weibel1997generalization}, is how to \emph{logically} and \emph{graphically} abstract a set of geographic objects that appear on a map in response to the effects of scale reduction, such that the map remains legible and relevant. A collection of map generalization work has been compiled by Mackaness, Ruas, Sarjakoski~\cite{mackaness2011generalisation}, which may function as an entry point into this complex field.

\minisec{Constraints}
The generalization process has be modelled in different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, quantifiable \emph{map constraints} act as the descriminator between feasible and infeasible maps. In other words, constraints define the limits of for instance perceptibility, density, and consistency~\cite{weibel1999generalising}. As an example, a proximity constraint defines the limit for how close geographic objects (or rather their symbols) may appear on a map in order to be considered \emph{distinct}~\cite{nutanong2012multiresolution}. As another example, a topological constraint may state that geometric objects must retain their topological relationship~\cite{egenhofer1991categorizing,randell1992spatial}, such as \emph{containment}. A concrete example of map constraints are the six design goals that were used to evaluate the iOS 6 Apple Maps API~\cite{samet2012duking}.
...
While constraints can be classified according to many different typologies~\cite{burghardt2007investigations}, a high-level framework categorizes constraints based on the input they apply to: \emph{single object}, \emph{pair of objects}, \emph{set of objects}, and \emph{set of scales} (see Table~\ref{tab:introduction:map:constraints}). While this simple functional classification can be nuanced greatly, which has been done among other by McMaster and Shea~\cite{mcmaster1992generalization}, choosing a semantics-neutral classification avoids taking a stand on issues that are debated lively in the cartographic community~\cite{rieger1993consensus}. Furthermore, it allows constraints to be formulated independently of the computational framework in which they are used.

\begin{table}[htdp]
\caption{A constraint framework based on arity and input type (with examples)}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Constraint input} & \textbf{Examples} \\
\hline
Single object & Minimum dimensions~\cite{ssc1977minimumdimensions}, complexity~\cite{de2004simplifying} \\
\hline
Pairs of objects & Minimum proximity~\cite{nutanong2012multiresolution}, topological relationship~\cite{egenhofer1991categorizing,randell1992spatial} \\
\hline
Sets of objects & Density~\cite{topfer1966principles}, connectivity~\cite{schmid2013opensciencemap,shreve1966statistical}, coverage~\cite{van1995gap}, distribution~\cite{sarma2012fusiontables} \\
\hline
Set of scales & Zoom-consistency~\cite{sarma2012fusiontables,foerster2010challenges} \\
\hline
\end{tabular}
\end{center}
...
\minisec{Objectives}
Quantifiable \emph{map objectives} measure the utility of a map, and therefore allow two (feasible) maps to be compared to each other. For example, an objective may be to maximize the sum of weighted importance (using some weight function) of objects that appear on the map~\cite{sarma2012fusiontables}. 
