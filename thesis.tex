...
For thousands of years, maps have provided humans with the visual means to gain spatial intelligence about the Earth. Maps visualize relevant geographic information in a legible and comprehensive way on a practical medium -- most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. Maps exploit that the visual sense is keen in humans. While other forms of information, such as data tables, demand that information be held in human memory and steadily processed by the brain, maps are graphics that allow information to be understood at a glance~\cite{bertin1967semiologie}. 

Over time, maps have been created for a multitude of reasons. For example, maps have helped seafarers to master the challenge of navigation and helped rulers gain an overview of their territories. Indeed, one of the oldest use cases for maps is the registration of land ownership that is recorded in a cadastre. For example, information about land ownership is etched into a clay tablet that was discovered near present-day Kirkuk, Iraq in 1930. The clay tablet, which is most likely more than four thousand years old, illustrates a plot of land between two rivers together with the owner's name, Azala.

Over time, the capacity of humans to collect and record geographic information has grown explosively. Today, trillions of spatial facts about the Earth and the rest of the Universe have been gathered and stored~\cite{openstreetmap,york2000sloan}. However, while human knowledge has grown unbounded, the capacity for visual perceptive in humans has probably remained stable since the stone age. Consequently, modern map makers must accomodate the same limitations of human visual perception as ancient map makers -- albeit while attempting to communicate vastly larger quantities of information.

\subsection{Maps}
Maps display geographic information that has been projected onto the plane~\cite{snyder1997flattening} and rendered at a certain scale. The concept of map scale has important implications for the amount of detail that a map can convey. For example, large scale maps (e.g. city maps) can display individual buildings, streets, parks and ponds. In contrast, a small scale map, (e.g. a country map), must necessarily omit detail. For example, an entire city and all that it contains may be represented by a single dot on a large scale map or even omitted entirely on an even larger scale map (e.g. a world map). 

The information that appears on maps may correspond to both physical and abstract concepts in the real world, such as trees and administrative borders respectively. Furthermore, each concept in the real world can be viewed both logically (e.g. as a data object) and visually (e.g. as a symbol) in connection with a map. The distinctive characteristics of logical and visual abstractions are covered below.

\minisec{Logical Abstraction}
A typical way to logically represent geographic information is to use discrete data objects (conventionally called \emph{vector} data in geography). Alternatively, geographic information can be represented by a geographic data grid (usually called \emph{raster} data in geography). However, data grids can be represented by data objects that are regularly distributed in space. For this reason, the following assumes that geographic information is logically abstracted by data objects.

Each object logically represents a discrete concept with a spatial dimensions in the real world, such as a road, lake, whale, train line, or municipality. It associates a \emph{geometric} attributes -- such as a point, line, or area -- with a set of additional attributes, which may include \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. For example, geographic information for the restaurant ``Noma'' in Copenhagen can be represented by an object with the following attributes:

\begin{itemize}
\item The geometric attribute $point(\ang{55.6778}, \ang{12.5962})$ corresponds to the geographic location of the restaurant Noma on the harbor front in Copenhagen, Denmark.
\item The qualitative attribute ``Nordic'' correponds to the cousine of the restaurant Noma. Qualitative attributes allow objects to be distinguished from other objects. For example, ``Nordic'' is different from ``Japanese''.
\item The ordered attribute ``Best Restaurant 2014'' corresponds to the ranking in 2014 of restaurant Noma. Ordered attributes allow objects to be ranked against each other.
\item The quantitative attribute ``DKK 1.600'' corresponds to the current price of twenty servings at Noma. Quantitative attributes allow arithmitic to be performed on objects. For example, ``DKK 1.600'' plus ``DKK 1.000''' (the current price of the wine pairing at Noma) is ''DKK 2.600''.
\end{itemize}

When geographic objects are expressed visually, the distinction between these attribute types becomes important, which will be described in the next section.

\minisec{Visual Abstraction}
Visually, geographic information on a map is typically represented by a set of \emph{symbols}. For example, a building object can be represented by a black square symbol, while a road object can be represented by two parallel curve symbols. Depending on their type, object attributes are graphically expressed by one of seven \emph{visual variables}, which were first described by Bertin~\cite{bertin1967semiologie} (see Figure~\ref{fig:introduction:visual:variables}). Different visual variables have an affinity towards certain types of attributes. For example, colors have no inherent order so they express qualitative attributes particularly well (see Figure~\ref{fig:introduction:restaurant:colors}). In contrast, sizes can be compared and measured and therefore express ordered and quantitative attributes particularly well. 
