...
The information on a map can be modelled in two ways, either logically or graphically. While the logical map model defines \emph{what} geographic information should appear on a map, the graphical map model defines \emph{how} the information should appear. The differences and specifics of these two models are covered below.

\minisec{Logical Model}
Geographic information on a map is logically described by a set of \emph{geographic objects}, which represent discrete concepts in the real world, such as roads, lakes, buildings, and subdivisions. Each object associates a \emph{geometry} (point, line or area), with additional \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. Different attribute types permit different distinctions and types of operations to be performed. For example, the restaurant ``Noma'' in Copenhagen can be represented by an object with the following attributes:
...
\item ``Best Restaurant'' is an ordered attribute value of the Noma object, which has the property that it can be \emph{ranked} against other ordered attribute values. For example, the best restaurant is (obviously) better than the second-best restaurant.
\item DKK 1.600 (the current price of twenty servings at Noma) is a quantitative attribute value of the Noma object, which has the property that \emph{arithmetic} can be performed on it. For example, the sum of the DKK 1.600 and DKK 1.000 (the current price of the wine pairing at Noma) is DKK 2.600.
\end{itemize}

The logical state of a map can be altered by performing abstract operations -- i.e. selection, projection and aggregation -- on the geographic objects. Each of the three abstract operations can be implemented by several concrete algorithms:

\begin{itemize}
\item Selection means that a subset of geographic objects are retained in the map. For example, a type of selection is the select-distinct algorithm that selects objects based on their proximity and salience~\cite{nutanong2012multiresolution}, while another is the DFS thinning algorithm that selects objects based on information density and salience~\cite{sarma2012fusiontables}.

\item Projection means that the attributes of geographic objects in the map are modifed. For example, a type of projection is the Douglas-Peucker simplification algorithm that reduces their geometric complexity of objects~\cite{douglas1973algorithms}, while another is the displacement algorithm by Bader that shifts object positions according to a holistic energy minimization criterium~\cite{bader2001energy}.

\item Aggregation means that several geographic objects in the map are merged into one geographic object. For example, the landcover-merge algorithm by Haunert and Wolff aggregates objects by on a holistic optimization process that considers both object areas and object classes~\cite{haunert2006landcover}.
\end{itemize}


\minisec{Graphical Model}
Geographic information on a map is graphically modelled by a set of \emph{symbols} that correspond one-to-one to the geographic objects in the logical model. For example, a building object can be represented by a black square symbol. Object attributes are graphically expressed by seven \emph{visual variables}, which were first described by Bertin~\cite{bertin1967semiologie} (see Figure~\ref{fig:introduction:visual:variables}). Different visual variables have an affinity towards certain types of attributes. For example, colors have no inherent order so they express qualitative attributes particularly well (see Figure~\ref{fig:introduction:restaurant:colors}). In contrast, sizes can be compared and measured and therefore express quantitative and ordered attributes particularly well. 
...
Bertin's work on the theory of graphics is immense, and this short section does not aim to fully cover it. A work that is related to Bertin's, is the ``the Grammar of Graphics'' work by Wilkinson, Wills, Rope, Norton, and Dubbs~\cite{wilkinson2006grammar}. Between them, these works cover the fundamental aspects of representing information as graphics, which includes the representation of geographic information on maps.
