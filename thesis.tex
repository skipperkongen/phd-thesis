...
Two important opposite approaches have been studied for generalization of geographic information, namely \emph{real-time} (or on-the-fly) generalization~\cite{} and \emph{batch-oriented} generalization~\cite{frank1994multiscaletree}. While the requirements for \emph{what} should be the produced are the same for the two approaches, vis-a-vis map requirements and objectives, these two approaches differ in \emph{when} the results are produced. The distinctions as well as the pros and cons of each approach is discussed below.

\minisec{Real-time approach}
Real-time generalization adapts geographical information at the time when a user request for information is recieved~\cite{bereuter2013real,van1995gap}. At first, real-time selection seems like an ideal approach for two reasons. 
...
Typically, an indexing structure, such as a quadtree~\cite{bereuter2013real} or GAP-tree~\cite{van1995gap}, is the factor that facilitates fast real-time generalization. However, there are several issues with real-time generalization in a Web context. While indexing structures enable fast generalization, the complexity of any selection algorithm is high compared to $O(1)$, which is the complexity of retrieving pre-selected data from a key-value store~\cite{pilchin2007dynamo}. Furthermore, the scalability of real-time generalization algorithms is usually only considered for increases in data volume, while it is ignore for increases in concurrency level. This is critical, because in a Web context there is typically a high and variable level of concurrency, which implies that the generalization method must be scalable in the number of concurrent queries. Unfortunately, there is a lack of studies into real-time generalization approaches that efficiently scale in the number of concurrent requests. The na{\"i}ve approach is to replicate the entire index which is costly.


\minisec{Batch-oriented approach}
Batch-oriented selection of geographic information compute materialized multi-scale views over the original state of the data~\cite{sarma2012fusiontables,frank1994multiscaletree,van2011towards}. Subsequently, the selected data can be partioned and replicated on a content delivery network (CDN), which allows immense scalability in the distribution phase of the web mapping pipeline. Of course, this scalability comes at a price, which is primarily composed of increased storage cost, bounded staleness of data, and inability to respond flexibly to user preferences during selection. However, batch-oriented solutions offer a better trade-off than on-the-fly solutions when maps are distributed to large audiences. 

First, while batch-oriented solutions need more storage, disk space have become the cheapest resource to scale in cloud infrastructures. Furthermore, entirely in-memory storage architectures, such as RAMCloud~\cite{ousterhout2011ramcloud}, have become budget feasible and highly resilient to failure, which is a trend that is bound to strengthen over time.



but indexed for fast algorithmic transformation. ince it can respond individually to user requests for maps. 

applies a selection algorithm in response to a user's map request. The advantage is that 
