...
\emph{Legible} maps enable humans to \emph{comprehend} geographic information. One way to ensure a legible map is to \emph{select} data holistically according to constraints and objectives that ensure the feasibility and utility of a map. Thus, maps breed spatial intelligence.
\end{emphpar}
...
Databases with spatial datatypes and functions include single-instance databases~\cite{postgres,mssql,oracle} and distributed databases~\cite{greenplum, vertica}.
...
Scalable spatial analytics systems can be built on top of a scalable and general purpose cluster computing system, such as Apache Hadoop~\cite{apachehadoop} or Apache Spark~\cite{zaharia2010spark}. The advantage is that these systems offer unlimited scalability in storage and compute capacity through a scale-out architecture.

Example of spatial analytics systems on clusters include Hive~\cite{witayangkurn2012spatialhive}, SpatialHadoop~\cite{aji2012hadoopspatial} and HadoopGIS~\cite{aji2013hadoopgis}.


such as Hive with spatial UDFs~\cite{witayangkurn2012spatialhive}, SpatialHadoop~\cite{aji2012hadoopspatial} and HadoopGIS~\cite{},  Mature cluster computing systems are freely available, such as the disk-based Apache Hadoop~\cite{apachehadoop} as well as the in-memory Apache Spark~\cite{zaharia2010spark} systems. A spatial analytics systems is typically implemented on top of cluster systems by developing a set of spatial UDFs in the extension language of the cluster, for example any JVM language for Hadoop. Once the UDFs are in place, a high-level query language, such as HiveQL~\cite{thusoo2010hive}, MapReduce jobs~\cite{}, and Pig Latin~\cite{eldawy2014pigeon}, can be used to execute highly scalable spatial analytics queries over large amounts of data that is stored in the distributed filesystem of the cluster.

Typically, a spatial analytics systems on top of a consiste, such as   have been implementes on top of these systems. 

Programmed as MapReduce jobs~\cite{}, or queries in HiveQL~\cite{witayangkurn2012spatialhive} or other declarative languages, such as Pig Latin with spatial extensions~\cite{eldawy2014pigeon}.

Very flexible, very scalable, query languages does not require knowledge of $n$. However, spatial data is usually not managed in the filesystem of the cluster, so before it can be processed the data needs to be moved from its primary location, which is typically an SDBMS.
