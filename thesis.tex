...
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}.  

\minisec{Visual Cognition of Maps} 
For thousands of years, humans have use their visual senses to comprehend spatial information on geographic maps. While legible maps enable humans to comprehend relevant geographic information in an instant, cluttered maps inhibit any comprehension. The visual cognitive ability of Homo Sapiens, the principal consumer and designer of maps, has probably remained unchanged for thousands of years. Even as the map visualization discipline itself has undergone revolution after revolution, there are important limitations to how the human eye can capture and the human brain can comprehend visual information. Consequently, maps that aim to foster spatial comprehension must be designed with these limitations in mind. While the three dimensions of the image make the visual sense our most powerful channel of perceptions, images have only three dimensions and the consequences of this limitation are important~\cite{bertin1967semiologie}. 

\minisec{Consequences of scale reduction}
A basic challenge in map visualization is how graphics are affected by a reduction in scale. Maps have been produced at many different scales, which the term scale signifies the ratio between distance as it is measured on the map medium and distance as it is measured on the ground. On a 1:500.000 map, one centimeter on the map surface, say the diameter of a city on the map, corresponds to five kilometers on the ground. At the extremes, one map may show just a single city (a large scale map), while another map may depict the entire world (a small scale map). As a graphic is scaled down the visual perceptibility of the graphic is reduced, which is caused by a number of important effects.

One important effect that occurs when a graphic is scaled down is that the \emph{size} and \emph{separation} of objects in the graphic decreases. Consequently, objects may approach the limit of perceptibility and distinguishability. For example, if an object becomes too small it may become impercievable to the human eye and appear to vanish. Correspondingly, if the proximity of objects becomes too close they may become indistinguishable from each other. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent a road, should be separated by at least 0.25 mm. 
