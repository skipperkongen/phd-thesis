...
For thousands of years, humans have visualized geographic information on maps, most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. During those years, maps have helped humans to comprehend a multitude of spatial phenomena. For millennia, maps have helped seafarers to comprehend the dangers that lie beneath the surface of oceans and rulers to comprehend the advances of foreign armies on their territories. For example, a four thousand year old clay tablet, which was discovered near present-day Kirkuk, Iraq in 1930, illustrates the boundaries of a land parcel together with the owners name, Azala. As such, cadastral maps probably constitute one of the earliest forms of data visualization in human history. 
...
\item ``Best Restaurant 2014'' is an ordered attribute value of the Noma object, which has the property that it can be \emph{ranked} against other ordered attribute values. For example, the best restaurant is (obviously) better than the second-best restaurant.
...
When geographic objects are expressed visually, the distinction between these attribute types become important, which will be described in the next section.

\minisec{Graphical Model}
Geographic objects are graphically represented by \emph{symbols}. For example, a building object can be represented by a black square symbol, while a road object can be represented by two parallel curve symbols. Depending on their type, object attributes are graphically expressed by one of seven \emph{visual variables}, which were first described by Bertin~\cite{bertin1967semiologie} (see Figure~\ref{fig:introduction:visual:variables}). Different visual variables have an affinity towards certain types of attributes. For example, colors have no inherent order so they express qualitative attributes particularly well (see Figure~\ref{fig:introduction:restaurant:colors}). In contrast, sizes can be compared and measured and therefore express ordered and quantitative attributes particularly well. 
