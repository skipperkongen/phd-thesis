...
\section{State-of-the-Art Mapping Pipelines}
\label{sec:introduction:web:mapping:pipelines}

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.5]{figs-thesis/mapping-states.pdf}
\caption{\textbf{Stages in a digital mapping pipeline}: data modeling (1) captures reality as a set of information objects (primary data model); data abstraction (2) refines the set of information objects into a logical model of content on a map (logical model); visual abstraction (3) specifies how information objects should be translated into symbols on a map (graphical model); finally, rendering (4) applies the graphical model to the logical model and produces output that is displayed on a map medium.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

The digital mapping process can be modeled as a pipeline that translates between ``reality'' and a graphical output on a digital map medium (see Figure~\ref{fig:introduction:requirements:pipeline}). Instead of leaping directly from reality to graphical output, it is useful to model information using some intermediate stages. A common approach is to model information at three intermediate stages: the \emph{primary data model}, the \emph{logical model} and the \emph{graphical model} of the digital map~\cite{gruenreich1992atkis}. To produce a high-quality map, it is often necessary to iterate back and forth between intermediate stages in the pipeline. For example, choosing symbols of larger size in the graphical model means that symbols are more likely to overlap. This in turn may trigger a renewal of the logical model, e.g. by selecting fewer objects to avoid symbol overlap.

The data modeling process is outside the scope of this thesis. It has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. Also, the rendering process is outside the scope of this thesis. Moreover, the rendering process depends to great extent on the map medium of choice. The intermediate stages, i.e. the logical and graphical model, were described in Section~\ref{sec:introduction:map:generalization}.

It is crucial that we understand the requirements of a digital mapping pipeline, especially since the Web has dramatically impacted what these requirements are (as mentioned in Section~\ref{sec:impact:of:the:web}). In a web scenario, the importance of map creation is only as important as map distribution: there is little point in creating a beautiful map if it does not reach the intended audience! Therefore, the evaluation of a mapping pipeline should take into account how map content is distributed to audiences.

A key distinction between state-of-the-art mapping pipeline is whether the map creation process is synchronous or asynchronous in relation to the map distribution process. This distinction is represented by two extreme approaches: real-time systems and batch-oriented systems. In the following sections, the advantages and drawbacks of both approaches are analyzed and discussed.

\subsection{Real-time Map Systems}
In real-time systems, the map creation process is synchronous with the map distribution process: the map is created on-the-fly when a map request is received by the system. For example, in systems that implement the Web Mapping Service (WMS) protocol, a map request typically invokes the entire mapping pipeline from the primary data model, through data abstraction, visual abstraction, and rendering.

In the academic literature, real-time systems have been studied because they have some important advantages:  

\begin{itemize}
\item Data is only stored only once, namely in its original state. While this obviously saves storage, a more important benefit is that information that appears on a map is never stale. Given an atomic update mechanism for the data model, such as transactions~\cite{gray1981transaction}, a map always displays the most recent and consistent state of the data. However, this is only true if the atomic update propagates to the index as well, which is a significant consideration given that data is typically managed outside the map generalization framework.
...
Real-time mapping relies on low-latency data abstraction, since the map creation process is synchronous with the map distribution process. Low latency is typically ensured by using an efficient data structure to index the geographic information. Typically, the proposed data structure will be some kind of tree, such as a quadtree~\cite{bereuter2013real} or GAP-tree~\cite{van1995gap}.

While tree-based real-time mapping seemingly ensure low-latency data abstraction, there are two key disadvantages when compared to serving pre-computed content from a key-value stores or content delivery network (CDN). First, the complexity of any tree-based access method is high compared to the $O(1)$ of looking pre-computed content up in a hash-based key-value store~\cite{pilchin2007dynamo}. Second, the proposed tree data structures are typically not designed for concurrent access. This is critical, because there is typically a high and variable level of concurrency in a web context, which implies that the data abstraction process must be scalable in the number of concurrent queries as well as data volume. The na{\"i}ve approach, which is to replicate the entire index multiple times and dispatch requests round-robin is costly, especially when data grows in size.

In conclusion, current real-time map systems are inappropriate for maps that target large web audiences, since their base performance is significantly lower than key-value store and they do not scale with concurrent users.

\subsection{Batch-oriented Map Systems}
Batch-oriented selection of geographic information compute materialized multi-scale views over the original state of the data. For example, multi-scale trees~\cite{frank1994multiscaletree,van2011towards} and solutions to integer programming instances~\cite{sarma2012fusiontables} can be used to represent data at multiple scales. There are two obvious advantages to a batch-oriented approach.

First, since there is no real-time requirement, more computational effort can be spent on producing a high-quality result. Distributed data analytical systems, such as spatial parallel databases and spatial cluster computing~\cite{eldawy2014spatialhadoop,aji2013hadoopgis}, can be wielded to make the process highly scalable with regard to data volume. For example, while highly scalable batch-oriented systems, such as Apache Hadoop or Apache Spark, cannot be used for real-time generalization, they are intrinsically appropriate for batch-oriented generalization of geographic information.

Second, materialized multi-scale views can be easily partioned and replicated on a content delivery network (CDN), which facilitates immense scalability in the distribution phase of the web mapping pipeline. Failure to scale in concurrent requests is a make-or-break requirement of web mapping pipelines. For this reason, a batch-oriented approach is the only viable option for Web mapping, at least until better distributed methods for real-time generalization have been discovered.

There are three drawbacks of a batch-oriented approach that are quite obvious. However, they are also pratically negligle in a Web context compared to the drawbacks of real-time generalization. The first issue has to do with storage cost. While materialized views incur an increased storage cost, disk space have become perhaps the cheapest resource to scale in cloud infrastructures. Moreover, in-memory storage systems, such as RAMCloud~\cite{ousterhout2011ramcloud}, show the way to cheap, highly performant and highly fault tolerant memory-backed storage, which is a trend that is bound to strengthen over time. The second issue has to do with staleness, which is also arguably neglible. While materialized views are potentially stale the very instant they have been produced, they can also be refreshed quite fast as demand dictates, since highly scalable data processing systems can be used in batch-oriented generalization. The third issue is more critical, namely that batch-oriented generalization cannot take individual user preferences into account, since they are geneally not know at processing time. A suboptimal work around is to materialize views for several different user profiles and choose between them at query time.

In conclusion, batch-oriented generalization is the only viable option for scalable web mapping pipelines. While batch-oriented generalization suffers from one significant drawback,  namely the difficulty of adapting content to individual users, this drawback must be tolerated since it seems that there is currently no better alternative.

%\subsection{Example: How to Distribute Maps Using Tiles}
%A method to achieve scalable maps is to partition the geographic data over a fixed multi-resolution geographic grid. Each cell, or \emph{tile}, in the grid contains a representation of data that is intersected by the tile. Once the tiles have been created, a content delivery network (CDN) can store and distribute the tiles to mapping clients with high scalability, availability and performance~\cite{dilley2002cdn}. An advantage of tiles is that they can be addressed using discrete and three-dimensional cell coordinates. For this reason, tiles can be accessed in a key-value query model. There are two types of geographic data, and therefore two types of tiles: \emph{raster tiles} that contain graphics and \emph{vector tiles} that contain information or data. Below, the pros and cons of each type of tile is discussed.

%\minisec{Raster Tiles}

%\minisec{Vector Tiles}
%A method to achieve scalable maps is to partition the geographic data over a fixed multi-resolution geographic grid. Each cell, or \emph{tile}, in the grid contains a representation of data that is intersected by the tile: either a list of identifiers or the data objects themselves. Tiled vector data can be distributed to mapping clients, and only then will it be graphically expressed. As such, distribution of tiled vector data corresponds to distribution of a \emph{logical map}, i.e. \emph{what} information the map contains, not \emph{how} the information is graphically expressed.

%~\cite{gaffuri12vectortiles}

%There are several advantages to tiled vector data and logical maps. First, distribution of logical maps allows many degrees of freedom with regard to the graphic expression of the map. For example, clients may choose the \emph{size}, \emph{value}, \emph{texture}, \emph{color}, \emph{orientation}, and \emph{shape} of information symbols such that the efficiency of graphic communication is maximized~\cite{bertin1967semiologie}. Second, content delivery network (CDN) can store  tiled vector data for highly scalabe, available and performant distribution~\cite{dilley2002cdn}. Third, each mapping client that retrieves data from the CDN, essentially contributes a CPU to the mapping pipeline, which is utilized to render information as graphics.

%Since small scale maps generally intersect larger quantities of geographic objects than large scale maps, there is a need to bound the amount of data in each tiles. Otherwise, small scale tiles would be very larger and take much longer to transmit over the Internet than large scale tiles. To keep the amount of information in each tile constant independently of the scale, the vector data must be generalized. A batch-oriented approach to generalization of content for vector tiles, is particularly useful for scalable web maps. This content generalization and the distribution steps for vector tiles is schematized in Figure~\ref{fig:introduction:approaches:vector:tiles}.

%\begin{figure}[htbp]
%\begin{center}
%\includegraphics[scale=.45]{figs-thesis/batch-oriented-poi.pdf}
%\caption{\textbf{Creation and distribution of tiled vector data}: a generalization algorithm selects geographic information for each scale (1); a multi-scale tiling scheme partitions information over variable resolution grids (2); multi-scale tiles are stored on a CDN (3); mapping clients pull data from CDN (4). Tiles contain vector data (A), or identifiers of intersected data (B).}
%\label{fig:introduction:approaches:vector:tiles}
%\end{center}
%\end{figure}
