...
Digital map visualization studies how to visually represent the geography of the Earth at multiple scales on restricted digital mediums, such as computer screens. On the web, geographical mash-up is the process by which designers of maps will layer new and interesting geographical data on top of commonly shared background maps. While the background maps are generally of high-quality, the result of many hours of expert labor, new and interesting data presents a raw challenge for creators of mash-ups. Simply displaying symbols for raw geographical data on top of a background map, without appropriate abstraction, will work only for the tiniest datasets (tens of records)~\cite{jones, weibel1999generalising}. One of the most fundamental problems in digital map creation is the mismatch between data volume and the small representational budget of computer screens. For example, a laptop display (e.g. 2880 x 1800 pixels) can display just a few thousand symbols without overlap, which is entirely indadequate for displaying individual records for most geographical datasets, i.e. datasets of more than a hundred spatially skewed records. Necessarily, geographical data must be abstracted to scale, for example by selection or aggregation. To create an useful and interactive map, geographical data must be \emph{holistically selected} according to criteria that maximize relevance and minimize visual conflict for the whole map.
