...
\emph{What are the challenges?} Visualization designers must address the crucial problem of data abstraction when creating high-quality visual representations of large and possibly heterogenous datasets. As the data grows in size, the finite resources available to visually represent the data, such as screen real-estate and memory, are quickly depleted~\cite{topfer:selection, weibel, stonebraker}. At this point, the data must be abstracted to fit the media, e.g. by selecting a representative subset or aggregating the data. However, designers can not manually abstract the data as it is too time consuming. Thus, the data abstraction process must be at least partially automated. Some crucial questions are: what is the ideal data abstraction for a given visualization, and how can designers control a partially automated data abstraction process to achieve a desired result? 

\emph{What are the challenges?} Given an appropriately abstracted dataset, system operators must ensure that the data is delivered to applications with low latency and high availability. A crucial problem is to operate a system that reaches these goals while minimizing the cost. Data providers must cost-efficiently operate services for data visualizations with low latency and 24 x 7 availability. \emph{at this point, we should know that data-driven applications require both availability/performance and visual quality, and that it is not comletely straight forward}.
...
\emph{What is the state of the art?} To address these crucial problems, the research community have worked on many useful techniques. First, the data abstraction problem has been the subject of research for many years (in the case of cartography, thousands!) and in many areas, e.g. map generalization~\cite{weibel}, visual analytics~\cite{see:db} and information graphs~\cite{metro:stops}. Second, the distributed systems and database communities has done extensive work on the fields of data partitioning and data replication, which are techniques for making access to data highly available, performant, and scalable~\cite{datacenter:as:a:computer}. The trade-offs involved are well-known, and have been the subject of important studies~\cite{danger:of:replication, cap:theorem}. Additionally, data partitioning schemes have been specifically developed for data that is visualized, e.g. hierarchical linerization of spatial data~\cite{hilbert:curve, z:curve, xyz:protocol}.  Third, Key-Value stores (K-V stores) with inherent support for data partitioning, data replication and ``scale out'' have been popularized by internet giants like Google, Amazon and Facebook~\cite{dynamodb, bigtable, haystack}. K-V stores feature a schema-less data model that is appropriate for data that is visualized (e.g. objects and multimedia data). However, the advantages of K-V stores are offset by severe limitations: they generally lack powerful query languages like SQL. Hence, they are not a good fit for computing data abstractions. In constrast, relational database systems (RDBMS) have the power to express the complex selection and aggregation queries needed for data abstraction. However, data abstraction queries are generally not easy to express in pure SQL, as data abstraction is a holistic process that generally considers the whole dataset. It seems that the ideal system to power data-driven visualizations would have the scalability of a K-V store (or better) and the expressibility of a RDBMS (or better). In this thesis we focus on developing such a system.
...
Outside geography, research on data abstraction is still in its infancy. As a result, most of the related work focuses on geographical data. There is an opportunitiy to transfer ideas from map generalization to visualization of general relational data. i.e. multi-scale data abstraction for tables. This thesis focuses on building such a system. The list below enumerates some important problems that need solving to realize this opportunity.

\begin{enumerate}
\item Data Abstraction Algebra
\item Programming Languages
\item RDBMS Integration
\item Adaptive Materialization
\end{enumerate}
...
\subsection{Adaptive Materialization} Using a log of previous requests, predict the requests that will be seen tomorrow. Account for trends, seasonal variation and toblers first law.
