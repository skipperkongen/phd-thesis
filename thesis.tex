...
For thousands of years, humans have visualized geographic information on maps, most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. During those years, maps have been created for a multitude of reasons. They have helped seafarers to navigate the oceans and governments to track who owned the land. For example, a more than four thousand year old clay tablet that was found in 1930 inside contemporary Iraq, depicts the boundaries of a land parcel together with the owners name, Azala. This probably constitutes one of the earliest data visualizations in human history. 

Over time, human knowledge of map making and capacity for geographic information gathering have evolved hand in hand. All the while, Homo Sapiens, the principal maker and consumer of maps has probably remained biologically unchanged since since the stone age. Consequently, modern map makers must accomodate the same limitations of human visual perception as ancient map makers.
...
Different visual effects occur as a map is scaled down  (see Figure~\ref{fig:introduction:background:raw:poi}). One effect is that the size of symbols -- if they are shown to scale -- decreases. For example, if a building symbol is 2 mm wide on a 1:500.000 map it will be 1 mm wide on a 1:1.000.000 map. Consequently, map symbols may approach the limit of perceptibility. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be at least 0.35 mm in length. while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}. A generalization technique that deals with diminutive symbols sizes is \emph{exaggeration}.

Another effect is that the separation between symbols -- if they are shown at accurate positions -- decreases. Consequently, symbols may come so close to each other as to become fused. For example, two symbols that are 0.4 mm apart on a 1:500.000 map will be 0.2 mm apart on a 1:1.000.000 map, which is below the limit at which symbols can be distinguished from each other. Two generalization techniques that deal with symbols in close proximity are \emph{displacement} and \emph{selection}.
...
A map that shows just a single city may be called a large-scale map, while a map that depicts the entire world may be called a small-scale map. A number of fundamental effects occur as a graphic, such as a map, is scaled down.


% Since ancient times, the requirements for this process have been to accurately and legibly} represent as much of the relevant information as possible on restricted map mediums~\cite{jones2005webage}. 


\minisec{Map Visualization}
 This implies a holistic reduction in information, which is frequently an NP-hard problem~\cite{sarma2012fusiontables,haunert2010area}.




\subsection{Multi-scale Maps}
Multi-scale maps allow users to zoom the view in and out (change scale), as well as to pan the focal point of the map. Each scale of the map shows geographic information at a different level of detail. The state of a map is divided between two models, the \emph{logical model} and the \emph{graphical model}. While the logical map model defines \emph{what} geographic information appears on a map at each scale, the graphical map model defines \emph{how} information appears on a map at each scale. The differences and specifics of these two models are covered below.

\minisec{Logical Model}
Geographic information on a map is logically represented by a set of \emph{geographic objects}. Each object associates a \emph{geometry} (point, line or area), with additional \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. For example:
...
The geographic objects constitute the logical state of a map. Consequently, the logical state can be altered by performing operations on the geographic objects. At an abstract level the operations are:

\begin{itemize}
\item \textbf{Selection}: Subsets of geographic objects can be omitted or kept according to criteria
\item \textbf{Projection}: The attributes of geographic objects are modifed
\item \textbf{Aggregation}: Several geographic objects are merged into one geographic object
\end{itemize}

Each of the abstract operations -- selection, projection, and aggregation -- can be implemented by several concrete operators. For example, \emph{select-distinct}~\cite{nutanong2012multiresolution} is a selection that filters objects according to their proximity; \emph{simplification}~\cite{douglas1973algorithms} is a projection that reduces the complexity of geometries; \emph{landcover-merging}~\cite{haunert2006landcover} is an aggregation that merges objects based on the area of their geometries.

\minisec{Graphical Model}
Geographic information on a map is graphically represented by a set of \emph{symbols}, with a one-to-one correspondence with geographic objects in the logical model. Seven \emph{visual variables}, which were first described by Bertin~\cite{bertin1967semiologie}, visually express the attributes of geographic objects (see Figure~\ref{fig:introduction:visual:variables}). Certain visual variables have an affinity towards certain types of attributes. For example, as colors have no inherent order they express qualitative attributes best (see Figure~\cite{fig:introduction:restaurant:colors}). In contrast, size can be compared and measured, and therefore express quantitative and ordered attributes best. 

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.72]{figs-thesis/visual-variables.pdf}
\caption{\textbf{Bertin's Visual Variables}: The visual variables, of which Bertin identified seven, can express qualitative, ordered and quantitative attributes of geographic objects. For example, color can express different categories of restaurnants. The figure is reproduced from \texttt{infovis-wiki.net}.}
...
A basic challenge in map design is how to derive a \emph{logical} and \emph{graphical} map state after a scale reduction, such that the map remains legible and relevant. This process is known as \emph{generalization}~\cite{mcmaster1992generalization,weibel1997generalization}. A collection of studies in generalization has been compiled by Mackaness, Ruas, Sarjakoski~\cite{mackaness2011generalisation}, which may function as possible entry point into this complex field.

The generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, \emph{constraints} act as the descriminator between feasible and infeasible map states~\cite{harrie1999constraint,ware2003generalization_sa}.
