...
Map visualization is a process that displays geographic information, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}. It is a challenging process, since opposing goals must balanced in order to achieve an optimal map. For example, the requirement that a map must be legible is at odds with the goal that a map should represent as much as possible the available information.

Geographic information is typically represented by discrete \emph{geographic objects}. Each object associates a \emph{geometry} (point, line or area), with a \emph{set of attributes} (qualitative, ordered and/or quantitative)~\cite{bertin1967semiologie}. For example, a restaurant may be represented by a geographic object, which associates the location of the restaurant (a point) with attributes such as name and cousine (qualitative component), price category (ordered component), and average rating in reviews (quantitative component).

Geographic object are graphically represented on a map by choosing appropriate \emph{visual variables}, which include \emph{position},  \emph{size}, \emph{value}, \emph{texture}, \emph{color}, \emph{orientation}, and \emph{shape}~\cite{bertin1967semiologie}. The position, which is a two-dimensional variable, is usually reserved for the coordinates of the geometry, which are first projected onto the plane using a standard map projection~\cite{}. It should be noted that the position could perfectly well represent other attributes, such as the average life expectancy and economic wealth of a country~\cite{rosling2007poverty}. Normally, one or two other attributes are represented by one visual variable each. However, different visual variables are most appropriate for certain types of attributes. For example, colors do not have an intrinsic order or value, and therefore intuitively express qualitative attributes, such as the cousine of a restaurant. In contrast, size is quantitative and consequently also ordered, which means that it intuitively expresses both quantitative and ordered attributes, such as a restaurant's average rating in reviews or its price category.

\minisec{Visual Perception of Maps} 
For thousands of years, humans have used their visual senses to comprehend information on geographic maps, from clay tablets~\cite{fathulla2007frameworks} to maps on computer screens. Humans have learned that a legible map enables instant comprehension of relevant geographic information, while a cluttered map inhibits such comprehension. Consequently, the map visualization discipline itself has witness epitomy after epitomy, and is today as much an art as it is a science. During these years of technological and aesthetic improments, the visual cognitive capacity of Homo Sapiens, the principal consumer and designer of maps, has probably remained unchanged. Indeed, there are fundamental limitations to how the human eye can capture and the human brain can comprehend visual information presented in a graphic~\cite{bertin1967semiologie}. 
