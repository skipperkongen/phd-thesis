...
1: The web today is home to an eco-system of providers and consumers who create, combine and visualize a huge diversity of open data. In symphony, open data covers the full spectrum of human interests, e.g. culture~\cite{rapgenius,wikipedia}, economics~\cite{worldbank}, geography~\cite{openstreetmap,danish:geodata:agency}, demography~\cite{unstats} and politics~\cite{data:gov, data:co:uk}. Indeed, the value proposition of open data is clear: open data has the potential to increase transparency, efficiency and enlightenment in our societies~\cite{hans:rosling, offentlige:data:i:spil}. Consequently, the availability of open data has created a great demand for online data-driven visualizations, i.e. applications that allow online users to understand and (sometimes) take action on the data by visual and interactive means~\cite{google:maps, nytimes, guardian}. For example, The New York Times and The Guardian rutinely feature data-driven visualizations as part of the news content that they provide~\cite{guardian:wikileaks, nytimes:obama:budget}. 

There are several crucial problems that must be addressed in order to operate a successful web application for data-driven visualization. The first problem is the design effort required to provide a high-quality visual experience, i.e. an uncluttered and informative presentation of the data (see Figure~\ref{fig:introduction:cluttered}). The second problem is the cost to run a data-driven service with 24 x 7 availability and fast loading times. The third problem that must be addresses is the scalability of services that suddenly become popular (see Figure~\ref{fig:introduction:latency:with:workload}). While devices in our pockets can reach any point in the world of open data at the speed of light, data can take a very long time to escape the black hole of an overloaded endpoint.


\emph{Talk about data APIs. Talk about user expectations: visual requirements + performance requirements. Maybe support visual requirements by the controls of generalization.}




What is needed now is to transform that thing we carry in our pockets into a powerful and visual sense making tool. As with any other application on the web, users expect data-driven visualizations to have 24 x 7 availability,   performance of a data-driven visualization.
