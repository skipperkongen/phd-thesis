...
\section{A View of Geospatial Data Systems}
...
% Separate use cases: data visualization (DATAVIZ), data creation (OLTP) and data analysis (OLAP)
% database visualization, journalistic visualization, 
% non-experts are increasingly working with data

\subsection{The Online Eco-system for Geospatial Data}
...
\subsection{Requirements for Future Systems}
...
All maps are multi-scale, with either discrete or continuous zoom-levels. All maps are multi-layer, each layer has a type and a map can mix layers of different types. 

To make things easier, we might assume that the world is flat?
...
%We refer to a unit of information in geospatial data as a geospatial datum. Each geospatial datum is conceptually embedded in a four-dimensional space with dimensions: \emph{longitude}, \emph{latitude}, \emph{altitude} and \emph{time}. Each geospatial datum states information about the spatio-temporal subset it covers.

%Because because the dimensions have fixed semantics: \emph{longitude}, \emph{latitude}, \emph{altitude} and \emph{time}.

%because the dimensions have concrete semantics.

%More formally, a geospatial datum (an image or an object) associates data with a spatio-temporal subset, i.e. a subset of a metric space with four dimensions:  \emph{longitude}, \emph{latitude}, \emph{altitude}, \emph{time}. While the image-based data model adopts a discrete view of space-time, the object-based model treats space-time as continuous.


%While both images and objects model spatio-temporal entities by associating data tuples with spatio-temporal subsets, the mechanism of association is different for the two.
...
%Each unit of information in geospatial data is conceptually embedded in a four-dimensional space with dimensions: \emph{longitude}, \emph{latitude}, \emph{altitude}, \emph{time}. Each geospatial datum lists information about the spatio-temporal subset it covers.


%In other words, a geospatial subsets of a four-dimensional \emph{metric space} corresponding to the physical Earth plus time. .

%Images use pixel values to represent discrete subsets of space and time. 

%For example... and for example...



Images store data in a matrix of pixels. Each pixel value is interpreted as either a color or as data. The outline of an image is mapped to a region on Earth.

in the matrix corresponds to a rectangular region using a matrix $A$ of cells (or pixels). Each cell, indexed by $A_{i,j}$, contains a data tuple that represents the region that is covered by the cell.

where each cell represents a discrete region of space-time by a pixel value (possibly multi-dimensional). rectangular region formed by a regular subdivision of space and time. An index $A_{i,j}$ points to a data tuple that represents the corresponding region. For example, an image can represent the elevation of the Earth's surface, by choosing a regular subdivision of earth.


For geospatial data, the four dimensions are continuous and have fixed semantics: \emph{longitude}, \emph{latitude}, \emph{altitude} and \emph{time} -- which makes geospatial data a special case of multi-dimensional data. For any dimension, the subset may extend infinitely in the positive or negative direction (or both). In summary, we can use the simple technique of associating data tuples with a spatio-temporal subsets to represent any entity, physical or abstract, that has an extent in 3D space and period of existence of over time.

% \emph{polyhedral} (actually only in the projected space), (conventionally), polyhedral (actually only in the projected space) 

By polyhedral subset we specifically mean 
%For example, images can be used to represent the elevation of the Earth's surface by letting each pixel represent a subdividing the surface into a set of rectangles and setting pixel values to the average altitude of rectangular regions.

 Objects can be used to model images, using a separate object for each cell or pixel in the image.  

models are image-based data (also called raster data) and object-based data (also called vector data). Each unit of information in geospatial data is conceptually embedded in a four-dimensional space with dimensions: \emph{longitude}, \emph{latitude}, \emph{altitude}, \emph{time}. To ease the display of information on a computer screen, geospatial data are \emph{projected} onto a two-dimensional plane. Furthermore, data can be selected on one or several dimensions, e.g. only show data for a particular altitude or time. Each unit states a set of facts about a given subset of four-dimensional space. Typical subsets are points, lines, polygons, surfaces, and volumes.

The atomic unit of information in the image-based data model is a cell (or pixel), which associates an index $M_{i,j}$ in a matrix $M$ with a data tuple, e.g. the two-tuple $\langle A, 42\rangle$. The atomic unit of information in the object-based data modelAn image consists of cells in a matrix. and each cell is associated with one or more values (of arbitrary data type). Objects consist of 

\subsection{Data Structures and Indexing}
% linearization, quad-trees, r-trees, 

\subsection{Query Models}


\subsection{Design Principles for Geographical Maps}

\subsection{System Architectures}
Client-server model. Data can be streamed continuously or requested once.

% review various pipelines for maps. This motivates 
Map production and map consumption.
