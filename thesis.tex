...
% Background
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}. Map visualization is fundamentally a ``big data'' challenge, since amount of geographic information usually exceed the representational budget of the medium. Therefore, the geographic information must be adapted to fit the budget, which is a fundamentally hard problem~\cite{weibel1997generalization}.

% The changes and challenges caused by the web.
The Web has structurally impacted \emph{how}, \emph{when}, \emph{where} and by \emph{whom} maps are produced and consumed. Consequently, new challenges of map creation and map distribution have emerged, which can not be adequately addressed by traditional geographic information systems (GIS) alone. 

The Web has resulted in a shift in the types of maps that need to be created, which is caused by two root factors. First, there is an abundant supply of background maps online -- provided commercially~\cite{google2014maps,stamen2014tiles,vizzuality2012cartodb} and by national mapping agencies~\cite{gst2014digitalmapsupply} -- which means that there is less of a need for the creation of these types of maps. Second, there is an great supply of geographic datasets online -- both curated~\cite{mst2014miljo}, user-contributed~\cite{twitter2014twitter,zooniverse2014oldweather}, and machine generated~\cite{planetlabs2014flock1} -- which grow by millions of information objects each day~\cite{openstreetmap2014stats}. The combination of these two factors means that demand has shifted towards the creation of thematic maps. For example, journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} create valuable thematic maps that increase transparency and can even save lifes. However, the visualization of vast geographic information repositories on maps calls for scalable processing methodologies that can handle the volume.

The Web has also caused a change in the people who create maps. While map creation was traditionally the domain of cartographic experts, the demand for thematic maps on the web has grown faster than the number of cartographic experts who can create them. This has created a human resources challenge, which is being addressed by the development of more intuitive and expressive design interfaces for map makers. The need for rapid thematic mapping has driven the development of fully-automatic mapping systems~\cite{sarma2012fusiontables,nutanong2012multiresolution}. However, these systems have completely sacrificed expressibility in order to gain productivity. Arguably, a fully-automatic mapping system is likely to be suboptimal for most cases. The alternative is to use traditional mapping systems, which were chiefly created for the production of complex maps of the Earth~\cite{lamy1999application,ware2003generalization_sa}. They are certainly powerful enough. However, these systems are hard and time-consuming to use, which in turn makes them costly.

The Web has finally caused a change in how maps reach their audience. Today, a map can be consumed by anyone, anywhere, at any time. Many online map are consequently consumed by millions of concurrent users. Web users today expect 24 x 7 availability and acceptable latency for online applications, which included maps. Maintaining a high service level is costly, which has driven the development of lower cost solutions. Tiling schemes that break up the map into raster~\cite{mbtile12} or vector tiles~\cite{gaffuri12vectortiles} have emerged as a solution to the availability and performance challenge. This works because discrete and uniquely identifiable tiles can be distributed on content delivery networks, which exploit the fundamental principles of data partitioning and data replication. However, the number of tiles in a tile pyramid grow exponentially in the number of zoom levels that a map supports. Consequently, tiles require a substantial amount of resources to materialize. While parallel processing, such as MapReduce can materialize a set of tiles in a short amount of time, the computation and I/O involved with the processing is fundamentally costly. What is needed is methods that have the benefits of data partitioning and data replication, while avoiding the cost of full materialization.
...
Finally, it is informative to review the six design goals that were stated for the iOS6 Apple Maps API: (1) minimize overlaps, (2) respect relative importance of entries, (3) maximize spatial fullness, (4) provide panning/zooming consistency, (5) enable efficient sampling, and (6) support filtering conditions.


\subsection{Stage Four: Content distribution} In stage four, the map content is distributed over the Internet to web and mobile mapping clients. A service level aggrements (SLA) establishes the absolute conditions that must be met by a map distribution:

\begin{itemize}
\item \textbf{High availability}: map content must be highly available, which can be expressed in terms of so-called ``nines''. For example, ``two nines'' equals 99\% availability or 1.68 hours of downtime per week, while ``five nines'' equals 99.999\% availability or 6.05 seconds of downtime per week. In the EU, national mapping agencies are contractly obligated to met ``two nines'' of availability~\cite{gst2014digitalmapsupply}. In contrast, private corporations -- such as Google and Microsoft -- rutinely operate at ``four nines'' or ``five nines''. As online map users have come to expect the relative high availability of commecial map distribution, national mapping agencies are presented with a challenge. 

\item \textbf{High scalability}: map content distribution must be highly scalable, that is, acceptable latency must be sustained for an increasing number of concurrent clients.
 \end{itemize}
...
% describe and identify
Selection of map content can be modelled as a combinatorial problem, which can be addressed by integer programming~\cite{haunert2006landcover,sarma2012fusiontables}. Na{\"i}vely, integer programs model the selection of information by binary decision variables, one for each object that may appear in the map. The decision variables appear in the objective function and in a set of inequalities. For this reason, the concrete data objects must be known in advance, since they are explicitly referenced by the integer program.
% discuss
...
Key-value paradigm. Data partitioning, data replication, map reduce (dean lessons)
