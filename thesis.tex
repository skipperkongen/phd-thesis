...
\section{Motivation}
Web-based multiscale visualizations allow users to explore large datasets on the go, using heterogenous devices (smartphones, tablets, personal computers) and in different life situations (at home, at the office, on the bus).  Specifically, discrete datasets can be explored at different focal points and at several levels of data- and visual abstraction.

Driven by the exponential growth in online data, multiscale visualizations have recently attracted much larger audiences and must integrate much larger and constantly evolving repositories of data. This poses two key challenges to designers and engineers. On one hand, visualization designers must rapidly address the dual problems of \emph{data abstraction} (e.g. selection and aggregation of information) and \emph{visual abstraction} (e.g. choice of styles and layouts)~\cite{stolte2003multiscale}. On the other hand, engineers must cost-effectively manage a brutal query workload as users connect to visualizations running in a distributed system.

Online visualizations are important because they allow professional and private users to interact with information on the go (on multiple devices and in different situations). facilitating data insights and supporting decision making -- e.g. using a plot to understand complex financial data or using a map to find a restaurant to have dinner at. 
