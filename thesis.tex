...
The following sections describe four gaps that that will be addressed by this thesis.
...
\item \textbf{Tileheat experimental evaluation}: TODO. We evaluate the Tileheat algorithm using map request log files from the Danish Geodata Agency. The results show that Tileheat greatly improves on the method currently used by the agency. The details about this part are described in Chapter~\ref{chapter:tileheat}
...
Tileheat has been tested with the workload of a background map. Experiments show that the skew that is exhibited by this workload is highly stable over time. That is, the map sections that are most requested coincide to a large degree with densely populated areas (by visual inspection of the heatmaps). Tileheat has not been evaluated for maps that exhibit more dynamic skew, i.e. where the focal point of hot spots changes quickly over time. We expect Tileheat to perform badly in this case, but this would have to be evaluated experimentally to be sure. Since Tileheat uses exponential smoothing with trend detection, there is a possibility that a shift in the area that is most requested on a map could be quickly detected. However, the shift can only be detected if it is present in the log. Thus, experimenting with shorter cycles could be an area for future work.

\section{Conclusion}
TODO
This thesis has developed new theory and performed experimental evaluations for the following subject areas: (a) Selection for maps and data visualizations. (b) Request prediction for maps. The general approach has been to investigate batch-oriented systems. Specifically, the design goals were to contribute systems that can:

\begin{itemize}
\item Process large amounts of data (millions of objects) for map and data visualizations.
\item Allow relatively novice designers or programmers to time-efficiently specify and compute data selection for complex, multi-scale data visualizations (minutes or hours end-to-end).
\item Support high availability and low latency serving of map visualizations.
\item Scale map visualizations to large numbers of concurrent consumers (millions).
\end{itemize}

\subsection{Strategies Used}
The main strategy used in the selection study was to move programs to the data instead of vice versa. Today, data for visualizations is often stored in relational database management systems (RDBMS). Therefore, a natural idea is to generate SQL statements that compute holistic selection entirely inside the database. Furthermore, SQL seems to be the lingua franca query language for the new class of emerging and highly scalable data processing systems, spearheaded by as Apache Hadoop projects such as Hive, Drill, and Spark.

The main strategy used for the map serving study was to allow map serving systems to learn from their own access log. While an entire map can be materialized and place in fast-access storage, it is costly to do so. By learning the hot data from an access log, caches can be seeded with just the hot set potentially hours ahead of peak load. 
