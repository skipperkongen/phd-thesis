...
% The data eco-system (data and visualizations) and requirements for high availability, high performance and scalability.
The web is home to an eco-system of data providers and data consumers who exchange and combine, e.g., geographical, temporal and socio-economical datasets~\cite{wikipedia,worldbank,unstats,data:gov, data:co:uk,gst,openstreetmap}. Open data has become a commodity that is freely available online. Indeed, the value proposition of open data is clear: open data contributes to the transparency, progress and enlightenment of our societies~\cite{hans:rosling, offentlige:data:i:spil}. Commoditized data drives the demand for data-driven visualizations that are beginning to realize the potential of our open data~\cite{usecase1, usecase2, usecase3}. In turn, the momentum of data-driven visualizations drives the demand for more open data in an upwards spiral. In anticipation of this demand, most large web sites and organizations (e.g. Wikipedia and the United Nations) offer data-only APIs that are designed to meet this demand~\cite{wikipediaapi,unapi}. 
...
\chapter{Towards a Unified Data Abstraction Theory}
\label{chapter:data:abstraction:survey}

\section{Introduction}

What are the important areas where data abstraction is applied? Any area that has recorded data and where the concept of generalization can help us to get an overview: History, Geography, Culture, Economics, Politics, Databases, etc.

What is data abstraction? Data abstraction is the art of transforming a dataset into a useful ``sketch''. While it is unimportant how the sketch should be interpreted (e.g. an image, a video, a data table, etc.), it is important that the process of interpretation it is efficient for humans. The sketch should provide relevant insights in humans, which are not provided by the original dataset.

The sketch is useful if it is legible, representative, and relevant. Getting really philosophical, data abstraction means to represent one bitset by another bitset that is somehow more useful; it is not a requirement that one bitset is smaller than the other, nor that the two bitsets are interpreted in the same way. However, it is important that the latter bitset (when interpreted in the right way) triggers cognitive insights in the human brain that are not yielded by the first bitset.


Getting into the nitty-gritty, what are the main types of data abstraction? Selection, aggregation, and modification.

What factors control a data abstraction process? See Weibel, controls of data abstraction.

What does data abstraction achieve? . balances legibility and representation, aims at aesthetic and efficient delivery of information.

What are the fundamental challenges?

%"MassifMontBlanc7438" by Gnomefilliere - Own work. Licensed under Creative Commons Attribution-Share Alike 3.0-2.5-2.0-1.0 via Wikimedia Commons - http://commons.wikimedia.org/wiki/File:MassifMontBlanc7438.JPG#mediaviewer/File:MassifMontBlanc7438.JPG.

\section{Related Work}

\subsection{Geographical data}
From a source map, compute a target map such that...

\begin{itemize}
\item See notes.tex and possibly latex in old and old2.
\item See Weibel and Dutton: (1) what generalisation of spatial data is, (2) why it is necessary, (3) some techniques for performing it in the digital domain, and (4) what tools
currently exist to support it, (5) providing pointers to some of the recent research literature.
\end{itemize}
...
?the selection and simplified representation of detail appropriate to the scale and/or the purpose of a map? (ICA 1973: 173)


\subsection{Relational data}
From a source database, compute a target database such that...


\subsection{Graph data}
From a source graph, compute a graph minor such that..


\section{Controlling Factors of Data Abstraction}
...
\subsection{Geographical data}
From a source map, compute a target map
\subsection{Relational data}
From a source database, compute a target database
\subsection{Graph data}
From a source graph, compute a graph minor

\section{Contraint-based Data Abstraction}

Given a source $G=(V,E)$, compute a target $G'=(V',E')$ over scales $Z = \lbrace z_0, ... z_n \rbrace$ by $\lbrack recursion, iteration\rbrack$, such that \emph{constraints} and \emph{objectives}... using \emph{operators}.

Brief coverage of related work
...
Selection, Aggregation, Modification
...
\subsection{Conflict Elimination}
