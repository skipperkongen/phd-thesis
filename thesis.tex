...
Over time, technology and map making has evolved hand-in-hand. For example, cloud computing is one of the latest technologies that has impacted map making and made possible a host of new use cases for maps. However, the visual perception of Homo Sapiens -- the principal user of maps -- has probably remained unchanged since the Neolithic period. Thus, despite the tremendous advances in technology, modern map makers must accommodate the same limitations of visual perception as ancient map makers.  In a sense, the map making challenge is greater today than ever. While the volume of geographic data keeps growing exponentially, the map medium itself is shrinking. While previously a map maker could use a table-sized map to communicate geographic information, today's map makers must often make due with a pocket-sized mobile device to communicate even more geographic information.

\subsection{Modeling Geographic Information}
\label{sec:modeling:geographic:information}
Geographic information is information that is associated with locations in physical space. In the context of this thesis, we will talk about information that appears on a map. This information can be modelled in two ways, either logically or graphically. While the logical information level defines \emph{what} geographic information should appear on a map, the graphical information level defines \emph{how} the information should appear on a map. The differences and specifics of these two models are covered below.

\minisec{Logical Model}
Geographic information on a map is logically modelled by a set of \emph{geographic objects}. Each object represents a discrete concept in the real world, such as a road, lake, building, or subdivision. Each object associates a \emph{geometry} (point, line or area) with a set of additional attributes, which may include \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. For example, the restaurant ``Noma'' in Copenhagen can be represented by an object with the following attributes:

\begin{itemize}
\item \ang{55.6778}, \ang{12.5962} is a geometric attribute value of the Noma object, which corresponds to a geographic position on the harbor front in Copenhagen, Denmark.
\item ``Nordic Cousine'' is a qualitative attribute value of the Noma object, which can be distinguished from other qualitative attribute values, such as ``Japanese Cousine''.
\item ``Best Restaurant 2014'' is an ordered attribute value of the Noma object, which enables it to be ranked against other objects having the same attribute, such as an object with a ``Second-best Restaurant 2014'' attribute value.
\item DKK 1.600 (the current price of twenty servings at Noma) is a quantitative attribute value of the Noma object, which has the property that \emph{arithmetic} can be performed on it. For example, the sum of the values DKK 1.600 and DKK 1.000 (the current price of the wine pairing at Noma) is DKK 2.600.
\end{itemize}
...
Geographic objects are graphically represented by \emph{symbols}. For example, a building object can be represented by a black square symbol, while a road object can be represented by two parallel curve symbols. Depending on their type, object attributes are graphically expressed by one of seven \emph{visual variables}, which were first described by Bertin in his book ``Semiology of Graphics: Diagrams, Networks, Maps''~\cite{bertin1967semiologie} (see Figure~\ref{fig:introduction:visual:variables}). Different visual variables have an affinity towards certain types of attributes. For example, colors have no inherent order but are clearly distinguishable from each other (some more than others). Therefore, symbol color can be used to distinguish between different qualitative attribute values (see Figure~\ref{fig:introduction:restaurant:colors}). In contrast, sizes can be compared and measured. Therefore, symbol size can express the relative ordering as well as quantity of attribute values that permit quantification and/or ranking. 
...
Bertin's work on the theory of graphics is immense, and this short section does not aim to fully cover it. A work that is related to Bertin's, is the ``the Grammar of Graphics'' by Wilkinson et al.~\cite{wilkinson2006grammar}. This collection of previous work covers the fundamental aspects of representing information as graphics, which subsumes the representation of geographic information on maps.

\subsection{Map Generalization}
\label{sec:introduction:map:generalization}
The Earth is huge compared to the size of a normal map, say a paper map that you might pick up at a tourist information desk. One of the key challenges of map making is how to represent planet-sized information on a roughly human-sized map medium, be it a computer screen, mobile device or piece of paper. The difficulties arise from the large difference in \emph{scale} between the two material bodies, the Earth and the map medium (see Definition~\ref{def:scale}); you can fit roughly 500 trillion non-overlapping paper maps of the Earth onto the surface of the Earth. Consequently, each centimeter on a map has to convey a lot of information. For example, on a 1:500.000 map, one centimeter on the map surface corresponds to five kilometers on the ground. In the real world, a lot of information is contained within those five kilometers! Necessarily, information must be abstracted in order to emphasize the important and suppress the irrelevant, as otherwise maps will become cluttered and illegible. In geographic information science, the complex process of information abstraction for maps is called \emph{map generalization}~\cite{mcmaster1992generalization,weibel1997generalization}.

\begin{definition}[Map Scale] The scale of a map is the ratio of a distance on the map to the corresponding distance on the ground. For example, on a 1:500.000 map, one centimeter on the map surface corresponds to five kilometers on the ground.\label{def:scale}\end{definition}
...
Different visual effects occur as a map is scaled down. These effects can be seen in Figure~\ref{fig:introduction:background:raw:poi}, which illustrates a point dataset that has been rendered at different scales. There are three important effects that can be observed:
...
\item The size of symbols -- if they are shown ``to scale'' -- decreases when the map scale is reduced. For example, if a lake is 2 mm wide on a 1:250.000 map would be 0.5 mm wide on a 1:1.000.000 map. Consequently, map symbols may become too small to be visible to the human eye. Similarly, map mediums may even be unable to reproduce symbols when they become too small. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be at least 0.35 mm in length. while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.
%A generalization technique that deals with diminutive symbols sizes is \emph{exaggeration}.

\item The separation between symbols -- if they are shown at accurate positions -- decreases when the map scale is reduced. Consequently, symbols may come so close to each other as to become fused. For example, two symbols that are 0.8 mm apart on a 1:250.000 map will be 0.2 mm apart on a 1:1.000.000 map, which is below the limit at which symbols can be distinguished from each other.
%Two generalization techniques that deal with symbols in close proximity are \emph{displacement} and \emph{selection}.

\item The information density --  the amount of information per unit area --  increases when the map scale is reduced. Information density can be measured according to different metrics, such as Tufte's density metric~\cite{tufte1983visual}. The important \emph{principle of constant information density} states that information density on a map should remain constant as the scale changes, which was first formally recognized by T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that the number of settlements per unit area in published map series is independent of the map scale. The rationalization of this observation is that a linear reduction in scale implies a reduction in area according to geometrical progression, which in turn must lead to a reduction in information at the same rate.
...
A generalization operator transforms the set of information objects that appear on a map in order to ensure that a map remains legible and relevant at a given scale  (see Figure~\ref{fig:introduction:background:mashups}). One way to classify operators is according to three categories: \emph{selection} (see Definition~\ref{def:selection}), \emph{projection}  (see Definition~\ref{def:projection}), and \emph{aggregation} (see Definition~\ref{def:aggregation}). 

It should be noted that, in geography, the term projection is often used interchangeably with the term \emph{map projection}, i.e. the projection of spherical coordiates onto the plane. However, in this thesis, the term projection has a broader meaning that is adopted from databases~\cite{ramakrishnan2000database}.
...
A procedure that accepts a set of objects and outputs the same set of objects but in a modifed state. \label{def:projection}.
\end{definition}

\begin{definition}[Aggregation]
A procedure that accepts a set of objects and merges subsets of these objects into single objects in the output. Crucially, at least one object that appears in the output must not be a members of the input, as otherwise the procedure would be equivalent to selection. \label{def:aggregation}.
\end{definition}

Each of the three high-level operators -- selection, projection, and aggregation -- can be implemented by several \emph{algorithms}. A key distinction is whether an algorithm processes objects \emph{independently} or \emph{holistically}. Thus each algorithm can be categorized in two dimensions, i.e. the category (selection, projection, aggregation) and subcategory (independent, holistic). For example, random sampling is a type of independent selection, since objects are selected or omitted independently of other objects (by flipping a coin, rolling a dice, etc.). In contrast, geometric displacement of objects such that no two object geometrically overlap is a type of holistic projection, since objects are modified in the context of other objects. Many different algorithms have been proposed in the literature (see Table~\ref{tab:operators}). 
...
The categorization of generalization operators the dimensions (selection, projection, aggregation) and (independent, holistic), which is presented here, is much more abstract than the typologies usually proposed in the generalization literature. Various authors have attempted to come up with a final and fundamental classification, hitherto without any consensus being reached among cartographers~\cite{rieger1993consensus}. For example, competing and derivative classifications have been proposed by Foerster, Morales, and Stoter~\cite{foerster2008classification}, McMaster and Shea~\cite{mcmaster1992generalization}, and Weibel and Dutton~\cite{weibel1999generalising}. While these different classifications of operators provide insight into the concrete aspects of the generalization process, it is hard to argue that either one of them is conclusive. Therefore, this section has covered generalization operators by grouping them in terms of some fundamental differences rather than specific differences. The goal of this section has been to provide the reader with a clear framework in which generalization operators can be understood. Later sections of this thesis will focus more narrowly on holistic selection and simple projection.

\subsection{Constraint-based Generalization}
\label{sec:contraint:based:generalization}
The generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, \emph{constraints} act as the discriminator between feasible and infeasible map states~\cite{harrie1999constraint,ware2003generalization_sa}, while \emph{objectives} act a measure of optimality of a map. Together, constraints and objectives define an optimization problem where an optimal map must be found in a search space of maps. The search space contains the set of maps that can be derived from a source geographic information. Maps can be derived by application of generalization operators to the objects and symbols on the map (see Section~\ref{sec:introduction:map:generalization}).
...
\item Unit areas may be required to satisfy constraints on the sets of objects they contain, such as maximum density.
\item Networks may be required to satisfy topological constraints, such as logical flow. For example, a river system that is displayed on a map should not be disconnected from itself or its outlet.
\end{itemize}
