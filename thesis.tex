...
As the human capacity to gather data has increased, the creation of maps has become more challenging; finite map mediums can only accommodate a finite number of legible symbols. This challenge has lead to a plethora of cartographic information abstraction techniques that are collectively known as \emph{map generalisation}~\cite{brassel1988generalization,mcmaster1992generalization,weibel1999generalising}. Map generalisation essentially solves the problem of creating a legible, useful and accurate maps for any map scale and for any amount of geographic information. Naturally, this process involves a compromise between readability and precision for most real-world cases. For example, maps of the physical world must carefully select the subset of the available information that is visualized, or else the map would soon become cluttered and illegible. Similarly, demographic maps must often aggregate information about individuals by area in order to clarify the important trends in local populations. Finally, small scale maps must usually distort the information in order to make it more legible, for example by enlarging symbols that would otherwise be imperceivable or displacing symbols that would otherwise overlap.
...
Each of the three high-level operators -- selection, projection, and aggregation -- can be implemented by several \emph{algorithms}. A key distinction is whether an algorithm processes objects \emph{independently} or \emph{holistically}. Thus each algorithm can be categorized in two dimensions, i.e. the category (selection, projection, aggregation) and subcategory (independent, holistic). For example, random sampling is a type of independent selection, since objects are selected or omitted independently of other objects (by flipping a coin, rolling a dice, etc.). In contrast, geometric displacement of objects that eliminates geometric object overlap is a type of holistic projection, since objects are modified in the context of other objects. Many different algorithms in all categories have been proposed in the literature (see Table~\ref{tab:operators}). 
...
This mathematically-oriented operator categorization (selection, aggregation, projection) is more abstract than the typologies usually proposed in the map generalization literature. In the literature, more domain-specific and fine-grained categories tend to be proposed. Various authors have attempted to come up with a final and fundamental classification, however without a consensus being reached~\cite{rieger1993consensus}. Various competing and derivative classifications have been proposed by Foerster, Morales, and Stoter~\cite{foerster2008classification}, McMaster and Shea~\cite{mcmaster1992generalization}, and Weibel and Dutton~\cite{weibel1999generalising}. While these different classifications of operators provide insight into the concrete aspects of the generalization process, it is hard to argue that either one of them is conclusive. Therefore, this section has covered generalization operators by grouping them by their fundamental mathematical properties rather than more specific properties.

Conceptually, the process is that derives an appropriate symbol from an information object is also a type of map generalization, which is a projection from objects to symbols.
