...
I dedicate this thesis to Marusa, my son Morgan and my daughter Athena and thank you for the magic and motivation and for suffering me when I was overworked and frustrated. I want to thank the Ministry of Higher Education and Science in Denmark, Grontmij A/S and the Danish Geodata Agency, and in particular Anders Friis-Christensen, Thorben Hansen and Nils Bo Wille-J{\"o}rgensen, for taking a chance and sponsoring this thesis. Next, and with very deep gratitude, I wish to thank my primary supervisor Marcos Vaz Salles for his tireless dedication and support, our great discussions and his warm friendship. You are a true light. Furthermore, I wish thank my supervisor Martin Zachariasen, for always keeping a cool head, believing in my ideas and for giving me the right push whenever I needed it. With a wam heart, I want to thank my father Georgios Kefaloukos, my mother Helena Hodgson and my step-father Stuart Hodgson for their great support and just for being proud of me. A special warm thought goes out to Dora and Tonci Balic-Zunic, for many fond memories with the family and for stepping in as supportive grand-parents time and again. Another special thought goes out to the faculty at DIKU and all PhD students, especially Vivek Shah, for fun and thought-provoking discussions over lunch in the cantine. It was sometimes silly and often enlightning. I wish to thank Professor Robert Weibel and everyone at UZH who offered me their feedback, help and sometimes cake during my stay at the University in Z{\"u}rich, Switzerland. My former colleagues deserve a special thank you for valuable feedback offered during my studies. In no particular order they are: Lars Loldrup, Martin Bj{\"a}rge Jensen, Brian Haunstrup, Asger and Gregers Petersen, the rest of my colleagues at GIS \& IT Grontmij and all my colleagues at the Digital Map Supply. Close to my heart are my friends, who forced me to have some fun once in a while, take the necessary breaks that I needed to come back and think even harder about the issues at hand. As a special commemoration, I would like to thank the late S{\"o}ren Aundal, a primary school teacher (not mine) and friend of the family when I was a kid. He showed me how to train a pair of binoculars at birds, stars and planets alike, thus nurturing the curiosity that has been part of my personality since childhood. Last but not least, I would like to thank Hans Skov-Petersen and Bernhard Snizek for kick-starting my interest in the field of geographical data science, and by transitivity, in data science as a whole. In conclusion, I acknowledge that the set of influences that direct our lives is impossible to trace, so instead I close my eyes and humbly accept the path that lays before me and the footsteps I have trodden in the great labyrinth of our existence. I have never walked alone.
...
% How has the world changed; what is the new situation?
1a (data): The web today is home to an eco-system of providers and consumers who create, combine and visualize a huge diversity of open data. In symphony, open data covers the full spectrum of human interests: e.g. culture~\cite{rapgenius,wikipedia}, economics~\cite{worldbank}, geography~\cite{openstreetmap,danish:geodata:agency}, demography~\cite{unstats} and government~\cite{data:gov, data:co:uk}. Indeed, the value proposition of open data is clear: open data has the potential to increase transparency, efficiency and enlightenment in our societies~\cite{hans:rosling, offentlige:data:i:spil}. Consequently, the availability of open data has created a great demand for data-driven visualizations, i.e. applications that help us understand and (some times) take action on the data by visual means~\cite{google:maps, nytimes, guardian}. With great haste, we will continuously move towards a world where access to data and the ability to understand it is as basic as turning on the light. Already, the fundamental infrastructure is in place; devices in our pockets with backlit screens and the ability to reach any point in the world of data at the speed of light. What is needed now is to transform that we carry in our pockets permanently connect us to the world of data and data applications. \emph{elaborate more on the situation?}

1b (data-driven visualizations):
 


As with other web applications, users expect 24 x 7 availability and high performance of a data-driven visualization.

A successful data-driven visualizations must be satisfy the expectations of web users. 


Data-driven visualization are evaluated by metrics that fall into two categories: visual metrics and performance metrics.

In the eyes of users, both data and data-driven visualizations have become a commodity. Consequently, data-driven visualizations must legibly and truthfully visualize data to users, the must also  24 x 7 availability, low latency, and high scalability, which is expected by web users today. Failure to recognize the importance of these usability and performance metrics will dramatically impact the success of a service~\cite{usability:study, performance:study}.



Web users expect 24 x 7 availability, low latency, and high scalability for web applications such as 


The demand for data-driven visualizations drive the demand for open data in an upwards spiral. In anticipation of this trend, most large web sites and organizations (e.g. Wikipedia and the United Nations) now offer APIs that are specifically designed to efficiently meet the demand for data and data visualizations~\cite{wikipediaapi,unapi}. Accordingly, web users, consciously or unconsciously, have come to expect 24 x 7 availability, low latency, and high scalability in these services. Addtionally, the utility of a data-driven visualization greatly depends on the visual qualities of the presentation. Alas, a failure to recognize the importance of any of these quality metrics will have drastic consequences for the success of a service~\cite{usability:study, performance:study}.

% How does the situation motivate the topic of the thesis?
%TODO: Flow should go like this after describing the situation: Data partitioning~\cite{stuff}, data replication~\cite{stuff} and data abstraction~\cite{mapgen, dbgen} are important techniques for providing high quality web services, i.e web services with 24 x 7 availability, low latency and high scalability. The advantages and pitfalls of using data partitioning and data replication are generally well understood, for example, the relationship between replication and consistency. In this thesis, the focus will be on data abstraction primarily, and data partitioning and replication secondarily. In the area of digital maps, tiling is a concept that merges together the techniques of data partitioning with data abstraction~\ref{xyz}. Next, describe what tiles are and how the concept can be used also for time lines, cluster visualizations, etc.
...
%In the field of multi-scale map generalization a narrow form of data abstraction has been extensively studied., e.g. in the field of , the full potential of data abstraction for other types of data data such in combination with data partitioning and data replication has not been realized yet. In the area of digital maps, tiling is a concept that merges together the techniques of data partitioning with data abstraction~\ref{xyz}. Next, describe what tiles are and how the concept can be used also for time lines, cluster visualizations, etc.

% What are key challenges and unanswered questions regarding the topic?
...
% Key-value stores, advantages and limitations.
% they support partitioning, replication and scale out => high availability, performance and scalability
%Today, users expect 24 x 7 availability and low latency access for data and visualizations. Consequently, data providers and researchers are focusing attention on techniques for high availability, high performance and scalability of data systems. Key-Value stores (K-V stores) have inherent support for partitioning, replication and scale out, which facilitates high availability, high performance and scalability~\cite{dynamodb, andfriends}. Furthermore, K-V stores have an opaque data model that is appropriate for storing heterogenous data, e.g. geographical data, images, videos and user data~\cite{usecase1, usecase2, usecase3, usecase4}. These characteristics make K-V stores a popular choice among data providers who experience high demand for their data~\cite{googlecdn, haystack}. In a nutshell, the K-V stores are scalable because they are simple; data is retrieved by primary key only. For example, in the XYZ protocol, the key ``15/17527/10252'' points to a map segment that shows the location of the University of Copenhagen, Department of Computer Science (see Figure~\ref{fig:introduction:example:tile:pyramid:access}). However, these advantages of K-V stores are balanced by severe limitations; K-V stores lack the expressibility of more advanced query engines like relational database (RDBMS). \emph{Write about materialized data objects versus computed data objects.}

% Current research in K-V stores
%Current research in K-V stores is searching for the right trade-off between the capabilities of relational database systems and first-generation K-V stores. Important areas include stronger consistency models, enhanced programmability, indexing, etc. \emph{at some point, we will present our contributions for closing this gap, specifically in the area of data visualizations. This is going to be so much fun!}



\emph{end of writing, comments in source code}

% Geographical data and the Danish Geodata Agency.
%Geographical data is an important type of data. In Denmark, the Danish Geodata Agency has offered an API for geographical data, which has been public and free of charge since 2013. The Agency too has felt the upwards spiral in demand for data. Over the past five years, the agency has experienced an exponential rise in the number of requests that are received per day. While the first version of the API was launched back in 2001, when data mash-ups were still in their infancy, it has been redesigned several time to keep up with demand. 



%In contrast, query engines that can evaluate select-project-join queries are more expressive but also harder to scale~\cite{Scalability:of:Select:Project:Join}. For example, in the WMS protocol, a portion of the map is retrieved by evaluating a select-project query (e.g. in an RDBMS). Generally, the difference in speed between evaluating a K-V query and a select-project query is $O(1)$ vs. $O(\log n)$, e.g. a memory address lookup vs. an index traversal to find matching records~\cite{rtree, btree}. In conclusion, K-V stores trade expressibility for scalability, which is a good compromise when the demand for data is high.

%Recently, the Danish Geodata Agency introduced a WMTS API, which is logically equivalent to an XYZ API. 

%The key reason behind this is scalability. Sites like YouTube and Facebook make images and video available though Content Delivery Networks (CDN), which are K-V stores at a planetary scale. Without these CDNs, these popular websites would crumble beneath the crushing workload. There is a key difference between using a K-V store for images and videos and using a K-V store for geographical data. While images and videos 

%Furthermore, data-driven visualizations like maps~\cite{google:maps, cartodb}, time lines~\cite{something}  and charts~\cite{something} have become a commodity. You can create a map in minutes by specifying a background map and add some overlay data to display on top (e.g. data from eBird.org). Data reuse is high, as popular background maps are used again and again in mash-ups around the world. Consequently, great effort has gone into providing background map data of high quality to satisfy this demand. While users expect 24 x 7 availability and low latency for data, a service like Google Maps is powered by a planet scale distributed system and an army of engineers.


% 
%While mash-ups contribute to transparency and knowledge in our societies, the high demand for data presents a critical problem in the eco-system of data providers and consumers. Mash-ups like geographical maps draw data from multiple providers, with a strong skew in the workload that each provider has to handle. For example, a geographical map visualization may overlay bird observation data on top of background map data. While the demand across all mash-ups might be small for bird data (e.g. eBird.org), it could be order of magnitude higher for background map data (e.g. Google Maps). This creates a problem for the providers of popular datasets, because users expect 24 x 7 availability and low latency. Luckily, services like Google Maps can afford the cost of a ~\cite{something}. However, providers of government curated data are not generally so lucky.





%Similarly, organizations that do not own data centers will typically buy services from a cloud provider that in turn is powered by global datacenters~\cite{aws}.


%high demand also presents a problem for the data providers. Handling the popularity is costly! In response  of an information repository like Wikipedia can be crushing. In capitulation and 

%a crucial problem is the effort and cost to create and serve data-driven visualizations to an unforgiving audience. 

%\begin{itemize}
%\item Cloud computing has made it significantly easier to build an own data-driven visualization.

%\item The supply of public datasets feeds the demand for more public dataset as mash-ups are continuously gaining momentum, e.g. in journalism and governance. This puts pressure on the whole eco-system, which includes base data like the background maps often used in geospatial mash-ups~\cite{google, mapbox, gst}.
%\item This has forced providers to rethink how data is made available. In the area of geographical data, two dominant approaches have been to provide deltas~\cite{openstreetmap:delta, wikipedia:delta} or to expose data with a simple key-value access model. The latter approach fits the pattern of a content delivery networks~\cite{cloudfront}, highly scalable key-value store~\cite{redis, memcached} and even plain file systems. Consequently, new standards for providing geospatial data have emerged, which are based on the key-value access model~\cite{WMTS, XYZ}.
%\item This creates a new set of challenges, i.e. key-value selection.
...
\begin{enumerate}
\item Briefly mention discuss general research in data-driven visualizations and in particular data abstraction which is the focus of thesis
\item Introduce need and problem of global selection (modelling, algorithms) for data abstraction
\item Introduce need and problem of programming/query languages for data abstraction
\item Introduce need and problem of database integration for data abstraction
\item Introduce need and problem of adaptive pre-emptive caching of geographical data in key-value stores\end{enumerate}
...
%This work draws heavily on ideas developed in the field of map generalization. Instead of copying in the entire theory of map generalization, the reader is encouraged to read Weibel and Dutton. While the rich diversity of planet earth has inspired such artful map generalization algorithms that can capture that vast reality on a piece of paper, this dissertation takes a side-ways leap: moving in parallel with the cartographernd, this dissertation will attempt to draw a vast universe of data tables on 
