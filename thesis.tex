...
While much has changed, some things remain constant. Homo Sapiens, the principal designer and audience of map visualizations, has remained biologically unchanged probably for thousands of years. While the Web has impacted when, why and how map visualizations are created and consumed, the basic ability of humans to visually absorb three dimensional images, such as maps, is limited in important and constant ways~\cite{jacques1967semiologie}. Therefore, maps on the Web must observ the same well-studied principles as maps on paper:




\subsection{Multi-scale Maps}
Map visualization studies how the geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a restricted medium~\cite{jones2005webage,weibel1997generalization,bertin1973semiologie}. Naturally, a map must trade-off \emph{acurate representation} of data that intersects the geographic boundary of the map with \emph{lossy abstraction} of data, which may omit~\cite{nutanong2012multiresolution,sarma2012fusiontables,schmid2013opensciencemap}, modify~\cite{douglas1973algorithms,lonergan2001iterative} or aggregate~\cite{haunert2006landcover} map objects in order to emphasize the important and suppress the unimportant. This process is known as \emph{map generalization}~\cite{harrie2007modelling,mackaness2011generalisation,weibel1997generalization}.
