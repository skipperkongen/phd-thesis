...
Map visualization studies how to visually represent the geography of the Earth at multiple scales on a restricted medium, such as a computer screen. Necessarily, geographical data must be abstracted according to scale in order to fit the representational budget of a small pixel space, for example the 2880 x 1800 pixels of a laptop display. Within map visualization, the disciplin of map generalization studies how geographical data should be abstracted as the scale changes. In contrast to selection, projection and aggregation in the relation algebra, map generalization must select, project and aggregate data \emph{holistically} according to visual constraints and objectives~\cite{weibel1999generalising}. While the goal of generalization is clear, i.e., balance legibility against representation, this goal is deceptively hard to achieve.

% People know this
%Maps layer different strata of geographical data on top of each other. While geographical data are very versatile, they are always geometric. For example, points can represent the location of animals and hospitals~\cite{openstreetmap2010haiti}; lines can represent the network of waterways and streets; polygons can represent the areas of lakes and national territories. 

% Challenges
More and more, organizations without a strong tradition for geography are beginning to exploit the availability of geographical data to commicate spatial naratives to their online users~\cite{nytimes2010iraq}. As a consequence, more and more professionals will need to acquire the skills to craft map visualizations. To power map visualizations, the web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{zooniverse2014oldweather} datasets, Day by day, the amount of geographical data on the web grows bigger and more detailed~\cite{openstreetmap2014stats}. Furthermore, nothing prohibits a geometric interpretation of otherwise non-spatial data, such as news stories~\cite{shahaf2013informationcartography} or the vocabulary of musicians~\cite{daniels2014vocabulary}, just to pick two real however exotic examples.

As mobile devices increasingly bond millions of people to an online lifestyle, digital maps and geographical information can be consumed by anybody, at any time, and from anywhere.

% Why GIS is inadequate
Traditionally, maps have been crafted by experts in Geographical Information Systems (GIS). However, a new situation exists on the web today that significantly marginalizes the use for traditional GIS. First, the demand for multi-scale maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS); therefore, map visualizations must be \emph{expressible by non-experts}. Second, web maps that run on top of cloud infrastructures can now integrate huge datasets (billions of data items); therefore, map visualization systems must be \emph{scalable in the size of data}. Third, web maps today can quickly reach millions of users who can interact with map visualization 24 x 7 using mobile devices; therefore, map visualization systems must be \emph{scalable in the number of concurrent users}. In short, GIS was not designed to satisfy these requirements, which merits the study of a new category of systems.
...
\caption{\emph{QPS vs latency correlation:} Default}
\label{default}
\end{center}
%\vspace*{-4ex}
\end{figure}
