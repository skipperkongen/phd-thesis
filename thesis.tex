...
\label{introduction:web:mapping:pipelines}
...
It is important to study the tasks and requirements of each stage in the pipeline. This allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. Stage one (data modeling) has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. Stage three (graphical abstraction) has been studied among others in the seminal work by Bertin~\cite{bertin1973semiologie}. This thesis will focus on stage two (logical abstraction) and stage four (content distribution), since this is were the biggest challenges in web mapping are currently found.

\subsection{Logical Abstraction}
\label{introduction:web:mapping:pipelines:logical:abstraction}
% describe
In stage two of the web mapping pipelines, high-fidelity information in the primary data model is logically adapted for display on multi-scale maps. The outcome of this stage is a logical data model which contains the subset of information that should appear at each scale of a multi-scale map. The logical information objects are derived from the primary data model in a process which is known as \emph{model generalization} in the geographic literature~\cite{gruenreich1985cag}. 

Homo Sapiens, the principal designer and audience of map visualizations, has probably remained cognitively unchanged for thousands of years. Even as the Web has impacted when, why and how maps are created and consumed, the basic ability of humans to perceive information on maps has remained powerful, yet fundamentally limited~\cite{bertin1973semiologie}. Consequently, modern maps on the Web must be guided by the same well-studied principles as ancient maps on print.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.35]{figs-thesis/holistic-selection.pdf}
\caption{\textbf{Binary and space constraints}: binary constraints, such as minimum proximity (left image), set up conditions that must be met by all qualifying pairs of objects. Space constraints, such as a maximum density (right image), set up conditions that must be met by all qualifying spaces. In the figure, objects that violate a constraint are circled by red lines; objects that are consequently deleted are shown as white disks; objects that are kept are shown as black disks.}
...
\minisec{Constraints and Objectives} Map content must by legible, relevant and consistent with reality. These high-level design goals are quantified by \emph{map constraints} and \emph{map objectives}. Map constraints set up quantifiable conditions that can be evaluated during map design, which enables the distinction between feasible and infeasible maps. Map constraints serve to ensure legible and consistent map. For example, a proximity constraint may cause map symbols to be deleted, when two or more symbols overlap, which increases legibility. A network topology constraint may prohibit the deletion of river segments because it would result in disconnected river networks, which increases inconsistency. However, map constraints can not ensure relevant maps, which is the purpose of map objectives. 

Map constraints can be classified as follows: 
...
Map constraints only distinguish feasible maps from infeasible maps. They do not quantify the degree of relevance of a feasible map. \emph{Map objectives} quantify the total utility of a map through an mathematical \emph{objective functions}. One type of objective is to penalize violated constraints, and minimize the sum of penalties. Another type of objective is to score objects by their importance, and maximize the sum of scores, which is arguably the most intuitive approach (see Figure~\ref{fig:introduction:selection:constraints}). 


\minisec{State-of-the-Art Logical Map Design} Current approaches to logical map design are characterized by two extreme categories: fully-automatic selection of geographic information and complex map generalization of geographic information. 

\begin{itemize}
\item \textbf{Automatic Selection}
...
% identify
Map generalization requires the holistic application of multiple operators -- such as selection, displacement, amalgamation, exaggeration, and collapse~\cite{weibel1999generalising} -- which has been modelled in complex frameworks with agent-based~\cite{lamy1999application} and metaheuristic search~\cite{ware2003generalization_sa} methodologies.
