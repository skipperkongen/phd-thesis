...
\item \textbf{Scalability} is the property that performance (i.e. latency) can be retained for increasing data volumes and/or number of concurrent users. In the mapping pipeline that was studied for this thesis, the latency roughly doubles in the middle of the day when the number of concurrent requests is highest.
...
\subsection{On-the-fly vs. Batch-oriented Selection}
\minisec{On-the-fly selection}
At first, on-the-fly selection of geographic information objects for maps seems like an ideal approach for a number of reasons. 
...
Second, user preferences can be considered part of the input. Therefore, selection of geographic information can be tailored to individual users needs, for example by letting a recommendation algorithm assign relevance measures to objects.

However, the complexity of any selection algorithm is certainly greater than $O(1)$, which is the complexity of retrieving pre-selected data from a key-value store. To speed up on-the-fly selection, the data will be indexed. While indexing structures, such as quad-trees and GAP-trees, allow polynomial-time evaluation of selection, this is true only of one-shot queries. To interface with large audiences, a mapping system must also be scalable. Unfortunately, on-the-fly selection is hard to scale in a cost-efficient way. In contrast, retrieval of pre-selected data from a key-value store is easy and cheap to scale to any number of concurrent users if certain trade-offs can be accepted (bounded staleness of information).

\minisec{Batch-oriented solutions}
Batch-oriented selection of geographic information compute materialized views over the original state of the data. Subsequently, the selected data can be partioned and replicated on a content delivery network (CDN), which allows immense scalability in the distribution phase of the web mapping pipeline. Of course, this scalability comes at a price, which is primarily composed of increased storage cost, bounded staleness of data, and inability to respond flexibly to user preferences during selection. However, batch-oriented solutions offer a better trade-off than on-the-fly solutions when maps are distributed to large audiences. 
