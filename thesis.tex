...
\emph{How has the world changed; what is the new situation?} The web today is home to an eco-system of data providers and data consumers who create, combine and visualize a huge diversity of open datasets. In combination, open data covers the full spectrum of human interests, e.g. culture~\cite{rapgenius,wikipedia}, economics~\cite{worldbank}, geography~\cite{openstreetmap,danish:geodata:agency}, demography~\cite{unstats} and politics~\cite{data:gov, data:co:uk}. Indeed, the value proposition of open data is clear: open data increases the level of transparency, efficiency and enlightenment in our societies, but only if we can make sense of it~\cite{hans:rosling, offentlige:data:i:spil}. Luckily, we are rapidly gaining the ability to make sense of the data through the means of powerful data visualizations. Consequently, the availability of open data, both curated and user-contributed, has created a great demand for online data-driven visualizations, e.g. maps~\cite{guardian:sunrise:on:twitter}, bubbles~\cite{nytimes:obama:budget}, and buckets~\cite{largest:vocabulary:in:rap}. Since data visializations are so useful, they can quickly attract an audience of millions, for instance, when they are shared on social media. 
