...
% Background
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}. Map visualization is fundamentally a ``big data'' challenge, since the amount of geographic information typically exceed the representational budget of the medium (see Figure~\ref{fig:introduction:background:raw:poi}). Therefore, the geographic information must be adapted to fit the budget, which is a fundamentally hard problem~\cite{weibel1997generalization}.
...
The Web has structurally impacted \emph{why}, \emph{how}, by \emph{whom}, \emph{when}, and \emph{where} maps are produced and consumed. Consequently, new challenges of map creation and map distribution have emerged, which cannot be adequately addressed by traditional geographic information systems (GIS). 
