...
\item \textbf{Holistic Multi-scale Selection} Geographical information is typically spatially skewed, which leaves some areas of geometric space dense with data while other are sparse. Simple selection, which filters a dataset row-by-row according to simple predicates, can not capture binary or multi-way object relations~\cite{egenhofer1991categorizing,egenhofer1991point}. Consequently, simple selection can not be used to ensure that a map is thinned to a legible level. In contrast, a holistic selection process will filter records according to global constraints and objectives~\cite{woodruff1998constant, topfer1966principles, harrie1999constraint}, such as proximity and density constraints and relative object importance (see Figure~\ref{fig:introduction:holistic:selection}). To enable coherent selection for multi-scale maps, the process of selection should be carried through to multiple scales, using a process model such as star or ladder generalization~\cite{foerster2010challenges}. A model abstration process based on multi-scale selection is appropriate for non-expert map designers, as it can be intuitively understood and moreover produces maps that are interactive, since the identity of records is preserved.

While a few recents map visualization systems have implemented automatic multi-scale selection~\cite{sarma2012fusiontables}, there is currently a lack of adequate algoritmic models for holistic multi-scale selection that allow flexible expression of user-defined constraints and objectives. This dissertation focuses on the development such models in the context of map visualizations.
