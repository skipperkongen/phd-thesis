...
\minisec{Visual Cognition} 
The visual cognitive ability of Homo Sapiens, the principal consumer and designer of maps, has probably remained unchanged for thousands of years, the map visualization discipline itself has undergone revolution after revolution. For thousands of years, humans have use their accute visual senses to comprehend spatial information on geographic maps. While legible maps enable humans to comprehend relevant geographic information in an instant, cluttered maps in constrast inhibit any comprehension. A basic challenge in map visualization is how graphics are affected by a reduction in scale. While the three dimensions of the image make the visual sense our most powerful channel of perceptions, images have only three dimensions and the consequences of this limitation are important~\cite{bertin1967semiologie}. 

\minisec{Consequences of scale reduction}
Maps, which are a type of graphic, have been produced at many different scales. Scale is the ratio between distance measured on the visualization medium and distance measured in reality. For example, for a 1:500.000 map, 1 cm on the map corresponds to 5 km on the ground. While one map may show just a single city (a large scale map), another map may depict the entire world (a small scale map). As a graphic is scaled down the visual perceptibility of the graphic is reduced, which is caused by a number of important effects.

One important effect that occurs when a graphic is scaled down is that objects in the graphic will appear to be smaller and closer together. Consequently, objects may approach the limit of perceptibility and distinguishability. For example, if an object becomes too small it may become impercievable to the human eye and appear to vanish. Correspondingly, if the proximity of objects becomes too close they may become indistinguishable from each other. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent a road, should be separated by at least 0.25 mm. 

An other important effect that occurs when a graphic is scaled down is that the information density of a graphic will appear to rise. Several metrics for information density have been studied, such as Tufte's density metric~\cite{tufte1983visual} along the other density metrics surveyed by Nickerson~\cite{nickerson1994visual}. The \emph{principle of constant information density} states that information density should remain constant across changes in scale, which was formally established by the cartographers T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that maps in published atlasses contain a roughly fixed average number of settlements per a unit area independently of the map scale. The rationalized their observation by observing that a linear reduction in scale implies a reduction in area according to geometrical progression, which must also lead to a reduction in information per unit area according to geometrical progression, if the information is to remain legible.
