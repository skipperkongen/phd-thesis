...
\subsection{Stage Four: Content distribution} In stage four, the map content is distributed over the Internet to web and mobile mapping clients. Service level aggrements (SLA) establish the conditions that must be met during the distribution of map content. The main requirements are:
...
\section{State-of-the-art Specification}

This thesis makes the case for declarative specification of logical map content. Arguably, this proposal occupies a balanced point in the design space, in-between fully-automatic and specification-less systems and highly complex programmable systems. In this section tours the design space of logical map content specification and argues the point for declarative specification.

Current proposals for logical abstraction of geographic information are characterized by two extremes: (1) automatic selection for simple maps, and (2) full-blown map generalization for complex maps. 

\subsection{Automatic Selection} 
% describe and identify
The primary use case for automatic selection of geographic information is the rapid design of thematic point maps over databases of geocoded information~\cite{google2014maps}. Users are assumed to have limited mapping skills. Therefore, the constraints and objectives of selection are completely encapsulated by the system. Additionally, different systems satisfy different constraints. For example, systems may automatically apply either a proximity~\cite{nutanong2012multiresolution} or density constraint~\cite{sarma2012fusiontables} without support for parameterization. Additionally, map objectives are unspecified, which means that at best an opaque ranking method is applied by the system.
...
\subsection{Procedural Map Generalizaiton}
% describe and identify
...
\chapter{Map Content Distribution}
