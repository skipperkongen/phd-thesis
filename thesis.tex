...
\emph{How has the world changed; what is the new situation?} The web today is home to an eco-system of providers and consumers who create, combine and visualize a huge diversity of open data. In symphony, open data covers the full spectrum of human interests, e.g. culture~\cite{rapgenius,wikipedia}, economics~\cite{worldbank}, geography~\cite{openstreetmap,danish:geodata:agency}, demography~\cite{unstats} and politics~\cite{data:gov, data:co:uk}. Indeed, the value proposition of open data is clear: open data will increase transparency, efficiency and enlightenment in our societies~\cite{hans:rosling, offentlige:data:i:spil}. In other words, properly designed data visualizations enable people to see the big picture of otherwise unfathomable phenomena, such government budgets, the geography of the Earth and complex human culture. Consequently, the availability of open data (e.g. user-contributed data) has created a great demand for online data-driven visualizations, e.g. maps~\cite{guardian:sunrise:on:twitter}, bubbles~\cite{nytimes:obama:budget}, and buckets~\cite{largest:vocabulary:in:rap}. To stimulate inquisitive behaviour, data visualizations often allow users to explore a dataset at multiple focal points and levels of detail; data can be explored at the global level or deep down in the detail. Since data visializations are so useful, they can quickly attract an audience of millions, for instance, when they are shared on social media. 
...
Second, given an appropriately abstracted dataset, system operators must ensure that the data is delivered to applications with low latency and high availability. A crucial problem is to operate a system that reaches these goals while minimizing the cost.
