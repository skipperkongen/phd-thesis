...
The web has transformed how map visualization should ideally be approached. This has opened up important new problems that must be solved, and which are not adequately addressed by state-of-the-art systems. Map visualizations must scale well both in the size of the data~\cite{Im2013visreduce, battle2013dynamicreduction} and the size of the audience~\cite{viegas2007manyeyes}. Moreover, designers must be able to create compelling and complex map visualizations using inituitive and expressive interfaces~\cite{wu2014case, battle2013scalar, fekete2012dmvis}. This dissertation focuses on how to build map visualization systems for the web. In particular, the following problems are addressed:

\begin{itemize}
% EXPRESSING
% DONE 1
\item \textbf{High-level Languages}: Designers, specifically those who are not GIS-experts, must be able to intuitively and concisely express complex multi-scale map visualizations over large geographical tables. A map visualization pipeline based on multi-scale selection, is easy to understand and produces maps that are interactive. Compelling map visualizations follow certain visual principles, which can be expressed as constraints, objectives and styles on the visual representation~\cite{woodruff1998constant, topfer1966principles, harrie1999constraint}. A productive pipeline for multi-scale selection, one that is programmable by designers, will require the specification only of high-level goals, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. Ideally, such a task can be expressed in a high-level programming language for map visualization, which depends only on the schema, not on a priori knowledge of the individual data items. 
...
% PROCESSING
% PENDING
\item \textbf{In-database Processing}: Map visualization systems must be able to scalably process a specification for multi-scale selection of geographical data.
