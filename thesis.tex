...
% The change: the web
Roger Tomlinson was one of the pioneers who kick-started the digital age of maps in the 1960's, with the introduction of geographic information systems (GIS). However, the rise of the Web has dramatically changed the activity of map visualization; a shift which has significantly marginalized the role of traditional GIS. First, interactive maps are now distributed to and consumed by billions of people 27 x 7 and on the go~\cite{foo}. Second, maps must now be designed by data enthusiasts with heterogenous skill levels, not only by GIS specialists, and often in conditions with high time-pressure~\cite{openstreetmap2010haiti,lomet2012warstories}. Third, vast repositories (billions of records) of geographic information are now available for map visualization and millions of new records are created each day~\cite{openstreetmap2014stats}. Fourth, focus has shifted from design of general-purpose background map layers, such as physical or topographic maps, to the design of thematic overlays, suchs as point of interest (POI) or news-worthy events (see Figure~\ref{fig:introduction:background:mashups}). In these ways, the Web has significant altered the stage for map visualization, which creates new challenges that can not adequately addressed by GIS alone.

On the Web, user audiences generally expect services to have high availability and low latency, and map providers like all other service providers must recognize the importance of these expectations. However, satisfying the constraints while keeping costs down is a challenge, which can potentially be addressed by application of distributed systems theory~\cite{beaver2010haystack,gaffuri12vectortiles,pilchin2007dynamo}. 

On the Web, map designers of various skill levels must craft map visualizations that meet specific design goals. However, map design is a holistic information optimization task of high computational complexity, often an NP-hard problem~\cite{haunert2006landcover,sarma2012fusiontables}, which means that humans and computers must work together. Thus, efficient and effective specification (by humans) of map visualizations is a challenge, which should be addressed by human-computer interaction~\cite{satyanarayan2014lyra} and programming language~\cite{meliou2011reverse,wilkinson2006grammar} theory. 

On the Web, map visualizations can seemlessly integrate with massive datasets in the cloud, which are frequently stored and exposed by relational database management systems (RDBMS). Scalable processing of massive datasets in RDBMS is a challenge, which must be addressed by appropriate application of database theory~\cite{wu2014case}.

% The constant: the human
While much has changed, some things remain constant. Homo Sapiens, the principal designer and audience of map visualizations, has remained essentially unchanged for thousands of years. While the Web has impacted when, why and how map visualizations are created and consumed, the ability of humans to visually absorb the three dimensional image of a map is limited in important and constant ways~\cite{jacques1967semiologie}. Consequently, map visualizations on computer displayes are, to a great extent, subject to the same visual constraints and objectives as maps on papyrus. It is important that knowledge that has been obtained in the pre-Web years of maps is not disregarded.

% The sum of both: success
In a sense, one chapter has closed in the book of map visualization, while another chapter is just about to open. On one hand, the chapter that has mapped the mountains, roads, cities and oceans of the physical Earth has closed, which is evidenced by the many online background maps provided by private companies~\cite{googlemaps,bingmaps,stamen} and national mapping agencies~\cite{gst2014digitalmapsupply}. On the other hand, the daily emergence of billions of geocoded data records on Web~\cite{cohen2011journalism,planetlabs2014flock1,openstreetmap,sankaranarayanan2009twitterstand,zooniverse2014oldweather} is opening a new chapter that must be written. This thesis will focus on the efficient and effective processing of emerging geographic information for thematic map visualization, both from a human and system perspective. The life cycle of a map has two stages; it is first created and then distributed. This thesis will study and address selected and important challenges in both stages of the map life cycle.
...
\subsection{Map Consumer Interfaces}
Besides HQ maps...
High Availability
High Scalability
Customizable
