...
During the design process for this SQL language extensions, some key requirements where identified: An extension to SQL for multi-scale selection must (1) allow map designers to express binary and set-based record constraints that should be satisfied by the selection, (2) allow map designers to define how record importance should be evaluated for each scale of a map, (3) allow map designers to freely define the notion of a set of scales in a map, (4) allow multi-scale selection in both the star and ladder paradigm~\cite{foerster2010challenges},  (5) allow programs to reference data stored in relational database tables.
