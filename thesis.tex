...
The previous chapters have described the systems that has been developed during this PhD project: Glossy, CVL, and Tileheat. While the evaluation of systems show that crucial progress has been made, they are also limited in some important ways. The sections below discuss some of these limitations and will sometimes indicate directions for future work that can potentially overcome them.

\section{Limitations of Glossy and CVL}
Glossy and CVL are systems for declarative and holistic selection of tabular data for multi-scale map and data visualizations. The systems have been built on-top-of the PostgreSQL database engine, which is a mature transactional database system. The approach has been to compile queries in Glossy SQL and CVL into SQL in the PostgreSQL dialect. The compiler output is an SQL program that solves an optimization problem (set multicover problem), which corresponds to the holistic selection problem.
