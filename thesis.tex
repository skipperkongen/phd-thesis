...
While tree-based real-time mapping seemingly ensures low-latency data abstraction, there are two key disadvantages when compared to pre-computed content that is served from a key-value stores or content delivery network (CDN): 

\begin{itemize}
\item The complexity of any tree-based access method (let alone generalization method) is high compared to the $O(1)$, which is the complexity of looking up pre-computed content in a hash-based key-value store~\cite{pilchin2007dynamo} or CDN.
...
In conclusion, current real-time map systems are inappropriate for maps that target large web audiences, since their base performance is significantly lower than key-value store and they do not cost-effectively scale with concurrent users.

\subsection{Batch-oriented Systems}
In batch-oriented systems, the map creation process is asynchronous with the map distribution process: the logical and/or graphical models are materialized separate from the primary data model. At this point, the pre-computed map can be partioned, e.g. using a tile pyramid, and replicated onto a content delivery network (CDN). For example, systems that implement the XYZ web mapping protocol, such as OpenStreetMap, typically function in this way.
...
\item Pre-computed maps can be partioned and replicated onto CDNs, which scale extremely well with concurrent queries.
...
Dispite the drawbacks, batch-oriented mapping systems are more appropriate than real-time systems when the number of concurrent users is high, as in a large-scale web context. Failure to scale in concurrent requests and data volume is simply a make-or-break requirement of mapping systems on the Web.  Furthermore, the disadvantages are not too severe. First, disk space have become perhaps the cheapest resource to acquire and memory is also bound to become cheaper over time.Second, materialized view can be refreshed quite fast as demand dictates, since highly scalable data processing systems can be used to power a batch-oriented system. Finally, user preferences can to some extent be accommodated by materializing views for different user profiles and choose between them at query time.
