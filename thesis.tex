...
% Map generalization
The three dimensions of an image (x, y, color) make the visual perception our most powerful perceptive system. But images have only three dimensions and the consequences of this limit are important~\cite{bertin1973semiologie}. Digital map visualization is the process by which map designers visually represent the complex geography of Earth in the finite 3-D matrix of pixels on a computer screen. On the web, maps are frequently used to illustrate new and exciting geographical datasets on top of existing background maps~\cite{google2014maps,stamen2014tiles}. Informally, this process is known as a geographical ``mash-up''. While existing background maps are generally of high quality, the result of many hours of expert labor, new datasets presents a raw challenge for the would-be designer of a geographical mash-up: creating legible and informative maps over new datasets is time consuming! Simply displaying unfiltered symbols for raw geographical data on top of an existing background map will not suffice (see Figure~\ref{fig:raw:poi:data}). Indeed, one of the most fundamental challenges in digital map creation is a process known as \emph{map generalization}. Map generalization is a complex information abstraction process by which map designers transform data to legibly represent as much as possible of the relevant information on (relatively) small scale maps~\cite{weibel1999generalising}. For example, a map designer may choose to \emph{select}~\cite{topfer1966principles}, \emph{simplify}~\cite{douglas1973algorithms},  \emph{displace}~\cite{lonergan2001iterative}, and \emph{amalgamate}~\cite{haunert2006landcover} geographical information in order to emphasize the important and suppress the unimportant.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.5]{figs-thesis/data-draws-the-world.pdf}
\caption{Data vs territory: On a small scale map of Europe, the data symbols for points of interest (left map) almost entirely cover the national territories (right map). It illustrates how for large datasets (thousands of records or more) it does not make sense to display unfiltered data. For example, there are 28 thousand points of interest near Paris; which one is the Eiffel Tower on the left map?}
\label{fig:raw:poi:data}
\end{center}
\vspace*{-4ex}
\end{figure}

The web has transformed the activity of map visualization since Roger Tomlinson first conceived geographic information systems (GIS) in the 1960's~\cite{ucgis2014tomlinson}. First, the crowd of map designers has changed, from a few GIS specialists in offices to many data entusiasts on the web~\cite{hanrahan2012enthusiast}. Second, the geographic data has changed, from moderately-sized and mostly curated datasets inside organizations to huge and often user-contributed datasets on the web~\cite{gst2014digitalmapsupply, openstreetmap, zooniverse2014oldweather}. Third, the audiences of map visualizations have changed, from a few people in offices who interact with mapping systems during business hours, to millions of people on the web who interact with mapping systems 24 x 7 and on the go. Fourth, the purpose of mapping has changed, from being mostly a professional activity, to being simultaneously a professional, personal and social activity. Admitedly, these four points are somewhat a caricature of reality, however the directionality of where map visualization is going is evident all around us. Undoubtedly, the impact of the web has caused important problems that are not adequately addressed by state-of-the-art GIS. 

Given these new opportunities on the web, i.e. so much interesting data (millions of records) and potentially huge audiences (millions of people), more and more individuals and organizations (e.g. news agencies) are motivated to communicate spatial narratives on interactive web maps~\cite{bradshaw2013online}. Some of these maps are easy to produce, whle others are more difficult. There are two extremes in the types of maps that are being created. At one extreme, map such as choropleth maps~\cite{jenks1963generalization} and heatmaps~\cite{wiki2014heatmap} present a highly statistical view of information~\cite{guardian2013ananas}. For example, maps that show the distribution of votes by region in an election. At the other extreme, map such as thematic point maps present an immersive, interactive and detailed view of information~\cite{nytimes2010iraq}. For example, maps that show the location of restaurants, hotels and tourist attractions. While both types of map are highly valuable, the former maps are straight-forward to produce using spatial statistics~\cite{ripley2005spatial}. In contrast, the latter maps present a fundamentally harder challenge to map designers due to the need for map generalization. For the latter type of maps, the generalization methods of selection and simplification seem to be most appropriate, since they maintain the identity of individual records where amalgamation does not. For this reason, this thesis will focus on the specification and scalable processing of multi-scale selection of large geographical information sets.

This thesis picks up a challenge for computer science, which was put forward by geographers more than fifteen years ago~\cite{weibel1998perspectives}. While numerous works have addressed the challenge, the web has dramatically changed the base conditions for performing map visualization in the recent years. Therefore, the goal of this thesis is to contributes new knowledge that can be put into scalable map visualization systems on the web; systems that are specialized for data enthusiasts who wish to design immersive, interactive web maps over large and constantly emerging geographical datasets.
...
Map generalization is traditionally the study of very domain-specific methods with a high focus on the geographical context. No doubt, the computational methods that are needed to produce cartographically challenging maps, suchs as physical and topographic maps, need to be highly aware of the geographic context~\cite{}. However, multi-scale selection of data for geographic mash-ups is a arguably a much simpler problem than generalizing data for a topographic map. For this reason, the selection problem can conceivably be mapped to a more abstract and less geography-specic problem, which however still captures the relevant elements of multi-scale selection for maps. If this was possible, and the abstract problem was well-known, we would theoretically be able to exploit previous work on efficient algorithms for this problem. Additonally, we would possibly be able to reuse the algorithmic framework for non-geography related data visualization as well.

The key question is: what are the key components of multi-scale selection for map visualization and can they be captured by a more abstract and well-known problem for which efficient algorithms exist?

%Typically, real-life geographical information is spatially skewed~\cite{xu2008skew}. Therefore, unless data is appropriately reduced, maps may appear cluttered in some areas while being completely empty in other areas (see Figure~\ref{fig:raw:poi:data}). While, random sampling and simple record-at-a-time filtering seem like obvious ways to reduce clutter in a map, they will not work. These reduction processes do not capture binary or multi-way object relations~\cite{egenhofer1991point, randell1992spatial} are not captured. Hence, these simple techniques can not ensure that a map legibly displays as much of the data as possible nor even that record symbols will still not overlap after filtering.

%In contrast, \emph{Holistic multi-scale selection} is a more sophisticad sampling process that filters data, according to constraints~\cite{woodruff1998constant, topfer1966principles} and objectives~\cite{harrie1999constraint} that must be satisfied. For example, a map visualization may require that unit areas of the map stay below a certain object density or that objects are separated by a minimum distance (see Figure~\ref{fig:introduction:holistic:selection}). Moreover, since in general not all data points are equally important, it is desirable to prioritize conflicted objects based on some measure of importance. Finally, to enable coherent maps over multiple scales, it is important that holistic multi-scale selction can be performed consistently over a set of scales~\cite{foerster2010challenges}.
...
\item \textbf{Declarative Specification}: There are currently two extremes in how state-of-the-art systems let users influcence the selection of data for multi-scale maps. At one extreme, fully-automated selection systems completely encapsulate the logic inside an algorithm library~\cite{sarma2012fusiontables}. Such systems exclude the map designer from any influence over the data selection process, including how record importance should be determined and which constraints should be enforced. At the other extreme, selection frameworks require map designers to write advanced and very geography-specific code~\cite{ware2003generalization_sa,lamy1999application}. Intuitively, the complexity of the solution should match the complexity of the problem. While selection of data for thematic overlays is not as complex as the generalization of a topographic map, it is also not so trivial as to varant completely unsupervised methods. First, there are many ways to score the importance of objects based on their attributes. Second, there are at least two different ways to make maps more legible, which are to bound the proximity of symbols and the density of symbols. It makes sense that map designers should be able to influence which of these controls are used during the selection process.

Ideally, designers could specify just the high-level constraints and objectives for a map in a versatile and concise specification language, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. Declarative languages, such as SQL, fit the bill for such as language, which has driven their prolific use in data management. However, there is a lack of declarative langauges that can control a multi-scale selection pipeline for map visualizations. Since visual abstraction has been addressed by previous work~\cite{jacques1967semiologie, nickerson1994visual, wilkinson2006grammar}, this thesis will focus on model abstraction stage of the pipeline (see Figure~\ref{fig:introduction:multiscale:selection:pipeline}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.4]{figs-thesis/abstraction-pipeline.pdf}
\caption{\emph{Multi-scale selection pipeline: the pipeline illustrated in the figure transforms consists of two stages. Stage one (model abstraction) transforms Hi-Fi tables of geographic data into filtered Lo-Fi views corresponding to the data elements that should appear on a map. Stage two (visual abstraction) transforms data records into data symbols on the map. The pipeline is controlled by a declarative specification of goals (tanned boxes) and executes processes (white boxes).}}
\label{fig:introduction:multiscale:selection:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

The key question is: what is an appropriate declarative language that allows map designers to express control over multi-scale selection pipelines in map visualizations?

% IN-DATABASE PROCESSING
\item \textbf{In-database Processing}: The web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap, zooniverse2014oldweather} datasets that can be visualized in geographical mash-ups. While geographical data is characterized by great diversity, is also reaches great volume. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to online location-based services, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and shopping~\cite{}, geographical information can be produced by anybody, anytime, and anywhere.

A map visualization systems must be able to scalably process a specification for multi-scale selection, since the data may be large. One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a higher-level language such as SQL, which reuses optimization techniques that are built-in to SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Additionally, since data for map visualizations is typically stored and managed in RDBMS~\cite{wu2014case} it makes sense to process the data where it is stored (see Figure~\cite{fig:introduction:indatabase:execution}). Despite the obvious opportunity, state-of-the-art map visualization systems do not significantly leverage RDBMS technology to process multi-scale selection~\cite{wu2014case}.

The key question is: how we can leverage RDBMS technology to its full potential to scalably process multi-scale selection for map visualizations?
...
The key question is: How can we exploit a log of previous map requests to cost-effectively materialize a hot set of tiles and lower the average latency of map requests?
\end{itemize}
...
First, this thesis contributes an identification of equivalence between the selection problem in map generalization and the set multi-cover problem. There is an downside and upside to this equivalence relation. The downside is that the set multicover problem is NP-hard~\cite{vazirani2001approximation}. The upside is that several efficient approximation algorithms have been contributed for this problem~\cite{chekuri2012setmulticover,rajagopalan1998primal}. The upside of the equivalence relation forms the background for a contribution of this thesis, namely that the application of a single efficient algorithm can solve many different variations of the multi-scale selection problem in map visualization, which supports a very versatile class of user-defined constraints and record importance measures.

Second, this thesis proves that simple sorting can solve the set multi-cover problem to optimality for a common and important case, namely the weighted selection of geographic points subject to a tile density constraint.

Third, this thesis contributes a formulation that captures a host of spatial constraints in the form for binary and set-based selection constraints. Furthermore, these two classes of constraints are formulated in terms of more abstract edge-selection constraints in a bi-partite graph. It is shown that an edge-selection constraint in bi-partitie graphs captures diverse and concrete spatial constraints such as object proximity and object density in geographical space (see Figure~\ref{fig:introduction:selection:constraints}).

The first and second contributions are presented in Chapter~\ref{chapter:cvl}  and the third contribution is presented in Chapter~\ref{chapter:glossy}.
...
\subsection{Glossy SQL Syntax and Semantics}
\label{sec:introduction:contributions:glossy}
% NOTFINISHED
Section~\ref{introduction:problem:statement} posits the need for declarative languages to control a multi-scale selection pipeline in map visualizations.

First, this thesis contributes Glossy SQL, a syntactical extension to SQL that allows visualization designers to declaratively specify multi-scale selection for map visualizations. Furthermore, it is shown that the language captures selection, not only for multi-scale maps, but for a substantially larger class of multi-dimensional data visualizations, which includes timelines. The choice of an extension to SQL allows programmers to concisely and intuitively express multi-scale selection in a familiar and productive programming style.

Second, this thesis contributes a structural operational semantics for Glossy SQL in terms of equivalent relational algebra queries. Furthermore, this facilitates compilers that translate Glossy SQL into relational queries for scalable execution in relational databases.

These contributions are presented in Chapter~\cite{chapter:glossy}..

\subsection{Glossy Compiler}
\label{sec:introduction:contributions:glossy}

In Section~\cite{introduction:problem:statement} the case for scalable in-database processing of multi-scale selection was made.

First, this thesis contributes a compiler for Glossy SQL that translates queries for multi-scale selection into a query in pure relational algebra. The compiler targets the commercial PostgreSQL RDBMS, which is prolifically used in the geographic community due to high-quality spatial extensions.

Second, this thesis contributes an experimental evaluation of the Glossy SQL compiler for a range of real-life datasets.

These contributions are presented in Chapter~\cite{chapter:glossy}. A prototype for the Glossy language and compiler is presented and experimentally evaluated in Chapter~\cite{chapter:cvl}.
...
\begin{itemize}
\item \textbf{Glossy SQL compiler}: pending
\item \textbf{Glossy SQL prototype}: \url{https://github.com/skipperkongen/declarative-cartography}
\item \textbf{Tileheat}: \url{https://github.com/skipperkongen/tileheat}
\end{itemize}
