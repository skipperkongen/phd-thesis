...
While some form of holistic multi-scale selection has appeared in a few recent map visualization systems~\cite{sarma2012fusiontables}, there is a lack of better models for holistic multi-scale selection that can incorporate user-defined constraints, objectives, and notions of scale. This dissertation will focus on this problem by developing a richer model for holistic selection.

% DECLARATIVE SPECIFICATION
% DONE 1
\item \textbf{Declarative Specification}: Map designers, especially those who are not GIS-experts, should ideally be able to express holistic multi-scale selection over large geographical tables in a highly productive way. One approach is to express holistic selection in low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, non-experts can not be expected to write low-level code. In a better approach, designers specify just the high-level constraints and objectives for a map in some intuitive specification language, while leaving the minute selection of data to the system. This goal fits exactly the vision of reverse data management as presented by Meliou at al.~\cite{meliou2011reverse}. The authors propose a declarative langauge for what they call \emph{how-to} queries. The idea behind a how-to query is specify a set of constraints and an objective for a target database that must be computed from a source database. This fits exactly the general pattern for holistic multi-scale selection queries, namely, given an input table of geographical data, compute an output table that satisfies a set of constraints and objectives.

Latching on to the vision of reverse data management, this dissertation will study and design declarative langauges for holistic multi-scale selection. Crucially, such languages must: (1) be intuitive and productive for geographical non-experts, (2) allow map designers to express constraints and objectives that should be enforced during selection, (3) allow map designes to express both the star and ladder paradigm of multi-scale data abstraction~\cite{foerster2010challenges},  (4) target geographical data that is stored in relational databases, and (5) encourage code sharing and reuse through schema-oriented language features, similar to SQL. In the literature no such language or its equivalent exist. To fill the gap, this dissertation will study and design declarative languages for expressing holistic multi-scale selection.
