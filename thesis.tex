...
\begin{emphpar}
\textbf{Intelligence}: from Latin \emph{intellect-}, formed from \emph{intellegere},, ``to understand, to comprehend''; from \emph{inter-}, ``between'' plus \emph{legere}, ``to pick out, to read''; \emph{Legere} is the etymological root of the English words \emph{select} and \emph{legible}. Maps present geographic information in legible form, which is selected with the purpose that a viewer may gain intelligence about the World.
\end{emphpar}
...
\caption{\textbf{Multi-scale web mapping pipeline}: a typical web mapping pipeline transforms geographic data between progressively more abstract and scale-dependent models of reality: First, reality is modelled as a set of discrete high-fidelity objects in a primary data model (1). Second, the primary data model is geometrically and semantically abstracted into an logical multi-scale data model (2). Third, the logical multiscale data model is transformed into a graphical multi-scale map model (3). Fourth, multi-scale models are distributed over the Internet for display in mobile and web mapping clients (4). Fifth, when multi-scale data models are distributed to clients, they must be transformed into multi-scale map models client-side (5).}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

A typical multi-scale web mapping pipeline has many moving parts, which consists of \emph{components} that encapsulate geographic data in various states and \emph{pipes} that move and transform geographic data between the components (see Figure~\ref{fig:introduction:requirements:pipeline}). Left-to-right, the geographic information transitions from a set of high-fidelity objects stored in a database to a set of scale-dependent graphics displayed in a zoomable map interface. It is important to identify the requirements for each component and pipe in the system along with any human-computer interfaces, since this allows us to characterize any gaps that may exist between state-of-the-art mapping pipelines and a hypothetical mapping pipeline, which meets all of the requirements.

This section focuses on the requirements for online thematic mapping applications, which is a precursor for the problem statement in Section~\ref{sec:introduction:problem:statement}.

\subsection{Map Constraints}

Satisfaction of basic map constraints is perhaps the most fundamental requirement of a mapping system. Homo Sapiens, the principal designer and audience of map visualizations, has remained biologically unchanged probably for thousands of years. Even as the Web has impacted when, why and how map visualizations are created and consumed, the basic ability of humans to perceive graphic information has remained powerful, yet fundamentally limited~\cite{bertin1973semiologie}. Consequently, modern maps on the Web must be guided by the same well-studied principles as ancient maps on print. In the schematic representation of mapping pipeline that is shown in Figure~\ref{fig:introduction:requirements:pipeline}), map constraints guide the data abstraction over transitions 2, 3 and 5.

Map constraints can be classified into two broad categories and five fundamental types: the first three types quantify the need for change (unary, binary and space constraints); the last two types prohibit specific changes (topological and cross-scale constraints):
...
\item \textbf{Space constraints} state conditions that must be satisfied by spaces, for example for each tile in a tiled map. One of the earliest impirical observations about geographic maps is that information density on published maps tends to be constant and independent of the map scale~\cite{topfer1966principles}. This principle is termed the \emph{Radical Law} and is also known as the \emph{Principle of Constant Information Density}~\cite{woodruff1998constant}. When information density on parts of the map reaches some threshold, the information should be abstracted by selection or aggregation in order to reduce the density (see Figure~\ref{fig:introduction:selection:constraints}).
...
In order to satisfy constraintsA map that simultaneously optimizes objectives and satisfies constraints must generally trade accurate representation of geographic information for abstract representation of geographic information. Intuitively, data is abstracted to the point where the information becomes legible and the map useful. Geographic information may be selected~\cite{nutanong2012multiresolution,sarma2012fusiontables,schmid2013opensciencemap}, projected~\cite{douglas1973algorithms,lonergan2001iterative} or aggregated~\cite{haunert2006landcover} in order to better emphasize the important and suppress the unimportant on a map. This process is known as \emph{map generalization}~\cite{harrie2007modelling,mackaness2011generalisation,weibel1997generalization}. A major goal of this thesis is to help the fields of map visualization and relational data management to converge. Therefore, this thesis adopts a relationally inspired operator set to describe the transformations during the map generalization process:
...
Humans must control two aspects of the map design process: (1) what information to display, and (2) how to display the information. For example, what information to display can be altered by selection, projection and aggragation of the available data; how to display the information can be altered for example by representing information by different visual variables~\cite{bertin1973semiologie}. Because the map design process is partly a human task, it must ideally be performed by time-efficient and intuitive methods. Furthermore, the map design method must allow humans to create maps that satisfy and optimize the types of constraints and objectives that were outlined in the previous section.
...
\subsection{Online Map Applications}
Besides HQ maps...
High Availability
High Scalability
Customizable
...
\item \textbf{Declarative Specification}: State-of-the-art mapping systems are currently polarized into two extreme approaches for selection of geographic data: fully-automated selection~\cite{sarma2012fusiontables, nutanong2012multiresolution} and complex programmable map generalization frameworks~\cite{lamy1999application, ware2003generalization_sa}. There is currently a lack of systems that offer map designers a balanced solution that is simple, yet programmable. In contrast, existing solutions are either too inflexible or too time-consuming.
