...
\minisec{Geographic Objects}
Geographic information is typically represented by discrete and multi-dimensional \emph{geographic objects}. For example, geographic objects can represent roads, buildings, lakes, locations, restaurants and much more. Each object associates a \emph{geometry} (point, line or area), with an additional \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. For example:

\begin{itemize}
\item Geometric attributes specify spatial \emph{location} and \emph{extent}. For example, \ang{55.6778}, \ang{12.5962} may be the location of a restaurant
\item Qualitative attributes allow \emph{distinction} between their possible values. For example, Nordic cousine is different from Japanese cousine
\item Ordered attributes allows a \emph{ranking} of their possible values. For example, gourmet restaurants are more expensive than fast-food restaurants
\item Quantitative attributes allow \emph{aritmetic} to be performed on their values. For example, DKK 1.600 plus DKK 1.000 is DKK 2.600.
\end{itemize}

\minisec{Visual Variables}
Geographic objects are visually represented on a map by choosing appropriate \emph{visual variables} for some or all of their attributes (see Figure~\ref{fig:introduction:visual:variables}). The visual variables of \emph{position},  \emph{size}, \emph{value}, \emph{texture}, \emph{color}, \emph{orientation}, and \emph{shape} were formalized in the seminal work by Bertin~\cite{bertin1967semiologie}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.7]{figs-thesis/visual-variables.pdf}
\caption{\textbf{Bertin's seven visual variables}: The variables express qualitative, ordered and quantitative attributes (reproduced from \texttt{infovis-wiki.net}).}
\label{fig:introduction:visual:variables}
\end{center}
\vspace*{-4ex}
\end{figure}

The position, which is a two-dimensional variable, is usually reserved for the coordinates of the geometry, which are first projected onto the plane using a standard map projection~\cite{snyder1997flattening}. It should be noted that the position could perfectly well represent another pair of attributes, such as the average life expectancy and economic wealth of a country~\cite{rosling2007poverty}. 

Visual variables have an affinity towards certain types of attributes. For example, as colors have no inherent order they express qualitative attributes best. In contrast, size can be measured and therefore express quantitative and ordered attributes best.

\minisec{Map State}
Intuitively, the map state is consists of geographic objects and symbols that appear on a map. More formally, the map state can be divided into the \emph{logical state}, which is the set of geographic objects that appear on a map, and the \emph{graphical state}, which is the set of symbols that visually represent the objects on the map. 

Logical map states can be derived by performing operations on the geographic objects:
