...
Semi-structured data, such as XML and JSON, is used in many contexts on the Web. For example, web services typically return data as either XML or JSON. These data formats do not generally conform to the regular schemas of relational data. However, semistructured can be transformed sinto tabular form, e.g. by using a view~\cite{}, which is a data integration approach~\cite{halevy2006data}. This approach would allow the Glossy and CVL systems to compute multi-scale subsets of semi-structured data. Another approach would be to rewrite the multi-scale abstraction semantics for semi-structured data, e.g. by including some form of aggregation. However, this project has not attempted such a direction.

\minisec{Scale-out Processing}
Limitations in data size. Results are shown for Postgres, because emphasis on correctness and reasonable scalability. However, other and more scalable databases or cluster systems could be utilized with many advantages, such as x, y, z would certainly result in increased scalability of the system. What would it take?

\minisec{User-studies}
