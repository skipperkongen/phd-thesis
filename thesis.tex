...
\emph{What are the key questions?} To reason about a data visualization management systems, three crucial and orthogonal questions must be investigated. First, how do we express the task of transforming data into a useful visual representation, which includes a transformation from (possibly) non-geometric data to a geometric interpretation? Second, how can we scalably process the millions of records that are often the foundation of a data-driven visualization? Third, how can we scalably serve data-driven visualizations to millions of online users, who expect 24 x 7 availability and acceptable latency? Answering each of these questions will require us to investigate areas beyond pure data visualization. For example, programming languages address the issue of expressing data transformation~\cite{sql}; databases address the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; distributed systems address the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. It is crucial that we pull together the relevant knowledge from these diverse fields and combine it into a system for efficient and effective data visualization management.
