...
% Map generalization
Digital map visualization studies how the intricate geography of the Earth can be visually and geometrically represented by the finite pixels on a computer screen. Digital maps allow onlin users to explore geographical information at multiple focal points (panning) and levels of detail (zooming). On the web, a geographical mash-up is the process by which designers of maps will layer new and interesting geographical data on top of commonly shared background maps~\cite{vizzuality2012cartodb}. While the background maps are generally of high-quality, the result of many hours of expert labor, new datasets present a raw challenge for creators of mash-ups. Simply displaying symbols for raw geographical data on top of a background map will not work (see Figure~\ref{fig:raw:poi:data}). One of the most fundamental problems in map creation is the mismatch between data volume and the small representational budget of the medium. For example, a computer screen can display just a few thousand distinct and legible symbols, which is entirely indadequate for displaying all individual records of most real-life sized geographical datasets. To address this problem, the field of \emph{map generalization} studies how to legibly represent as much as possible of the relavant geographical information in mediums with finite representational budget~\cite{weibel1999generalising}. For example, geographical data can be generalized by \emph{selection}~\cite{topfer1966principles}, \emph{simplification}~\cite{douglas1973algorithms},  \emph{displacement}~\cite{lonergan2001iterative}, and \emph{amalgamation}~\cite{haunert2006landcover}.
...
Today, the web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap, zooniverse2014oldweather} datasets that can be visualized in geographical mash-ups. While geographical data is characterized by great diversity, is also reaches great volume. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to an online services, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and games~\cite{geocaching2014geocaching}, geographical information can be produced and consumed by anybody, anytime, and anywhere.

% lots of in-experiences designers who don't know GIS. Aggregation is too easy,  selection is too hard or not hard enough.
Given the availability of so much interesting data, more and more organizations such as news agencies~\cite{nytimes2010iraq,guardian2013ananas} are motivated to communicate spatial narratives using online maps. There are two extreme types of maps that need to be created. At one extreme, choropleth maps~\cite{jenks1963generalization} and heatmaps~\cite{wiki2014heatmap} present a statistical and aggregated view of information~\cite{guardian2013ananas}. For example, users may consult a choropleth map to understand how votes are distributed among parties in a region-by-region election. At the other extreme, point maps present an immersive, interactive and detailed view of information~\cite{nytimes2010iraq}. For example, users may consult a point maps to find a particular restaurant, hotel, gas station, or other point of interest. While both are valuable, the former maps are fairly easy to produce using statistics. In contrast, the latter maps present a fundamental challenge to map designers due to the need for map generalization. Selection, simplification and displacement all retain the possibility for immersive, interactive and detailed map, however only selection can generally solve the problem of cluttered maps for all cases (assuming that it is acceptable to display a subset). For this reason, this thesis focuses on how novice map designers can express multi-scale selections for maps in a highly productive and yet flexible way. Certainly, high-quality tools do exist that will produce a multi-scale selection of data, however they are either too simple (i.e. do not support user-defined selection)~\cite{sarma2012fusiontables} or too time consuming~\cite{mapnik, sld}.

% As a consequence, more and more professionals will need to acquire the skills to craft map visualizations for the web. However, demand for maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS). 

% lots of users
Serving maps to millions of people presents an orthogonal challenge to the problem of creating maps. Thanks to innovations in mobile mapping applications and ubiquitous web access, a map visualization can be consumed by large numbers of concurrent users. For this reason, maps must scalable in the number of visitors. Furthermore, maps must be highly available, since this web users expect 24 x 7 availabilty of services on the web. In areas such as image serving, this problems has been solved by employing data partitioning and data replication~\cite{haystack, barroso2009datacenter}. In the area of maps, one more technique is required in order to enable partitioning and replication: geographical data must be linearized into a one-dimensional key-space by using a bucketing technique such as tiling~\cite{decola93,mbtile12}. While tiles have traditionally been used mostly for serving map imagery~\cite{}, it has been proposed that tiles can also be used for serving data~\cite{gaffuri12vectortiles}. Creating a multi-scale tile pyramid can be achieved in many ways. If speed is the only concern, a mapreduce approach will quickly render a large number of tiles, since the problem is highly parallelizable~\cite{dean04,dean2009designs}. However, the number of tiles grow exponentially in the number of zoom levels of a map. So, if storage, I/O and processing cost are an issue, it may be useful to materialize only a subset. This thesis studies the question of how to pick a good subset of tiles to materialize, i.e. the subset that will most likely satisfy all requests in the near future.

Data visualizations must scale well both in the size of the data~\cite{Im2013visreduce, battle2013dynamicreduction} and the size of the audience~\cite{viegas2007manyeyes,scalablemaps}. Moreover,  In-database execution addresses the issue of scalable data processing where the data for visualizations is most frequently managed~\cite{wu,ferry, }; 

 declarative languages and visual design interfaces address the issue of intuitive creation of compelling and complex data visualizations~\cite{wu, someothers}. This dissertation investigates the design of a data visualization management system that meets all of these requirements, simultaneously. To address the short-comings of GIS for the web, this dissertation makes the case for a new category of systems: Map Visualization Management Systems (MVMS). The vision of MVMS is influences by the visions of Reverse Data Management~\cite{meliou2011reverse} and Data Visualization Management Systems (DVMS), while specialized for the particular challenges of map visualization (see Figure~\ref{fig:mvms}).
...
\begin{enumerate}
\item How can we intuitively express the creation of multi-scale map visualizations?
\item How can we scalably process millions of geographical records for map visualizations?
\item How can we scalably serve map visualizations to millions of online users, who expect 24~x~7~availability and acceptable latency?
\end{enumerate}
...
The web has transformed how map visualization should ideally be approached. This has opened up important new problems that must be solved, and which are not adequately addressed by the state of the art in geographical information systems (GIS). Specifically, a map visualization system must support geospatial non-experts who are increasingly tasked with designing map visualizations~\cite{wu2014case, battle2013scalar, fekete2012dmvis}. To benefit of this growing segment of map designers, systems most expose intuitive and productive map authoring interfaces. Moreover, to harness the explosion in available geograhical information and ubiquitous web access of mobile users, systems must scale infinitely both in data volume~\cite{Im2013visreduce, battle2013dynamicreduction} and the size of the audience~\cite{viegas2007manyeyes}. This dissertation focuses on how to build map visualization systems that fulfill these requirements. In particular, the following problems must be addressed:


\note{TEST: Each of the following sub-sections must (1) sound like a problem, (2) raise important and genuine concerns, (3) must not have been solved by previous work. }


\begin{itemize}
% HOLISTIC MULTI-SCALE SELECTION
% SEMICOMPLETE
\item \textbf{Holistic Multi-scale Selection} Typically, real-life geographical information is non-uniformly distributed~\cite{xu2008skew}. Therefore, unless data is appropriately reduced, maps will be cluttered in some areas while being completely empty in other areas (see Figure~\ref{fig:raw:poi:data}). While, selection is an intuitive way to reduce clutter in a map, simple selection, which filters a dataset row-by-row according to simple predicates, can not capture binary or multi-way object relations~\cite{egenhofer1991point, randell1992spatial}. Consequently, simple selection can not ensure that maps represent as much of the data as is legibly possible for all local areas. 

In contrast, \emph{Holistic multi-scale selection} is a sampling process that filters data according to constraints~\cite{woodruff1998constant, topfer1966principles} and objectives~\cite{harrie1999constraint} that are evaluated locally and globally. For example, a map visualization may require that unit areas of the map stay below a certain object density or that objects are separated by a minimum distance (see Figure~\ref{fig:introduction:holistic:selection}). Moreover, since in general not all data points are equally important, it is desirable to prioritize conflicted objects based on some measure of importance. Finally, to enable coherent maps over multiple scales, it is important that holistic multi-scale selction can be performed consistently over a set of scales~\cite{foerster2010challenges}.

The idea of holistic multi-scale selection for map generalization is not itself a new idea~\cite{weibel}. Furthermore previous work has either offered direct support for holistic multi-scale selection~\cite{sarma2012fusiontables} or proposed metaheuristic frameworks that do not necessarily preclude such an operator~\cite{lamy1999application, ware2003generalization_sa}. However, in all previously published systems, there is insufficient support for user-defined constraints, objectives, and notions of scale. This dissertation focuses on the problem of holistic multi-scale selection with strong, intuitive and explicit support for user-defined constraints, objectives, and notions of scale.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.4]{figs-thesis/abstraction-pipeline.pdf}
\caption{\emph{Multi-scale selection pipeline:} }
\label{fig:introduction:multiscale:selection:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}
...
% PROCESSING
% SEMICOMPLETE
\item \textbf{In-database Processing}: A Map visualization systems must be able to scalably process a specification for multi-scale selection of geographical data, since the data may be large. One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a higher-level language such as SQL, which reuses optimization techniques that are built-in to SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Additionally, since data for map visualizations is typically stored and managed in RDBMS~\cite{wu2014case} it makes sense to process the data where it is stored (see Figure~\cite{fig:introduction:indatabase:execution}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.4]{figs-thesis/indatabase-execution.pdf}
\caption{\emph{In-database processing:} }
\label{fig:introduction:indatabase:execution}
\end{center}
\vspace*{-4ex}
\end{figure}

While the paradigm of in-database processing for visualizations has been proposed in a recent vision by Wu et al.~\cite{wu2014case}, their vision does not adequately address the special challenges presented by map visualization. This dissertation will design and implement a compiler that translates high-level specifications of holistic multi-scale selection into pure SQL, which can be scalably evaluated inside an RDBMS.

% SERVING
% SEMICOMPLETE
\item \textbf{Predictive Materialization}: Map visualization systems must be able to serve tiled multi-scale maps to large web audiences who expect 24 x 7 availability and acceptable latency~\cite{gaffuri2012toward, pilchin2007dynamo, beaver2010haystack}. In particular, the advantages of scalable distribution systems, such as content delivery networks (CDN), must be cost-efficiently leveraged~\cite{beaver2010haystack, datacentre}. However, since the number of tiles grow expenentially in the number of supported scales, fully materializing a multi-scale map into tiles and storing it in a CDN is costly. 

%Linearization, data partitioning, data replication, and key-value stores address the issue of scaling data visualization services to large audiences who expect 24 x 7 availability and acceptable latency~\cite{gaffuri2012toward, pilchin2007dynamo, beaver2010haystack};
...
\note{(I guess) these are like mini summaries of the articles. They should of course explicitely connect to the problem statements above.}
...
\subsection{Declarative Multi-scale Selection}
\label{sec:introduction:contributions:cvl}
% NOTFINISHED

Something resembling the abstract

\subsection{In-database Processing of Multi-scale Selection}
\label{sec:introduction:contributions:glossy}
% NOTFINISHED

Something resembling the abstract. 

\subsection{Prediction for Tile Materialization}
...
Map generalization is a process performed by map designers to adapt and transform geographical information to retain as much as possible of the relevant geographical information in geographical map, subject to constraints of legibility~\cite{weibel1999generalising}. To create an uncluttered map, geographical data can be \emph{selected}~\cite{sarma2012fusiontables}, \emph{simplified}~\cite{douglas1973algorithms},  \emph{displaced}~\cite{lonergan2001iterative}, and \emph{amalgamated}~\cite{haunert2006landcover}. A more aggressive approach is \emph{statistical aggregation} of geographical information, such as choropleth maps, which eliminates the explicit representation of geographical data altogether~\cite{friendly2008milestones}. For a more complete overview of this complex field, you are refered to the works of Weibel and Dutton~\cite{weibel1999generalising} as well as Jones and Ware~\cite{jones2005webage}.  

The main types of geometric transformation are: reduction in the number of discrete 
features that are portrayed (geometric selection); reduction in the detail of individual 
line, area and surface features (typically reduction in sinuosity); amalgamation of 
neighbouring features, whether point, line or area-referenced; reduction in 
dimensionality, or collapse, from area-referenced features to line or point referenced 
features; exaggeration of the size of significant features that would otherwise be too 
...
This thesis proposes a new map generalization technique for multi-scale selection. To ensure that multi-scale selection is a solid solution, it must be evaluated in the context of well-known principles from the map generalization literature. This section reviews the most important principles.

\subsection{Constant Information Density}

Cite woodruff and toepfer.

\subsection{Mobile Map Design Objectives}
...
\subsection{Qualification of multi-scale selection}

The conclusion is that multi-scale selection mets all of these requirements.
...
Two-dimensional data structures, such as quadtrees~\cite{bereuter2013real} and GAP-trees~\cite{van1995gap}. One-dimensional data-structures, such as B-tress and Hash Tables, can efficiently emulate certain aspects of two-dimensional datastructures by employing the techniques of linearization~\cite{zcurve, hilbertcurve}. For example, the nodes of a quadtree can be adressed by one-dimensional strings over the alphabet $\lbrace A, B, C, D \rbace$, where testing the existance of a node in a subtree corresponds to testing for prefixes. Finding keys that match a prefix is an efficiently operation in a B-tree. However, what?
...
This thesis proposes a batch-oriented approach to perform multi-scale selection of geographical data.
Conclude on the sensibility of performing batch-oriented multi-scale selection by using context from the three previous sections.
...
In map generalization, Gr{\"u}nreich is attributed with a model that divides map generalization into two separate tasks of model generalization and cartographic generalization (see Figure~\ref{fig:state:of:the:art:gruenreich}). While this is a very useful distinction, the terms are not natural outside the domain of geography, for example in data visualization. To bridge the worlds of map visualization and data visualization, this dissertation will prefer the terms \emph{model abstraction} and \emph{visual abstraction}, instead of model generalization and cartographic generalization. Paraphrasing and perhaps stretching the original definitions, model abstraction is concerned with what to display, while visual abstraction is concerned with how to display it. For example, holistic multi-scale selection belongs to model abstraction, since we are choosing what records to display. In constrast, the choice of appropriate symbols, say a red circle, for the selected data would be a visual abstraction. Clearly, these tasks are not entirely orthogonal, as for example larger symbols chosen during visual generalization would impact the constraints that should be satisfied during model abstraction (larger symbols means room for fewer objects).

While the area of visual abstraction is somewhat outside the scope of this thesis, it must be briefly touched upon. The reason that visual abstraction is mostly outside the scope of this thesis is that visual abstraction is not an area with significant unsolved problems in the context of maps. Seminal work by Bertin~\cite{jacques1967semiologie} and Nickerson~\cite{nickerson1994visual} have thoroughly dealt with the topic of graphical expression including visual expression in the context of maps. The reason why visual abstraction still merits a discussion is the presence of recent work in data visualization that puts graphical abstraction languages as the center piece of the data visualization process. For example, the Ermac system~\cite{wu2014case} proposes a language for expressing data visualizations, which is heavily influenced by Wilkinson's work on the Grammer of Graphics~\cite{wilkinson2006grammar}. It is important to note that data visualization systems, such as Ermac, can safely ignore model abstraction, since the primary use case is visualization of statistical data. Statistical data can easily be transformed into well-known graphical representations, such as bar charts, without any concerns for the legibility problems that plague (say) a point of interest map. Another system which is related to Ermac is ggplot2~\cite{wickham2009ggplot2}, which is also influenced by the Grammar of Graphics. The basic idea behind the Grammar of Graphics is that a graphical abstraction framework should formalize the fundamental graphical primitives rather than simply maintain a catalogue of chart types.
