...
\caption{Logical stages of web mapping pipelines.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

Typical web mapping pipelines have many moving parts. However, all web mapping pipelines perform the same logical tasks (see Figure~\ref{fig:introduction:requirements:pipeline}). The tasks are to (1) capture ``reality'' in a primary data model, (2) derive logical map content from the primary data model, (3) derive graphical map content from the logical map content, and (4) distribute map content over the Internet to web and mobile mapping clients. Optionally, logical map content can be distributed to clients and in this case graphical map content is derived client-side~\cite{gaffuri12vectortiles}. It is important to describe the requirements for each part in the pipeline, since this allows us to compare state-of-the-art pipelines to a hypothetical pipeline that meets all of the requirements. With the exception of stage 1, the important requirements for all stages are outlined in the following sections. 

\subsection{Map Content}
Logical mapping is concerned with \emph{what} geographic information should appear on a map, which corresponds to stage 2 in Figure~\ref{fig:introduction:requirements:pipeline}. In contrast, graphical mapping is concerned with \emph{how} geographic information should appear on a map (i.e. the graphical expression of data), which corresponds to stage 3 in Figure~\ref{fig:introduction:requirements:pipeline}. In truth, these two views of the map content are interdependent. For example, bigger graphical symbols may result in less data being selected since symbols are more likely to overlap. For this reason, the requirements for logical and graphical map content are treated together in the following.
...
Map constraints alone do not enable the ranking of feasible maps, which requires the introduction of an \emph{map objective}. One approach is to introduce penalties for violated constraints, and minimize the sum of penalties. Another approach is to introduce a scoring function for objects, and maximize the sum of scores for objects that appear in the map (see Figure~\cite{fig:introduction:selection:constraints}). 
