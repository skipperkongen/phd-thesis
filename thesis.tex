...
The quality of selections produced by Glossy SQL can be measured by several object metrics, which include density, proximity and distribution. By definition, the density and proximity of a global selection fulfills the requirements, since the these acceptable limits of these are ensured by constraints in the query. 

In contrast to density and proximity, the preservation of spatial distribution is not explicitely handled by Glossy SQL. However, there are two way to ensure this property in Glossy SQL: random sampling and spatial scaling. First, both Glossy SQL and pure SQL can easily compute random samples of the input by using a randomized predicate in the where clause. Second, a Glossy-specific feature is provided by optional keywords that can be used with a locus constraint. The spatial scaling keywords SCALE LINEAR and SCALE LOG ensure that the number of items that are selected per locus is proportional or in logarithm relationship to the average density for all loci. For example, if relatively many objects fall within a locus, then a global selection will choose more objects for this locus. 

In Figure~\ref{}, the spatial distribution of an input (airports) is compared to the spatial distribution of a global selection that uses a locus constraint and the linear spatial scaling keyword. While the global selection contains seveal orders of magnitude fewer items than the input, it can be seen that the spatial distribution is the same.
