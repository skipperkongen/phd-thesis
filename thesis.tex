...
Thematic maps are characterized by two extremes. At one extreme, highly aggregated maps, such as choropleth maps~\cite{guardian2013ananas} and heatmaps~\cite{wiki2014heatmap}, present a statistical view of geographic information (left map in Figure~\ref{fig:introduction:background:mashups}). Such aggregated maps are fairly trivial to produce using spatial statistics~\cite{ripley2005spatial}. At the other extreme, object maps, such as point of interest (POI) maps~\cite{bereuter2013real, sarma2012fusiontables,nutanong2012multiresolution}, present a detailed view of information (right map in Figure~\ref{fig:introduction:background:mashups}). Since map generalization is generally required, thematic maps of objects are usually much more challenging to produce than statistic maps.
...
This example list of \emph{constraints} and \emph{objectives} illustrates one way to quantify the feasibility and utility of a map. More formally: 
...
\subsection{Distribution of Geographic Information} In stage four of the web mapping pipeline, map content is distributed over the Internet to web and mobile mapping clients. A service level aggrements (SLA) establishes the success criteria for this process. There are three typical dimensions of an SLA:

\begin{itemize}
\item \textbf{Availability} is the property that geographic information can be reached and downloaded by mapping client. It can be defined in terms of so-called ``nines''. For example, ``two nines'' equals 99\% availability or 1.68 hours of downtime per week, while ``five nines'' equals 99.999\% availability or 6.05 seconds of downtime per week. In the EU, national mapping agencies are typically obligated to met at most ``three nines'' of availability~\cite{gst2014digitalmapsupply}. In contrast, private corporations -- such as Google and Microsoft -- rutinely operate at ``four nines'' or ``five nines''. As online map users have come to expect the relative high availability of commecial map distributions, national mapping agencies are presented with a challenge. 

\item \textbf{Performance} is the property that requests for geographic information can be evaluated with low latency. As online 
...
Given the requirements for an online wepping mapping pipeline, which were outlined in Section~\ref{sec:introduction:web:mapping:pipelines}, this section lists four important gaps between state-of-the-art mapping systems and a hypothetical mapping system that satisfies all the requirements.

The four gaps that have been identified are:

\begin{itemize}

% MATHEMATICAL FOUNDATION
\item \textbf{Unified Selection Model}
While selection of geographic information can be intuitively understood as filtering objects subject to some criteria, many different selection models have been proposed~\cite{sarma2012fusiontables,nutanong2012multiresolution,rocha2011efficient}. While each model has clear merits, there is a crucial lack of a unified selection model that subsumes them all. What is needed is a model for holistic selection of geographic data that supports: user-defined constraints, i.e. unary, object-relative, space-relative, and cross-scale constraints, as well as user-defined map objectives, either minimization of error or maximization of relevance. This thesis pursues the latter.

Ideally, a well-studied and efficient algorithm can solve all instances in the unified model, in the same fashion that sorting solves all instances in the top-k selection model.

% DECLARATIVE SPECIFICATION
\item \textbf{Declarative Specification}: Selection systems today are polarized into automatic selection systems (suboptimal for most real cases) and highly-programmable map generalization systems (complex and time-consuming even for experts). There is currently a lack of a balanced systems in-between these two extremes, which can express selection for a wide range of thematic maps and can be efficiently programmed by more than GIS experts. As a programming approach, it makes sense that programmers state just the constraints and objectives of a map in a declarative geographic selection language, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. This captures the essential requirements of map design, while being highly productive since it is a concise formulation. However, there currently does not exist such a declarative programming language for map visualizations.

% IN-DATABASE PROCESSING
\item \textbf{In-database Processing}: The web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap,twitter2014twitter,zooniverse2014oldweather} datasets that can be visualized on maps, which are typically managed in a relational database management systems (RDBMS)~\cite{wu2014case}. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to location-based services online~\cite{endomondo2014endomondo,tinder2014tinder,twitter2014twitter} geographic information can be produced by anybody, anytime, and anywhere. Therefore, mapping systems must be able to scalably process multi-scale selection over large relational datasets (millions of records). 

One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a high-level language such as SQL, which reuses optimization techniques that are built-in to existing SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Furthermore, SQL are typically supported by emerging database systems, such as parallel in-memory databases~\cite{zaharia2010spark}. Moreover, since data is typically stored in RDBMS, it makes sense to process the data where it is stored. Despite the obvious opportunity, state-of-the-art mapping systems do not leverage the full potential of RDBMS technology or relational algebra as a query language to process selection of geographic information for maps.

% PREDICTIVE TILE MATERIALIZATION
\item \textbf{Predictive Tile Materialization}: 
Rendering and serving of so-called map tiles is currently the typical way to implement high performance maps~\cite{mbtile12}. There are two extreme approaches to rendering tiles. At one extreme, the entire tile set is preemptively materialized, for example on a MapReduce cluster~\cite{dean2009designs,dean04mapreduce}. However, this is wasteful since most tiles are never or rarely requested~\cite{fisher07hotmap}. At the other extreme, individual map tiles are lazily materialized or prefetched real-time. However, this results in high average latency since workloads are temporally skewed, which causes high concurrency levels during peak load~\cite{kefaloukos2012tileheat}. A better approach is to predict popular tiles well in advance, and materialize a hot set during a low-load period and serve it with low latency during high-load periods. This assumes that slightly stale geographic information is acceptable, which is typically the case~\cite{sarma2012fusiontables}. A hypothesis is that a request log can be used to predict future tile requests.
...
Section~\ref{sec:introduction:problem:statement} made the case for in-database processing of multi-scale selection. The Glossy compiler translates queries in Glossy SQL into relational algebra, which can be processed on top of relational databases.

First, this thesis contributes an optimized implementation of the Glossy compiler, which targets the commercial PostgreSQL RDBMS~\cite{PostgreSQL}. This database is popular in the geographic data community due to its high-quality spatial extension~\cite{PostGIS}.

Second, this thesis contributes an experimental evaluation of the Glossy SQL compiler for a range of real-life datasets. The results show that the Glossy selection system is scalable for a wide range of evaluated test cases, geographic as well as non-geographic.
