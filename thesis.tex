...
Outside geography, research on data abstraction is still in its infancy. The majority of the related work focuses on data abstraction geographical data, i.e. map generalization. There is an opportunity to export useful ideas from map generalization to the wider area of data visualization. To fullfill the potential of data visualization, we must bring data abstraction techniques to where the data is primary managed, i.e. relational database systems (RDBMS). Ideally, a class of RDBMS will emerge that is specialized towards data visualization, e.g. systems that can perform multi-scale data abstraction operations over data tables. This thesis focuses on four problems that must be solved in order to build such a system.
