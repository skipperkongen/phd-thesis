...
As the human capacity to gather data has increased, the creation of maps has become more challenging; finite map mediums can only accommodate a finite number of legible symbols. This challenge has lead to a plethora of cartographic information abstraction techniques that are collectively known as \emph{map generalization}~\cite{brassel1988generalization,mcmaster1992generalization,weibel1999generalising}. Map generalisation essentially solves the problem of creating a legible, useful and accurate maps for any map scale and for any amount of geographic information. Naturally, this process involves a compromise between readability and precision for most real-world cases. For example, maps of the physical world must carefully select the subset of the available information that is visualized, or else the map would soon become cluttered and illegible. Similarly, demographic maps must often aggregate information about individuals by area in order to clarify the important trends in local populations. Finally, small scale maps must usually distort the information in order to make it more legible, for example by enlarging symbols that would otherwise be imperceivable or displacing symbols that would otherwise overlap.
...
Logically, the information on a map can be modeled as a set of \emph{geographic objects}, which associate data with a certain location. Each object models a distinct geographic concept in the real world, such as a road, lake, mountain peak, or country, by a finite set of \emph{attributes}. Each attribute of an object can be categorized as either \emph{geometric}, \emph{qualitative}, \emph{ordered} or \emph{quantitative}~\cite{bertin1967semiologie}. For example, the restaurant ``Noma'' in Copenhagen can be represented by an object with four attributes $\langle$ (\ang{55.6778}, \ang{12.5962}), ``Nordic Cousine'', ``Best Restaurant 2014'',  DKK 1.600  $\rangle$:

\begin{itemize}
\item $(\ang{55.6778}, \ang{12.5962})$ is a geometric attribute of the Noma object, which corresponds to a geographic position on the harbor front in Copenhagen, Denmark.
\item ``Nordic Cousine'' is a qualitative attribute of the Noma object, which Noma to be distinguished from objects with a ``Japanese Cousine'' attribute.
\item ``Best Restaurant 2014'' is an ordered attribute of the Noma object, which enables it to be ranked against other ordered attribute of the same hierarchy (restaurant ranks).
...
Of these attribute types, the geometric attribute type is perhaps the one most intrinsic to geographic information. Formally, a geometric attribute corresponds to a set of points; in geography, these points are typically geographic coordinates. There are three frequently used types of point sets: 
...
Bertin's work on the theory of graphics is immense, and this short section does not aim to fully cover it. Also, a related work is the ``the Grammar of Graphics'' by Wilkinson et al.~\cite{wilkinson2006grammar}. This collection of previous work covers many of the fundamental aspects of representing information as graphics, which subsumes the representation of geographic information on maps. This work will focus mostly on abstraction of geographic object sets and less on graphical representation.
...
This section briefly covers the implications of reducing the scale of a map and how map generalization can help address the problems that arise. Furthermore, this section provides a quick overview of the key approaches to map generalization. For the reader who is interested in additional knowledge about the generalization process, Mackaness, Ruas, Sarjakoski have compiled material that provides a useful introduction into the field.~\cite{mackaness2011generalisation}. Furthermore, the seminal works by McMaster and Shea~\cite{mcmaster1992generalization} and Weibel~\cite{weibel1997generalization} are important entry points into the field of map generalization.
...
A procedure that accepts a set of objects and merges subsets of these objects into single objects. Crucially, at least one object that appears in the output must not be a members of the input, as otherwise the procedure would be equivalent to selection. \label{def:aggregation}
\end{definition}
...
Overall, the generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, constraints act as the discriminator between feasible and infeasible map states~\cite{harrie1999constraint,ware2003generalization_sa}, while objectives act a measure of map utility and optimality. Together, constraints, objectives and a set of geographic information objects  implies an optimization problem over a search space of maps. The search space consists of maps that can be derived from a source map by applying generalization operators (see Section~\ref{sec:introduction:map:generalization}) to the objects and symbols.
...
The overall purpose of map constraints is to distinguish between acceptable maps and unacceptable maps, or alternatively, to define the search space of feasible maps. Constraints may state conditions that should be met by the map as a whole, on subsets of information on the map or by individual pieces of information on the map. For example:
...
\item Subareas of the map may be required to satisfy locus constraints, such as maximum density.
\item Networks may be required to satisfy topological constraints, such as not disconnecting a river system from itself or its outlet.
\end{itemize}
...
\minisec{Focus on Thematic Maps}
The Web has shifted the the focal point of map creation, such that new thematic maps and overlays are more frequently created than new background maps. Part of the reason is that private companies and national mapping agencies already offer a great quantity of background maps for the Web~\cite{google2014maps,gst2014digitalmapsupply, stamen2014tiles,vizzuality2012cartodb}. In addition, the information that appears on background maps has a tendency to remain relatively stable over time, such as the location of mountains, lakes, coast lines, cities, and roads. In contrast, thematic data typically reflects phenomena that are intrinsically more dynamic and multi-faceted, such as human behaviour.  This makes thematic maps fundamentally more news-worthy and trendy than background maps, which is why journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} frequently create thematic maps.

While all geographic data is growing~\cite{openstreetmap2014stats}, thematic data has a clear tendency to grow faster than background data. Since the relevance of thematic data is intrinsically short-lived, it is crucial that we can process it and visualize it on maps expediently. However, given how the volume of data can accumulate really fast, it is crucial that data can be processed in a scalable way.


\minisec{Large Thematic Datasets}
...
To distribute maps, content delivery networks (CDN) offer a scalable and affordable way to distribute data to a global audience~\cite{dilley2002cdn}. Some of the key advantages of CDN are their superior availability, scalability and performance~\cite{barroso2009datacenter,pilchin2007dynamo}, which is why they are used by anyone seriously engaged in data distribution~\cite{beaver2010haystack}. To harness a CDN for map distribution, the map data must first be organized into key-value pairs. For example, the key may be the ID of a geographic grid cell, while the value may correspond to the data intersected by that grid cell~\cite{gaffuri12vectortiles}. Spatial data can be partitioned into key-value pairs by several techniques which includes spatial grids~\cite{decola1993pyramid} and linearization~\cite{hilbert1891ueber,morton1966computer} techniques. Once partitioned, the spatial data for a map can be distributed to millions of people online as a set of map tiles, either as images or data.
...
\begin{itemize}
\item Data is only stored only once, namely in the primary data model. While this obviously saves storage, a more important benefit is that information that appears on a map is never stale. Given an atomic update mechanism for the data model, such as transactions~\cite{gray1981transaction}, a map always displays the most recent and consistent state of the data.
\item User preferences and the context of map usage can be taken into consideration when creating the map. Thus, maps can be tailored to individual users needs, for example by letting a recommendation algorithm play a role in assigment of object importance.
\end{itemize}
...
\begin{itemize}
\item The complexity of any tree-based access method (let alone generalization method) is high compared to the $O(1)$, which is the complexity of looking up precomputed content in a hash-based key-value store~\cite{pilchin2007dynamo} or CDN.
\item The tree structures that have been proposed are typically not designed for concurrent access. This is critical, because there is typically a high and variable level of concurrency in a web context, which implies that the data abstraction process must be scalable in the number of concurrent queries as well as data volume. The na{\"i}ve approach, which is to replicate the entire index multiple times and dispatch requests round-robin is costly, especially when data grows in size.
\end{itemize}
...
\begin{itemize} 
\item Since there is no real-time requirement, more computational effort can be spent on producing a high-quality result. Furthermore, batch-oriented systems can exploit the machinery of distributed data analytical systems, such as spatial parallel databases~\cite{waas2009beyond} and spatial cluster computing~\cite{aji2012largespatial,aji2013hadoopgis,eldawy2014spatialhadoop}, which can process practically unlimited amounts of spatial data.
\item Map data produced by a batch-oriented system can be partioned as tiles and replicated onto key-value stores, such as CDNs, which scale extremely well in the number of concurrent queries.
\item Maps that are distributed via a CDN additionally gain high availability and low latency because content can be georeplicated to edge locations of the network, which are closer to the users.  
\end{itemize}

Some important studies in batch-oriented mapping systems are multi-scale trees~\cite{frank1994multiscaletree,van2011towards} and the Fusion Tables system~\cite{sarma2012fusiontables}. However, these systems only partially realize the full potential advantages of the batch-oriented paradigm! For example, distributed spatial processesing systems are not significantly leveraged by any of the systems, except for rendering of map tiles~\cite{dean2009designs}. Thus, there is a gap between what is theoretically possible and what has been tried.
...
Additionally, the disadvantages are not too severe. First, disk-based storage has become very cheap and pure in-memory storage is bound to become cheaper over time. Second, materialized view can be refreshed as demand dictates, either by leveraging batch-oriented parallel processing~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} or by employing techniques for materialized view maintenance~\cite{gupta1995maintenance}. Finally, user preferences can to some extent be accommodated by materializing views for different user profiles and choose between them at query time. However, this last problem is probably the most fundamental problem of batch-oriented systems.
...
For this problem, this thesis contributes compilers for the CVL and Glossy SQL languages as well as an evaluation of their performance and scalability:

\begin{itemize}
\item \textbf{CVL compiler}: The compiler translates CVL into SQL with stored procedures implemented in the Python programming language. The compiler targets the PostgreSQL RDBMS, which is popular in the geographic data community due to its high-quality spatial extension~\cite{postgis}. The compiler output can easily be adjusted to target a different spatial database engine~\cite{soliman2014orca} or cluster computing system~\cite{eldawy2014pigeon, thusoo2010hive}. The compiler implementation is described in Chapter~\ref{chapter:cvl}.
\item \textbf{CVL experimental evaluation}: The performance and scalability of the CVL compiler is evaluated for several real-world and synthetic datasets, which inlude both point, line and area datasets. The evaluation of CVL is described in Chapter~\ref{chapter:cvl}.
\item \textbf{Glossy SQL compiler}: The compiler translates Glossy SQL into pure SQL in the PostgreSQL dialect~\cite{postgresql,postgis}.  The compiler output can easily be adjusted to target a different spatial database engine or cluster computing system. The compiler is described in Chapter~\ref{chapter:glossy}.
\item \textbf{Glossy SQL experimental evaluation}: This thesis contributes an experimental evaluation of the Glossy SQL compiler for a range geographic and non-geographic use cases over real-life datasets. The evaluation measures the scalability and performance of queries produced by the Glossy SQL compiler. Furthermore, the evaluation compares the performance and scalability of output generated by the Glossy SQL compiler to output generated by the CVL compiler. This evaluation is included in Chapter~\ref{chapter:glossy}.
...
For this problem, this thesis contributes a prediction-based tile selection framework as well as an experimental evaluation of its predictive accuracy:

\begin{itemize}
\item \textbf{Tileheat prediction algorithm}: foo. The details about this part are described in Chapter~\ref{chapter:tileheat}
\item \textbf{Tileheat experimental evaluation}: foo. The details about this part are described in Chapter~\ref{chapter:tileheat}
...
% Not good enough. Tim uses bullets that reference the contributions, Marcos used a single sparagraphs to similar effect
...
\begin{figure}[!t]
\centering
\scalebox{0.5}{\includegraphics{figs-glossy/density-glossy.pdf}}
\caption{Spatial distribution for three types of selection (approximately 2000 airports each): (A) random sampling. (B) global selection w. locus constraint and linear scaling. (C) global selection w. locus constraint but w/o spatial scaling.}
...
The previous chapters have described the contributions of this PhD project: Glossy, CVL, and Tileheat. While the evaluation of these systems show that crucial progress has been made in important areas, the contributions are also limited in some important ways. The sections below will discuss some of these limitations and will sometimes indicate directions for future work that could potentially overcome them.

\section{Limitations of Glossy and CVL}
...
\minisec{Aggregation and Projection}
...
\minisec{Graphs and Semi-structured Data}
...
In summary, Glossy and CVL does not run on any Hadoop-based system that we know of. Glossy and CVL might run on Greenplum with some light modification, but this has not been tested.

\minisec{Usability testing}
...
\section{Limitations of Tileheat}
% Tileheat discussion
...
\minisec{Real-time prediction}
Real-time prediction of content that will requested next by a user can help improve perceived latency. Ahead of time, the predicted content can be materialized and brought into a fast-access storage layer just before it is needed. The assumptions are that the actions of each user are sufficiently predictable and that the content requested by different users is sufficiently different. Tileheat was not designed for personalized prediction and therefore does not support it. While the actions of users have been shown to be sufficiently predictable, it has also been shown that the map content requested by different users follows common patterns. For this and other reasons, tileheat was designed to process access logs and prepare a common hot set many hours ahead of peak load. If a rapid shift in user behaviour should occur during peak load or a user suddenly behaves in an anomalous manner, Tileheat will be unable to react until the next scheduled access log processing.

\section{Conclusion}
This thesis contributes new knowledge within on two areas of online multi-scale map and data visualization: i.e. (a) declarative and holistic selection for map and data visualization and (b) prediction of map requests. The general approach has been to investigate batch-oriented systems. Specifically, the design goals were to contribute systems that can:
...
\minisec{Strategies}
The main strategy used in the selection study was to move programs to the data instead of vice versa. Today, data for visualizations is often stored in relational database management systems (RDBMS). Therefore, a natural idea is to generate SQL statements that compute holistic selection entirely inside the database. Furthermore, SQL seems to be the lingua franca quey language for the new class of emerging and highly scalable data processing systems, spearheaded by as Apache Hadoop projects such as Hive, Drill, and Spark.
