...
The web has transformed how map visualization should ideally be approached. This has opened up important new problems that must be solved, and which are not adequately addressed by the state of the art. Map visualizations must scale infinitely both in data volume~\cite{Im2013visreduce, battle2013dynamicreduction} and the size of the audience~\cite{viegas2007manyeyes}. Moreover, designers must be able to create compelling and complex map visualizations using inituitive and expressive interfaces~\cite{wu2014case, battle2013scalar, fekete2012dmvis}. This dissertation focuses on how to build map visualization systems for the web that fulfill these requirements. In particular, the following problems are addressed:

\begin{itemize}
% EXPRESSING
% DONE 1
\item \textbf{High-level Languages for Map Visualization}: Designers, especially those who are not GIS-experts, must be able to express multi-scale map visualizations over large geographical tables. A map visualization pipeline based on multi-scale selection is easy to understand and moreover produces maps that are interactive, since the identity of records is preserved. Compelling map visualizations follow certain visual principles, which can be expressed as constraints, objectives and styles on the visual representation~\cite{woodruff1998constant, topfer1966principles, harrie1999constraint}. A productive pipeline for multi-scale selection, one that is programmable by designers, will require the specification only of high-level goals, while leaving the minute selection of data to the system~\cite{meliou2011reverse} (see Figure~\ref{fig:introduction:multiscale:selection:pipeline}). Ideally, the specification requires only the schema to be known, as opposed to a priori knowledge of the data itself. This would facilitate code reuse across datasets.

A high-level programming langage for multi-scale selection must: (1) must be intuitive and productive for non-experts, (2) capture both the star and ladder paradigm for multi-scale selection~\cite{foerster2010challenges}, (3) allow designers to intuitively express user-defined and high-level constraints and objectives for multi-scale selection, (4) target geographical data that is stored in relational databases, (5) encourage code sharing and reuse through general-purpose language features. Subject to these requirements, the language should be designed to minimize the average programs length.
