...
Map generalization approaches can be divided into two polar opposites, namely on-the-fly~\cite{bereuter2013real,van1995gap,nutanong2012multiresolution} generalization and batch-oriented~\cite{schmid2013opensciencemap,haunert2006landcover,sarma2012fusiontables} generalization. Initially, on-the-fly generalization seems to have all the benefits, such as only storing the data once and the opportunity to react individually to users' behavior. However, on-the-fly methods do not cost-effectively scale to massive amounts of concurrent users, since they rely on datastructures that are efficient for single queries but are difficult to partition and scale for concurrent queries~\cite{}. In contrast, batch-oriented methods  gets an input data set and returns an output data set using algorithms, rules, or constraints without the intervention of humans. To distribute the resulting map to massive amounts of users, the materialized information abstraction can be partitioned and replicated inside highly scalable content delivery networks (CDN)~\cite{barroso2009datacenter}, for example as vector tiles~\cite{gaffuri12vectortiles} that partition the data in a pyramid scheme~\cite{decola93}. The most crucial drawback of a batch-oriented approach is that it sacrifices consistency. When the base data changes, the materialized information abstraction becomes an inconsistent view of the data. To make it consistent, the abstraction must be computed from scratch. Given a requirement that a map must by highly scalable, for example in a geospatial data infrastructure~\cite{gst2014digitalmapsupply}, batch-oriented approaches are presently the only viable option. However, the current state-of-the-art in batch-oriented map generalization is suboptimal for abstraction of thematic foreground data. % Why?
Batch-oriented approaches are characterized by two extremes, either fully automatic systems or very advanced and domain-oriented systems. % Yeah, what is wrong with that?
