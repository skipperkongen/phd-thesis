...
\emph{What are the challenges?} There are two crucial problems that must be addressed in order to successfully develop a data-driven visualization. The first problem is the design effort required to create a high-quality visual representation of a large and perhaps heterogenous dataset. Very quickly, the representation budget of finite resources, like screen real-estate, are depleted as the data grows in size~\cite{topfer:selection, weibel, stonebraker}. At this point, the data must be abstracted to fit the medium, for example by selecting subsets or by aggregating. An important question is: how should the data be abstracted and how can users (designers) control the process to get the results they desire? The second problem is the cost to operate a data-intensive service with 24 x 7 x 365 availability and low latency. While devices in our pockets can reach any point in the world of open data at the speed of light, that data may be stuck in the black hole of an overloaded endpoint. While techniques exist that minimize the risk of data being stuck on overloaded servers, using these techniques generally require a monetary investment, e.g. in cloud infrastructure. In many technology companies, the costs allocated to cloud infrastructure rival the cost of hiring expensive engineers. \emph{at this point, we should know that data-driven applications require both availability/performance and visual quality, and that it is not comletely straight forward}.
