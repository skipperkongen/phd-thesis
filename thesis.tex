...
A log of requests is typically recorded by a map provider. The map provider studied in this PhD project has a log of every request that has been received for the past five years, which includes information about the time of day and the area of the map that was requested. This is probably typical. However, information about which person made the request is not logged. This is probably also typical. From this log, this conducted the preliminary studies that where mentioned above. However, the log may not only explicitely tell us something about the past, it may also implicitely tell us something about the future. Currently, there is a lack of methods that optimize the preemptive materialization of tiles, temporally and spatially. On the one hand, if such a method could predict the set of hot tiles in the futre, which could save a lot of CPU-cycles and I/O during preemptive (partial) tile materialization by only materializing the hot set. On the other hand, if such a method could tell us when to preemptively materialize the hot tiles so as to not collide with many concurrent user requests, we could improve the average latency of map requests. 
...
\subsection{Equivalence to Set Multicover Problem}
...
First, this thesis contributes an identification of equivalence between the selection problem in map generalization and the set multicover problem. There is an downside and upside to this equivalence relation. The downside is that the set multicover problem is NP-hard~\cite{}. The upside is that several efficient approximation algorithms have been studied for this problem, which includes an algorithm with $O(??)$ complexity. The upside of the equivalence relation forms the background for a contribution of this thesis, namely that the application of a single efficient algorithm can solve many different variations of the multi-scale selection problem in map visualization, which supports a very versatile class of user-defined constraints and record importance measures.

Second, this thesis contributes a formulation that expresses a host of concrete binary and set-based selectin constraints in terms of edge-selection constraints in a bi-partite graph. For example, this formulation captures diverse constraints such as object proximity and object density in geographical space (see Figure~\ref{fig:introduction:selection:constraints}).

Third, this thesis proves that simple sorting can solve the set multi-cover problem to optimality for common and important cases, namely the selection of geographic points subject to a tile density constraint.
...
\subsection{Glossy SQL Extension}
\label{sec:introduction:contributions:glossy}
% NOTFINISHED
Section~\ref{introduction:problem:statement} posits the need for declarative languages to express multi-scale selection in map visualizations. This thesis contributes an extension to SQL which addresses this need, not only for map visualizations, but for a substantially larger class of data visualizations, which includes zoomable timelines. Moreover, the choice of an extension to SQL allows programmers to concisely and intuitively express multi-scale selection in a familiar and productive programming style. This was important, since programmer productivity is a major design goal.

During the design process, some key design goals where identified: An extension to SQL for multi-scale selection must (1) allow programmers to express scale-dependent binary and set-based constraints that should be satisfied by a valid selection, (2) allow programmers to define how record importance should be evaluated for each scale, (3) allow programmers to freely define a set of tuples that generalize the notion of scales, (4) allow multi-scale selection in both the star and ladder paradigm~\cite{foerster2010challenges},  (5) allow programmers to reference data tables in relational databases.
...
In Section~\cite{introduction:problem:statement} the case for in-database processing of multi-scale selection was made. This thesis makes the contribution of a translation approach that will compile a specification written in a declarative language for multi-scale selection into a query in pure relational algebra. Consequently, the translated query can be executed entirely inside a relational database, where the data is assumed to be stored.
