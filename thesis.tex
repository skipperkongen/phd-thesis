...
% Map generalization
The three dimensions of an image (x, y, color) make the visual perception our most powerful perceptive system. But images have only three dimensions and the consequences of this limit are important~\cite{bertin1973semiologie}. Digital map visualization is the process by which map designers visually represent the geography of Earth in the finite 3-D pixel matrix on a computer screen. Alas, it is fundamentally challenging to represent something as enormous as planet Earth on something as small as a computer screen~\cite{dillard1974pilgrim}.
...
The web has transformed the activity of map visualization since Roger Tomlinson first conceived geographic information systems (GIS) in the 1960's~\cite{ucgis2014tomlinson}. First, the crowd of map designers has changed, from a few GIS specialists in offices to many data entusiasts on the web~\cite{hanrahan2012enthusiast}. Second, the geographic data has changed, from moderately-sized and mostly curated datasets inside organizations to huge and often user-contributed datasets on the web~\cite{gst2014digitalmapsupply, openstreetmap, zooniverse2014oldweather}. Third, the audiences of map visualizations have changed, from a few professionals who interact with mapping systems during business hours, to millions of people on the web who interact with maps 24 x 7 and on the go. Fourth, the purpose of digital mapping has changed, from being mostly a professional activity, to being simultaneously a professional, personal and social activity. Admitedly, these four points are somewhat a caricature, however the directionality of where map visualization is going is evident all around us. Undoubtedly, the impact of the web has caused important challenges that are not adequately addressed by state-of-the-art GIS. 

The new opportunities on the web, comprised by mountains of data (millions of records) and potentially huge audiences (millions of people), more and more individuals and organizations (e.g. news agencies) seek to communicate spatial narratives on interactive web maps~\cite{bradshaw2013online}. Some of these maps are easy to produce, while others are more difficult. There are two extremes in the types of maps that are being created. At one extreme, map such as choropleth maps~\cite{jenks1963generalization} and heatmaps~\cite{wiki2014heatmap} present a highly statistical view of information~\cite{guardian2013ananas}, however they are straight-forward to create using spatial statistics. At the other extreme, map such as thematic point maps present an immersive, interactive and detailed view of information~\cite{nytimes2010iraq}. While both types of map are highly valuable, the latter maps present a fundamentally harder challenge to map designers due to the need for map generalization. To retain individual records in the map with the possibility for interaction, thematic data can be selected or displaced, but not aggregrated or amalgamated. Of the former two, only selection can be guaranteed to produce a legible map when the data is dense, as displacement requires empty spaces somewhere in the map. 

For the reasons mentioned above, this thesis will concentrate on methods for selection of thematic data for (multi-scale) maps. A specific goal of this thesis is the study of methods that can be incorporated into map systems that are specialized for data enthusiasts (i.e. not GIS specialists) who wish to rapidly design interactive web maps over large and constantly emerging geographical point sets.
