...
The Web has structurally impacted \emph{why}, \emph{how}, by \emph{whom}, \emph{when}, and \emph{where} digital maps are produced and consumed. 


In its wake, a dual challenge of map creation and distribution has surfaced, which cannot be adequately addressed by traditional geographic information systems (GIS). GIS was developed before the World Wide Web and primarily as a single-user system~\cite{ucgis2014tomlinson}. Later on, GIS has been retrofitted with web service capabilities, such as the Web Map Service (WMS) and Web Feature Service (WFS), which are protocols designed for executing live queries on the data over the Web. However, these protocols do not scale because they rely on heavy processing of the data at query time. While this may work well for a few concurrent users, it becomes prohibitively expensive at scale. As a consequence, almost all succesful online maps rely on pre-materialized map tiles, which can additionally be complemented by client-side rendering.

% END REVISIT

\minisec{Huge Emerging Datasets}
...
%\minisec{National Mapping Agencies in the Future}
%National mapping agencies create and distribute curated map content of on the Web~\cite{gst2014digitalmapsupply}. The maps are of high quality and potential valuable to society~\cite{gst2012friedata}. However, similar maps are also being distributed by private corporations, such as Google and Microsoft, which creates a competive situation. In fact, multi-national corporations, such as Google and Microsoft -- rutinely operate at one or two orders of magnitude higher availability (99.999\% versus 99.5\%) and performance (40 msec versus 400 msec per map requrest) compared to national mapping agencies. Evidently, computer science heavy organizations like Google and Microsoft have adapted better to the reality of Web mapping than national mapping agencies -- while simultaneously serving maps to orders of magnitude larger audiences. This puts pressure on national mapping agencies, who must catch up or become irrelevant. To support the adoption of best practice Web maps in national mapping agencies, this thesis aims to disclose a set of low-cost techniques for serving map content online with high availability and high scalability.
