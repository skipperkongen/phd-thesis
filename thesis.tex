...
\begin{emphpar}
\textbf{Intelligence}: from Latin \emph{intellect-}, formed from \emph{intellegere},, ``to understand, to comprehend''; from \emph{inter-}, ``between'' plus \emph{legere}, ``to choose, to read''; \emph{Legere} is the etymological root of the English words \emph{select} and \emph{legible}. 

Map makers select geographic information for presentation on legible maps; maps help their viewers comprehend the World.
\end{emphpar}

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.47]{figs-thesis/geographic-mashups.pdf}
\caption{\textbf{Thematic maps}: Two examples of thematic maps that show highly specific foreground data on top of general-purpose background maps: (left) a choropleth map of  \% people who speak a non-english language as their first language in the UK; (right) a point map of Wikileaks data for casualties during the 2003-2011 Iraq War. The maps are reproduced from The Guardian's Data Blog including background maps \textcopyright 2014 Google.}
...
% Background
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a highly restricted medium~\cite{jones2005webage}. Map visualization is fundamentally a ``big data'' challenge, since the amount of geographic information typically exceed the representational budget of the medium (see Figure~\ref{fig:introduction:background:raw:poi}). Therefore, the geographic information must be adapted to fit the budget, which is a fundamentally hard part of the mapping process that is known as \emph{generalization}~\cite{shea1989cartographic,weibel1997generalization}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.495]{figs-thesis/data-draws-the-world.pdf}
\caption{\textbf{Unfiltered geographic information}: On a small scale map of Europe, the symbols for unfiltered points of interest from OpenStreetMap (left) almost entirely cover most national territories (right). It illustrates how for large datasets (thousands of records or more) it generally does not make sense to display unfiltered geographic information on a map.}
\label{fig:introduction:background:raw:poi}
\end{center}
\vspace*{-4ex}
\end{figure}

This thesis has studied the mapping operations of a national mapping agency~\cite{gst2014digitalmapsupply}. Recently, national maping agencies have come under pressure, as commercial mapping alternatives, such as Google and Bing Maps, routinely dominate in terms of price, ease-of-use, availability, performance and scalability. While little has been written about this, the effect is evident to anyone working with maps and geographic information on the web. The outset of this thesis was to understand the techniques that have enabled the success of commercial mapping providers, see if the techniques can be improved, and finally transfer the knowledge into the public domain. While this promethean goal implicitly motivates this thesis, its contributions are necessarily more focused on fewer, yet important challenges within this larger picture.
...
First, the Web has shifted the reasons why maps are created, which is caused by two root factors. On one hand, private companies~\cite{google2014maps,stamen2014tiles,vizzuality2012cartodb} and national mapping agencies~\cite{gst2014digitalmapsupply} supply an abundance of online background maps, which means that there is less need to create these types of maps. On the other hand, there is an great supply of thematic geographic datasets online -- both curated~\cite{mst2014miljo}, user-contributed~\cite{twitter2014twitter,zooniverse2014oldweather}, and machine generated~\cite{planetlabs2014flock1} -- which grow by millions of information objects each day~\cite{openstreetmap2014stats}. The combination of these two factors means that demand has shifted towards the creation of thematic maps~\cite{bereuter2013real}. For example, journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} create thematic maps that are important and can ultimately even save lifes. However, the increase in data volume calls for map processing methods that efficiently scale with the size of data.

Not all thematic maps are challenging to create. On one hand, highly aggregated maps, such as choropleth maps~\cite{guardian2013ananas} and heatmaps~\cite{wiki2014heatmap}, present a statistical view of geographic information (left map in Figure~\ref{fig:introduction:background:mashups}). Such aggregated maps are fairly trivial to produce using spatial statistics~\cite{ripley2005spatial}. On the other hand, object maps, such as point of interest (POI) maps~\cite{bereuter2013real, sarma2012fusiontables,nutanong2012multiresolution}, present a detailed view of information (right map in Figure~\ref{fig:introduction:background:mashups}). Since map generalization such as holistic selection is generally required for detailed object maps, these maps are generally challenging to produce.

Second, the Web has caused a change in how and by whom maps are created. While map creation was traditionally the domain of cartographic experts, this is no longer the case. This is partly because demand for thematic maps on the web has grown faster than the number of cartographic experts who can create them. This has created a human resources challenge, which is being addressed by the development of more intuitive and expressive design interfaces, which promise to enable anyone to create maps. The need for rapid thematic mapping has driven the development of fully-automatic mapping systems~\cite{sarma2012fusiontables,nutanong2012multiresolution}. However, these systems have completely sacrificed expressibility in order to gain productivity. Arguably, a fully-automatic mapping system is likely to be suboptimal for most cases. The alternative is to use traditional mapping systems, which were chiefly created for the production of complex maps of the Earth~\cite{lamy1999application,ware2003generalization_sa}. They are certainly powerful enough. However, these systems are hard and time-consuming to use, which in turn makes them costly. Clearly, there is a need for expressible map design interfaces that can be time-efficiently operated by cartographic novices.
...
To understand the process of web mapping, it must be modeled at a more granular level. The web mapping process can be modeled as a four-stage logical pipeline that moves geographic information through a life cycle that begins with data modeling and ends with graphic display on a map (see Figure~\ref{fig:introduction:requirements:pipeline}). It is important to study the tasks and requirements of each stage in the logical pipeline, since this allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. 

Stage one (data modeling) has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. Stage three (graphical abstraction) has been studied in the seminal work by Bertin~\cite{bertin1973semiologie}. This thesis will focus on the remaining stages, namely stage two (logical abstraction) and stage four (content distribution), since the biggest challenges in web mapping are currently found here.


\subsection{Holistic Selection of Geographic Information} 
Geographic information must be logically adapted for display on maps, which generally requires the derivation of an appropriate subset of information, or alternatively aggregation. This thesis focuses on \emph{selection} as the primary means for information abstraction. Selection is a challenging but useful way to adapt geographic information for maps. In contrast, aggregation is mostly, while not always, a straight-forward process. The remaining generalizion techniques, such as simplification and collapse, do not preclude selection. For example, simplification is primarily a means for saving data storage and I/O, which is orthogonal to the goals of selection~\cite{weibel1997generalization}.

It is important to understand the requirements of selection. Even as the Web has impacted when, why and how maps are created and consumed, maps must (still) be \emph{legible}, \emph{consistent} (with reality), and \emph{relevant} (for the purpose of the map). The six design goals of the iOS6 Apple Maps API concretize this further: 
...
This example list of \emph{constraints} and \emph{objectives} illustrates concrete ways to quantify the feasibility and utility of a map. A more abstract and formal view of constraints and objectives is the following: 

\begin{itemize}

\item \textbf{Constraints}, such as unary~\cite{de2004simplifying,welch1982spatial}, binary~\cite{nutanong2012multiresolution}, space~\cite{topfer1966principles, woodruff1998constant}, cross-scale~\cite{sarma2012fusiontables,foerster2010challenges} and topological~\cite{egenhofer1991categorizing,randell1992spatial,schmid2013opensciencemap,shreve1966statistical,van1995gap} constraints, define the feasibility of a map. Objects or spaces that violate the constraints are said to be \emph{in conflict}, which is a situation that must be remedied by the application of an appropriate map generalization operator~\cite{shea1989cartographic,weibel1997generalization}.

\item \textbf{Objectives}, such as minimization of conflict~\cite{bader2001energy,haunert2006landcover,sester2000generalization} or maximization of content~\cite{sarma2012fusiontables,nutanong2012multiresolution}, quantify the utility of a map. Quantifiable objectives facilitate combinatorial optimization over a search space of maps.
...
While the concrete formulation of constraints and objectives depends on the map that is being designed, any map design process must be guided by some set of constraints and objectives, such as the ones described above. Specifically, holistic selection of geographic information for maps requires holistic constraints and objectives.

\subsection{Distribution of Geographic Information} In stage four of the web mapping pipeline, map content is distributed over the Internet to web and mobile mapping clients. The success criteria are established in a service level aggrements (SLA), which states the acceptable thresholds of quantifiable service metrics. While SLAs differ in their formulation, usually, acceptable thresholds for the following three metrics are defined:
...
\subsection{Achitectures}

Two important high-level approaches have been proposed for generalization of geographic information, namely \emph{real-time} (or on-the-fly) generalization~\cite{bereuter2013real,van1995gap} and \emph{batch-oriented} generalization~\cite{sarma2012fusiontables,frank1994multiscaletree,van2011towards}. While the requirements for \emph{what} should be the produced are the same for the two approaches, vis-a-vis map constraints and objectives, these two approaches differ in \emph{when} the results are produced. The distinctions as well as the pros and cons of each approach in a Web context is discussed below.

\minisec{Real-time approach}
...
However, there are several issues with real-time generalization in a Web context. While indexing structures enable fast generalization, the complexity of any selection algorithm is high compared to $O(1)$, which is the complexity of retrieving pre-selected data from a key-value store~\cite{pilchin2007dynamo}. Furthermore, the scalability of real-time generalization algorithms is usually only considered for increases in data volume, while it is ignore for increases in concurrency level. This is critical, because in a Web context there is typically a high and variable level of concurrency, which implies that the generalization method must be scalable in the number of concurrent queries. Unfortunately, there is a lack of studies into real-time generalization approaches that efficiently scale in the number of concurrent requests. The na{\"i}ve approach is to replicate the entire index which is costly.

In conclusion, real-time generalization is currently an infeasible approach for scalable web mapping pipelines that are accessed by large numbers of concurrent users.

\minisec{Batch-oriented approach}
Batch-oriented selection of geographic information compute materialized multi-scale views over the original state of the data~\cite{sarma2012fusiontables,frank1994multiscaletree,van2011towards}. For example, multi-representational trees can be used to represent data at multiple scales. There are two obvious advantages to this approach.
...
In conclusion, batch-oriented generalization is the only viable option for scalable web mapping pipelines that are accessed by large numbers of concurrent users. However, it suffers from one significant drawback that must be tolerated, namely that it is cumbersome to customize the selection information based on individual user preferences.

\section{Problem statement}
\label{sec:introduction:problem:statement}

Given the requirements for an online wepping mapping pipeline, which were outlined in Section~\ref{sec:introduction:web:mapping:pipelines}, this section lists four important gaps between state-of-the-art mapping systems and a hypothetical mapping system that satisfies all the requirements. Specifically, a selection-based mapping system for large web-audiences is considered. Since on-the-fly selection is expensive to scale to large concurrent audiences, a batch-oriented approach is considered as the most viable option.

This thesis studies four gaps in state-of-the-art batch-oriented, selection-based web mapping systems, namely:

\begin{itemize}

% MATHEMATICAL FOUNDATION
\item \textbf{Unified Selection Model}.
While selection of geographic information can be intuitively understood as filtering objects subject to some criteria, many different selection models have been proposed~\cite{sarma2012fusiontables,nutanong2012multiresolution,rocha2011efficient}. However, each of the proposed models offer distinct semantics for selection. Clearly, there is a lack of a unified selection model that subsumes all of the proposed selection semantics. Specifically, what is needed is a model for holistic selection of geographic data that supports: \emph{user-defined constraints}, i.e. unary, object-relative, space-relative, and cross-scale constraints, as well as \emph{user-defined objectives}, either minimization of error or maximization of relevance. Ideally, a well-studied and efficient algorithm can solve all instances in the unified model, in the same fashion that sorting solves all instances in the top-k selection model.
...
\item \textbf{Low-cost Tile Materialization}.
...
Instead of presenting a complete web mapping pipeline, this thesis addresses the gaps that were identified in Section~\ref{sec:introduction:problem:statement}.
 
\subsection{Set Multi-cover Formulation}
\label{sec:introduction:contributions:set:multicover:problem}

Section~\ref{sec:introduction:problem:statement} made the case for a unified model for selection of geographic information.

First, this thesis contributes a unified model for selection of geographic information. The model is based on an equivalence between the problem of selecting geographic information for maps and the more abstract set multi-cover problem. Consequently, selection of geographic information can be interpreted as selection of edges in a bi-partite graph. There is an downside and upside to this mathematical equivalence relation. The downside is that the set multi-cover problem, and therefore also the geographic selection problem, is NP-hard~\cite{vazirani2001approximation}, which has also been noted by others~\cite{sarma2012fusiontables}. The upside is that several efficient approximation algorithms have been proposed for the set multi-cover problem~\cite{chekuri2012setmulticover,rajagopalan1998primal}, which can be directly used for selection of geographic information.

Second, it is shown that a restricted case of the set multi-cover problem, which frequenty occurs in maps, can be solved to optimality by an efficient sorting based algorithm. Specifically, the restricted case is weighted selection of geographic points subject to a tile density constraint. In this frequently occuring case the sets are disjoint.

Third, it is shown how to express a variety of use cases in the unified selection model, which includes both geographic and non-geographic use cases. It is shown how diverse constraints, such as proximity and density constraints, can be modeled as a single constraint on selection of edges.
...
Section~\ref{sec:introduction:problem:statement} argued the need for a declarative language for selection of geographic information in a web mapping  pipeline. This section introduces the declarative language contributions of this thesis: Glossy SQL

First, this thesis contributes the \emph{syntax} of Glossy SQL, which allows programmers to declaratively express multi-scale selection of geographic and non-geographic data sources. The use cases that Glossy SQL covers include multi-scale timelines, maps and cluster visualizations. Programmers can declaratively express constraints that must be satisfied by a multi-scale selection. Additionally, programmers can declaratively define how the importance of records should be measured at each scale of the selection. Finally, programmers can define an ordered set of scales for visualizaton. The syntax expresses both iterative or recursive queries, with semantics that correspond to the star and ladder approach in generalization, respectively~\cite{foerster2010challenges}. A particular design goal that has been met is that queries in Glossy SQL must be compact yet highly readable. The longest Glossy SQL query in this thesis is just 14 lines of code!

Second, this thesis contributes the \emph{semantics} of Glossy SQL, which is expressed in terms of relational algebra. This semantics forms the foundation of the Glossy Compiler.
...
Section~\ref{sec:introduction:problem:statement} made the case for in-database processing of multi-scale selection. The Glossy compiler translates queries in Glossy SQL into relational algebra, which can be processed on top of spatial relational databases management systems (SDBMS).

First, this thesis contributes an optimized implementation of the Glossy compiler, which targets the commercial PostgreSQL SDBMS~\cite{postgresql,postgis}. This database is popular in the geographic data community due to its high-quality spatial extension~\cite{postgis}.

Second, this thesis contributes an experimental evaluation of the Glossy SQL compiler for a range geographic and non-geographic use cases using real-life datasets. The results show that the Glossy selection system is scalable and performant for the evaluated test cases.
...
\subsection{Tileheat: Prediction Algorithm}
...
\item Chapters~\ref{chapter:cvl}, ~\ref{chapter:glossy}, and ~\ref{chapter:tileheat} describe the contributions this thesis. 
...
\section{Map Generalization Process Models}

\subsection{Constraint-based Modelling}

 which is a type of constraint-based modeling of the map generalization process. More approaches in 1.3.3, 1.3.4, 1.3.5, 1.3.6 of \cite{harrie2007modelling}.

\minisec{Combinatorial Optimization} In combinatorial optimization a solution that maximizes/minimizes a real-valued objective function is found in a discrete search space, which is confined by a set of constraints. For selection, the search space consists of any solution that can be achieved by deleting a subset of an input set of geographic information objects. If a solution violates a constraint, then the solution is deemed infeasible. The objective function takes as input a (feasible) solution and outputs a real value which represents the utility of the solution. The objective is either to maximize or minimize the objective function.

This thesis proposes a combinatorial optimization approach to selection of geographic information on multi-scale maps.

This thesis categorizes constraints in map generalization as follows:

\begin{itemize}

\item \textbf{Unary constraints}, which establish conditions that must be met be individual map objects; a map object that violates a unary conditions is in conflict and may by definition not appear in its current form on a map. For example, impirical studies have shown that building symbols should be no smaller than 0.35 mm and roads should be no narrower than 0.25 mm. These limits are known as \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. Furthermore, when displaying detailed multi-lines and polygons on a computer display, consequtive vertices may be indistinguishable to the human eye. Such objects should be \emph{simplified} to the coarsest level of detail that is indistinguishable from all finer levels of detail. This saves storage, speeds up processing, cuts down on I/O, for example when geographic data is transmitted over the Internet.

\item \textbf{Binary constraints}, which set up conditions that must be satisifed by all qualifying pairs of objects. Two objects that violate a binary constraint are \emph{in conflict} and may by definition not appear in their current form on a map. For example, two objects that overlap or are very close to each other may be defined to be in conflict~\cite{weibel1997generalization, nutanong2012multiresolution}, which is a geometric binary constraint. However, not all binary constraints are geometric. For example, a constraint that limits the number of bulls per cow herd to one, is an example of a semantic binary constraints. Here, two bulls in the same cow herd would be in conflict with each other, no matter their geometric relationship to each other.

\item \textbf{Space constraints}, which state conditions that must be satisfied by spaces in the map, for example for each tile in a tiled map. One of the earliest impirical observations in geographic maps is that information density on published maps tends to be constant and independent of the map scale~\cite{topfer1966principles}. This principle is termed the \emph{Radical Law} and is also known as the \emph{Principle of Constant Information Density}~\cite{woodruff1998constant}. When information density on parts of the map reaches some threshold, the information should be abstracted by selection or aggregation in order to reduce the density (see Figure~\ref{fig:introduction:selection:constraints}).

\item \textbf{Cross-scale constraints}, which apply to maps that are zoomed in and out. Cross-scale constraints state limitations on how logical map content may transition over a set of ordered map scales. For some map objects, for example point of interest (POI) maps, it is more logical if POI do not disappear when zooming in, which is termed the Zoom-Consistency constraint~\cite{sarma2012fusiontables}. For other map objects, most notably place names, it is entirely appropriate that objects disappear when zooming in. For example, global place names (e.g. ``Europe'') should probably be replaced by local place names (``Paris'', ``London'', or ``Copenhagen'') when zooming in on a map. In map generalization process models these two approaches are termed the \emph{ladder} and \emph{star} approach respectively~\cite{foerster2010challenges}.

\item \textbf{Topological constraints}, which express invariants over the relationships between geographic information objects, which must not change during an abstraction process~\cite{egenhofer1991categorizing,randell1992spatial}. For example, the relative positions of distinct objects must be maintained; the primary logic of connectivity in a road network must be maintained~\cite{schmid2013opensciencemap}; holes may not appear in surfaces where no holes were before~\cite{van1995gap}; river networks must be pruned from the leaves~\cite{shreve1966statistical}; etc.

\end{itemize}

However, this is not the only way to categorize constraints. More in 1.3.2 of \cite{harrie2007modelling}.

\subsection{Map Generalization Operators}

This thesis studies a selection framework that supports the following operators:

\begin{itemize}
\item \textbf{Selection}
\item \textbf{Simplification}
\item \textbf{Exaggeration}
\item \textbf{Collapse}
\end{itemize}

However, these are not the only operators that have been proposed in the map generalization literature. More in 4.2 of \cite{weibel1999generalising}.

\subsection{Process Modelling}

This thesis models the generalization process, specifically the selection process, as a global optimization problem. However, this is not the only way to model the generalization process. 

\subsection{Real-time versus Batch-oriented}

This thesis proposes a batch-oriented approach to map generalization. However, on-the-fly methods have also been proposed. Bereuter and Van Oosterom.



\section{Must Consider}
Map Generalization: Nutanong, QuadTree, Fusion Tables

Spatial Analysis: HadoopGIS, SpatialHadoop, Pigeon, Greenplum, IBM Netezza, Teradata, Oracle Spatial, MS SQL Server Spatial, and PostGIS.
