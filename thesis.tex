...
\minisec{Constraints and Objectives}
The generalization process has be modelled in different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, quantifiable \emph{map constraints} act as the descriminator between feasible and infeasible maps. For instance, constraints may define the limits of acceptable size, complexity and density in a map~\cite{weibel1999generalising}. More concretely, \emph{proximity constraints} define the limit for how close geographic objects symbols may appear on a map in order to be considered \emph{distinct}~\cite{nutanong2012multiresolution}; network topology constraints may state that the principal logic of circulation should be retained in a street network before and after weeding out minor roads~\cite{schmid2013opensciencemap}.
