...
Short intro... talk about mult-scale seleciton.

\note{TEST: Each of the following sub-sections must (1) sound like a relevant problem, (2) must not have been adequately addressed by previous work.}

% in-database processing: BOYAA

\begin{itemize}

% MATHEMATICAL FOUNDATION
\item \textbf{Efficient Abstract Modelling} 
Map generalization is traditionally the study of very domain-specific methods with a high focus on the geographical context. No doubt, the methods that allow production of complex topographic maps need to consider geography in a very domain-oriented fashion~\cite{}. However, multi-scale selection of data in geographic mash-ups is a much simpler problem than generalizing data for a topographic map. For this reason, the selection problem can conceivably be mapped to an abstract problem, which however captures the relevant elements of multi-scale selection. If this was possible, and the abstract problem was well-known, we would theoretically be able to exploit previous work on efficient algorithms for this problem.

The key question is: what are the key components of multi-scale selection for map visualization and can they be captured by a more abstract and well-known model for which efficient algorithms exist?
...
\item \textbf{Declarative Specification}: There are currently two extremes in how state-of-the-art systems let users influcence the selection of data for multi-scale maps. At one extreme, fully-automated selection systems completely encapsulate the logic inside an algorithm library~\cite{sarma2012fusiontables}. Such systems exclude the map designer from any influence over the data selection process, including how record importance should be determined and which constraints should be enforced. At the other extreme, selection frameworks require map designers to write advanced and very geography-specific code~\cite{ware2003generalization_sa,lamy1999application}. While such frameworks are needed for complex map generalization tasks, such as those needed to produce a high-quality topographic map, they are not suitable for generalizing thematic overlays for geographical mash-ups. 

The main issue with these approaches is the degree of specification needed to get the job done: either too little or too much to be useful for map designers who craft geographical mapsh-ups. Ideally, designers should specify just the high-level constraints and objectives for a map in a versatile and concise specification language, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. Declarative languages, such as SQL, fit the bill for such as language. However, there is a lack of declarative langauges that can efficiently express multi-scale selection for map visualizations.

The key question is: what is a declarative language that allows map designers to intuitively and concisely express multi-scale selection for map visualizations?

% IN-DATABASE PROCESSING
\item \textbf{In-database Processing}: The web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap, zooniverse2014oldweather} datasets that can be visualized in geographical mash-ups. While geographical data is characterized by great diversity, is also reaches great volume. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to an online services, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and games~\cite{geocaching2014geocaching}, geographical information can be produced by anybody, anytime, and anywhere.
...
%this thesis focuses on how to enable novice map designers to express multi-scale selections for maps in a highly productive and yet flexible way. Certainly, tools for multi-scale selection do exist that can be operated by novice map designers, however they are either too simple (i.e. do not support user-defined selection)~\cite{sarma2012fusiontables} or too time consuming to use~\cite{mapnik, sld}. Therefore, it is crucial that intuitive, efficient and complete tools for selection of geographical information are included in map visualization systems.

% PREDICTIVE TILE MATERIALIZATION
\item \textbf{Predictive Tile Materialization}: Map visualization systems must be able to serve tiled multi-scale maps to large web audiences who expect 24 x 7 availability and acceptable latency~\cite{gaffuri2012toward, pilchin2007dynamo, beaver2010haystack}. In particular, the advantages of scalable distribution systems, such as content delivery networks (CDN), must be cost-efficiently leveraged~\cite{beaver2010haystack, datacentre}. However, since the number of tiles grow expenentially in the number of supported scales, fully materializing a multi-scale map into tiles and storing it in a CDN is costly. 
...
While existing geographical information systems (GIS) can be used to craft geographical mash-ups for the web, there are three major reasons why the current state of the art is a suboptimal solution. 
...
label{sec:introduction:contributions:cvl}
 ...
 Figure~\ref{fig:introduction:selection:constraints} represents a contribution, but not the whole contribution.
...
While how-to queries fit nicely with the generali requirements for selection, namely, given a geographical dataset, compute a multi-scale selection that satisfies a set of constraints and objectives, they have not been considered in a multi-scale context.

Crucially, a declarative language for holistic multi-scale selection must: (1) allow map designers to intuitively and concisely express constraints and objectives, (2) allow map designes to express both the star and ladder paradigm of multi-scale selection~\cite{foerster2010challenges},  (3) allow map designers to explicitly target geographical data in relational database tables. In the literature no such language has been proposed nor designed for map visualization in general, and multi-scale selection specifically. To fill this gap, this dissertation will study and design declarative languages for expressing holistic multi-scale selection that meet the requirements stated above.
