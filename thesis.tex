...
An effect that occurs when a graphic is scaled down is that the \emph{size} of objects in the graphic decreases. Consequently, object symbols, if shown to scale, may approach the limit of perceptibility. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.

Furthermore, as scale reduction causes the size of objects to decrease, it also causes the gaps that \emph{separates} the objects to narrow. Consequently, pairs of objects may come so close as to appear indistinguishable from each other. For example, two symbols that are 0.4 mm apart on a 1:500.000 map will be 0.2 mm apart on a 1:1.000.000 map, which is probably below the limit at which symbols can be distinguished from each other (see Figure~\ref{fig:introduction:background:raw:poi}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.495]{figs-thesis/data-draws-the-world.pdf}
\caption{\textbf{Effects of scale change}: On a small scale map of Europe, the hundreds of thousands of symbols for unfiltered points of interest from OpenStreetMap are indistinguishable from each other (left). The density of symbols is so high that they entirely cover the physical territories of most national territories. In constrast, the subset of those symbols that fit on a large scale map are clearly separated from each other (right).}
...
Another way that the reduction in scale can be percieved is that the \emph{information density} of a graphic increases, that is the amount of information per unit area. Several metrics for information density have been studied, such as Tufte's density metric~\cite{tufte1983visual} and the density metrics surveyed by Nickerson~\cite{nickerson1994visual}. The \emph{principle of constant information density} states that information density should remain constant as the scale changes. The principle which was empirically established by the cartographers T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that the number of settlements per unit area in published map series is independent of the map scale. The rationalized their observation by observing that a linear reduction in scale implies a reduction in area according to geometrical progression, which in turn must lead to a reduction in information at the same rate, if the information is to remain legible.
