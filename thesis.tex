...
The web is home to an eco-system of data providers and data consumers who exchange and combine, e.g., geographical, temporal and socio-economical datasets~\cite{wikipedia,worldbank,unstats,data:gov, data:co:uk,gst,openstreetmap}. Open data has become a commodity that is freely available online. Indeed, the value proposition of open data is clear: open data contributes to the transparency, progress and enlightenment of our societies~\cite{hans:rosling, offentlige:data:i:spil}. Commoditized data drives the demand for data-driven visualizations that are beginning to realize the potential of our open data~\cite{usecase1, usecase2, usecase3}. In turn, the momentum of data-driven visualizations drives the demand for more open data in an upwards spiral. In anticipation of this demand, most large web sites and organizations (e.g. Wikipedia and the United Nations) offer data-only APIs that are designed to meet this demand~\cite{wikipediaapi,unapi}. Today, internet users consciously or unconsciously expect 24 x 7 availability, low latency and high scalability of these APIs~\cite{userstudy}.
