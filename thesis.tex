...
Batch-oriented selection of geographic information computes a materialized multi-scale view over the original state of the data. For example, multi-scale trees~\cite{frank1994multiscaletree,van2011towards} and solutions to combinatorial problems~\cite{sarma2012fusiontables} can be used to represent data at multiple scales. There are two obvious advantages to a batch-oriented approach.

One advantage of batch-oriented selection is that more computational effort can be spent to produce a high-quality result. For example, cluster-based spatial analytics~\cite{eldawy2014spatialhadoop,aji2013hadoopgis}, which is too slow for real-time selection, can be used for scalable, batch-oriented selection regardless of data volume. Alternatively, parallel spatial databases can be used in the same way.
