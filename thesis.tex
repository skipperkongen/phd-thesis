...
As the human capacity to gather data has increased, the creation of maps has become more challenging; finite map mediums can only accommodate a finite number of symbols of legible size. This challenge has lead to a plethora of cartographic information abstraction techniques that are collectively known as \emph{map generalisation}. Map generalisation essentially solves the problem of creating a legible, useful and accurate maps for any map scale and for any amount of information. Naturally, this process involves a compromise between readability and precision for most real-world cases. For example, maps of the physical world must carefully select the subset of the available information that is visualized, or else the map would soon become cluttered and illegible. Similarly, demographic maps must often aggregate information about individuals by area in order to clarify the important trends in a local population. Finally, small scale maps must usually distort the information in order to make it more legible, for example by enlarging symbols that would otherwise be imperceivable or displacing symbols that would otherwise overlap.
