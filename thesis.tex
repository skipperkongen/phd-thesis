...
Maps present geographic information in legible form, which is selected such that a viewer may acquire intelligence about the World.
\end{emphpar}
...
% The change: the web
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on highly restricted mediums~\cite{bertin1973semiologie,jones2005webage,weibel1997generalization}. Map visualization is a big data abstraction challenge: there is generally more geographic information available than what the restricted mediums can be legible represent without abstracting the information. Roger Tomlinson was one of the pioneers who kick-started the digital age of maps in the 1960's, with the introduction of geographic information systems (GIS). However, the rise of the Web has significant altered the stage for map visualization, which has created new challenges that can not be adequately addressed by traditional GIS alone. The Web has transformed mapping from a single-track endevour of map content production into a dual endevour of map content production and map content distribution. Additionally, the Web has structurally impacted how, when and where maps are produced and consumed.

National mapping agencies, for example in Denmark~\cite{gst2014digitalmapsupply}, create and distribute highly currated map content on the Web. The maps are generally of high quality and of potential value to society. However, similar maps are also being distributed by private corporations, such as Google and Microsoft, which creates a competive situation. In fact, multi-national corporations, such as Google and Microsoft -- rutinely operate at one or two orders of magnitude higher availability (99.999\% versus 99.5\%) and performance (40 msec versus 400 msec per map requrest) compared to national mapping agencies. Evidently, computer science heavy organizations like Google and Microsoft have adapted better to the reality of Web mapping than national mapping agencies -- while simultaneously serving maps to orders of magnitude larger audiences. A critical challenge, for example for national mapping agencies, is how to serve map content with the high quality of service that users have come to expect, while simultaneously keeping down the cost. Therefore, this thesis aims to disclose a set of low-cost techniques for serving map content online with high availability and high scalability.

The Web has also impacted the types of maps that need to be created. An abundant supply of background maps online has all but dismantled the general need for background map creation. Currently, the biggest challenge in mapping is how to scalable express thematic maps over vast online repositories of curated~\cite{} and user-contributed~\cite{} geographic datasets. First, the repositories keep growing in size~\cite{openstreetmap2014stats}. Second, the demand for thematic maps is rising faster than the supply of highly-trained map designers. This creates a challenge, namely how to design scalable, intuitive and highly expressible design interfaces for map makers.
...
Web mapping pipelines execute four logical stages (see Figure~\ref{fig:introduction:requirements:pipeline}). It is important to study the tasks and requirements of each stage in the logical pipeline. This allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. 
...
\subsection{Stage Two: Logical abstraction} Even as the Web has impacted when, why and how maps are created and consumed, the basic ability of humans to comprehend maps has remained powerful, yet fundamentally limited~\cite{bertin1973semiologie}. Consequently, geographic information must be appropriately adapted for the various scales of a multi-scale map. Crucially, geographic information on a map must simulataneously be \emph{legible}, \emph{consistent} (with reality), and \emph{relevant} (for the purpose of the map). These conditions can be evaluated by \emph{constraints} and \emph{objectives}, which quantify the feasibility and utility of maps~\cite{harrie1999constraint,weibel1997generalization}:


\begin{itemize}

\item \textbf{Constraints}: map constraints, such as unary~\cite{de2004simplifying,douglas1973algorithms,welch1982spatial}, binary~\cite{nutanong2012multiresolution}, space~\cite{topfer1966principles, woodruff1998constant}, cross-scale~\cite{sarma2012fusiontables,foerster2010challenges} and topological~\cite{egenhofer1991categorizing,randell1992spatial,schmid2013opensciencemap,shreve1966statistical,van1995gap} constraints, establish quantifiable conditions that must be satisfied by the map state. Objects or spaces that violate the constraints are said to be \emph{in conflict}, which is a situation that must be remedied by the application of an appropriate map generalization operator~\cite{weibel1997generalization}.

\item \textbf{Objectives}: the utility of maps is quantified by an objective function, which computes a numeric sum based on the map state. One type of map objective is to assign a penalty to violated map constraints, and minimize the sum of penalties. Another type of map objective is to score objects by their importance, and maximize the sum of scores, which is arguably the most intuitive approach (see Figure~\ref{fig:introduction:selection:constraints}).
...
\subsection{Stage Four: Content distribution} In stage four, the map content is distributed over the Internet to web and mobile mapping clients. Service level aggrements quantify the requirements that map content distrubtions must met. There are two main requirements for the distribution of map content:

\begin{itemize}
\item \textbf{High availability}: map content must be highly available. This can be quantified in terms of so-called ``nines''. For example, ``two nines'' equals 99\% availability or 1.68 hours of downtime per week, while ``five nines'' equals 99.999\% availability or 6.05 seconds of downtime per week. Many national mapping agencies are contractly obligated to met ``two nines'' of availability~\cite{gst2014digitalmapsupply}. In contrast, private corporations -- such as Google and Microsoft -- rutinely operate at ``four nines'' or ``five nines''. Consequently, online map users have come to expect high availability, i.e. ``four nines'' or more, which presents a challenge to national mapping agencies. 
\item \textbf{High scalability}: map content distribution must be highly scalable, which means that acceptable latency must be sustained as the number of concurrent clients increases. Latency can be quantified as the number of milliseconds it takes to 
 \end{itemize}

One of the most critical challenges in content distribution on the Web -- including map content -- is how to sustain high availability and acceptable latency while keeping down the cost.

\section{State-of-the-Art}

Current approaches to logical map design are characterized by two extremes: (1) automatic selection for simple maps, and (2) full-blown map generalization for complex maps. 

\subsection{Automatic Selection} 
% describe and identify
The primary use case for automatic selection of geographic information is the rapid design of thematic point maps over databases of geocoded information~\cite{google2014maps}. Users are assumed to have limited mapping skills. Therefore, the constraints and objectives of selection are completely encapsulated inside system libraries. Additionally, different systems offer support for different constraint types. For example, systems may support automatically applied proximity~\cite{nutanong2012multiresolution} or density constraints~\cite{sarma2012fusiontables}. Additionally, map objectives are unspecified, which means that at best an opaque ranking method is applied by the system.

% discuss
%While automatic selection is highly time-efficient for map designers it also highly non-expressive. Map designers have no influence over the constraints and objectives that are applied during the selection process (see Section~\ref{introduction:web:mapping:pipelines}). This severely limits the control that map designers have over the design process, which is probably suboptimal for most real cases. Automatic selection frameworks obviously lack alternative map generalization operators, such as displacement and aggregation. However, this is arguably less of a problem, because selection is probably sufficient for most thematic point map designs. In summary, expressivity is entirely traded for productivity.

\subsection{Map Generalization}
% describe and identify
The primary use case for full-blown map generalization is the production of complex cartographic representations of the Earth, such as topographic and physical maps. When creating maps that contain diverse geographic information categories -- suchs as streets, hydrology, land use, mountain ranges, and urban constructions -- complex methods are warranted. Map generalization requires the holistic application of multiple operators -- such as selection, displacement, amalgamation, exaggeration, and collapse~\cite{weibel1999generalising} -- which has been modelled in complex frameworks with agent-based~\cite{lamy1999application} and metaheuristic search~\cite{ware2003generalization_sa} methodologies.
