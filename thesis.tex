...
Digital map visualization studies how the geography of the Earth can be represented on a computer screen. No doubt, it is fundamentally challenging to represent something as enormous as planet Earth on something as small as a computer screen~\cite{dillard1974pilgrim} (TODO: find pages quoted by Weibel and Dutton and add to bibtex). While the three dimensions of an  image make the visual perception our most powerful perceptive system, images are finite and have only three dimensions and the consequences of these limits are important~\cite{bertin1973semiologie}.
...
Web and mobile mapping applications typically combine emerging thematic foreground data with the spatial context of existing background map, which is a process informally known as a ``geographic mash-up'' (see Figure~\ref{fig:introduction:background:mashups}}). % Why is it cool? 
Mash-ups are important because they combine new and relevant spatial information with existing high-quality background maps that provide the spatial context in which the new information should be understood~\cite{bereuter2012algorithms} % Why is it hard?
Any high-quality map must satisfy several and mutually adversarial design goals. Tricky trade-offs are required to display as much as possible of the relevant information while also remaining legible~\cite{jones2005webage}: as data grows in size beyond a few thousand records, symbols on the map will become indistinquishable from each other (see Figure~\ref{fig:raw:poi:data}). At this point, there is a need for geographic information abstraction, which is also known as map generalization~\cite{weibel1999generalising}. For example, a map designer may choose to \emph{select}~\cite{topfer1966principles}, \emph{simplify}~\cite{douglas1973algorithms},  \emph{displace}~\cite{lonergan2001iterative}, and \emph{amalgamate}~\cite{haunert2006landcover} the geographic information in order to emphasize the important and suppress the unimportant. % Why is the state-of-the-art not good enough?
...
\chapter{Geographic Information Abstraction}
...
\subsection{Model versus Visual Abstraction}
