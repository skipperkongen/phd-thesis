...
% TODO: Maybe introduce the notion of generalization operators early? Would make it easier to distinguish from DB terms like selection.

% TODO: More importantly, introduce the term multi-scale data abstraction (MSDA) and stick with it. Define the data abstraction operators in this context, i.e. global select, global project and global aggregate.

% Relational Databases (RDBMS) on the other hand are certainly powerful enough to express MSDA. While a full system for MSDA could be realized on top of a relational database, this thesis concentrates on using RDMBSs for multi-scale selection and projection.
...
\begin{enumerate}
\item What is the research context and the area of focus for this thesis? Data abstraction, more specifically selection. Semi-lazy materialization for geographical key-value stores. 
\item What is the multi-scale global selection problem?
\item What are programming/query languages for data abstraction?
\item What is database integration for data abstraction?
\item What is predictive caching for geographical key-value stores?
\end{enumerate}

\emph{What is the research context and the area of focus for this thesis?}

\emph{What is the multi-scale global selection problem?} 

\emph{What are query languages for data abstraction?} Languages that can express multe-scale global selection.

\emph{What is database integration for data abstraction?} Data abstraction queries are translated into efficient queries in relational algebra.

\emph{What is predictive caching for geographical key-value stores?} Using a log of previous requests, predict the requests that will be seen tomorrow. Account for trends, seasonal variation and toblers first law.
