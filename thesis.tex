...
\subsection{Effect of Scale Reduction}
...
\minisec{Decreasing Size and Separation}
One important effect that occurs when a graphic is scaled down is that the \emph{size} and \emph{separation} of objects in the graphic decreases. Consequently, objects may approach the limit of perceptibility and distinguishability. For example, if an object becomes too small it may become impercievable to the human eye and appear to vanish. Similarly, if the proximity of objects becomes too close they may appear indistinguishable from each other. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.

\minisec{Increasing Density}
An other important effect that occurs when a graphic is scaled down is that the \emph{information density} of a graphic increases. Several metrics for information density have been studied, such as Tufte's density metric~\cite{tufte1983visual} along the other density metrics surveyed by Nickerson~\cite{nickerson1994visual}. The \emph{principle of constant information density} states that information density should remain constant across changes in scale, which was formally established by the cartographers T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that maps in published atlasses contain a roughly fixed average number of settlements per a unit area independently of the map scale. The rationalized their observation by observing that a linear reduction in scale implies a reduction in area according to geometrical progression, which must also lead to a reduction in information per unit area according to geometrical progression, if the information is to remain legible.

Thus, map visualization is fundamentally a ``big data'' challenge, since the amount of geographic information typically exceed the representational budget of the medium (see Figure~\ref{fig:introduction:background:raw:poi}).

\subsection{Map Generalization}
A basic challenge in map design, known as map generalization~\cite{shea1989cartographic,weibel1997generalization}, is how to \emph{logically} and \emph{graphically} abstract a set of geographic objects that appear on a map in response to the effects of scale reduction, such that the map remains legible and relevant. In the present description of map generalization, we have aimed at the highest sensible level of description. This is a delibarate attempt to avoid getting involved in the well-known disagreement that exist among cartographers~\cite{rieger1993consensus}. A collection of map generalization work has been compiled by Mackaness, Ruas, Sarjakoski~\cite{mackaness2011generalisation}, which may function as an entry into this complex field.
...
Constraints can be classified according to many different typologies~\cite{burghardt2007investigations}. A very high-level typology, which focuses on the arity rather than semantics of constraints, is to distinguish between \emph{unary}~\cite{de2004simplifying,welch1982spatial}, \emph{binary}~\cite{nutanong2012multiresolution}, \emph{locus}~\cite{topfer1966principles, woodruff1998constant} (authors term), \emph{global}~\cite{egenhofer1991categorizing,randell1992spatial,schmid2013opensciencemap,shreve1966statistical,van1995gap}, and \emph{cross-scale}~\cite{sarma2012fusiontables,foerster2010challenges} constraints (see Table~\ref{tab:introduction:map:constraints}). While this simple functional classification can be nuanced greatly, which has been done among other by McMaster and Shea~\cite{mcmaster1992generalization}, choosing a functional rather than semantic classification of constraints distances the analysis from issues that are still debated in the cartographic community.

\begin{table}[htdp]
\caption{A typology of constraints based on their arity (with examples)}
\begin{center}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Type} & \textbf{Evaluated for} & \textbf{Examples} \\
\hline
Unary & Single objects & Minimum dimensions, sinuosity \\
\hline
Binary & Pairs of objects & Proximity, containment \\
\hline
Locus & Subsets & Density \\
\hline
Global & All objects  & Network connectivity, distribution \\
\hline
Cross-scale & Sets of objects at distinct scales  &  Zoom-consistency \\
\hline
\end{tabular}
\end{center}
...
\caption{\textbf{Binary and space constraints}: binary constraints, such as minimum proximity (left image), set up conditions that must be met by all qualifying pairs of objects on a map. Space constraints, such as a maximum density (right image), set up conditions that must be met by all qualifying spaces in the map. Objects that violate a constraint are in conflict (circled by red lines); this situation must be remedied, for example by selection (kept objects are shown as black disks; deleted objects are shown as white disks.}
