...
Cloud computing refers to the situation where a computational resource is exposed to online users via self-service web service interfaces, which is entirely or partially managed by the cloud provider~\cite{mell2009nist}. Today, powerful data analytics systems, such as mapreduce~\cite{apache2015hadoop} and column-stores~\cite{bauer2013redshift}, can be provisioned and rented by the hour~\cite{amazon2010aws}. On top of on-demand provisioning of computational resources, cloud computing offers practically unbounded storage of information. In combination, cloud computing potentially creates a powerful opportunity to process large-scale spatial data for maps~\cite{aji2013hadoopgis,eldawy2014spatialhadoop}.

Today, most large-scale information distributers, such as video and social networking sites, use cloud-based content delivery networks (CDN) to distribute their product to a global audience~\cite{dilley2002cdn}. These systems are founded on solid theory from distributed systems, such as key-value stores, data partitioning and data replication techniques~\cite{barroso2009datacenter,pilchin2007dynamo}. Any type of data that can be organized as a set of key-value pairs can be distributed via a content delivery network. For example, a photo has a filename or identifer (the key) and some bytes that make up the image content (the value)~\cite{beaver2010haystack}. Spatial data can be partitioned into key-value pairs by using a spatial grid~\cite{decola1993pyramid} or linearization~\cite{hilbert1891ueber,morton1966computer}. Once partitioned, the spatial data for a map can be efficiently and concurrently distributed to millions of people online, for example as so-called vector tiles~\cite{gaffuri12vectortiles}.
