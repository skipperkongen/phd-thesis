...
\emph{Legible} maps enable humans to \emph{comprehend} geographic information. One way to ensure a legible map is to \emph{select} data holistically according to constraints and objectives that ensure the feasibility and utility of a map. Maps breed spatial intelligence.
\end{emphpar}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.6]{figs-thesis/mapping-states.pdf}
\caption{States and transitions of a mapping pipeline.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

The digital mapping process can be modelled as a pipeline that translates between ``reality'' and a configuration of pixels on a computer screen. The mapping pipeline has three intermediate states that correspond to increasingly abstract representations of reality. The intermediate states are: the \emph{data model}, the \emph{logical model} and the \emph{graphic model}~\cite{gruenreich1992atkis} (see Figure~\ref{fig:introduction:requirements:pipeline}).

While the final state is a straight-forward rendering of the graphic model, the three intermediate states require further explanation:

\begin{enumerate}
\item \textbf{Data model}: the data model captures reality as a set of discrete geographic information objects. Geographic information objects associate a geometry, such as a \emph{point}, \emph{line} or \emph{area}, with a set of \emph{qualitative}, \emph{ordered} and \emph{quantitative} components~\cite{jacques1967semiologie}.
\item \textbf{Logical model}: the logical model logically abstracts the data model by \emph{selection}, \emph{projection} and \emph{aggregration} of the information objects, in order to emphasize the important and suppress the irrelevant. The logical model captures \emph{what} information the digital map contains. It should be noted that the term projection is used here as it is used in databases, i.e. the modification of an information object.
\item \textbf{Graphic model}: the graphic model graphically abstracts the logical model into a set of graphic symbols, in order to maximize the efficiency of visual \emph{perception} and \emph{comprehension} in the digital map. Symbols use \emph{size}, \emph{value}, \emph{texture}, \emph{color}, \emph{orientation}, and \emph{shape} to express the \emph{qualitative}, \emph{ordered} and \emph{quantitative} components of the geographic information~\cite{jacques1967semiologie}. The symbols are arranged according to the geometries of the geographic information objects, either by a strict geometric projection into the plane or in a topologically equivalent arrangement. The graphic model captures \emph{how} the information should be visually represented on a digital map.
\end{enumerate}

It is important to study the tasks and requirements that are associated with each transition and state in the pipeline, since this allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. 

Transition one (data modeling) has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}; Transition three (graphical abstraction) has been studied in the seminal work by Bertin~\cite{bertin1973semiologie}. Transition four (rendering) is outside the scope of this thesis. This thesis will focus on transition two (logical abstraction), since the biggest challenges in web mapping are currently found here. The requirements for logical abstraction are discussed in Section~\ref{sec:introduction:requirements:logical:model}.

The above view of a mapping pipeline is agnostic of the existance of the Web and the Internet. In contrast, a \emph{web mapping pipeline} allows the transfer of state over the Internet, from one state container to another state container. Typically, one state container (the server) transfers state to many state containers (the clients). It is important to understand the requirements of state transfer on the Web.


\subsection{Logical Abstraction} 
\label{sec:introduction:requirements:logical:model}
...
Geographic information must be logically adapted for display on maps, which generally requires the derivation of an appropriate subset of information, or alternatively aggregation. This thesis focuses on \emph{selection} as the primary means for logical abstraction of information. Selection challenging but useful way to adapt geographic information for maps. In contrast, aggregation is mostly, while not always, a straight-forward process. The remaining generalizion techniques, such as simplification and collapse, do not preclude selection. For example, simplification is primarily a means for saving data storage and I/O, which is orthogonal to the goals of selection~\cite{weibel1997generalization}.

It is important to understand the requirements of selection. Even as the Web has impacted when, why and how maps are created and consumed, maps must (still) be \emph{legible}, \emph{consistent} (with reality), and \emph{relevant} (for the purpose of the map). These are properties that can be ensured by \emph{constraints} and \emph{objectives}~\cite{harrie2007modelling}.

\newpage
The six design goals of the iOS6 Apple Maps API concretize the concept of constraints and objectives: 
...
This example list of constraints and objectives illustrates concrete ways to quantify the feasibility and utility of a map. A more abstract and formal view of constraints and objectives is the following: 

\begin{itemize}

\item \textbf{Constraints}, such as unary~\cite{de2004simplifying,welch1982spatial}, binary~\cite{nutanong2012multiresolution}, space~\cite{topfer1966principles, woodruff1998constant}, cross-scale~\cite{sarma2012fusiontables,foerster2010challenges} and topological~\cite{egenhofer1991categorizing,randell1992spatial,schmid2013opensciencemap,shreve1966statistical,van1995gap}, define a search space of feasible maps. Objects or spaces that violate the constraints are said to be \emph{in conflict}, which is a situation that must be remedied by the application of an appropriate map generalization operator~\cite{shea1989cartographic,weibel1997generalization}.

\item \textbf{Objectives}, such as minimization of conflict~\cite{bader2001energy,haunert2006landcover,sester2000generalization} or maximization of content~\cite{sarma2012fusiontables,nutanong2012multiresolution}, quantify the utility of a map. Real-valued objective functions facilitate combinatorial optimization over a search space of maps.
