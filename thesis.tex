...
% for boxes
\usepackage{mdframed}
\newmdenv[topline=false,bottomline=false,rightline=false,linewidth=2]{emphpar}
...
\begin{emphpar}
\textbf{Intelligence}: from Latin \emph{intellect-}, formed from \emph{intellegere},, ``to understand, to comprehend''; from \emph{inter-}, ``between'' plus \emph{legere}, ``to choose, to read''; \emph{Legere} is the etymological root of the English words \emph{select} and \emph{legible}. Maps present geographic information in legible form, which is selected such that viewers may acquire intelligence about the World.
\end{emphpar}

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.47]{figs-thesis/geographic-mashups.pdf}
\caption{\textbf{Thematic maps}: Two examples of maps that visualize thematic foreground data on top of common background maps. UK statistics for \% people who speak a non-english language as their first language (left); Wikileaks data for casualties during the 2003-2011 Iraq War (right). The maps are reproduced from The Guardian's Data Blog including background maps \textcopyright 2014 Google.}
...
% The change: the web
Map visualization studies how the vast geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on highly restricted mediums~\cite{bertin1973semiologie,jones2005webage,weibel1997generalization}. Map visualization is a big data abstraction challenge: there is generally more geographic information available than what the restricted mediums can be legible represent without abstracting the information. Roger Tomlinson was one of the pioneers who kick-started the digital age of maps in the 1960's, with the introduction of geographic information systems (GIS). However, the rise of the Web has significant altered the stage for map visualization, which has created new challenges that can not be adequately addressed by traditional GIS alone.

This thesis aims to study and close some important gaps between traditional GIS and requirements for map visualization on the web, with a focus on selection of geographic information for maps. Specifically, this thesis will study three orthogonal topics: (1) Effective and efficient map design; (2) scalable data processing for maps; (3) highly available and scalable distribution of maps. The focus will be on thematic maps since they are currently the most challenging, useful and simultaneously most frequently created type of maps (see Figure~\ref{fig:introduction:background:mashups}}). In contrast, the challenge of producing ``general purpose'' maps, such as topographic and physical maps, has arguably been adequately addressed by previous work~\cite{lamy1999application, ware2003generalization_sa}.

\minisec{Thematic Maps on the Web} % Thematic maps are important
Web and mobile mapping applications are typically thematic~\cite{bereuter2013real} Thematic mapping applications are important since they allow map designers -- such as journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} -- to communicate relevant and focused spatial information to highly motivated target groups. Thematic maps increase the level of transparency and enlightenment in our society -- they can even save lives. % What are they exactly?
Thematic maps are typically composed of two distinct layers of information: First, the \emph{thematic map layer} consists of foreground data, such as news-worthy geographic events, which is the primary information provided by the map. Second, the \emph{contextual map layer} consists of background data, such as a topographic map, which provides that spatial context in which the former information may be understood. % Why is it hard?? 

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.495]{figs-thesis/data-draws-the-world.pdf}
\caption{\textbf{Raw geographic information}: On a small scale map of Europe, the data symbols for ``raw'' points of interest (left) almost entirely cover the national territories (right). It illustrates how for large datasets (thousands of records or more) it does not make sense to display unfiltered data. For example, there are 28 thousand points of interest near Paris; which one is the Eiffel Tower on the left map?}
\label{fig:raw:poi:data}
\end{center}
\vspace*{-4ex}
\end{figure}

While contextual map layers have become a commodity that is readily availably from both private companies~\cite{googlemaps, bingmaps} and national mapping agencies~\cite{gst2014digitalmapsupply}, thematic map layers often presents a challenge to map designers due to the need for \emph{map generalization}~\cite{harrie2007modelling,mackaness2011generalisation,weibel1997generalization}. The source mateterial for a thematic map layer typically consists of raw and emerging geographic information, such as user-contributed data~\cite{openstreetmap,zooniverse2014oldweather}, which must first be molded into a presentable visual representation (see Figure~\ref{fig:raw:poi:data}).

% Why is the state-of-the-art not good enough?
There are two extremes types of thematic map layers that are being created. At one extreme, choropleth~\cite{guardian2013ananas} and heatmap~\cite{wiki2014heatmap} layers represent a statistical and aggregated view of geographic information, which is fairly straight-forward to produce using spatial statistics~\cite{ripley2005spatial}. An example is the left map in Figure~\ref{fig:introduction:background:mashups}. At the other extreme, thematic point maps represent an immersive and detailed view of information~\cite{bereuter2013real, sarma2012fusiontables,nutanong2012multiresolution}, which fundamentally challenges map designers due to the need for holistic abstraction of source map data. An example is the right map in Figure~\ref{fig:introduction:background:mashups}, which shows holistic selection of geographic points in Google Fusion Tables~\cite{lomet2012warstories}.  

%\begin{figure}[htbp]
%\begin{center}
%\includegraphics[scale=.5]{figs-thesis/vector-tiles.pdf}
%\caption{\textbf{Vector tiles}: TODO.}
%\label{fig:introduction:background:vector:tiles}
%\end{center}
%\vspace*{-4ex}
%\end{figure}
...
\section{Web Mapping Pipelines}
\label{introduction:requirements}

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.5]{figs-thesis/map-visualization-pipeline-3.pdf}
\caption{Logical tasks in a web mapping pipeline.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

Typical web mapping pipelines have many moving parts. However, most web mapping pipelines perform the same logical tasks (see Figure~\ref{fig:introduction:requirements:pipeline}). The logical tasks are to (1) capture ``reality'' in a primary data model, (2) derive logical map content from the primary data model, (3) derive graphical map content from the logical map content, and (4) distribute map content over the Internet to web and mobile mapping clients. Optionally, graphical interpretation of the logical data may be performed client-side~\cite{gaffuri12vectortiles}.

It is important to understand the tasks and requirements of each stage in the pipeline. This allows us to test for gaps between state-of-the-art pipelines and a hypothetical pipeline that meets all of the requirements. Stage one (data modeling) has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. Stage three (graphical abstraction) has been studied among others in the seminal studies by Bertin~\cite{bertin1973semiologie}. This thesis will focus on stage two (logical abstraction) and stage four (content distribution), since the most present challenges in web mapping are found here.

\subsection{Requirements}
\label{sec:introduction:requirements:map:content}

In stage 1 of a mapping pipeline, ``reality'' is captured in a primary data model that consists of discrete data objects. While the data modeling process and its requirements is beyond the scope of this thesis, it has been thoroughly described in previous work~\cite{friis2001requirements,goodchild1992geographical}.

In stage 2 and 3 of the mapping pipeline, the primary data model is transformed into a legible and consistent geographic map content. The outcome of stage 2 is a logical map content model, i.e. \emph{what} is the geographic information that should appear on the map. The outcome of stage 3 is a graphical map content model, i.e. \emph{how} should the geographic information be graphically represented on the map. In truth, these two processes are interdependent. For example, bigger graphical symbols may results in less information since symbols are more likely to overlap. For this reason, the requirements for logical and graphical map content are treated together in the following.

Map content must satisfy \emph{map constraints} that ensure the legibility and consistency of a map. For example, a proximity constraint may trigger that a map symbol is deleted because it overlaps a more important map symbol, which would compromise legibility. As another example, a topology constraint may prohibit the deletion of a river segment because it would result in an disconnected river network, which would be inconsistent with reality. Homo Sapiens, the principal designer and audience of map visualizations, has probably remained cognitively unchanged for thousands of years. Even as the Web has impacted when, why and how map visualizations are created and consumed, the basic ability of humans to perceive information on maps has remained powerful, yet fundamentally limited~\cite{bertin1973semiologie}. Consequently, modern maps on the Web must be guided by the same well-studied principles as ancient maps on print.

Some important map constraints are:

\begin{itemize}

\item \textbf{Unary constraints} establish conditions that must be met be individual map objects; a map object that violates a unary conditions is in conflict and may by definition not appear in its current form on a map. For example, impirical studies have shown that building symbols should be no smaller than 0.35 mm and roads should be no narrower than 0.25 mm. These limits are known as \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. Furthermore, when displaying detailed multi-lines and polygons on a computer display, consequtive vertices may be indistinguishable to the human eye. Such objects should be \emph{simplified} to the coarsest level of detail that is indistinguishable from all finer levels of detail. This saves storage, speeds up processing, cuts down on I/O, for example when geographic data is transmitted over the Internet.

\item \textbf{Binary constraints} set up conditions that must be satisifed by all qualifying pairs of objects. Two objects that violate a binary constraint are \emph{in conflict} and may by definition not appear in their current form on a map. For example, two objects that overlap or are very close to each other may be defined to be in conflict~\cite{weibel1997generalization, nutanong2012multiresolution}, which is a geometric binary constraint. However, not all binary constraints are geometric. For example, a constraint that limits the number of bulls per cow herd to one, is an example of a semantic binary constraints. Here, two bulls in the same cow herd would be in conflict with each other, no matter their geometric relationship to each other.

\item \textbf{Space constraints} state conditions that must be satisfied by spaces in the map, for example for each tile in a tiled map. One of the earliest impirical observations in geographic maps is that information density on published maps tends to be constant and independent of the map scale~\cite{topfer1966principles}. This principle is termed the \emph{Radical Law} and is also known as the \emph{Principle of Constant Information Density}~\cite{woodruff1998constant}. When information density on parts of the map reaches some threshold, the information should be abstracted by selection or aggregation in order to reduce the density (see Figure~\ref{fig:introduction:selection:constraints}).

\item \textbf{Cross-scale constraints} apply to maps that are zoomed in and out. \emph{Cross-scale constraints} state limitations on how the logical and visual map may transition between neighboring zoom levels. For some map objects, for example point of interest (POI) maps, it is more logical if POI do not disappear when zooming in, which is termed the Zoom-Consistency constraint~\cite{sarma2012fusiontables}. For other map objects, most notably place names, it is entirely appropriate that objects disappear when zooming in. For example, global place names (e.g. ``Europe'') should probably be replaced by local place names (``Paris'', ``London'', or ``Copenhagen'') when zooming in on a map. In map generalization process models these two approaches are termed the ladder and star approach respectively~\cite{foerster2010challenges}.

\item \textbf{Topological constraints} express invariants over the relationships between geographic information objects, which must not change during an abstraction process~\cite{egenhofer1991categorizing,randell1992spatial}. For example, the relative positions of distinct objects must be maintained; the primary logic of circulation in a road network must be maintained~\cite{schmid2013opensciencemap}; holes may not appear in surfaces where no holes were before~\cite{van1995gap}; river networks must be pruned from the leaves~\cite{shreve1966statistical}; etc.

\end{itemize}

Map constraints alone do not enable the ranking of feasible maps, which requires the introduction of \emph{map objectives}. One type of objective is to introduce a penalty for violated constraints, and minimize the sum of penalties. Another type of objective is to introduce a scoring function over the map objects, and maximize the sum of scores for objects that appear in the map subject to the constraints (see Figure~\ref{fig:introduction:selection:constraints}). 

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.35]{figs-thesis/holistic-selection.pdf}
\caption{\textbf{Object-relative and space-relative conflicts}: object-relative constraints, such as minimum proximity (left image), set up conditions that must be met by all qualifying pairs of objects. Space-relative constraints, such as a maximum density (right image), set up conditions that must be met by all qualifying spaces. In the figure, objects that are in conflict are circled by red lines; objects that are deleted in order to resolve the conflicts are represented by white disks; objects that are selected are represented by black disks.}
...
In stage 4, the map content is distributed over the Internet to web and mobile mapping clients. The primary requirements for this process is that map content is highly available and that the content serving process is highly scalable, i.e. can sustain acceptable latency for an increasing number of concurrent map clients.One of the most critical challenges is to fullfill these requirements while keeping down the cost.

\subsection{State-of-the-Art Map Design}

Stages 2 and 3 in the web mapping pipeline are concerned with map design. Currently, state-of-the-art map design systems are fall into two extreme categories: fully-automatic selection of geographic information and complex map generalization of geographic information. 

\minisec{Automatic Selection}
% describe
The primary use case for automatic selection is the rapid design of thematic point maps from databases of geocoded information. Apart from graphical customization, such as the styling of map symbols, the adaptation of geographic information for the map proceeds without the involvement of humans~\cite{google2014maps,}.

% identify
State-of-the-art automatic selection systems arbitrarily offer support for either unary~\cite{mapnik,sld,vizzuality2012cartodb}, binary~\cite{nutanong2012multiresolution}, or space constraints~\cite{sarma2012fusiontables}. Furthermore, map designers have no way to indicate how important objects are relative to each other, which at best means that an opaque ranking method is applied by the system.

% discuss
While automatic selection is highly time-efficient for map designers it also highly non-expressive. Map designers have no influence over the constraints and objectives that are applied during the selection process (see Section~\ref{sec:introduction:requirements:map:content}). This severely limits the control that map designers have over the design process, which is probably suboptimal for most cases. Automatic selection frameworks obviously lack alternative map generalization operators, such as displacement and aggregation. However, this is arguably less of a problem, because selection is probably sufficient for most thematic point map designs.

\minisec{Map Generalization}
% describe
The primary use case for full-featured map generalization is the production of complex cartographic representations of the Earth, such as topographic and physical maps. When creating maps over diverse types of geographic information -- suchs as streets, river networks, mountain ranges, and urban constructions -- complex methods are warranted.

% identify
Map generalization requires the holistic application of multiple operators -- such as selection, displacement, amalgamation, exaggeration, and collapse~\cite{weibel1999generalising} -- which has been modelled in complex frameworks with agent-based~\cite{lamy1999application} and metaheuristic search~{ware2003generalization_sa} methodologies.

% discuss
While map generalization encompasses powerful methodologies, the demand for the kind of maps that can be produced by such powerful tools is arguably vaining. The abundant supply of high-quality background maps from private companies and national mapping agencies, together with the constant emergence of user-contributed and currated thematic data, has shifted demand towards the design of simpler foreground layers. Furthermore, the demand for foreground map design is rising faster than the number of domain experts who master map generalization techniques, which is a fundamental human resources challenge. Finally, map generalization is a complex and time-consuming, and thus costly, even for domain experts.


\subsection{State-of-the-Art Map Serving}
...
Given the requirements for online maps that were outlined in Section~\ref{introduction:requirements}, this section will list some important gaps between state-of-the-art mapping systems and a hypothetical mapping system that satisfies all the requirements. These problems are addressed by the thesis contributions that are introduced in Section~\ref{sec:introduction:contributions}.

\begin{itemize}

% MATHEMATICAL FOUNDATION
\item \textbf{Unified Selection Model}
While selection of data for maps can be intuitively understood as filtering objects subject to some criteria, many different selection models have been proposed~\cite{sarma2012fusiontables,nutanong2012multiresolution,rocha2011efficient}. Critically, there is a lack of a unified selection model that can express all of them. What is needed is a model for holistic selection of geographic data that supports: scale-dependent and user-defined map constraints, i.e. unary, object-relative, space-relative, and cross-scale constraints, as well as scale-dependent and user-defined map objectives.

Ideally, a well-studied and efficient algorithm can solve all instances, in the same fashion that sorting solves top-k selection.

% DECLARATIVE SPECIFICATION
\item \textbf{Declarative Specification}: There is currently a lack of systems that offer map designers a balanced solution between automated selection and map generalization. Ideally, a map design interface for thematic maps is simple, yet programmable. In contrast, state-of-the-art interfaces are either too inflexible or too time-consuming.

It makes sense that programmers can declaratively specify map constraints and objectives in a high-level geographic selection language, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. Declarative specifications are concise and intuitive and therefore highly productive for programmers. Moreover, many declarative languages are highly optimizable since computational frameworks can be swapped out in the background~\cite{eldawy2014pigeon,thusoo2009hive}, for example by targeting a 

% IN-DATABASE PROCESSING
\item \textbf{In-database Processing}: The web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap} datasets that can be visualized on maps. Typically, data for visualizations is stored in a relational database~\cite{wu2014case}. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to location-based services online~\cite{endomondo2014endomondo,tinder2014tinder,twitter2014twitter} geographic information can be produced by anybody, anytime, and anywhere. Therefore,  a mapping systems must be able to scalably process multi-scale selection over large relational datasets (millions of records). 

One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a high-level language such as SQL, which reuses optimization techniques that are built-in to existing SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Furthermore, high-level languages such as SQL are typically supported by emerging parallel in-memory data processing systems~\cite{zaharia2010spark}. Orthogonally, data for visualizations is typically stored and managed in relational database management systems (RDBMS); from an I/O perspective, it makes sense to process the data where it is stored. Despite the obvious opportunity, state-of-the-art map visualization systems do not significantly leverage RDBMS technology or relational algebra to process multi-scale selection of geographic information, other than to extract geographic data for post-processing in external systems.

% PREDICTIVE TILE MATERIALIZATION
\item \textbf{Predictive Tile Materialization}: 
Rendering and serving of so-called map tiles is currently the typical way to implement high performance maps~\cite{mbtile12}. There are two extreme approaches to rendering tiles. At one extreme, the entire tile set is preemptively materialized~\cite{dean2009designs}. However, this is wasteful as most tiles are never or rarely requested~\cite{fisher07hotmap}. At the other extreme, individual map tiles are lazily materialized and only rendered when they are actually requested by a user. However, this results in high average latency since requests tend to be highly concurrent~\cite{kefaloukos2012tileheat}. While several studies have proposed real-time predictive prefetching of tiles for individual users, this approach does little to address the latency problem under highly concurrent workloads. While the problem can be addressed by deploying more servers, this is a high-cost approach. Furthermore, it is suboptimal to prefetch tiles for individual users, since tile sets are finite and many users request the same tiles. A better approach is to predict popular tiles well in advance, and materialize a hot set during a low-load period and serve it with low latency during high-load periods. A hypothesis is that a log of previous requests can be used to predict future tile requests.
...
\subsection{Tileheat: Geographic Request Prediction}
...
%\begin{quote}
%This thesis argues that GIS has become marginalized on the web, and therefore it is necessary to study the design of a next-generation map visualization system, designed from the ground up for the realities of the web. A case is made for the gold standard of map visualization systems that deliver high-quality multi-scale map visualizations, while being superbly integrated with relational database, highly productive for non-experts, highly scalable in data volume (millions of records), highly available even through system maintenance, and can effortlessly serve concurrent requests to large numbers (millions) of online users. The system does not have to be perfect, or even better in all dimensions than previous work. It simply has to be better-suited for map visualizations on the web. Specifically, some previous work will propose more features or features that more cleverly through out. However, case by case, gaps will be identified between the related work and what the gold standard for map visualization systems should be on the web.

%Answering each of these questions will require us to investigate areas beyond pure map visualization. For example, work in \emph{map generalization} addresses the problem of geographic data abstraction for multiple scales~\cite{weibel1999generalising}; work in \emph{data management} addresses the issue of managing data for visualizations; work in \emph{programming languages} addresses the issue of expressing the data transformations~\cite{sql}; work in \emph{graphics} addresses the issue of representating data visually~\cite{wickham2009ggplot2, wilkinson2006grammar}; work in \emph{data management} addresses the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; work in \emph{distributed systems} addresses the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. All of these topics play a key role in map visualization. This chapter scans the intersection between map visualization and the related work and compares the results against the requirements of a Map Visualization Management System (see Chapter~\ref{cha:introduction}).
%\end{quote}
...
\chapter{Selection of Map Content}
...
In contrast to the older geography literature on map generalization, recent studies of map generalization tend to make fewer geography-specific distinctions; it is as though the recent literature deliberately departs from the heavy complexity of ``traditional'' map generalization and seeks out a simpler analysis. An explanation could be that map generalization for complex cartographic problems, such as maps of the physical earth, is generally a closed field. The many background maps that are provided online by private companies, as well as the national mapping agencies, certainly support this explanation. While arguably map generalization has been solved for the production of topographic, physical and other types of background maps, the web has opened up new challenges for the generalization of emerging foreground data. This claim is supported by the appearance of a new wave of map generalization studies that focus on the generalization of emering foreground data, and mostly point sets. Indeed, the challenges that are addressed are different from those that are addressed by traditional cartography and generalization.

In modern map generalization, an important distinction is between \emph{on-the-fly} and \emph{batch-oriented} generalization. However, both approaches have to deal with the same situation that is generally faced by any kind of data visualization on the web. First, hitherto unseen quantities of data must be handled in a scalable fashion. Second, efficient and intuitive human-computer interfaces must provide designers with facilities to effectively turn data into visual communication means. Third, map visualization platforms must be well-integrated with existing and emerging data repositories online. Fourth, visual communication means must be scalably distributed to millions of online users. Of course, the same underlying principles that have guided traditional generalization must also guide modern generalization. In other words, human visual cognitive ability is tied to the human biology and remains unchanged by the appearance of the web.

The challenges brought on by the web are not isolated to map visualization. Outside the fields of geography and map generalization, highly similar data abstraction challenges are being addressed in the field of data visualization. Here, works also addresses the issues outlined above, such as how to express data visualizations and how to handle large quantities of data~\cite{wu2014case}. While spatial is still special, there is no doubt that opportunities exist for cross pollination across these two fields. \emph{enumerate}. It is therefore relevant to briefly position the contributions of this thesis in a larger landscale of data visualization. However, these contributions will be discussed together with work in map generalization.

This chapter covers related work in the following areas:

\begin{itemize}
\item General principles of map generalization
\item Requirements for map visualization on the web
\item On-the-fly generalization
\item Batch-oriented generalization
\end{itemize}

For each of these areas, the work is judged in relation to the requirements brought on by the web.


% Big data: The next frontier for innovation, competition, and productivity
% Breaking the chains: On declarative data analysis and data independence in the big data era

This thesis argues that GIS has become marginalized for addressing the current challenges in maps on the web, and therefore it is necessary to study the design of a next-generation map visualization system, designed from the ground up for the realities of the web. A case is made for map visualization systems that deliver high-quality multi-scale map visualizations, while being superbly integrated with relational database, highly productive for non-experts, highly scalable in data volume (millions of records), highly available even through system maintenance, and can effortlessly serve concurrent requests to large numbers (millions) of online users. The system does not have to be perfect, or even better in all dimensions than previous work. It simply has to be better-suited for map visualizations on the web. Specifically, some previous work will propose more features or features that more cleverly through out. However, case by case, gaps will be identified between the related work and what the gold standard for map visualization systems should be on the web.

Answering each of these questions will require us to investigate areas beyond pure map visualization. For example, work in \emph{map generalization} addresses the problem of geographic data abstraction for multiple scales~\cite{weibel1999generalising}; work in \emph{data management} addresses the issue of managing data for visualizations; work in \emph{programming languages} addresses the issue of expressing the data transformations~\cite{sql}; work in \emph{graphics} addresses the issue of representating data visually~\cite{wickham2009ggplot2, wilkinson2006grammar}; work in \emph{data management} addresses the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; work in \emph{distributed systems} addresses the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. All of these topics play a key role in map visualization. This chapter scans the intersection between map visualization and the related work and compares the results against the requirements of a Map Visualization Management System (see Chapter~\ref{cha:introduction}).

% Basic condition in cartography (while I'm not doing cartography, my methods abstract and prepare the data for cartographic visualization. Therefore, the conditions below are relevant):
...
\section{Overview of Map Generalization}
...
% mention where to get more info on traditional generalization
This thesis focuses on generalization of geographic data in relational databases. Map generalization is a big and complex field, which on record has been studied for decades. Off the record, it has probably been studied for thousands of years. Consequently, there is a lot of related work. However, the purpose of this section is not to give a comprehensive overview of map generalization, as this is too big an area. For that, the reader is refered to the survey by Weibel and Dutton~\cite{weibel1997generalization}, as well as a paper by Jones and Ware~\cite{jones2005webage}. A recent book by Mackaness, Ruas, and Sarjakoski attempts to collect the state of knowledge in generalization as of 2011~\cite{mackaness2011generalisation}. As yet, there seems to be a lack of surveys that investigates the recent challenges and approaches in map generalization, while also relating broadly the recent work in modern data visualization. While these sources might not give a accurate impression of where generalization is going, it will give a good impression of where generalization has come from.

% Older surveys from the 90's
...
Briefly explain what is meant by a unary constraint

\minisec{Minimum Dimensions}
  % Minimum dimensions
Minimum dimensions in cartography, such as 0.35 mm for the length of a black square (e.g. representing a building) and 0.25 mm between the double lines typically used to draw roads.

% Object-relative constraints
\subsection{Object-relative Constraints}

Briefly explain what is meant by an object-relative constraint

\minisec{Minimum Proximity}
  % Minimum proximity

% Space-relative constraints
\subsection{Space-relative Constraints}

Briefly explain what is meant by a space-relative constraint

\minisec{Constant Information Density}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.6]{figs-thesis/constant-information-density.pdf}
...
% see GAP-tree for muller-paper
M{\"u}uller et al. distinguish between batch and interactive generalization.


\subsection{Requirements for map visualization on the web}
...
\section{On-the-fly Map Generalization}
...
% approaches: expressing, online processing
\subsection{Indexing}
% approaches using data structures
...
% approaches: expressing, off-line processing
...
\chapter{Data Serving for Maps}

% approaches: expressing, processing, serving
