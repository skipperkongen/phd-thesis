...
In this section, we evaluate three crucial aspects of our implementation of Glossy SQL. First, we evaluate the performance and scalability of Glossy SQL queries when compiled to pure SQL by the translation of Section~\ref{sec:sql:translation} and run over an existing RDBMS (PostgreSQL). The goals of our evaluation are to show that: 
...
Second, we evaluate the character-count-efficiency and code-complexity of Glossy SQL compared to equivalent queries in pure SQL (the output of the compiler). The goal of this evaluation is to demonstrate that Glossy SQL is significantly less verbose and complex to write than the equivalent SQL in the PostgreSQL dialect.

Third, we evaluate quality of the selections that are computed by Glossy System. In particular, we show that when items have the same weight, the selection corresponds to random sampling.
