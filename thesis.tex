...
\abstract{\emph{should be rewritten completely}: This thesis argues that the role of traditional geographic information systems (GIS) has become marginalized on the web. Therefore, it is necessary to study the design of a next-generation map visualization system, designed from the ground up for the realities of the web. A case is made for map visualization systems that are highly productive and effective map design environments for geospatial novices, and which are superbly integrated with large geographic tables (millions of records) stored in relational database management systems (RDBMS). Furthermore, map visualization systems on the web must be highly available even through system maintenance, and must effortlessly and scalably serve concurrent requests to large audiences (millions of online users). The thesis argues that such a system does not have to be perfect, or even better in all dimensions than previous work in geographic information visualization. It simply has to be better-suited for map visualizations on the web. Specifically, some previous work will propose more features or features that are more powerful with respect to map visualization. However, case by case, gaps will be identified between the related work and the gold standard for map visualization systems on the web. Typically, previous work will complement rather than challenge the work presented in this thesis. For example, the availability of high-quality background maps is the fruit of many years of labor within the geographic information visualization community. The role of next-generation map visualization systems is not to replace the fruit of this labor, but rather to enable powerful geographic mash-ups that combine high-quality background maps with the new and excting geographic datasets that continuously emerge on the web.

The contributions of the thesis are organized into two orthogonal dimensions. First, this thesis focuses on two questions about of how to create geographic information visualizations. (1) how to effectively express the complex problem of holistic multi-scale selection in a declarative programming language style, which can be mastered even by novice map designers, and (2) how to scalably process multi-scale selections inside a relational database engine, which assumes that geographic data is stored in an RDBMS. Second, this thesis focuses on a question that relates to how content delivery networks (CDN) can be exploited in order to cost-effectively distribute interactive geographic maps with high availability, scalability and performance. This dissertation specifically focuses on (3) how to predict the arrival of requests in a map serving system in order to prioritize the materialization of map data in a CDN, which can dramatically cut down the cost for storage and processing. 
...
For thousands of years, humans have used their visual senses to comprehend geographic information on maps. Humans learned early on that legible maps enable instant comprehension of relevant geographic information, while cluttered maps inhibit comprehension~\cite{phillips1982investigation,springer1987retrieval}. Over time, the reasons that maps are created and the context they are created in have constantly evolved. Accordingly, novel methods for drawing maps have been invented, studied, and sometimes discarded. However, there exists no evidence to suggest that the human capacity for visual perception and comprehension has evolved significantly since the stone age. Consequently, modern map makers who wield modern map making tools must accomodate the same human cognitive limitations as ancient map makers.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.47]{figs-thesis/geographic-mashups.pdf}
\caption{\textbf{Maps on the Web}: Two examples of maps that show thematic foreground data on top of general-purpose background maps: (left) a choropleth map of  \% people who speak a non-english language as their first language in the UK; (right) a point map of Wikileaks data for casualties during the 2003-2011 Iraq War. The maps are reproduced from The Guardian's Data Blog including background maps \textcopyright 2014 Google.}
\label{fig:introduction:background:mashups}
\end{center}
\end{figure}

\minisec{Map Visualization}
Map visualization studies the process by which geographic information is displayed on a three-dimensional medium (x, y, color), most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. Since ancient times, the requirements for this process have been to \emph{accurately} and \emph{legibly} represent as much as possible of the \emph{relevant} information on a finite map medium~\cite{jones2005webage}. This implies a holistic reduction in information, which is frequently an NP-hard problem~\cite{sarma2012fusiontables,haunert2010area}.

If you need convincing that map visualization is a difficult task, try to pick up a pencil and draw a detailed map of the World on a sheet of A4 paper. Soon, you will need to make hard choices about what to include and what to leave out, such as cities, waterways and roads. Now imagine that you have to draw a map of a different world than the physical Earth; a world you have no prior knowledge of; a world where you do not start with a mental model of what is important and what is irrelevant. A daunting task to be sure. This is the kind of world that is encountered when new spatial datasets emerge on the Web, such as the Wikileaks dataset which details spatial events that occured during the 2003-2011 Iraq War (see the left map in Figure~\ref{fig:introduction:background:mashups}).

\subsection{Multi-scale Maps}
Multi-scale maps allow users to zoom the view in and out (change scale), as well as to pan it (change focal point). Each scale of the map shows geographic information at a different level of detail. The state of a map is divided between two models, the \emph{logical model} and the \emph{graphical model}. While the logical map model defines \emph{what} geographic information appears on a map at each scale, the graphical map model defines \emph{how} information appears on a map at each scale. The differences and specifics of these two models are covered below.

\minisec{Logical Model}
In a logical model of maps, geographic information is represented by a set of \emph{geographic objects}. For example, geographic objects can represent roads, buildings, lakes, locations, restaurants and much more. Each object associates a \emph{geometry} (point, line or area), with additional \emph{qualitative}, \emph{ordered} and/or \emph{quantitative} attributes~\cite{bertin1967semiologie}. For example:

\begin{itemize}
\item \ang{55.6778}, \ang{12.5962} is and example of a geometric attribute value, which allows geographic information to be \emph{positioned} on Earth. For example, \ang{55.6778}, \ang{12.5962} is located on the harbor front in Copenhagen, Denmark.
\item ``Nordic'' and ``Japanese'' are examples of qualitative attribute values, which can be \emph{distinguished} from each other. For example, Nordic cousine is different from Japanese cousine, but neither better nor worse.
\item ``Gourmet'' and ``Fast-food'' are examples of ordered attributes, which can be \emph{compared} to each other. For example, gourmet restaurants are more expensive than fast-food restaurants.
\item DKK 1.600 and DKK 1.000 are examples of quantitative attributes, which can be \emph{added}, \emph{subtracted}, etc. together. For example, the sum of the values is DKK 2.600.
\end{itemize}

Thus, the logical state of a map is the set of geographic objects that appear on each scale of the map. Consequently, the logical state can be altered by performing abstract operations on the set of geographic objects: 

\begin{itemize}
\item Subsets of geographic objects can be \emph{selected}
\item Geographic objects can be \emph{projected}, i.e. their attributes and/or their values changed
\item Several geographic objects can be \emph{aggregated} into one geographic object
\end{itemize}

Each abstract operation on the logical model can be implemented by concrete operators that serve a specific purpose. For example, \emph{displacement}~\cite{bader2001energy}, \emph{simplification}~\cite{douglas1973algorithms}, \emph{exaggeration}, \emph{collapse} are all types of projections, but each modifes an object in a unique way.

\minisec{Graphical Model}
In a graphical model of maps, geographic information is represented by a set of \emph{symbols}, i.e. one symbol per geographic object in the logical model. Seven \emph{visual variables}, which were first described by Bertin~\cite{bertin1967semiologie}, visually express the attributes of geographic objects (see Figure~\ref{fig:introduction:visual:variables}). Certain visual variables have an affinity towards certain types of attributes. For example, as colors have no inherent order they express qualitative attributes best. In contrast, size can be compared and measured, and therefore express quantitative and ordered attributes best. 

The symbol position usually represents the object's geometry, which has been projected onto the plane using a standard map projection~\cite{snyder1997flattening}. Alternatively, symbols can be positioned in a topologically equivalent layout, which is a technique that is frequently used on Metro maps. Moreover, the symbol position can perfectly well represent something other than the geometry, such as the average life expectancy and economic wealth of a country~\cite{rosling2007poverty}. 

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.7]{figs-thesis/visual-variables.pdf}
\caption{\textbf{Bertin's visual variables}: The seven variables express qualitative, ordered and quantitative attributes (reproduced from \texttt{infovis-wiki.net}).}
\label{fig:introduction:visual:variables}
\end{center}
\vspace*{-4ex}
\end{figure}

Bertin's work on the theory of graphics is immense, and this short section does not aim to fully cover it. A work that is related to Bertin's, is the book ``the Grammar of Graphics'' by Wilkinson, Wills, Rope, Norton, and Dubbs~\cite{wilkinson2006grammar}. Between them, these works cover the fundamental aspects of representing information as graphics.


\subsection{Effects of Scale Reduction}
A basic challenge in map visualization is how the perceptability of graphics is affected by a reduction in scale. Maps have been produced at many different scales, where the term scale signifies the ratio between distance as it is measured on the map medium and distance as it is measured on the ground. For example, on a 1:500.000 map, one centimeter on the map surface corresponds to five kilometers on the ground. A map that shows just a single city may be called a large-scale map, while a map that depicts the entire world may be called a small-scale map. A number of fundamental effects occur as a graphic, such as a map, is scaled down.

\minisec{Size and Separation}
An effect that occurs when a graphic is scaled down is that the \emph{size} of object symbols -- if they are shown to scale -- decreases. Consequently, object symbols may approach the limit of perceptibility. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be no shorter than 0.35 mm. while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.

Furthermore, scale reduction causes the \emph{separation} between objects to decrease. Consequently, pairs of objects may come so close as to appear indistinguishable from each other. For example, two symbols that are 0.4 mm apart on a 1:500.000 map will be 0.2 mm apart on a 1:1.000.000 map, which is probably below the limit at which symbols can be distinguished from each other (see Figure~\ref{fig:introduction:background:raw:poi}).

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.495]{figs-thesis/data-draws-the-world.pdf}
\caption{\textbf{Effects of scale reduction}: Symbols for POI objects on a large scale map are clearly separated from each other (left). In constrast, unfiltered symbols for POI objects on a small scale map are indistinguishable from each other (right). The density of symbols is so high that they entirely cover the physical territories of most national territories.}
\label{fig:introduction:background:raw:poi}
\end{center}
\vspace*{-4ex}
\end{figure}

\minisec{Information Density}
Another way to view the effects of scale reduction is to say that the \emph{information density} of a graphic increases, that is the amount of information per unit area. Several metrics for information density have been studied, such as Tufte's density metric~\cite{tufte1983visual} and the density metrics surveyed by Nickerson~\cite{nickerson1994visual}. The central \emph{principle of constant information density} states that information density on a map should remain constant as the scale changes. The principle which was empirically established by the cartographers T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that the number of settlements per unit area in published map series is independent of the map scale. The rationalized their observation by observing that a linear reduction in scale implies a reduction in area according to geometrical progression, which in turn must lead to a reduction in information at the same rate, if the information is to remain legible.

\subsection{Map Generalization}
A basic challenge in map design, known as generalization~\cite{shea1989cartographic,weibel1997generalization}, is how to derive a \emph{logical} and \emph{graphical} map state after a scale reduction, such that the map remains legible and relevant. A collection of map generalization work has been compiled by Mackaness, Ruas, Sarjakoski~\cite{mackaness2011generalisation}, which may function as possible entry point into this complex field. Other entry points into the generalization field are the works by Weibel~\cite{weibel1997generalization}, Weibel and Dutton~\cite{weibel1999generalising}, as well as McMaster and Shea~\cite{mcmaster1992generalization}.

\minisec{Constraints}
The generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, \emph{constraints} act as the descriminator between feasible and infeasible map states.

Constraints can be classified according to many different typologies~\cite{burghardt2007investigations,mcmaster1992generalization}. However, constraints generally accept a (partial) map state and either reject it or accept it. For example:

\begin{itemize}
\item A partial map state can be a single building object, which must satisfy a minimum dimensions constraints. If the building is too small, the map state is rejected
\item A partial map state can be a pair of buildings, which must satisfy a minimum proximity constraint. If the buildings are too close to each other, the map state is rejected
\item A partial map state can be the set of all points of interest in the map, which must satisfy a density constraint. If the density of the points is too high for any unit area of the map, the map state is rejected.
\item A partial map state can be a set of segments in a river system, which must satisfy a flow constraint. If the river system is disconnected from itself or its outlet, the map state is rejected.
\end{itemize}

In the examples above, the flow constraint requires knowledge of a previous state if a river segment is to be meaningfully categorized as ``missing''. In constrast, the minimum dimensions constraint can be evaluated in isolation, since a building is either too small or it is ok.

first constraint is \emph{context-independent} while the second constraint is \emph{context-dependent}. The minimum dimensions constraint is context-independent because a building object is either too small or it is ok, regardless of the context. The flow constraint is context-dependent, because a river system is only disconnected if another map state exists in which it is connected.

a constraint could be evaluated for all individual building objects in a map, and reject the partial  of inputs that involve geographic objects, a partial states could be a single ``building'' object, a pair of ``point of interest'' objects, or all objects of the ``river'' class that are part of the same river system.


In a functional view, constraints are \emph{functions} that accept a (partial) map state and output a boolean value of \emph{true} if and only if the input satisfies the constraint.
Constraints that is formulated over map states can be sclassied as either 


Constraints may define the limits of many different properties of the map, such as symbol size, complexity and density~\cite{weibel1999generalising}. For example, \emph{proximity constraints} define the limit for how close geographic objects symbols may appear on a map in order to be considered \emph{distinct}~\cite{nutanong2012multiresolution}; \emph{topological constraints} restrict which segments of a river network can be pruned to ensure that the river remains connecting to itself and its outlet~\cite{horton1945erosional,shreve1966statistical,strahler1957quantitative}.

Regardless of the many constraint typologies 

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.32]{figs-thesis/holistic-selection.pdf}
\caption{\textbf{Pair and set constraints}: pair constraints, such as minimum proximity (left image), set up conditions that must be met by all qualifying pairs of objects on a map. Set constraints, such as a maximum density (right image), set up conditions that must be met by all qualifying sets of objects in the map. Objects that violate a constraint are in conflict (circled by red lines); this situation must be remedied, for example by selection (kept objects are shown as black disks; deleted objects are shown as white disks.}
\label{fig:introduction:selection:constraints}
\end{center}
\vspace*{-4ex}
\end{figure}

While constraints can be classified according to many different typologies~\cite{burghardt2007investigations,mcmaster1992generalization}, one approach is to view constraints as \emph{functions} that map input to a boolean value that is \emph{true} if and only if the input satisfies the constraint. In this \emph{functional typology} constraints are classified by their input signature: \emph{object}, \emph{pair of objects}, \emph{space} (see Table~\ref{tab:introduction:map:constraints}). 

 This pattern is well-known from object-oriented design theory.



\begin{table}[htdp]
\caption{An arity-based typology of map constraints}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Input} & \textbf{Examples} \\
\hline
Object & Minimum dimensions~\cite{ssc1977minimumdimensions}, complexity~\cite{de2004simplifying} \\
\hline
Pair of objects & Minimum proximity~\cite{nutanong2012multiresolution}, topological relationship~\cite{egenhofer1991categorizing,randell1992spatial} \\
\hline
Space & Density~\cite{topfer1966principles} \\
\hline
Pair of sets & Topology~\cite{schmid2013opensciencemap,shreve1966statistical}, zoom-consistency~\cite{sarma2012fusiontables,foerster2010challenges}, distribution~\cite{sarma2012fusiontables}, coverage~\cite{van1995gap} \\
\hline
\end{tabular}
\end{center}
\label{tab:introduction:map:constraints}
\end{table}%

\minisec{Map objectives}
Objectives seek to optimize the utility of a map, which is quantified by an objective function. Therefore objectives allow two (feasible) maps to be compared to each other. For example, an objective may be to maximize the sum of weighted importance (using some weight function) over all objects that appear on the map~\cite{sarma2012fusiontables}. Given this objective, one map is ``better'' than another map if the sum of weighted importance of visible geographic objecst is greater on the former map.

A real-world set of constraints and objectives were used to evaluate the map for Apple iOS 6~\cite{samet2012duking}.

\minisec{Logical Abstraction}
Logical abstraction is the process that derives, from a set of base geographic objects, a set of geographic objects that should appear on a map. At a high level, geographic objects in the base set can be \emph{selected}, \emph{aggregated}, and have their attributes \emph{projected} (in the general sense, not only cartographic sense) in order to accomodate the legibility and relevance goals of the map. Therefore, logical abstraction it is the process that decides \emph{what} information should be appear on a map.

TODO: discuss operators

One way to ensure a legible and relevant maps is to select data holistically according to constraints and objectives that ensure that humans will find relevant information on the map. 
...
\includegraphics[scale=.47]{figs-thesis/geographic-mashups.pdf}
\caption{\textbf{Maps on the Web}: Two examples of maps that show thematic foreground data on top of general-purpose background maps: (left) a choropleth map of  \% people who speak a non-english language as their first language in the UK; (right) a point map of Wikileaks data for casualties during the 2003-2011 Iraq War. The maps are reproduced from The Guardian's Data Blog including background maps \textcopyright 2014 Google.}
\label{fig:introduction:geographic:mashups}
\end{center}
\end{figure}
...
\section{When to Process Selection}
\label{sec:introduction:architectures}

A key distinction between state-of-the-art mapping pipeline is the timing of the map generalization process. Two extreme approaches are \emph{real-time}~\cite{bereuter2013real,van1995gap} and \emph{batch-oriented}~\cite{sarma2012fusiontables,frank1994multiscaletree,van2011towards} approaches. In the following sections, the advantages and drawbacks of both approaches are analyzed.

\subsection{Real-time Approach}
Real-time generalization adapts geographical information at the time when a user requests geographic information for a map. Typically, an indexing structure, such as a quadtree~\cite{bereuter2013real} or GAP-tree~\cite{van1995gap}, is the factor that facilitates fast real-time generalization algorithms. At first, real-time selection seems like an ideal approach for two reasons. 

First, the data is stored only once, namely in its original state. While this obviously saves storage, a more important benefit is that selected information is never stale. Given an atomic update mechanism, such as transactions~\cite{gray1981transaction}, a selection algorithm always computes solutions based on the most recent and consistent state of the data. However, this is only true if the atomic update propagates to the index as well, which is a significant consideration given that data is typically managed outside the map generalization framework.

Second, user preferences and context of usage can be considered as part of the input to selection. Therefore, selection of geographic information can be tailored to individual users needs, for example by letting a recommendation algorithm play a role in assigment of object importance.

However, there are several issues with real-time generalization in a Web context. While indexing structures enable fast generalization, the complexity of any selection algorithm is high compared to $O(1)$, which is the complexity of retrieving pre-selected data from a key-value store~\cite{pilchin2007dynamo}. Furthermore, the scalability of real-time generalization algorithms is usually only considered for increases in data volume, while it is ignore for increases in concurrency level. This is critical, because in a Web context there is typically a high and variable level of concurrency, which implies that the generalization method must be scalable in the number of concurrent queries. Unfortunately, there is a lack of studies into real-time generalization that efficiently scale in the number of concurrent requests. The na{\"i}ve approach, which is to replicate the entire index, is costly.

In conclusion, real-time generalization is currently an infeasible approach for web mapping pipelines that target large Web audiences.

\subsection{Batch-oriented Approach}
...
Geocouch, MongoDB, Cassandra, Riak.

Programmed in a variety of native languages.
