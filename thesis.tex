...
\emph{What are the key questions?} Three crucial and orthogonal questions relate to data visualization on the web. First, how can we express the task of data transformation, from non-geometric to geometric representation? Second, how can we process millions of records? Third, how can we serve data visualizations to millions of demanding online users? Answering each of these questions will require us to investigate areas beyond pure data visualization. For example, programming languages address the issue of expressing data transformation~\cite{sql}; databases address the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; distributed systems address the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. It is crucial that we pull together the relevant know-how from these diverse fields and combine it into systems for efficient and effective data visualization.
