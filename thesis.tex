...
Geographic information is typically represented by discrete and multi-dimensional \emph{geographic objects}. For example, geographic objects can represent roads, buildings, lakes, locations, restaurants and much more. Each object associates a \emph{geometry} (point, line or area), with an additional \emph{set of attributes} of qualitative, ordered and/or quantitative types~\cite{bertin1967semiologie}: \emph{qualitative attributes}, such as the cousine of a restaurant, allow basic distinction between its possible values. For example, Italian is different from Japanese; \emph{ordered attributes}, such as price category, allows a ranking of its possible values. For example, gourmet restaurants are more expensive than fast-food restaurants; \emph{quantitative attributes}, such as restaurant rating on a 0-5 scale, allow aritmetic to be performed on their values.

Geographic objects are graphically represented by symbols, that give a visible body to often area-less geometries, such as points, lines and the boundaries of polygons. For example, a geometric point may be visually represented by a disk, while a geometric line may be visually represented by a band.
