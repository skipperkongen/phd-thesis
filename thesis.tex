...
Maps are a type of graphic, which like all graphics consists of symbols. The symbols represent the geometric and topological relationship between entities and themes in some space. As a special case, geographic maps symbolize entities and themes that are found on or near the Earth's surface. The position and extent of entities on the (roughly) spherical Earth is most conveniently given in spherical coordinates, which by convention are stated as degrees north or south of the Equator, degrees east or west of the Greenwich line, and meters above or below the mean sea level. 

Given a position on Earth, there are several steps that are required before a symbol can be drawn on a flat map medium. Map projections translate between spherical coordiantes on Earth and planar coordinates on the map medium~\cite{snyder1997flattening}. While some map projections may preserve angles, other projections may preserve areas or distances. Which projection should be used depends on the purpose of the map. For example, preserving angles is important on navigational charts, while preserving areas is arguably more important on political maps. After projection, symbols can be displaced in ways that optimize readability while retaining (say) topological relationships. For example, a metro stations and lines map should be topologically correct (e.g. display stations where lines intersect) and also be highly readable (since commuters are often in a hurry); in this setting, geometric accuracy is less important.
...
\minisec{Data Abstraction}
It is natural to interpret the world as being inhabited by distinct entities that are ordered in a hierarchy. For example, the idea that a tree in a forest has a set of leaves perfectly illustrates this idea of hierarchical entities. However, real-world entities are full of inifinite detail. In contrast, maps can not represent infinite detail. Therefore, data for maps must abstract all this detail into a finite set of information values for each described entity.
...
While the hierarchy of entities (leaves, on trees, in a forest) can be captured by attributes that point to a set of objects, most maps do not require the entity hierarchy to be explicitely modelled in the data model. An exception is an interactive map that supports active drill-down on information. For example, clicking on the restaurant ``Noma'' on a map, could open up a new map of the table plan.

While this section has focused on objects, an alternative data modelling technique is to model a spatial entities by a data grid (conventionally called \emph{raster} data in geography). A data grid usually represents aggregated spatial information, such as population count per unit area or elevation of territory. While grid models are often very practical, the object model subsumes the grid model. For example, a set of uniformly distributed objects is equivalent to a data grid.
