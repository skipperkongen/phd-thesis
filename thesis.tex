...
Digital map visualization studies how to visually represent the geography of the Earth at multiple scales on restricted digital mediums, such as computer screens. On the web, geographical mash-up is the process by which designers of maps will layer new and interesting geographical data on top of commonly shared background maps. While the background maps are generally of high-quality, the result of many hours of expert labor, new and interesting data presents a raw challenge for creators of mash-ups. Simply displaying symbols for raw geographical data on top of a background map, without appropriate abstraction, will work only for the tiniest datasets (tens of records). One of the most fundamental problems in digital map creation is the mismatch between data volume and the small representational budget of computer screens. For example, a laptop display (e.g. 2880 x 1800 pixels) can display just a few thousand symbols without overlap, which is entirely indadequate for displaying individual records for most geographical datasets, i.e. datasets of more than a hundred spatially skewed records. Necessarily, geographical data must be geometrically abstracted according to scale. Useful geometric transformations include: reduction in the number of discrete objects (geometric selection), displacement and simplification of objects (geometric modification), and aggregation of grouped objects (geometric aggregation). In the geographical literature, the field that studies the process of data and visual abstraction for maps is field known as map generalization. For an overview of this complex field, you are refered to the work of Weibel and Dutton~\cite{weibel1999generalising} as well as Jones and Ware~\cite{jones2005webage}. A simple and effective way to deal with too much data, while retaining the possibility to interact with relevant geographical objects, is to perform geometric selection. However, to create an useful map, geographical data must be \emph{holistically selected} according to constraints and objectives that ensure a proper balance between inclusion of relevant source information and cartographic considerations of legibility.
