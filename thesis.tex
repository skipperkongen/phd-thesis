...
Two important opposite approaches have been studied for generalization of geographic information, namely \emph{real-time} (or on-the-fly) generalization~\cite{bereuter2013real,van1995gap} and \emph{batch-oriented} generalization~\cite{sarma2012fusiontables,frank1994multiscaletree,van2011towards}. While the requirements for \emph{what} should be the produced are the same for the two approaches, vis-a-vis map requirements and objectives, these two approaches differ in \emph{when} the results are produced. The distinctions as well as the pros and cons of each approach in a Web context is discussed below.

\minisec{Real-time approach}
Real-time generalization adapts geographical information at the time when a user request for information is recieved. Typically, an indexing structure, such as a quadtree~\cite{bereuter2013real} or GAP-tree~\cite{van1995gap}, is the factor that facilitates fast real-time generalization algorithms. At first, real-time selection seems like an ideal approach for two reasons. 

First, the data is stored only once, namely in its original state. While this obviously saves storage, a more important benefit is that selected information is never stale. Given an atomic update of data, such as a transaction, a selection algorithm always computes solutions based on the most recent and consistent state of the data.
