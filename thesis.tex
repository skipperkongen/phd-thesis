...
\caption{Logical web mapping pipeline.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\vspace*{-4ex}
\end{figure}

Logically, most web mapping pipelines consist of the same high-level stages (see Figure~\ref{fig:introduction:requirements:pipeline}). It is important to study the tasks and requirements of each stage in the logical pipeline. This allows us to evaluate state-of-the-art pipelines against a hypothetical pipeline that meets all of the requirements. Stage one (data modeling) has been studied in previous work by Friis-Christensen~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. Stage three (graphical abstraction) has been studied in the seminal work by Bertin~\cite{bertin1973semiologie}. This thesis will focus on stage two (logical abstraction) and stage four (content distribution), since this is were the biggest challenges in web mapping are currently found.

\subsection{Logical Abstraction: Requirements}
\label{introduction:web:mapping:pipelines:logical:abstraction}
% describe
Homo Sapiens, the principal designer and audience of map visualizations, has probably remained cognitively unchanged for thousands of years. Even as the Web has impacted when, why and how maps are created and consumed, the basic ability of humans to comprehend maps has remained powerful, yet fundamentally limited~\cite{bertin1973semiologie}. Consequently, modern maps on the Web must be guided by the same well-studied principles as ancient maps on print.

In stage two of the previously defined web mapping pipeline, the primary data model is logically adapted for display on multi-scale maps. The outcome of this process is a multi-scale logical model of the information that should appear on the map, i.e. what should be dispayed rather than how it should be displayed. This process is known as \emph{model generalization} in the geographic literature~\cite{gruenreich1985cag}. 


\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.35]{figs-thesis/holistic-selection.pdf}
\caption{\textbf{Binary and space constraints}: binary constraints, such as minimum proximity (left image), set up conditions that must be met by all qualifying pairs of objects on a map. Space constraints, such as a maximum density (right image), set up conditions that must be met by all qualifying spaces in the map. In the figure, objects that violate a constraint are circled by red lines; objects that are consequently deleted are shown as white disks; objects that are kept are shown as black disks.}
...
Map constraints only distinguish feasible maps from infeasible maps. They do not by themselves quantify the relevance of maps. In contrast, \emph{map objectives} quantify the total utility of a map through a measurable \emph{objective function}. One type of map objective is to penalize violated map constraints, and minimize the sum of penalties. Another type of map objective is to score objects by their importance, and maximize the sum of scores, which is arguably the most intuitive approach (see Figure~\ref{fig:introduction:selection:constraints}).

\subsection{Logical Abstraction: State of the Art}

Current approaches to logical map design are characterized by two extremes: (1) automatic selection for simple maps, and (2) full-featured map generalization for complex maps. 

\minisec{Automatic Selection} The primary use case for automatic selection of geographic information is the rapid design of thematic point maps over databases of geocoded information~\cite{google2014maps}. Users are assumed to have no knowledge of advanced topics such as map generalization. Therefore, the constraints and objectives for selection are encapsulated inside system libraries that arbitrarily offer support for a certain constraint types. For example, systems may support automatically applied binary~\cite{nutanong2012multiresolution} or space constraints~\cite{sarma2012fusiontables}. Additionally, map objectives are unspecified, which means that at best an opaque ranking method is applied by the system.

% discuss
While automatic selection is highly time-efficient for map designers it also highly non-expressive. Map designers have no influence over the constraints and objectives that are applied during the selection process (see Section~\ref{sec:introduction:requirements:map:content}). This severely limits the control that map designers have over the design process, which is probably suboptimal for most real cases. Automatic selection frameworks obviously lack alternative map generalization operators, such as displacement and aggregation. However, this is arguably less of a problem, because selection is probably sufficient for most thematic point map designs. In summary, expressivity is entirely traded for productivity.

\minisec{Map Generalization} The primary use case for full-featured map generalization is the production of complex cartographic representations of the Earth, such as topographic and physical maps. When creating maps over diverse types of geographic information -- suchs as streets, river networks, mountain ranges, and urban constructions -- complex methods are warranted. % describe
...
\subsection{Content Distribution: State of the Art}
