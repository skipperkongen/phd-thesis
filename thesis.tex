...
Digital map visualization studies how to represent the geography of the Earth, as captured by geographical data, in highly restricted pixel spaces, such as computer screens. While geographical data are always spatial, they are otherwise very heterogenous. For example, points can represent the location of animals and hospitals~\cite{openstreetmap2010haiti}; lines can represent the network of waterways and streets; polygons can represent the areas of lakes and national territories. Furthermore, nothing prohibits a spatial interpretation of otherwise non-spatial data, such as news~\cite{shahaf2013informationcartography} and musicians~\cite{daniels2014vocabulary}. Every day, the amount of geographical data grows bigger and more detailed. For example, more than one hundred thousand new objects are added to the OpenStreetMap database each day~\cite{openstreetmap2014stats}.


% Maybe maps need no introduction.
Maps have been used for thousands of years and for good reason: they help us to reason about our planet and even the universe~\cite{eisenstein2011sdss}; they increase transparency and efficiency in our societies~\cite{offentlige:data:i:spil}; they help in urban planning~\cite{agrawal2012bigdata}; they help us locate places to visit~\cite{}; they help us navigate safely between origin and destination~\cite{}. On the web, the demand for maps is driven by an active eco-system of geographical data creators, who continue to produce a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap} datasets. Every day, the data grows bigger and more detailed. For example, more than one hundred thousand new objects are added to the OpenStreetMap database each day~\cite{openstreetmap2014stats}. Furthermore, geographical data consists of highly flexible geometric objects that span the full spectrum of human spatial interests. For example, points can represent the location of animals and hospitals~\cite{openstreetmap2010haiti}; lines can represent the network of waterways and streets; polygons can represent the areas of lakes and national territories.

% Maybe gear more towards mash-ups?

\section{Challenges in Map Visualization}
\label{introduction:challenges:in:map:visualization}

% Why GIS is inadequate
Traditionally, maps have been crafted by experts in Geographical Information Systems (GIS). However, a new situation exists on the web today that significantly marginalizes the use for traditional GIS. First, the demand for multi-scale maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS); therefore, map visualization systems must be \emph{expressive for non-experts} who wish to craft multi-scale map visualizations. Second, web maps that run on top of cloud infrastructures can now integrate huge datasets (billions of data items); therefore, map visualization systems must be \emph{scalable in the size of data}. Third, web maps today can quickly reach millions of users who interact with map visualization 24 x 7 on mobile devices; therefore, map visualization systems must be \emph{scalable in the number of concurrent users}. In short, GIS was not designed to satisfy these requirements, which merits the study of a new category of systems.

% Requirements for new system: MVMS
To address the challenges of GIS for the web, this dissertation introduces a new category of systems: Map Visualization Management System (MVMS). To begin answering what an MVMS should be, we must first investigate three crucial questions:

\begin{enumerate}
\item How do we intuitively express the creation of multi-scale map visualizations?
\item How can we scalably process millions of geographical records?
...
Answering each of these questions will require us to investigate areas beyond pure map visualization. For example, work in \emph{map generalization} addresses the problem of abstracting data for multiple scales~\cite{weibel1999generalising}. work in \emph{programming languages} addresses the issue of expressing data transformations~\cite{sql}; work in \emph{graphics} addresses the issue of visually representing data~\cite{wickham2009ggplot2, wilkinson2006grammar}; work in \emph{data management} addresses the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; work in \emph{distributed systems} addresses the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. It is crucial that we pull together the relevant knowledge from these diverse fields and combine it into systems for efficient and effective data visualization.

Some early, but incomplete, examples of MVMS include Google Fusion Tables~\cite{google2014fusiontables}, and CartoDB~\cite{cartodb2014cartodb}. \emph{move to related work.}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.45]{figs-thesis/europe-as-data.pdf}
\caption{\emph{Data vs Territory:} On a low scale map of Europe, 5 x 5 pixel data symbols for tourism points of interest (left map) fully cover the territory (right map). Thus, for large datasets (thousands of records) displaying unfiltered data symbols is non-sensical. First, a users can not discrimiate between interesting data and less interesting data. Second, crucial spatial context is lost when map labels are eclipsed by data symbols. Third, users can not accurately interact with the data symbols to get more information. For example, there are 28 thousand points of interest near Paris; how would a user click on the Eiffel Tower?}
\label{default}
\end{center}
%\vspace*{-4ex}
\end{figure}
