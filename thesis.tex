...
% The change: the web
Map visualization studies how the geography of the Earth can be displayed, most recently on computer screens and historically on pieces of paper. Since ancient times, the requirements for this process have been to purposefully and legibly represent as much as possible of the relevant information on a restricted medium~\cite{bertin1973semiologie,jones2005webage,weibel1997generalization}. This is an acute example of a big data challenge, since there is generally more geographic data available for visualization than what can be legibly represented on (say) a computer screen. Roger Tomlinson was one of the pioneers who kick-started the digital age of maps in the 1960's, with the introduction of geographic information systems (GIS). However, the rise of the Web has significant altered the stage for map visualization, which has created new challenges that can not be adequately addressed by traditional GIS alone. This thesis aims to study and close some important gaps between traditional GIS and requirements for map visualization on the web. Specifically, the thesis will study three orthogonal topics in map visualization: (1) User-friendly map design, (2) scalable data processing for maps, and (3) high-availability and high-scalability distribution of map data. The focus will be on non-aggregated thematic maps since they are currently the most challenging and simultaneously one of the most frequent maps that are created. 

\minisec{Thematic Mapping Applications on the Web} % Thematic maps are important
Thematic mapping applications on the Web are important since they allow map designers -- such as journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} -- to communicate relevant and focused spatial information to highly motivated target groups. Thematic maps increase the level of transparency and enlightenment in our society -- they can even save lives. % What are they exactly?
Thematic maps are typically composed of two distinct layers of information: First, the \emph{thematic map layer} consists of foreground data, such as news-worthy geographic events, which is the primary information provided by the map. Second, the \emph{contextual map layer} consists of background data, such as a topographic map, which provides that spatial context in which the former information may be understood. Such a composition of thematic and contextual layers is sometimes informally called a ``geographic mash-up'' (see Figure~\ref{fig:introduction:background:mashups}}). % Why is it hard?? 
...
\caption{\textbf{Multi-scale web mapping pipeline}: Typical web mapping pipelines transform geographic data between progressively more abstract and scale-dependent models of reality: First, reality is modelled as a set of discrete high-fidelity objects in a primary data model (1). Second, the primary data model is geometrically and semantically transformed into an multi-scale data model (2). Third, the multiscale data model is (optionally) visually transformed into a multi-scale map model (3). Fourth, multi-scale models are distributed over the Internet for display in mobile and web mapping clients (4). Fifth, when multi-scale data models are distributed to clients, they must be transformed into multi-scale map models client-side (5).}
...
A typical multi-scale web mapping pipeline has many moving parts (see Figure~\ref{fig:introduction:requirements:pipeline}). Mapping pipelines consists of \emph{components} that encapsulate geographic data in various states and \emph{pipes} that move and transform geographic data between the components. Left-to-right, the geographic information transitions from a set of high-fidelity objects stored in a database to a set of scale-dependent graphics displayed in a zoomable map interface. It is important to identify the requirements for each component and pipe in the system along with any human-computer interfaces, since this allows us to characterize any gaps that may exist between state-of-the-art mapping pipelines and a hypothetical mapping pipeline, which meets the all requirements of web-based mapping applications.
...
While selection of data for maps can be intuitively understood as filtering objects subject to some criteria, many different selection models have been proposed~\cite{sarma2012fusiontables,nutanong2012multiresolution,rocha2011efficient}. Critically, there is a lack of a unified selection model that can express all of them. What is needed is a model for holistic selection of geographic data that supports:
\begin{enumerate}
\item user-defined unary constraints (e.g. attribute filtering), object-relative constraints (e.g. minimum proximity), as well as space-relative constraints (e.g. maximum density) for multi-scale maps; 
\item scale-dependent and user-defined ranking of objects;
\item (optional) cross scale constraints, i.e. zoom-consistency.
\end{enumerate}
...
\item \textbf{Declarative Specification}: There are currently two extremes in how map designers select geographic data in state-of-the-art mapping systems. At one extreme, fully-automated selection systems encapsulate the logic of data selection inside an algorithm library~\cite{sarma2012fusiontables, nutanong2012multiresolution}. However, they bar programmers from any influence over the constraints and objectives that are enforced during selection, which is most likely a problem in any real-world map production scenario. At the other extreme, complex map generalization frameworks are powerful enough to produce even complex topographic maps. However, they require a high level of domain-specific expertise and are time-consuming to configure or program even for experts~\cite{lamy1999application, ware2003generalization_sa}. There is a critical lack of systems that offer programmers a balanced choice in-between the two extremes.

It makes sense that programmers can specify map constraints and objectives in a high-level geographic selection language, while leaving the minute selection of data to the system. In a recent vision, Meliou, Gatterbauer, and Suciu proposed a class of declarative how-to queries that let programmers focus on holistic constraints and objectives for a target database, which should be computed from a source database~\cite{meliou2011reverse}. The core idea is that programmers declaratively defines a space of feasible databases together with an objective function which can be used to compare databases. A combinatorial solver then computes an optimal database without further action by the user. This approach saves time, since programmers only need to focus on high-level goals which can be concisely expressed. Moreover, many declarative languages are highly optimizable since computational frameworks can be swapped out in the background~\cite{eldawy2014pigeon,thusoo2009hive}, for example by targeting a language with runs on emerging parallel in-memory systems~\cite{zaharia2010spark}. Ideally, a declarative language for geographic how-to queries could be generalized to support how-to queries for other types of data visualizations as well, i.e. data visualization that require holistic selection of data.

% IN-DATABASE PROCESSING
\item \textbf{In-database Processing}: The web offers a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap} datasets that can be visualized on maps. Typically, data for visualizations is stored in a relational database~\cite{wu2014case}. Each day, millions of new objects are added to the OpenStreetMap database alone, which already contains billions of objects~\cite{openstreetmap2014stats}. Moreover, as mobile devices increasingly bond millions of people to location-based services online, such as dating~\cite{tinder2014tinder}, transportation~\cite{drivr2014drivr} and shopping~\cite{}, geographic information can be produced by anybody, anytime, and anywhere. Therefore,  a mapping systems must be able to scalably process multi-scale selection over large relational datasets (millions of records). 

One approach is to translate a specification for selection into low-level code (e.g. LLVM, C++, OpenMP, or MPI), which can potentially result in very efficient execution. However, low-level code is harder to optimize across platforms. A better approach is to target a high-level language such as SQL, which reuses optimization techniques that are built-in to SQL query engines~\cite{boncz2005pathfinder, boncz2006monetdb, grust2009ferry}. Additionally, since data for visualizations is typically stored and managed in relational database management systems (RDBMS)~\cite{wu2014case} it makes sense to process the data where it is stored. Despite the obvious opportunity, state-of-the-art map visualization systems do not significantly leverage RDBMS technology to process multi-scale selection of geographic information, other than to extract data for post-processing using simple bounding-box and attribute filtering queries~\cite{wms, }.
