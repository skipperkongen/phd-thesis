...
\subsection{Analysis of Real-time Mapping Systems}
...
While tree-based real-time mapping seemingly ensures low-latency data abstraction, there are two key disadvantages when compared to precomputed content that is served from a key-value stores or content delivery network (CDN)~\cite{dilley2002cdn}: 
...
\begin{itemize} 
\item Since there is no real-time requirement, more computational effort can be spent on producing a high-quality result. Furthermore, distributed data analytical systems, such as spatial parallel databases~\cite{waas2009beyond} and spatial cluster computing~\cite{eldawy2014spatialhadoop,aji2013hadoopgis} can process practically unlimited amounts of spatial data.
\item Precomputed maps can be partioned and replicated onto CDNs, which scale extremely well with concurrent queries.
\item Maps that are distributed via a CDN additionally offer high availability and low latency because content is georeplicated to locations near the users.  
\end{itemize}

Some important studies in batch-oriented mapping systems are multi-scale trees~\cite{frank1994multiscaletree,van2011towards} and the Fusion Tables system~\cite{sarma2012fusiontables}. Supporting work are studies in scalable batch-oriented spatial analytics, such as SpatialHadoop~\cite{eldawy2014spatialhadoop} and HadoopGIS~\cite{aji2013hadoopgis}. Related work for map distribution is in-memory and fault-tolerant stores, such as RAMCloud~\cite{ousterhout2011ramcloud}. These stores offer extremely low latency, without sacrificing availability.
...
While real-time systems present an ``ideal'' for mapping systems (see the advantages above), state-of-the-art real-time mapping systems are inappropriate for large-scale web maps. First, these systems have relatively low availability and high latency compared to precomputed content that is served from a CDN. Second, they do not cost-effectively scale with the number of concurrent users. These problems are critical in a web context.

Dispite the drawbacks, state-of-the-art batch-oriented mapping systems are appropriate for large-scale web maps. On the hand, the ability to scale in concurrent requests is simply a make-or-break requirement for any large-scale web map today. Furthermore, the ability to process unlimited amounts of geographic information is often crucial. On the other hand, the disadvantages are not too severe. First, disk-based storage has become very cheap and pure in-memory storage is bound to become cheaper over time. Second, materialized view can be refreshed as demand dictates, either by leveraging batch-oriented parallel processing~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} or by employing techniques for materialized view maintenance~\cite{gupta1995maintenance}. Finally, user preferences can to some extent be accommodated by materializing views for different user profiles and choose between them at query time. However, this last problem is probably the most fundamental problem of batch-oriented systems.
