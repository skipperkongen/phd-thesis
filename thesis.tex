...
For thousands of years, humans have used their visual senses to comprehend geographic information on maps. Humans have learned over time that a legible map enables instant comprehension of relevant geographic information, while a cluttered map inhibits such comprehension. Over time, the reasons that maps are created and the context they are created in have constantly evolved. Accordingly, novel methods for drawing maps have been invented, studied, and sometimes discarded. However, there exists no evidence to suggest that the human capacity for visual perception and comprehension has evolved significantly since the stone age. Consequently, modern map makers must accomodate the same cognitive limitations in humans as ancient map makers.
...
Map visualization studies the process by which geographic information is displayed on a three-dimensional medium (x, y, color), most recently on computer screens and historically on pieces of paper or even clay tablets~\cite{friendly2008milestones,robinson1982early}. Since ancient times, the requirements for this process have been to \emph{accurately} and \emph{legibly} represent as much as possible of the \emph{relevant} information on a highly restricted medium~\cite{jones2005webage}. It is a challenging process, since opposing goals must balanced in order to achieve an optimal map. Frequently, there will be too much information to be visualized compared to the representational budget of the highly restricted map medium. If you need convincing, try to pick up a pencil and draw a full world map on a sheet of A4 paper. Almost immidiately, you will need to select which cities, islands, lakes, etc. to include, and which to leave out. Having made the hard choices of what to include, next you will need to decide how to graphically render the information you have chosen to include. A daunting task to be sure.

\minisec{Geographic Information}
Geographic information is typically represented by discrete and multi-dimensional \emph{geographic objects}. For example, geographic objects can represent roads, buildings, lakes, locations, restaurants and much more. Each object associates a \emph{geometry} (point, line or area), with a \emph{set of attributes} of qualitative, ordered and/or quantitative types~\cite{bertin1967semiologie}: \emph{qualitative attributes}, such as the cousine of a restaurant, have no inherent order. For example, Italian is neither more nor less than Japanese; \emph{ordered attributes}, such as price category, allows a ranking. For example, mid-range restaurants are more expensive than budget restaurants but cheaper than gourmet restaurants; \emph{quantitative attributes}, such as average rating (on a 0-5 scale), allow aritmetic to be performed on their values.
...
A basic challenge in map design, known as map generalization~\cite{shea1989cartographic,weibel1997generalization}, is how to alter the representation of geographic information in response to a reduction in scale, such that the graphic representation of geographic information remains legible and useful. 



Logical abstraction is the process that maps a set of information objects along with their individual components to another set of information objects. For example, information objects can be selected or aggregated, and component values can be modified. In constrast, graphical abstractions maps information objects to graphic symbols, primarily by using visual variables, such as \emph{color}, \emph{size}, or \emph{value}, to visually express the qualitative, ordered or quantitative components of an information object~\cite{bertin1967semiologie}. For example, symbol color can be used to distinguish between restaurants with different cousines, such as italian, japanese and hungarian restaurants, which expresses a qualitative component. Similarly, symbol size can be used to distinguish between cities of different size, such as villages, towns, and metropoles, which expresses a quantitative component.
...
\subsection{Requirements of Logical Abstraction} 
...
\subsection{Requirements of Content Distribution} 
...
State-of-the-Art mapping pipelines are distinquished, among other things, by \emph{when} selection of data is performed relative to when a request for map data is recieved, either \emph{real-time} or \emph{in advance}. Moreover, mapping pipelines are distinguished by the type of data that they send to mapping clients, which is either \emph{graphics} or \emph{information}, which can be turned into graphics by the client.

\subsection{When to Perform Selection}
...
\subsection{How to Distribute Maps Using Tiles}
