...
Geospatial data visualization studies how to usefully represent the complex realities of Earth on highly restricted graphical mediums, such as the tiny screens on mobile devices. A geographical map is an important type of geospatial data visualization that has been used for thousands of years. Indeed, maps have a clear value proposition: they help us to reason about our planet and even the universe~\cite{eisenstein2011sdss}; they increase transparency and efficiency in our societies~\cite{offentlige:data:i:spil}; they help in urban planning~\cite{agrawal2012bigdata}; they help us locate places to visit~\cite{}; they help us find our way~\cite{}. The demand for maps is driven by an active eco-system of geographical data creators, which has produced a huge diversity of curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap} datasets. Every day, the data grows bigger and more detailed. For example, more than one hundred thousand new objects are added to the OpenStreetMap database each day~\cite{openstreetmap2014stats}. Geospatial data is highly flexible; it includes points such as the location of gas stations and hospitals~\cite{openstreetmap2010haiti}; lines such as the network of streets and waterways; polygons such as the areas of lakes and countries.

Traditionally, maps have been created by experts using Geographical Information Systems (GIS). However, maps are increasingly developed for the web and increasingly leverage the power of cloud computing. This has created a new situation that significantly marginalizes the need for traditional GIS. There are three consequences of the web and cloud computing that traditional GIS was not developed for. First, web maps can quickly reach millions of mobile users who will engage maps 24 x 7. Second, maps running on cloud infrastructure can easily integrate huge datasets (millions of data items). Third, the demand for maps visualizations on the web is rising faster than the number of experts who know how to create them. This leads to a set of requirements for a map visualization system that is specifically aimed at the web and cloud computing:

This dissertation specifies the requirements for a systems that can address the short-comings of traditional GIS.



Therefore, web map systems must scale in the number of concurrent users; they must be highly available; they must have acceptable latency.  For this reason, map systems must scale with data volume. that are hard to process and visually represent.


For this reason, non-expert users must be able to rapidly craft compelling map visualizations.

However, these systems are incomplete.
...
Consequently, there is an increasing demand for techniques that online map visualizations that package all this geographical information into legible, interactive, and web-accessible interfaces~\cite{mapref}. The need for online maps creates a demand for more effective and efficient methods to produce and serve maps. 


Maps abstract and combine multiple layers of geospatial data, such as points, lines and polygons. 
...
\section{Challenges in Map Visualization}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=.5]{figs-thesis/europe-as-data.pdf}
\caption{On a low scale map of Europe, 5 x 5 pixel symbols for tourism points of interest (left map) fully cover the physical landmass (right map). Thus, displaying unfiltered points of interest is non-sensical. First, a users can not discrimiate between interesting sites and less interesting sites. Second, crucial spatial context is lost, such as city names on a background map. Third, users can not accurately interact with the points of interest to get more information. For example, there are roughly 28 thousand points of interest near Paris. Given this fact, how should a user click exactly on the Eiffel Tower on the left map?}
\label{default}
\end{center}
%\vspace*{-4ex}
\end{figure}
...
Currently, the infrastructure to support is 

Geographical Information Systems (GIS) have traditionally been used to manage and visualize geographical data. However, these systems are  , Data Visualization Management Systems (DVMS) have been proposed as a catch-all solution for data-driven visualizations on the web today~\cite{wu2014case, fekete2012dmvis}. The API of a fully-fledged DVMS must included facilities for both data management, visualization design, and visualization sharing~\cite{foo}. A DVMS must be scalable both in the size of data~\cite{lins2013nanocubes, Im2013visreduce, battle2013scalar, liu2013immens} and audience~\cite{viegas2007manyeyes}. Additionally, it is crucial that designers can craft visualizations using an interface that is both intuitive, efficient (in terms of wall-clock time) and expressive. In the geographical domain, prototypes of such systems include Google Fusion Tables~\cite{google2014fusiontables}, and CartoDB~\cite{cartodb2014cartodb}. To make the distinction between spatial and  clear, this dissertation introduces Geospatial Visualization Management Systems (GVMS) .
...
\emph{What are the key questions?} To reason about a data visualization management systems, three crucial and orthogonal questions must be investigated.
\begin{enumerate}
\item How do we intuitively \emph{express} the task of transforming (possibly non-geometric) data into useful visual representations?
\item How can we scalably \emph{process} the millions of records that are often the foundation of a data-driven visualization?
\item How can we scalably \emph{serve} data-driven visualizations to millions of online users, who expect 24 x 7 availability and acceptable latency?
\end{enumerate}

Answering each of these questions will require us to investigate areas beyond pure data visualization. For example, \emph{programming languages} have addressed the issue of expressing data transformations~\cite{sql}; \emph{graphics} have addressed the issue of visually representing data~\cite{wickham2009ggplot2, wilkinson2006grammar}; \emph{databases} have addressed the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; \emph{distributed systems} have addressed the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. It is crucial that we pull together the relevant knowledge from these diverse fields and combine it into systems for efficient and effective data visualization.
...
\note{Compare against tableau~\cite{wesley2011tableau}, Ermac~\cite{wu2014case}. }

\section{Definitions}

\note{Maybe this is a good section, but it should not be in the related work}

In general, data visualization is concerned with the creation and study of (interactive) visual representations of data to reinforce human cognition~\cite{friendly2008milestones}. While data can be both numerical and non-numerical, this thesis will focus on data that is stored in tables, e.g. rows in an RDBMS. At first, this may seem constricted, however tables are versatile and make up the bulk of all data that is visualized today~\cite{wu2014case}. Defined in this way, the distinction between information visualization and scientific visualization (see Munzner~\cite{munzner2008pitfalls}) becomes irrelevant. For example, a table may contain geographical objects where the spatial representation is given, or contain economical data where the spatial representation is chosen. In any event, the tables are assumed to contain rows that capture some aspect of ``reality'' -- ideally with a high level of granularity. For example, a table may contain roads with their high-precision geographical geometries~\cite{openstreetmap}, or the minute details of a federal budget~\cite{nytimes2012obamabudget}. Without loss of generality, the tables are assumed to have a fixed schema.

\section{Data Visualization Management Systems}

Ermac (don't know the short-comings), Scalar (don't know the short-comings), Fusion Tables (scalable data processing, scalable data serving, intuitive visual abstraction interface, however non-configurable data abstraction interface).
...
Why is it not good enough? Hard to write and customize.
