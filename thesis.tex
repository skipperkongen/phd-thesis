...
Digital map visualization is the process by which map designers represent the geography of Earth in the finite 3-D pixel matrix on a computer screen. The three dimensions of an image (x, y, color) make the visual perception our most powerful perceptive system. But images have only three dimensions and the consequences of this limit are important~\cite{bertin1973semiologie}. Indeed2, it is fundamentally challenging to represent something as enormous as planet Earth on something as small as a computer screen~\cite{dillard1974pilgrim}.
...
Web and mobile mapping applications typically combine emerging thematic foreground data with the spatial context of existing background map~\cite{bereuter2012algorithms}, which is informally known as a ``geographic mash-up'' (see Figure~\ref{fig:introduction:background:mashups}}). While existing background maps are generally of high quality, the result of many hours of expert labor, large thematic datasets presents a raw challenge for the would-be designer of a mash-up. Computer screens can legibly display just a few thousand distinct symbols, thus, if more data is intersected by the viewport it will have to be abstracted to remain legible.
% funnel from here.
% - Maps are really cool
% - Requirements for web and mobile maps: PoCID, iOS6
% - only maps of large data need information abstraction, whether area partitions (haunert, gap) or points (pia, da sarma).
% - in typical map application no need for complex information abstraction, since the hard maps (background maps) already exist.
% - On-the-fly methods do not cost-effectively scale to massive audiences
% - scalable batch-oriented data abstraction methods, which sacrifice consistency for query time performance, are the only approach that will cost-effectively scale to massive audiences (e.g. vector tiles); mention content delivery networks, data partionining and replication.
% - however, current batch-oriented methods either too automatic or too complex for map designers.
% - new paragraph:
% - from user perspective, only data that is requested needs to be consistent
% - prediction would enable us to batch-process only the data that is needed, which would decrease the window of inconsistency and would be cheaper as well.
