...
\item \textbf{Mathematical Selection Framework} 

OK, saying that there has not been mathematical modelling before is a false claim. Figure out what this problem is about and why it has not been addressed before. Then describe it here.
...
\item \textbf{Declarative Specification}: There are currently two extremes in how state-of-the-art systems let users influcence the selection of data for multi-scale maps. At one extreme, fully-automated systems completely encapsulate the logic of selection inside an algorithm library~\cite{sarma2012fusiontables}. Such systems exclude the map designer from any influence over the data selection process, including how record importance should be determined and which constraints should be enforced. At the other extreme, highly customizable systems require map designers to minutely express the selection of data through a (potentially large) set of simple predicates over record attributes~\cite{sld,mapnik}. It is very time-consuming, if not outright infeasible, to express a holistic multi-scale selection by a set of simple predicates that are evaluated record-by-record without considering the context or other records.

Ideally, designers should specify just the high-level constraints and objectives for a map in a versatile and concise specification language, while leaving the minute selection of data to the system. A recent vision by Meliou, Gatterbauer and Suciu makes the case for declarative \emph{how-to} queries. The core idea is that users specify a set of constraints and an objective for a target database that should be computed from a source database (they gave an example of optimizing a stock portfolio). While how-to queries fit nicely with the requirements for holistic selection, namely, given a geographical dataset, compute a multi-scale selection that satisfies a set of constraints and objectives, they have not been considered in a multi-scale context.
