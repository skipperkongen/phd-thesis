...
The previous chapters have described the systems -- Glossy, CVL, and Tileheat -- that has been developed during this PhD project. However, many potentials of these systems have not been fully explored or developed during the project due to time and resource limitations. Consequently, the Glossy, CVL lines of work, as well as the Tileheat line of work are limited in various ways. The sections below discuss these limitations and also indicate some interesting future directions for work.

\section{Limitations of Glossy and CVL}
The Glossy and CVL systems have implemented holistic selection of tabular data for multi-scale data visualizations. The systems harness the PostgreSQL database engine to process queries by compiling queries into SQL in the PostgreSQL dialect. While we have argued that Glossy and CVL are novel and highly useful systems, the choices that were made during the developement of these systems mean that they are limited in some important ways. Some of these limitations are covered below.
...
\section{Conclusion}
The thesis has studied on two subjects within multi-scale map and data visualization, with an emphasis on batch-oriented systems for map creation and map serving. The overall aim was strengthen the state of the art of in map visualization technology for a massively online, connected and data sharing population of users world-wide. Specifically, the design goals have been to contribute a systems for map visualization that can:

\begin{itemize}
\item Process large amounts of data (millions of objects) for map and data visualizations
\item Allow relatively novice designers or programmers to time-efficiently specify and compute information selection of complex, multi-scale data visualizations (minutes or hours end-to-end)
\item Support high availability and low latency serving of 
\item Scale to large numbers of concurrent consumers (millions).
\end{itemize}
