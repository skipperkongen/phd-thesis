...
Given the availability of so much interesting data, more and more organizations such as news agencies~\cite{nytimes2010iraq,guardian2013ananas} are motivated to communicate spatial narratives using online maps. As a consequence, more and more professionals will need to acquire the skills to craft map visualizations for the web. However, demand for maps on the web is rising faster than the number of GIS experts who know how to craft them (using traditional GIS). Maps can be characterized by two extremes. At one extreme, choropleth maps present a highly aggregegated view of information~\cite{guardian2013ananas}. At the other extreme, point maps present an highly immersive and detailed view of information~\cite{nytimes2010iraq}. While both are valuable, the former maps are fairly easy to produce using statistics. In contrast, the latter maps present a fundamental challenge to map designers due to the need for map generalization.


While tools have been developed that allow moderately experienced map designers to craft beautiful maps, they are either too automatic~\cite{sarma2012fusiontables} or very time consuming to use~\cite{mapnik, sld}. % hmm, need to have introduced selection at this point...
