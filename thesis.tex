...
Glossy and CVL are systems for multi-scale abstraction of data for map and data visualizations. The focus was on holistic selection, which other important abstraction operators, such as aggregation, were ignored. The systems were built on-top-of the PostgreSQL database engine. While this is a mature transactional database system, especially for spatial data, it it lacks the scalability of newer parallel databases and cluster computing systems. Thus, the scalability results for Glossy and CVL only hint at the true scalability of these approaches. Finally, the work with the systems was focused on algorithmic contributions, language semantics, and performance evaluation. Therefore, the usability of the Glossy SQL and CVL languages has only been informally evaluated.

\minisec{Aggregation and Projection}
The Glossy and CVL systems supports holistic selection of data for multiple scales, which is an important map generalization operator. However, other important operators are not supported by Glossy and CVL, such as holistic aggregation and holistic projection. There are several reasons for this, which can be thought of algorithmically as well as syntactically.

Algorithmically, selection in Glossy and CVL depends on a solver for the set multi-cover problem (SMCP), since the holistic selection problem corresponds to this problem. However, the project has not been able to establish that the same relationship exists between holistic aggregation, holistic projection and SMCP. One approach could be to bridge models for holistic aggregation and holistic projection with SMCP by means of a problem mapping. For example, Wollf and Launert have developed a modeling for land-cover aggregation, which is based on a coloring problem of an adjencency graph~\cite{haunert2006landcover}. If an efficient mapping from the coloring problem to SMCP was developed, any algorithm for SMCP could be used to solve holistic aggregation. A similar strategy could be used for mapping models for holistic projection to SMCP. However, it is not clear how well such an approach would work in practice. 

Syntactically, the Glossy SQL and CVL languages were developed for selection. While the project has attempted several times to develop a good syntax for specification of holistic aggregation and projection, these efforts were not successful. Perhaps the problem is structurally rooted. In selection there are only two outcomes for an object: it is either included or it is excluded. In contrast, for holistic aggregation and holistic projection there are sevaral possible outcomes for an object. For example, in land-cover aggregation an object can be merged with several of its neighbors, while in displacement an object can be displaced to several or even infinitely many positions. The syntax of Glossy SQL and CVL allow a user-defined object weight to be given, which is enough to prioritized selection. However, for land-cover aggregation a distance function must also be given. Thus, the syntax of Glossy SQL and CVL would likely have to be extended with more clauses.

\minisec{Graphs and Semi-structured Data}
The Glossy and CVL systems process tabular data for visualizations. However, tabular data is not the only type of data that can be visualized at multiple scales. Interesting data types include graphs, semi-structured, and unstructured data. There are many similarities between geographic maps and other types of visualization. For example, the effects of scale-reduction -- such as density, proximity and size change -- also apply to graph visualization.

Graphs model many important phenomena, such as social networks. For example, a vast social networks $G$ could meaningfully be visualized at multiple scales by showing an appropriately layed-out graph minor of $G$ at each scale. The operations that produce a valid graph minor from an input graph include selection (nodes and edges) as well as aggregation (edge collapse). In this model, small scale visualization levels could focus on community patterns and gradually transition to individual people toward larger scale visualization levels. Similar to recursive selection in Glossy SQL, the graph minor property could be recursive, such that the graph shown at scale $i$ was required be a graph minor of any graph shown at scale $j < i$. Similar to Glossy SQL, nodes in the social could be weighted, e.g. based on pagerank~\cite{brin1998anatomy}, which could be used to prioritize node selection and edge collapse.

Semi-structured data, such as XML and JSON, is used in many contexts on the Web. For example, web services typically return data as either XML or JSON. In general, the structure of XML and JSON is nested data and may optionally have a schema. However, semistructured can be transformed sinto tabular form and processed by Glossy or CVL, e.g. by writting a view. This would be a data integration approach~\cite{halevy2006data}. Another approach would be to rewrite the multi-scale abstraction semantics for semi-structured data, e.g. by including some form of aggregation. However, this project has not attempted such a direction.
