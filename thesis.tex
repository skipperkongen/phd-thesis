...
\todo{Wait to the contributions to narrow these down to the things I looked at.}

\emph{What is the research context and the area of focus for this thesis?}

\subsection{Data Abstraction Operators} Introduce the data model for mult-scale data abstraction: tables. Introduce the two types of multi-scale: ladder and star. Introduce constraints. Introduce objective functions. Introduce the operators: global selection, global projection, and global aggregation. Combine into full problem: ...

\subsection{Programming Languages} talk about the need for a concise, declarative language, and the elements it should contain, i.e. express the API.

\subsection{RDBMS Integration} Data abstraction queries are translated into efficient queries in relational algebra.

\subsection{Adaptive Data Serving} Using a log of previous requests, predict the requests that will be seen tomorrow. Account for trends, seasonal variation and toblers first law.
