...
% for degree symbol
\usepackage{siunitx}
% for tighter itemize environments
\usepackage{enumitem}
\setlist{nosep}
...
% for boxes
\usepackage{mdframed}
\newmdenv[topline=false,bottomline=false,rightline=false,linewidth=3]{emphpar}
...
\usepackage[top=1.2in,bottom=1.2in,left=1.30in,right=1.30in]{geometry}
\usepackage{graphicx} 
\usepackage{ifpdf}
\usepackage{listings}
\usepackage{multirow}
...
\sloppy
\title{Declarative and Scalable Selection for Map Visualizations}

%\title{Declarative Object Selection for Scalable Map Visualizations}
%\title{Declarative Cartography for Scalable Geographic Data Visualizations}
%\title{Declarative Cartography for Scalable Map Visualizations}
%\title{Scalable Selection for Geographic Data Visualizations}
%\title{Declarative Selection for Online Geographic Data Visualizations}
%\title{Declarative and Scalable Web Mapping Systems}
%\title{Declarative Web Mapping}
%\title{Declarative Cartography for Scalable Web Mapping Pipelines}
...
\date{\parbox{0.85\linewidth}{\centering%
  \today\endgraf\bigskip\bigskip
  This thesis has been submitted to the PhD School of The Faculty of Science, University of Copenhagen\endgraf\bigskip\bigskip
  This thesis was supervised by:\endgraf\bigskip
  Prof. Martin Zachariasen \hspace*{1cm} Dr. Marcos Vaz Salles\endgraf\bigskip
  Dept.\ of Computer Science \endgraf
  University of Copenhagen}}              % Activate to display a given date or no date
...
The talented employees at Grontmij A/S and the Danish Geodata Agency deserve a special thanks for frequent, useful, and often wonderfully pragmatic feedback on my work. Hans Skov-Petersen and Bernhard Snizek must be thanked for kick starting my interest in the field of geographic data science, when I worked as a research assistant in Skov \& Landskab.

As a commemoration, I would like to thank the late S{\o}ren Aundal, a primary school teacher who showed me how to train a pair of binoculars at the sky, where we would watch birds, stars and planets together, when I was a kid. He perhaps sparked my enduring curiosity about the natural world, which in turn matured into a curiosity about the other world of computer science.
...
\abstract{Geografiske kort er en af de ældste og vigtigste former for datavisualisering. Kartografi, som er læren om kortet som informationsbærer og medium, er blevet studeret i årtusinder. Efterhånden har vi opnået en nærmest komplet forståelse af alle delprocesser der indgår under skabelsen og brugen af et kort -- herunder generalisering af geografiske data. Dog har fremkomsten af internettet skabt et behov for at opdatere vores viden på dette vigtige område, da grundlaget for at skabe kort i denne nye virkelighed er blevet udfordret. Nogle vigtige tendenser og udfordringer er: (1) Offentligt tilgængelige baggrundskort er blevet et forbrugsgode, der udbydes af mange og som let kan integreres i en hvilken som helst kort-applikation. Derfor er der ikke længere det samme behov for at oprette og dele baggrundskort på Internetet. I modsætning til baggrundskort, er tematiske data noget der dagligt volder kort-designerere problemer. Derfor er der et behov for særligt at studere brugen af sådanne data, herunder deres generalisering. (2) Internettet har demokratiseret hele data visualiseringsprocessen, hvilket betyder at det ikke længere er forbeholdt eksperter at designe et landkort. Hvor det før var kartografer og GIS-specialister som var de primære designere af kort, ser vi nu at alle dele af samfundet bidrager med kort, såsom journalister, nødhjælpsarbejdere, web-designere, videnskabsfolk, og såkaldte data-entusiaster. Derfor er der et øget behov for at tilbyde disse brugergrupper nogle gode værktøjer til kort-design, som ikke er forbeholdt eksperter. (3) Internettet er en kilde til meget store mængder tematiske data, som med fordel kan fremvises på et kort, men traditionelle kort-værktøjer er ikke designet til at håndtere meget store data mængder. F.eks. kan traditionel GIS ikke udnytte den kapacitet, der forefindes i klyngecomputersystemer, såsom Hadoop og parallelle databaser. Derfor er der et behov for at integere nye værktøjer til kort-design med moderne klyngearkitekturer og systemer. (4) Kort bliver i dag tilgået døgnet rundt af mange brugere samtidigt. Derfor er det nødvendigt med kort-tjenester, som skalerer i antallet af samtidige brugere. Dette er blevet erfaret flere gange i Danmark under offentligegørelse af kort, som af den ene eller anden grund tiltrak sig offentlighedens interesse. Diverse tiltag til kort-tjenester, såsom Web Map Service og Web Feature Service, kan ikke i tilstrækkelig grad håndtere mange samtidige brugere. Derfor er der fremkommet mere skalérbare tjenestetyper, såsom Web Map Tile Service, som dog ikke er tilstrækkelige til at publicere et kort. For eksempel afhænger disse kort-tjenester af eksterne services til at generalisere og rendere geografiske data til brug på et kort. Selvom sådanne services eksisterer, er omkostningen ganske betydelig til at rendere et fler-skala kort i alle skalaer og udsnit. Da det samtidigt er langt fra alle dele af et fler-skala kort som bliver læst i praksis, er der derfor et potentiale for selektivt at processere de dele af et kort som vil blive læst i fremtiden. Dette skaber et behov for at forudsige hvordan et kort vil blive brugt, f.eks. baseret på hvordan lignende kort er blevet brugt før. \\

Denne afhandling bidrager med ny teknologi, som kan bruges til at imødegå nogle af de udfordringer som internettet har skabt design og brug af geografiske kort, i særdeleshed på nettet: (a) To nye deklarative sprog til kartografisk udvælgelse af data til et kort -- Glossy SQL og dets prototype Cartographic Visualization Language (CVL) -- er blevet udviklet og evalueret. Disse sprog tillader designeren af et kort at specificere holistiske mål for udvælgelse af data til et fler-skala geografisk kort. Metoden understøtter to typer af vigtige og meget dækkende bruger-definerede begrænsninger i.f.m. udvælgelsen. Disse begrænsninger bliver automatisk tilfredsstillet af systemet under kørsel. Derudover understøttes en bruger-defineret målfunktion for den kartografiske udvælgelse i form af brugerdefinerede objekt-vægte, som vil blive forsøgt maksimeret i.f.t. de udvalgte elementer på kortet. F.eks. kan restauranter udvælges til et kort både som funktion af deres egen vigtighed og som funktion af andre tætvedliggende restauranters vigtighed. (b) En oversætter fra Glossy SQL og CVL er blevet udviklet og evalueret. Oversætterens målsprog er SQL i PostgreSQL dialekten. Dette betyder at Glossy SQL og CVL -- og derved hele udvælgesesprocessen -- kan afvikles i PostgreSQL. Dette er en moderne relationel database, som ofte benyttes til opbevaring og analyse af geografiske data. Dermed undgås behovet for at eksportere data i forbindelse med udvælgelsesbaseret generalisering. (c) TileHeat er et rammesystem for forudsigelse af de dele af et kort, som vil blive forespurgt i fremtiden. Systemet er baseret på eksponentiel udglatning og en ny dissipationsteknik, således at forudsigelser af trafisk kan baseres på en relativt lille tilfældig udvælgelse af kort-forespørgsler.\\

Resultaterne viser at Glossy SQL og CVL kan beregne en kartografisk udvælgelse af data ved at afvikle en kompleks forespørgsel i en relationel database. Det er blevet påvist at denne metode er skalerbare optil en halv million objekter i databasen. Desuden tyder resultaterne på at metoden er skalerbar for databaser med flere millioner objekter, i særdeleshed hvis målsproget for oversætteren udskiftes med en variant af SQL, som understøttes af et klyngeberegningssystem (f.eks. såsom Hadoop). Mens CVL er blevet afprøvet i.f.t. flere use cases for geografiske kort, vises det at Glossy SQL derudover kan benyttes til udvælgelse af data til ikke-geografiske datavisualiseringer, såsom tidslinjer. Endeligt viser resultaterne for Glossy SQL og CVL at kartografisk udvælgelse kan udtrykkes meget præcist og kortfattet i disse deklarative sprog. Resultaterne for Tileheat viser en væsentlig forbedring af forudsigelsespræcisionen for korttrafik, i sammenligning med den metode, der p.t. benyttes af Geodatastyrelsen i Danmark. Dette betyder at en væsentlig mængde beregninger ville kunne spares i denne offentlige styrelse, som er ansvarlig for distribution af kortdata til den danske offentlighed.}
...
%``To affect thro' the eyes what we fail to convey to the public through their word-proof ears'' -- Florence Nightingale, mid-19th century
\end{emphpar}

For thousands of years, humans have visualized geographic information on maps, most recently on computer screens and historically on pieces of paper and clay tablets~\cite{friendly2008milestones,robinson1982early}. During those years, maps have helped humans comprehend a multitude of spatial phenomena, from natural bodies such as oceans, glaciers, and mountains to political artefacts such as borders, demographics and land ownership. An example of the latter is a four-thousand-year-old clay tablet that was discovered near present-day Kirkuk, Iraq in 1930. It shows the boundaries of a land parcel with the owner's name, ``Azala''. Given its age, this cadastral map is probably among the earliest maps in human history. 

As the human capacity to gather data has increased, the creation of maps has become more challenging; finite map mediums can only accommodate a finite number of legible symbols. This challenge has lead to a plethora of cartographic information abstraction techniques that are collectively known as \emph{map generalization}~\cite{brassel1988generalization,mcmaster1992generalization,weibel1999generalising}. The purpose of map generalization is to create legible, accurate, and ultimately useful maps for any map scale and for any amount of underlying geographic data. Naturally, this process involves a compromise between readability and precision for most real-world cases. For example, maps of the physical world often select a subset of the available information for display, as otherwise the map would become cluttered and illegible. Similarly, demographic maps often aggregate information about individuals by area, since this can clarify important trends in local populations. Finally, small-scale maps often distort the information in order to make it more legible. For example, symbols may be enlarged and displaced in order to make them easier to perceive or distinguish from each other. Thus, the focus of maps is on the correct and relevant comprehension of spatial phenomena, rather than on a precise and scaled-down reproduction of reality.

Over time, technology and map making has evolved hand-in-hand. A recent example is cloud computing, which has made it easier to share and process geographic information on maps. However, the visual perception of Homo Sapiens -- the principal user of maps -- has probably remained unchanged since the Neolithic period. Thus, despite the tremendous advances in technology, modern map makers must accommodate the same biological limitations of the visual perception in humans as ancient map makers.  

In a sense, the map making challenge is greater today than ever. While the volume of geographic data keeps growing exponentially, the map medium itself is shrinking. Furthermore, the demand for globally-distrubuted, highly-available information with low-latency access is a characteristic of a web-focused world where digital information is considered an online commodity.

\subsection{Modeling Geographic Information}
\label{sec:modeling:geographic:information}
Maps visually communicate geographic information using a set of symbols, which are graphical representations of logical information objects. These two information models are discussed below.

\minisec{Logical Model}
Logically, the information on a map can be modeled as a set of \emph{geographic objects}, which associate data with a certain location. Each object models a distinct geographic concept in the real world, such as a road, lake, mountain peak, or country, by a finite set of \emph{attributes}. Each attribute of an object can be categorized as either \emph{geometric}, \emph{qualitative}, \emph{ordered} or \emph{quantitative}~\cite{bertin1967semiologie}. For example, the restaurant ``Noma'' in Copenhagen can be represented by an object with four attributes $\langle$ (\ang{55.6778}, \ang{12.5962}), ``Nordic Cuisine'', ``Best Restaurant 2014'',  DKK 1,600  $\rangle$:

\begin{itemize}
\item $(\ang{55.6778}, \ang{12.5962})$ is a geometric attribute of the Noma object, which corresponds to the geographic position of Noma on the harbor front in Copenhagen, Denmark.
\item ``Nordic Cuisine'' is a qualitative attribute of the Noma object, which distinguishes Noma from restaurants with a ``Japanese Cuisine''.
\item ``Best Restaurant 2014'' is an ordered attribute of the Noma object, which enables it to be ranked against other ordered attributes of the same hierarchy (restaurant ranks).
\item DKK 1,600 (the current price of twenty servings at Noma) is a quantitative attribute value of the Noma object, which has the property that arithmetic can be performed on it. For example, the sum of the values DKK 1,600 and DKK 1,000 (the current price of the wine pairing at Noma) is DKK 2,600.
\end{itemize}

The distinction between these attribute types become especially important when objects are represented visually, which will be discussed in the next section.

Of these attribute types, the geometric attribute type is perhaps the one most intrinsic to geographic information. Formally, a geometric attribute corresponds to a set of points; in geography, these points are typically geographic coordinates.

There are three frequently used types geometric attributes that correspond to the following point sets: 

\begin{itemize}
\item \textbf{Point}: a single point, for example $(x,y)$
\item \textbf{Curve}: a one-dimensional set of points, for example the polygonal chain described by the points $((x_1, y_1), (x_2, y_2), \dots)$.
\item \textbf{Surface}: a set of points with an area, for example a polygon described by the closed polygonal chain $((x_1, y_1), (x_2, y_2), \dots, (x_1, y_1))$.
\end{itemize}

For simplicity, geometries with a volume as well as hierarchical attribute types such as lists are ignored in the following.


\minisec{Graphical Model}

\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.72]{figs-thesis/visual-variables.pdf}
\caption{\textbf{Bertin's Visual Variables}: The visual variables, of which Bertin identified seven, can express qualitative, ordered and quantitative attributes of geographic objects. For example, different colors can represent different cuisines. The figure is reproduced from \texttt{infovis-wiki.net}.}
\label{fig:introduction:visual:variables}
\end{center}
\end{figure}


Symbols graphically represent geographic objects. For example, a building object can be represented by a black square symbol, while a road object can be represented by two parallel curve symbols. Typically, the attributes of an object are mapped to one (or more) of seven \emph{visual variables}, which were first described by Bertin in his book ``Semiology of Graphics: Diagrams, Networks, Maps''~\cite{bertin1967semiologie} (see Figure~\ref{fig:introduction:visual:variables}). Different visual variables have an affinity towards certain types of attributes. For example, colors have no inherent order but are clearly distinguishable from each other (some more than others). This makes them useful for representing qualitative attribute values (see Figure~\ref{fig:introduction:restaurant:colors}). In contrast, sizes can be compared and measured. This makes them useful for representing both relative order as well as absolute quantity. 

A two-dimensional visual variable is position, which usually represents the object's geometry after a standard map projection~\cite{snyder1997flattening}. Alternatively, symbols can be positioned in a topologically equivalent layout, which is a technique that is frequently used for example in Metro maps. Moreover, the symbol position can perfectly well represent something other than the geometry, such as average life expectancy or economic wealth~\cite{rosling2007poverty}. 
...
Bertin's work on the theory of graphics is immense, and this short section does not aim to fully cover it. Also, a related work is the ``the Grammar of Graphics'' by Wilkinson et al.~\cite{wilkinson2006grammar}. This collection of previous work covers many of the fundamental aspects of representing information as graphics, which subsumes the representation of geographic information on maps. This thesis will focus mostly on abstraction of geographic object sets and less on graphical representation.

\subsection{Map Generalization}
\label{sec:introduction:map:generalization}
As mentioned in Section~\ref{sec:introduction:background}, map generalization is a collective term that covers various information abstraction techniques used to create geographic maps. In this section, we will have a closer look at the motivation for map generalization, as well as some of the well-known techniques.

The Earth is huge compared to the size of a normal map, say a paper map that you might pick up at a tourist information desk. One of the key challenges of map making is how to represent planet-sized information on a roughly human-sized map medium, be it a computer screen, mobile device or piece of paper. The difficulties arise from the large difference in \emph{scale} between the two material bodies, the Earth and the map medium (see Definition~\ref{def:scale}). 

\begin{definition}[Map Scale] The scale of a map is the ratio of a distance on the map to the corresponding distance on the ground. For example, on a 1:500,000 map, one centimeter on the map corresponds to five kilometers on the ground.\label{def:scale}\end{definition}

For comparison, one can fit roughly 500 trillion non-overlapping paper maps onto the surface of the Earth. Consequently, each centimeter on a map has to convey a lot of information. For example, on a 1:500,000 map, one centimeter on the map surface corresponds to five kilometers on the ground. In the real world, a lot of information is contained within those five kilometers! 

Necessarily, information must be abstracted in order to emphasize the important and suppress the irrelevant, as otherwise maps would become cluttered and illegible. In geographic information science, the complex process of information abstraction for maps is called \emph{map generalization}~\cite{mcmaster1992generalization,weibel1997generalization}.


This section briefly covers the implications of reducing the scale of a map and how map generalization can help address the problems that arise. Furthermore, this section provides a quick overview of the key approaches to map generalization. For the reader who is interested in additional knowledge about the generalization process, Mackaness, Ruas, Sarjakoski~\cite{mackaness2011generalisation} have compiled material that provides a useful introduction into the field. Furthermore, the seminal works by McMaster and Shea~\cite{mcmaster1992generalization} and Weibel~\cite{weibel1997generalization} are important entry points into the field of map generalization.


\minisec{Effects of Scale Reduction}
Different visual effects occur as a map is scaled down. These effects can be seen in Figure~\ref{fig:introduction:background:raw:poi}, which illustrates a point dataset that has been rendered at different scales. There are three important effects that can be observed as a map is rendered at lower scale (without applying map generalization):
...
\item \textbf{Decreasing symbol sizes:} The sizes of symbols -- if shown at correct relative size according to the real-world things they represent -- decrease when the map scale is reduced. For example, if a lake is 2 mm wide on a 1:250,000 map it would be 0.5 mm wide on a 1:1,000,000 map, i.e. barely visible to the human eye. Similarly, map mediums may even be unable to reproduce symbols when they become too small. The limits of perceptibility, which can and have been empirically established, are termed \emph{minimum dimensions} in cartography~\cite{weibel1997generalization}. For example, it has been found that the sides of a black square, which may represent a building, should be at least 0.35 mm in length, while two parallel curves, which may represent the curbs of a road, should be separated by at least 0.25 mm~\cite{ssc1977minimumdimensions}.
%A generalization technique that deals with diminutive symbols sizes is \emph{exaggeration}.

\item \textbf{Decreasing symbol separation:} The separation between symbols -- if shown at accurate positions -- decreases when the map scale is reduced. Consequently, symbols may come so close to each other as to become fused. For example, two symbols that are 0.8 mm apart on a 1:250,000 map will be 0.2 mm apart on a 1:1,000,000 map, which is below the limit at which symbols can be distinguished from each other.
%Two generalization techniques that deal with symbols in close proximity are \emph{displacement} and \emph{selection}.

\item \textbf{Increasing information density:} The information density --  the amount of information per unit area --  increases when the map scale is reduced. Information density can be measured according to different metrics, such as Tufte's density metric~\cite{tufte1983visual}. The \emph{principle of constant information density} states that information density on a map should remain constant as the scale changes, which was first formally recognized by T{\"o}pfer and Pillewiser in 1966~\cite{topfer1966principles}. They observed that the number of settlements shown per unit area in published map series seems to be roughly independent of the map scale. The rationalization of this observation is that a linear reduction in scale implies a reduction in drawing area according to geometrical progression, which intuitively must lead to a reduction in information at the same rate.
%  Information density can be kept constant by either \emph{aggregation} and \emph{selection} (see Figure~\ref{fig:introduction:background:mashups}).

\end{itemize}

\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.48]{figs-thesis/geographic-mashups.pdf}
\caption{\textbf{Aggregation and selection of geographic information}: On the left map, residents of London, United Kingdom are aggregated by region and the regions are shaded according to the share of people who speak a non-English language as their first language; darker values imply more foreign-language speakers. On the right map, points that represent casualties during the 2003-2011 Iraq War are selected from a much larger set of points. Both maps are reproduced from The Guardian's Data Blog with background maps from Google Maps \textcopyright 2014.}
\label{fig:introduction:background:mashups}
\end{center}
\end{figure}

\minisec{Generalization Operators}
The effects mentioned above generally require information to be abstracted as scale of a map is changed. Otherwise, maps may become illegible and cluttered or symbols may become imperceivable. Generalization operators are functions that transform geographic object sets and map symbols such that a useful and readable map is ensured. For example, information can be selected or aggregated (see Figure~\ref{fig:introduction:background:mashups}). One way to classify operators is according to three categories: \emph{selection} (see Definition~\ref{def:selection}), \emph{projection}  (see Definition~\ref{def:projection}), and \emph{aggregation} (see Definition~\ref{def:aggregation}). 

\begin{definition}[Selection]
A procedure that accepts a set of objects and outputs a subset of these objects, but does not modify the objects in any way. \label{def:selection}
\end{definition}

\begin{definition}[Projection]
A procedure that accepts a set of objects and maps each object to another object. \label{def:projection}
\end{definition}

\begin{definition}[Aggregation]
A procedure that accepts a set of objects and merges subsets of these objects into single objects. Critically, at least one object that appears in the output cannot be a member of the input, as otherwise the procedure would be equivalent to selection. \label{def:aggregation}
\end{definition}

In geography, the term projection is often used synonymously with the term \emph{map projection}, i.e. the projection of spherical coordinates onto the plane. However, in this thesis, the term projection is used in a broader mathematical sense that is adopted from databases~\cite{ramakrishnan2000database}, i.e. a mapping from objects to objects.

\minisec{Independent vs. Holistic Operation}
The three operators -- selection, projection, and aggregation -- can be subcategorized into \emph{independent} or \emph{holistic} variants. An independent operator can process objects separately (e.g. in parallel), because the decision that are made for one object does not impact any other object. For example, \emph{random sampling} and \emph{filtering by predicate} are independent selection operators, since an isolated inclusion/exclusion decision must be made for each object, which does not affect the decision made for other objects. By contrast, a holistic operator must process objects together.  For example, \emph{displacement} is a holistic operator that moves objects apart if they are indistinguishable from each other due to close proximity. In this example, moving an object away from one object may bring it into close proximity of another. Therefore, a holistic solution for the movement of objects must be found. Table~\ref{tab:operators} shows how selection, projection, and aggregation can be implemented by both independent and holistic algorithms.

\begin{table}[htdp]
\caption{Examples of generalization algorithms by category}
\begin{center}
\begin{tabular}{|l|p{10cm}|}
\hline
\textbf{Category (subcategory)} & \textbf{Generalization procedure} \\
\hline
Selection (independent)  & Random sampling~\cite{wang2014efficient} \newline Filtering by predicate~\cite{sld} \\
\hline
Selection (holistic)  & Route selection~\cite{agrawala2001rendering} \newline Street network selection~\cite{schmid2013opensciencemap} \newline Hydrological network selection~\cite{horton1945erosional,shreve1966statistical,strahler1957quantitative} \newline Proximity-based selection~\cite{nutanong2012multiresolution} \newline Density-based selection~\cite{sarma2012fusiontables} \\
\hline
Projection (independent)  & Coordinate projection~\cite{snyder1997flattening} \newline Exaggeration~\cite{weibel1999generalising} \newline Collapse~\cite{weibel1999generalising} \newline Simplification~\cite{douglas1973algorithms} \\
\hline
Projection (holistic) & Feature displacement~\cite{bader2001energy} \\
\hline
Aggregation (independent)  & Choropleth maps~\cite{choropleth}  \newline Heat maps~\cite{heatmap} \\
\hline 
Aggregation (holistic) & Landcover aggregation~\cite{haunert2006landcover} \\
\hline
\end{tabular}
\end{center}
\label{tab:operators}
...
\minisec{Typologies in the Literature}
This section has adopted a three-way categorization of operators into selection, projection, and aggregation. This typology is considerably more abstract and high-level than the usual typologies proposed in the map generalization literature. There, a more domain-specific and specific categorization tends to be prefered over this more mathematically pure categorization. However, dispite the numerous attempts in the literature to come up with a final typology for map generalization operators, no consensus has been reached yet~\cite{rieger1993consensus}. Various competing and derivative typologies have been proposed by Foerster, Morales, and Stoter~\cite{foerster2008classification}, McMaster and Shea~\cite{mcmaster1992generalization}, and Weibel and Dutton~\cite{weibel1999generalising}. While these different classifications of operators provide insight into the concrete aspects of the generalization process, it is hard to argue that either one of them is conclusive. Therefore, this section has covered generalization operators by grouping them by their fundamental mathematical properties rather than more specific properties.

\subsection{Constraint-based Generalization}
\label{sec:contraint:based:generalization}
Overall, the generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, constraints act as the discriminator between feasible and infeasible map states~\cite{harrie1999constraint,ware2003generalization_sa}, while objectives quantify map utility and thus define optimality. Together, constraints, objectives and a set of geographic information objects  implies an optimization problem over a search space of maps. The search space consists of maps that can be derived from a source map by applying generalization operators (see Section~\ref{sec:introduction:map:generalization}) to the objects and symbols.

There are many ways to measure the optimality of a map. Typically, an objective is to either maximize or minimize some objective function over the map content, for example to maximize the sum of ``relevance'' of objects that appear on a map~\cite{sarma2012fusiontables}.

\minisec{Constraints in Maps}
The overall purpose of map constraints is to distinguish between acceptable maps and unacceptable maps, or equivalently, to define the search space of feasible maps. Constraints may state conditions that should be met by the map as a whole, by subsets of information on the map or by individual pieces of information on the map. 

\newpage
Some examples of constraints in different categories are listed below:

\begin{itemize}
\item \textbf{Unary constraints}: individual objects must satisfy a condition. For example, the size of an object geometry must respect minimum dimensions.
\item \textbf{Pair constraints}: pairs of objects must satisfy a condidtion. For example, proximity of objects must be above a certain threshold.
\item \textbf{Locus constraints}: subareas of the map must satisfy a condidtion. For example, the information density within each subarea must be below a certain threshold.
\item \textbf{Topological constraints}: networks and geometries must satisfy a topological condition. For example, a river system must not be disconnected from itself or its outlet; topological properties such as containment, intersection, etc. must not change.
\end{itemize}

Like generalization operators, constraints have also been classified according to many different typologies. For more information on constraint typologies in the literature, refer to the work of Burghardt, Schmid, and Stoter~\cite{burghardt2007investigations} or  McMaster and Shea~\cite{mcmaster1992generalization}.

\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.4]{figs-thesis/holistic-selection.pdf}
\caption{\textbf{Pair and Locus Constraints}: pair constraints, such as minimum proximity (left), set up conditions that must be met by all qualifying pairs of objects on a map. Locus constraints, such as a maximum density (right), set up conditions that must be met by all qualifying loci (e.g. unit areas) in the map. Entities (such as objects or loci) that violate a constraint are in conflict. In the figure, entities that are in conflict are circled by red lines; this situation must be remedied, for example by selection. In the figure, kept objects are shown as black disks; deleted objects are shown as white disks.}
...
To evaluate the satisfaction of map constraints, they must be based on quantifiable measures. For example, a measure can be compared to a threshold value in order to determine satisfaction. Luckily, many different measures have been studied, which can form the basis of constraint formulations for maps:

\begin{itemize}
\item Unary measures quantify geometric and non-geometric properties of individual geographic objects and symbols in a map. They form the basis for unary constraints.  Geometric unary measures include complexity~\cite{douglas1973algorithms}, sinuosity~\cite{mueller1968introduction} and size. Non-geometric unary measures include all forms of logical predicates, such as equality, comparison, and set membership.

\item Spatial measures quantify properties of space, such as the density of information and the distribution of information. They form the basis of locus constraints, i.e. constraints on space. Information density is measured by a density metric, such as one of the metrics surveyed by Nickerson~\cite{nickerson1994visual}. Spatial distribution is measured as a distance to some ``original'' distribution, for example by using the Bhattacharyya distance~\cite{bhattacharyya1946measure} or the earth mover's distance~\cite{rubner2000earth}.

\item Topological measures characterize the topological relationship between geometric objects in the plane and elements in a network. They form the basis of topological constraints. Binary topological measures on geometric objects include the nine-intersection model~\cite{egenhofer1991categorizing} and the region connection calculus (RCC)~\cite{randell1992spatial}. Measures for network elements include stream-orderings~\cite{horton1945erosional,shreve1966statistical,strahler1957quantitative} and shortest paths~\cite{schmid2013opensciencemap}.
\end{itemize}

For a concrete example of real-world constraints and objectives that have been used to evaluate a map in industry, see the Apple iOS 6 design objectives~\cite{samet2012duking}.

% Background

%\section{Motivation}
%\label{sec:introduction:motivation}
\section{How the World of Maps has Changed}
\label{sec:impact:of:the:web}
%\label{sec:introduction:motivation}

Map have been crafted for thousands of years. So, why is there a sudden need to rethink the way maps are created? A crucial change in conditions has been caused by the recent rise of the Web as well as of cloud computing. In the following sections, the challenges and opportunities that have followed in the wake of these disrupting technologies are analyzed in more detail.

\subsection{The Impact of the Web}
% The changes and challenges caused by the web.
The Web has structurally impacted \emph{why}, \emph{how}, \emph{when}, and \emph{where} geographic maps are produced and consumed. In combination, the Web and mobile devices have resulted in an online eco-system where maps are created, accessed, and shared online by millions of people every day; people who carry out a multitude of everyday tasks that involve maps. These users expect 24 x 7 availability and acceptable latency for map services, as for any online service, which puts pressure on the existing geographic information system (GIS) infrastructure.

GIS was developed prior to the World Wide Web and primarily as a single-user system~\cite{ucgis2014tomlinson}. By contrast, the Web is a massively concurrent environment where millions of users access the same services. Therefore, GIS has been retrofitted with web capabilities, such as the Web Map Service (WMS) and Web Feature Service (WFS). However, these protocols do not fundamentally alter the architecture of traditional GIS. They simply expose a GIS interface over the Web while mostly maintaining the same computational architecture as before. Specifically, the focus has traditionally been on on-the-fly analysis and map rendering, since this represents an ideal.

While on-the-fly maps represent an ideal in user experience, they are fundamentally expensive to compute in a highly concurrent access scenario, such as is the norm on the Web. Consequently, all major successful map providers on the Web have switched to a new map architecture, which primarily relies on precomputed map data that is distributed to end users over a content delivery network (CDN)~\cite{mbtile12}. 

The section below investigates some of the key aspects of maps on the Web today, and why they cause challenges for map creators.

%Selection of data subsets is one of the most common tasks when creating a map. For example, a map that displays restaurants in a city must usually select a subset for display on the map, since the density of food shops in urban environments is typically too high to accomodate all of them on a map (depending on scale). A comprehensive literature on map generalization (including selection) already exists, so why is there a need to augment the literature with new studies and new methods? To answer this question, this section covers three recent trends in online computing that have dramatically impacted the map visualization process -- and which in turn requires new selection methods for maps to be developed. The trends present both hard challenges and powerful opportunities for new ways to create maps in an online environment.

%\begin{itemize}
%\item Mobile devices and the Web, in combination, have produced an online environment where information is created, accessed, and shared ubiquitously. Today, online maps are used by millions of people every day and have become a commodity. As with any online commodity, the users demand 24 x 7 availability and acceptable latency for map services.
%\item Advances in cloud computing have exposed highly-programmable and scalable data processing systems, such as mapreduce frameworks~\cite{apache2015hadoop} and data warehouses~\cite{bauer2013redshift}, to ordinary programmers who can rent and program these powerful systems by the hour~\cite{amazon2010aws}. Additionally, cloud computing provides practically unbounded storage of information. In combination, cloud computing potentially creates a powerful opportunity for processing data visualizations, such as maps, at scale~\cite{aji2013hadoopgis,eldawy2014spatialhadoop}.
%\item Today, most large-scale information distributers, such as video and social networking sites, use content delivery networks to distribute their product to a global audience. These systems are founded on solid theory from distributed systems, such as key-value stores, data partitioning and data replication~\cite{barroso2009datacenter,pilchin2007dynamo}. Similarly, maps that have been properly partitioned can be efficiently and concurrently distributed to millions of people online~\cite{gaffuri12vectortiles}.
%\end{itemize}

%In the following sections, these trends are investigated in more detail.
...
\minisec{Thematic Maps}
While thematic maps have always been prolifically created, the Web has further shifted the attention, away from creation of background maps towards creation of thematic maps and overlays. Part of the reason is that private companies and national mapping agencies already offer a great quantity of high-quality background maps for the Web, often free of charge~\cite{gst2014digitalmapsupply,google2014maps, stamen2014tiles,vizzuality2012cartodb}. In contrast, thematic data typically reflects phenomena that are intrinsically more dynamic and multi-faceted, such as human behaviour.  This makes thematic maps fundamentally more news-worthy and trendy than background maps, which is why journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} frequently create new thematic maps as part of their work. Finally, thematic data ranges over multitude of different domains, which multiplies the number of different thematic maps that can be created. By contrast, background maps tend to depict relatively static information from a limited number of domains, such as road networks, hydrology, urban structures, and administrative borders. Therefore, the number of different background maps that can sensibly be created is small in comparison to thematic maps.


\minisec{Large Datasets}
There is a steady rise in the supply of geographic datasets online -- both curated~\cite{mst2014miljo}, user-contributed~\cite{twitter2014twitter,zooniverse2014oldweather}, and machine generated~\cite{planetlabs2014flock1}. In some cases, these datasets grow by millions of information objects each day~\cite{openstreetmap2014stats}. Furthermore, in the future, data from low-orbit satellites~\cite{planetlabs2014flock1} and sensor networks~\cite{project2009european} could potentially dwarf today's data volumes. The challenges to mapping are both data integration~\cite{halevy2006data} as well as data processing of large volumes of heterogenous geographic data.


%The Web has shifted the the focal point of map creation, such that new thematic maps and overlays are much more likely to be created than new background maps.  This shift can be attributed to multiple sources. On the one hand, private companies and national mapping agencies already supply a great quantity of high-quality background maps for the Web~\cite{google2014maps,gst2014digitalmapsupply, stamen2014tiles,vizzuality2012cartodb}. Additionally, the information that appears on background maps is typically quite stable over time, such as the location of mountains, lakes, coast lines, cities, and roads. On the other hand, the supply of thematic data is constantly evolving as it reflects phenomona that are intrinsically more dynamic and multi-faceted, such as human behaviour. The sources of thematic data include curated~\cite{datagov}, user-contributed~\cite{twitter2014twitter,zooniverse2014oldweather}, and machine generated~\cite{planetlabs2014flock1} repositories. 

%While all geographic data is growing~\cite{openstreetmap2014stats}, thematic data has a clear tendency to grow faster than background data. Since the relevance of thematic data is intrinsically short-lived, it is crucial that we can process it and visualize it expediently. However, given how the volume of data can accumulate really fast, it is crucial that data can be processed in a scalable way.

%Journalists~\cite{lomet2012warstories}, aid workers~\cite{openstreetmap2010haiti}, and scientists~\cite{zooniverse2014oldweather} are frequent creators of thematic maps, which aim to increase transparency and knowledge in the World and potentially save human lives, as was the case in Haïti after the earthquake in 2010~\cite{openstreetmap2010haiti}.



\minisec{Novice Map Makers}
The Web has caused a change in how and by whom maps are created. While map creation was traditionally the domain of cartographic experts, this is no longer the case. Today, the data enthusiast has replaced the expert as the person most likely to create a map~\cite{hanrahan2012enthusiast}. This is partly because demand for thematic maps on the web has grown faster than the number of cartographic experts who can create them. This has created a human resources challenge, which is being addressed by the development of more intuitive and expressive design interfaces. These interfaces promise to enable anyone to create maps. The need for rapid thematic mapping has driven the development of fully-automatic mapping systems~\cite{sarma2012fusiontables,nutanong2012multiresolution}. 

While fully-automatic mapping systems promise map designer productivity, it is still difficult to create a non-default map. Arguably, a fully-automatic mapping system is likely to be suboptimal for most cases. The alternative is to use traditional mapping systems, which were chiefly created for the production of complex maps of the Earth~\cite{lamy1999application,ware2003generalization_sa}. They are certainly powerful enough. However, these systems are hard and time-consuming to use, which in turn makes them costly and inappropriate for novice to medium-skilled map designers. Clearly, there is a need for expressible map design interfaces that can be time-efficiently operated by non-specialist map makers, such as software developers, journalists, aid workers and scientists.

\minisec{Massively Concurrent Consumption}
The Web has caused a change in when and where maps are consumed by their audience. Today, an online map can be consumed by anyone, anywhere, at any time. Thanks partially to mobile devices in our pockets that are always connected to the Internet, users can access information on-demand. Consequently, many online map are concurrently consumed by thousands or even millions of users, who expect 24 x 7 availability and acceptable for online services including maps. 

In distributed systems, data partitioning and data replication are important techniques that can increase the availability and performance of data services~\cite{barroso2009datacenter,dilley2002cdn}. In mapping, a typical data partitioning scheme is to split the data into raster~\cite{mbtile12} or vector tiles~\cite{gaffuri12vectortiles}, which split the data among cells in a fixed multi-level grid called a tile pyramid~\cite{decola1993pyramid}. The advantage of tile pyramids, which have logical grid for each ``zoom level'' in a map, are that subsets of the data can be addressed in a fixed key-value scheme. Consequently, tiles can easily be distributed via a content delivery networks (CDN)~\cite{dilley2002cdn}. A CDN is a geo-distributed high-performance key-value store that serves data from edge-locations on the Internet, which are close to the users. While CDN can effectively serve data, the number of tiles in a tile pyramid grow exponentially in the number of zoom levels that a map supports. While a large set of tiles can be rapidly materialized in parallel, for example on a map-reduce cluster~\cite{dean04mapreduce,dean2009designs}, the computation and I/O cost is fundamentally high since it is exponential in the number of tile pyramid levels. For example, on a zoomable world map with twenty zoom levels (which is the norm), there are more than 300 billion distinct tiles. Therefore, there is a need for methods that reap the benefits of data partitioning and data replication, while avoiding the cost of full materialization.

\subsection{The Impact of Cloud Computing}
Cloud computing offers a powerful model of ubiquitous, convenient, and on-demand access to a shared pool of vast computing resources~\cite{mell2009nist}. The advent of cloud computing has surely created new opportunities both for map creation and map distribution.

\minisec{Impact on Map Creation}
To create maps, programmers, scientists and cartographers can rent highly-scalable data analytics~\cite{apache2015hadoop}, storage~\cite{palankar2008amazon} and database systems~\cite{bauer2013redshift} by the hour from a cloud provider, such as Amazon Web Services (AWS)~\cite{amazon2010aws}. Such systems provide the components needed to compute and store geographic maps over massive amounts of data. For example, map generalization (see Section~\ref{sec:introduction:map:generalization}) can essentially be broken down into a set of data analysis and transformation tasks that can be performed by a system like Spatial Hadoop~\cite{eldawy2014spatialhadoop}. This type of system allows spatial analytics to run on a scalable cluster of computers.

The availability of potent computing resources is matched by unprecedented access to curated~\cite{gst2014digitalmapsupply} and user-contributed~\cite{openstreetmap} geographic data online. Together, the availability of cloud computing, cloud storage and open data creates a neat opportunity to create previously unrealizable maps.

\minisec{Impact on Map Distribution}
To distribute maps, content delivery networks (CDN) offer a scalable and affordable way to distribute data to a global audience. Some of the key advantages of CDN are their superior availability, scalability and performance~\cite{barroso2009datacenter,pilchin2007dynamo}, which is why they are used by anyone seriously engaged in data distribution~\cite{beaver2010haystack}. To harness a CDN for map distribution, the map data must first be organized into key-value pairs. For example, the key may be the ID of a geographic grid cell, while the value may correspond to the data intersected by that grid cell~\cite{gaffuri12vectortiles}. Spatial data can be partitioned into key-value pairs by several techniques which includes spatial grids~\cite{decola1993pyramid} and linearization~\cite{hilbert1891ueber,morton1966computer} techniques. Once partitioned, the spatial data for a map can be distributed to millions of people online as a set of map tiles, either as images or data.

Recently, in-memory and fault-tolerant stores, such as RAMCloud~\cite{ousterhout2011ramcloud} have appeared. Systems like RAMCloud combine the speed of in-memory caches with the persistence of disk-based stores. The appearance of such systems will make the distribution of maps, which can be decomposed into key-value pairs, even more effective and affordable in the future. Already now, in-memory key-value stores are being offered as a service in the cloud.

\section{State-of-the-Art Digital Mapping Pipelines}
\label{sec:introduction:web:mapping:pipelines}

\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.5]{figs-thesis/mapping-states.pdf}
\caption{\textbf{Digital mapping pipeline}: the pipeline consists of several stages and transformations. Data modeling (1) captures reality as a set of information objects (primary data model); data abstraction (2) refines the set of information objects that appear on a map (logical model); visual abstraction (3) specifies how information objects are symbolized on a map (graphical model); finally, rendering (4) produces output that is displayed on a map medium.}
\label{fig:introduction:requirements:pipeline}
\end{center}
\end{figure}

The digital mapping process can be modeled as a pipeline that translates ``reality'' into graphical output on a digital map medium (see Figure~\ref{fig:introduction:requirements:pipeline}). Instead of leaping directly from reality to graphical output, it is useful to model some intermediate stages. A common approach is to model geographic information at three intermediate stages: the \emph{primary data model}, the \emph{logical model} and the \emph{graphical model} of the digital map~\cite{gruenreich1992atkis}. To produce a high-quality map, it is often necessary to iterate back-and-forth between the logical model and the graphical model. For example, choosing larger symbols in the graphical model may result in overlapping symbols. In response, the logical model can be renewed by selecting fewer objects such that symbol overlap is avoided. The edge from the graphical model to the logical model in Figure~\ref{fig:introduction:requirements:pipeline} models this iterative process.

The data modeling process is outside the scope of this thesis. It has been studied in previous work, for example by Friis-Christensen et al.~\cite{friis2001requirements} and Goodchild~\cite{goodchild1992geographical}. The rendering process is also outside the scope of this thesis. Moreover, the rendering process is highly dependent on the map medium of choice. The intermediate stages, i.e. the logical and graphical model, were described in Section~\ref{sec:introduction:map:generalization}.

Given a model of the digital mapping pipeline, it is crucial that we understand the requirements of its components. This is especially true since the Web has dramatically impacted what these requirements are (as mentioned in Section~\ref{sec:impact:of:the:web}). In a web scenario, the importance of map creation is only as important as map distribution: there is little point in creating a beautiful map if it does not reach the intended audience! Therefore, the evaluation of a mapping pipeline should take into account how map content is distributed to audiences, which is not included in Figure~\ref{fig:introduction:requirements:pipeline}.

A key distinction between state-of-the-art mapping pipeline is whether the map creation process is synchronous or asynchronous in relation to the map distribution process. While the synchronous paradigm is championed by real-time systems, the asynchronous paradigm is championed by batch-oriented systems. In the following sections, the advantages and drawbacks of both paradigms are analyzed and discussed.

\subsection{Real-time Mapping Systems}
In real-time systems, the map creation process is synchronous with the map distribution process: pieces of the map are created on-the-fly as map requests are received by the system. For example, in systems that implement the Web Mapping Service (WMS) protocol, a map request will typically invoke the entire mapping pipeline from the primary data model, through data abstraction, visual abstraction, and rendering.

In the academic literature, real-time systems have been studied because they have some important advantages:  

\begin{itemize}
\item In real-time mapping systems, data is only stored once, namely in the primary data model. While this obviously saves storage, a more important benefit is that information that appears on a map is never stale. Given an atomic update mechanism for the data model, such as transactions~\cite{gray1981transaction}, a map always displays the most recent and consistent state of the data.
\item In real-time mapping systems, user preferences and the context of map usage can be taken into consideration when creating the map. Thus, maps can be tailored to individual users needs, for example by letting a recommendation algorithm play a role in assigment of object importance.
\end{itemize}

Real-time mapping requires low-latency data abstraction, since the map creation process is synchronous with the map distribution process. Low latency is typically ensured by using an efficient data structure to index the geographic information. The index structure is usually some kind of tree, such as a quadtree~\cite{bereuter2013real} or GAP-tree~\cite{van1995gap}.

The drawbacks of real-time systems are primarily concerned with their relative lack of scalability: 

\begin{itemize}
\item In real-time mapping systems, the complexity of any tree-based access method (let alone generalization method) is high compared to the essentially contant-time complexity of looking up precomputed content in a key-value store~\cite{pilchin2007dynamo}, such as a CDN.
\item In real-time mapping systems, the tree structures that have been proposed are typically not designed for massively concurrent access. This is critical, because there is typically a high and variable level of concurrency in a web context, which implies that the data abstraction process must be scalable in the number of concurrent queries as well as data volume. The na{\"i}ve approach, which is to replicate the entire index multiple times and dispatch requests round-robin is costly, especially when data grows in size.
\end{itemize}

\subsection{Batch-oriented Mapping Systems}
In batch-oriented systems, the map creation process is asynchronous with the map distribution process: the logical and/or graphical models are materialized separate from the primary data model. At this point, the precomputed map can be partioned, e.g. using a tile pyramid, and replicated onto a content delivery network (CDN). For example, systems that implement the XYZ web mapping protocol~\cite{osm2014xyz}, such as OpenStreetMap, typically function in this way.

In the literature, batch-oriented systems have been studied for the distinct advantages that they offer over real-time systems:

\begin{itemize} 
\item In batch-oriented systems, more computational effort can be spent on producing a high-quality result. Furthermore, batch-oriented systems can exploit the machinery of distributed data analytical systems, such as spatial parallel databases~\cite{waas2009beyond} and spatial cluster computing~\cite{aji2013hadoopgis,eldawy2014spatialhadoop}, which can process practically unlimited amounts of spatial data.
\item In batch-oriented systems, the output can be partioned as tiles and replicated onto key-value stores, such as CDNs, which offer high-availability and scale extremely well in the number of concurrent requests.
\end{itemize}

Some important studies in batch-oriented mapping systems are multi-scale trees~\cite{frank1994multiscaletree,van2011towards} and the Fusion Tables system~\cite{sarma2012fusiontables}. However, these systems only partially realize the full potential advantages of batch-oriented systems. For example, distributed spatial processesing systems are not significantly leveraged by any of the systems, except for rendering of map tiles~\cite{dean2009designs}. Thus, there is a gap between what is theoretically possible and what has been tried.

There are three drawbacks of a batch-oriented approach that are quite obvious:

\begin{itemize}
\item Materialization of the logical and graphical model obviously incurs an additional storage cost compared to only storing the primary data model (plus an index). 
\item Materialized views of the primary data model may become stale. % lars harrie: term stale? 
\item Batch-oriented systems can not easily take individual user preferences into account, nor can they easily adjust the map for the usage context.
\end{itemize}

\subsection{Best Fit for Massively Concurrent Web Maps}

Real-time systems present an ``ideal'' for mapping systems. However, current real-time mapping systems are not ideal for creating and serving massively concurrent web maps. First, these systems have low scalability availability and high latency compared to precomputed content that is served from a key-value store or CDN. Second, they do not cost-effectively scale with the number of concurrent users. These problems are critical in a web context.

In contrast, batch-oriented mapping systems are appropriate for creating and serving web maps. First, batch-oriented systems can process maps by exploiting highly-scalable data processing systems, such as spatial mapreduce~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} and spatial parallel databases~\cite{waas2009beyond}. While highly scalable, these systems are not appropriate for real-time processing. Second, batch-oriented systems can serve maps online with high availability, high scalability, and low latency by using key-value stores or CDN to distribute pre-processed data. Today, scalability in data volume and concurrent users is a crucial requirement for large-scale web map systems. 

Additionally, the disadvantages of batch-oritented systems are not too severe in practice. First, map data typically does not change often enough for real-time systems be worthwhile. Second, disk-based storage has become very cheap and pure in-memory storage is bound to become cheaper over time. Third, materialized views can be refreshed as demand dictates, either by leveraging batch-oriented parallel processing~\cite{aji2013hadoopgis,eldawy2014spatialhadoop} or by employing techniques for materialized view maintenance~\cite{gupta1995maintenance}. Finally, user preferences can to some extent be accommodated by materializing views for different user profiles and choose between them at query time. However, this last problem is probably the most fundamental problem of batch-oriented systems, since user preferences are somewhat heterogenous.

\section{Problem Statement and Contributions}
\label{sec:introduction:problem:statement}
%Given the analysis of state-of-the-art web mapping pipelines (Section~\ref{sec:introduction:web:mapping:pipelines}) and the realization that batch-oriented pipelines best match these requirements (Section~\ref{sec:introduction:architectures}), this section lists four important gaps between state-of-the-art batch-oriented pipelines and a hypothetical batch-oriented pipeline that satisfies all the requirements.
In this section four gaps will be described along with the contributions of this thesis that address them.

\subsection{Unified Selection Model}
\label{sec:introduction:problem:statement:one}
% MATHEMATICAL FOUNDATION
Selection for maps can be intuitively understood as filtering geographic objects, possibly at multiple scales, subject to some constraints. However, the formulations of selection in the literature differ widely in their choice of constraints. For example, different authors have formulated selection for either unary~\cite{rocha2011efficient}, binary~\cite{nutanong2012multiresolution}, or locus~\cite{sarma2012fusiontables} constraints. Thus, there does not exist a complete formulation of selection of geographic data for maps, which allows users to mix and match constraints for a selection. Clearly, there is a need for a unified formulation that subsumes these incomplete formulations.

Specifically, this unified selection model must supports user-defined constraints of various types, such as unary, binary, locus, and/or cross-scale constraints. Furthermore, the model must support user-defined objectives, such as maximization of user-defined object weights. Ideally, all instances of the unified selection problem can be partially or completely solved by well-known, efficient algorithms, in the same way that e.g. that deduplication can be reduced to sorting and a linear scan (more efficient methods exist).

For this problem, this thesis makes the following contributions:

\begin{itemize}
\item \textbf{Selection model}: We present a general formulation of selection for maps that supports most constraints that have been proposed for in the literature. We refer to this problem as \emph{global selection}. The formulation is based on an equivalence relation between the selection problem in geographic maps and the more abstract set multi-cover problem. There is a downside and upside to this mathematical equivalence. The downside is that the set multi-cover problem, and therefore also the geographic selection problem as we have formulated it, is NP-hard~\cite{vazirani2001approximation}. However, others have proved that related selection problems for maps are also NP-hard~\cite{sarma2012fusiontables}. The upside is that several efficient approximation algorithms have been proposed for the set multi-cover problem~\cite{chekuri2012setmulticover,rajagopalan1998primal}, which can therefore be directly used for selection of geographic information. The model is presented in Chapter~\ref{chapter:cvl}.

\item \textbf{Sorting-based heuristic}: We present a sorting-based heuristic for global selection, which produces good quality solutions for most real-world cases. Furthermore, we show that the heuristic solves a frequent, special case to optimality. Specifically, the special case is weighted selection of geographic points subject to a tile density constraint, which corresponds to the formulation by Das Sarma, Lee, Gonzalez, Madhavan, and Halevy~\cite{sarma2012fusiontables}. The description of the sorting-based heuristic and its evaluation can be found in Chapter~\ref{chapter:cvl}.
...
Selection of geographic information for maps today is mostly polarized into two extremes: non-programmable selection~\cite{sarma2012fusiontables} and complex programmable selection~\cite{lamy1999application}. Neither extreme is optimal. While the former approach is probably suboptimal for most real cases since map designers are entirely excluded from the selection process, the latter approach is  very time-consuming and thus expensive to use even for experts. There is currently a lack of a simple, yet programmable approach for selection, which is at the same time expressive and productive for non-technical map designers. As a programming approach, it makes sense that map designers can focus on high-level constraints and objectives for a map in a declarative language, while leaving the minute selection of data to the system~\cite{meliou2011reverse}. Unfortunately, there currently does not exist such a declarative language for global selection of data for maps.

For this problem, this thesis contributes two declarative languages for global selection of data for visualization:

\begin{itemize}

\item \textbf{CVL language}: We present the Cartographic Visualization Language (CVL), which is a declarative, domain-specific language (DSL) for global selection of spatial data for multi-scale maps. It supports user-defined constraints, user-defined object weights and ladder-based map generalization (i.e. zoom-consistency). The syntax and informal semantics of CVL are described in detail in Chapter~\ref{chapter:cvl}.

\item \textbf{Glossy SQL language}: We present Glossy SQL, which extends the ideas of CVL and introduces an SQL-like syntax. Specifically, Glossy SQL is a declarative language for global selection of tabular data for multi-scale data visualizations, where CVL is only for spatial data and maps. Glossy SQL expands on the key ideas of the CVL language in the following ways: it extends the domain of global selection to include all forms of tabular data (both spatial and non-spatial); it classifies user-defined constraints as either unary, binary, set or cross-scale constraints; multi-scale selection can both recursively and iteratively~\cite{foerster2010challenges} (CVL only supports recursive selection); it allows users to define a set of user-defined scale tuples that combine with the data at each level of the visualization; it allows scale-dependent, context-independent (i.e. non-global) projection and selection of objects in an SQL style. Finally, the semantics of Glossy SQL are given by a mapping to pure SQL. Glossy SQL is covered in in Chapter~\ref{chapter:glossy}. 
...
For this problem, this thesis contributes with compilers for the CVL and Glossy SQL languages as well as an evaluation of their performance and scalability:

\begin{itemize}
\item \textbf{CVL compiler}: We present a compiler that translates CVL into SQL with stored procedures implemented in the Python programming language. The compiler targets the PostgreSQL RDBMS, which is popular in the geographic data community due to its high-quality spatial extension~\cite{postgis}. The compiler output can easily be adjusted to target a different spatial database engine~\cite{soliman2014orca} or cluster computing system~\cite{eldawy2014pigeon, thusoo2010hive}. The compiler implementation is described in Chapter~\ref{chapter:cvl}.
\item \textbf{CVL experimental evaluation}: We evaluate the performance and scalability of the CVL compiler for several real-world and synthetic datasets, which inlude both point, line and area datasets. The evaluation of CVL is described in Chapter~\ref{chapter:cvl}.
\item \textbf{Glossy SQL compiler}: We present a compiler that translates Glossy SQL into pure SQL in the PostgreSQL dialect~\cite{postgresql,postgis}.  The compiler output can easily be adjusted to target a different spatial database engine or cluster computing system. The compiler is described in Chapter~\ref{chapter:glossy}.
\item \textbf{Glossy SQL experimental evaluation}: We present an experimental evaluation of the Glossy SQL compiler for a range geographic and non-geographic use cases over real-life datasets. The evaluation measures the scalability and performance of queries produced by the Glossy SQL compiler. Furthermore, the evaluation compares the performance and scalability of output generated by the Glossy SQL compiler to output generated by the CVL compiler. This evaluation is included in Chapter~\ref{chapter:glossy}.

\end{itemize}


% PREDICTIVE TILE MATERIALIZATION
...
For this problem, this thesis contributes with a prediction-based tile selection framework as well as an experimental evaluation of its predictive accuracy:

\begin{itemize}
\item \textbf{Tileheat prediction algorithm}: We present a prediction algorithm for near-future geographic map requests. The prediction is based on an analysis of recent requests for the map, which combines exponential smoothing with a novel heat dissipation technique.  The details about this part are described in Chapter~\ref{chapter:tileheat}
\item \textbf{Tileheat experimental evaluation}: We have evaluated the Tileheat algorithm using a map request log from the Danish Geodata Agency. The results show that Tileheat greatly improves on the method currently used by the agency. The details about this part are described in Chapter~\ref{chapter:tileheat}
...
\item \textbf{Chapter 4}: \emph{Glossy: Global Selection for Data Visualizations}. Kefaloukos, Pimin K and Salles, Marcos V. and Zachariasen, Martin. In submission.
...
%Chapter~\ref{chapter:case:study} presents a case study of the Danish Geodata Agency, which as part of the public sector in Denmark publishes high-quality currated geospatial data, e.g. for visualization purposes. This chapter also contains a light-weight manual for anyone wishing to implement the contributions of this thesis. 
...
%  \end{minipage} \hfill
  \begin{minipage}{0.49\linewidth}
    ...
    \centerline{(b) Airports on the top-most zoom-level.}
  \end{minipage}
  \caption{Airport map before (a) and after (b) generalization by CVL (query in Figure~\ref{fig:cvl:example:airports}). While image (a) contains 7K airports, image (b) only contains the 16 busiest airports in the World.}
...
\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.68]{figs-cvl/cvl_stages.pdf}
...
\begin{figure}[tbp]
\begin{center}
\includegraphics[scale=.45,viewport=400 375 450 550]{figs-cvl/indatabase-execution.pdf}
...
In terms of quality, the difference between SGA and LPGA is not great for either constraint. The difference depends more on the constraint than on the solver, with visibility generally yielding the best solutions. However, the running time of SGA can be substantially shorter than that of LPGA. We analyze this effect in the following.
...
Airports (7K)  & Proximity & LPGA & 7s & 1.22 \\
...
% INSERTED
\minisec{Data reduction techniques}
...
% INSERTED
\minisec{Non-global versus global processing}
Data reduction techniques can be characterized by two extremes: \emph{global} (i.e. holistic) and \emph{non-global} (i.e. non-holistic) data reduction. While non-global data reduction treats items independently, global data reduction must process items in the global context of other items. In SQL, select-where and select-count statements are example of non-global data reduction operations. In contrast, map generalization operators typically processes items in a global context~\cite{weibel1997generalization} -- with some notable exceptions such as geometric simplification~\cite{douglas1973algorithms}. For example, global selection for maps may require items to be selected such that the distance between any two items is above a certain threshold~\cite{nutanong2012multiresolution}. Similarly, global aggregation for maps may merge together items in order to optimize the global legibility and aesthetic quality of a map~\cite{haunert2010area}. The difference in computational complexity between global and non-global data reduction is typically substantial. While the complexity of non-global data reduction, such as selection and aggregation in SQL, is linear in the number of items, the complexity of global data reduction is often NP-hard~\cite{sarma2012fusiontables,haunert2006landcover}.
 
Despite the wealth of existing work on data visualization, most systems have either focused on abstracting data by aggregation~\cite{blais2014generating,lins2013nanocubes,stolte2003multiscale,vartak2014seedb}, or on devising techniques to squeeze a whole dataset into the available pixels~\cite{keim1995visdb,kruskal1978multidimensional}. The problem of creating visualizations that explicitly show representative items, which are selected from a large underlying dataset, has received far less attention. Nevertheless, the importance of this problem is significant. Figures~\ref{fig:example:rappers} to~\ref{fig:example:placenames} illustrate a few visualizations that crucially depend on data selection. These visualizations vary substantially in dimensionality, going from a single axis in the case of Figure~\ref{fig:example:rappers} to a parallel coordinates plot of multiple dimensions in Figure~\ref{fig:example:clusters} to two-dimensional geographic maps in Figures~\ref{fig:example:airports} and~\ref{fig:example:placenames}. Furthermore, these visualizations employ multiple scales to allow users to zoom into the data and see more records from the underlying dataset in an interactive fashion. Moreover, these visualizations are becoming ever more necessary for human understanding as we move towards an age of large datasets and data enthusiasts~\cite{hanrahan2012enthusiast,markl2014breaking}. Even though previous work has targeted particular examples of such visualizations, e.g., for spatiotemporal data~\cite{jugel2014m4,kefaloukos2014declarative,sarma2012fusiontables}, a generic solution to multi-scale data selection has remained elusive. Such a solution should ideally be able to globally sample records, while respecting user-defined constraints and notions of record importance, and at the same time elegantly reuse database technology to operate on large datasets.
...
% INSERTED
\minisec{On-the-fly versus batch-oriented processing}
The information for a data visualization can either be processed \emph{on-the-fly} or in a \emph{batch-oriented} way. Ideally, information would only be processed on the fly, i.e. only when it is needed. However, data visualizations on the Web, such as Google Maps and Bing Maps, must have 24 x 7 availability and low latency while potentially being consumed by millions of concurrent users. Current on-the-fly methods are typically not designed to be accessed by millions of concurrent users with high demands for availability and performance. The problem is only made worse as the amount of data that backs a visualization grows. In order to met the demand for low-latency data visualizations over massive amounts of data, it is more realistic to precompute a data visualization before it is distributed to online audiences. For example, a pre-processed map can be partitioned using linearization~\cite{hilbert1891ueber,morton1966computer} and distributed using scalable data distribution systems~\cite{dilley2002cdn}.

\newpage

\minisec{Global, batch-oriented selection}
In this chapter, we introduce Glossy (GLObal Selection SYstem), a system that computes global selections over records stored in an RDBMS. In short, Glossy's novelty and utility rests on three pillars: (a) Glossy SQL, an extension of SQL that allows visualization developers to express global selection in a concise, yet familiar, query language, (b) an efficient translation of Glossy SQL into relational algebra, which allows Glossy to harness the power of mature query optimizers and indexed tables at runtime, (c) support for global selection over any type of tabular data, not just data within a certain domain (e.g., temporal or spatial data). With Glossy, multi-scale global selections of data for all the visualizations in Figures~\ref{fig:example:rappers}--\ref{fig:example:placenames} can be achieved concisely in a common declarative framework.  
...
\begin{usecase}[Points of interest (POI)]
\label{ex:poi}
Points of interest (e.g., hotels or tourist attractions) are often shown on zoomable maps as an overlay on top of a background map. On a good POI map, items must not disappear when zooming in, a property referred to as zoom consistency~\cite{sarma2012fusiontables}. In addition, modern maps are gridded into tiles at each scale, and web services often transfer tiles individually. It is thus natural to adopt tiles as a bucketing mechanism for information on a map. 
...
In this section, we evaluate three crucial aspects of our implementation of Glossy SQL.
\begin{enumerate}

\item We evaluate the performance and scalability of Glossy SQL by executing the compiler output on the PostgreSQL database system. The goals of our evaluation are to show that: (a)~The Glossy SQL compiler produces efficient and scalable SQL target queries for a range of Glossy SQL constructs; (b)~Glossy SQL can be used to efficiently compute the main use cases presented in Sections~\ref{sec:use:cases} and detailed in Section~\ref{sec:use:cases:solved}; and (c)~The performance of Glossy is either competitive or superior to the performance of the similar CVL selection system. 

\item We compare the character-count of Glossy SQL queries to the character-count of equivalent queries in pure SQL (the output of the compiler). The goal of this evaluation is to demonstrate that Glossy SQL is a concise formulation, which is objectively less complex to write than the equivalent SQL in the PostgreSQL dialect.

\item We evaluate the quality of the selections that are computed by Glossy System. The goals of this evaluation are to show that: (a)~the set of selected items never depends on the order in which items are stored by the RDBMS; (b)~when items have the same weight, the selection corresponds to random sampling.
\end{enumerate}
...
\hline
\textbf{Dataset} & \textbf{Data Source} & \textbf{Dimension and Type} & \textbf{Input Size (rows)}    \\
\hline
...
\begin{figure}[tbp]
\centering
\scalebox{0.6}{\includegraphics{figs-glossy/scalability.pdf}}
...
\begin{figure}[tbp]
\centering
\scalebox{.6}{\includegraphics{figs-glossy/scalability_vs_cvl.pdf}}
\caption{Scalability of equivalent recursive queries in Glossy vs. CVL with locus and pair constraints on scaled point of interest data.}
\label{fig:scalability:glossy:vs:cvl}
\end{figure}
...
We evaluated the scalability of Glossy SQL by deriving ten POI datasets with between 1,000 and 512,000 records, while attempting to scale dataset size exponentially. We adopt the same scaling procedure described by Kefaloukos et al.~\cite{kefaloukos2014declarative}. A sweep-line approach is employed to enlarge the dataset along the x-axis in the west to east direction. The main property of this procedure is that the size of the dataset can be gradually increased, while maintaining local density. The scalability results are reported in Figure~\ref{fig:glossy:scalability}. 
...
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Use Case} & \textbf{Query} & \textbf{Dataset} & \textbf{Response Time} \\
\hline
...
\subsection{Language Usuability}
Glossy SQL is designed to be an easy language to write global selection queries in. One way to estimate the ease of use is to measure the number of words that are needed to express an idea. Arguably, a programming language that requires only few words to express an idea is more useful than a programming language that requires many words to express the same idea. In this section, the verbosity of several Glossy SQL queries is compared the equivalent queries in pure SQL. Words, i.e. uninterrupted alpha-numeric sequences, cover several query components such language keywords, columns names, function names and certain operators (but not all). 

The chart in Figure~\ref{fig:glossy:words:per:query} show word counts for the three example queries that were introduced in Section~\ref{sec:use:cases:solved}. It clearly shows that queries in Glossy SQL are on average an order of magnitude more concise than equivalent queries in pure SQL. The pure SQL queries correspond to the output of the Glossy compiler. While it can be argued that a human programmer could write shorter and equivalent SQL queries by hand, a significant amount of effort has been spent on the compiler to make the translated queries both readable and highly efficient. Given the vast discrepancy in words used, we can conclude that Glossy SQL is at least very concise compared to pure SQL when expressing global selection queries.

Of course, conciseness by itself is not sufficient to classify a language as ``easy''. Ultimately, a proper usability test would have to be conduced, in order to definitely conclude that Glossy SQL offers an improvement in usability over pure SQL. However, since the syntax of Glossy SQL is very similar to SQL, we believe that it is fair to use conciseness as a significant proxy for usability.

\begin{figure}[tbp]
\centering
\scalebox{0.45}{\includegraphics{figs-glossy/words-per-query.pdf}}
\caption{Word counts for the queries (Glossy and SQL). The queries correspond to the examples shown in Figures~\ref{fig:glossy:sql:rappers}, ~\ref{fig:glossy:sql:poi}, and \ref{fig:glossy:sql:placenames}.}
\label{fig:glossy:words:per:query}
\end{figure}
...
\begin{figure}[tbp]
\centering
\scalebox{0.7}{\includegraphics{figs-glossy/density-glossy.pdf}}
\caption{Spatial distribution for three types of selection (approximately 2000 airports each): (a) Random sampling. (b) Global selection w. locus constraint and linear scaling. (c) Global selection w. locus constraint but w/o spatial scaling. Note that more airports are selected in remote regions (e.g. Sahara, Siberia, Pacific Ocean) on (c) compared to (a) and (b).}
\label{fig:glossy:spatial:distribution:evaluation}
\end{figure}


The quality of a global selection can be measured in several ways, which only make sense when the selected information is visualized. Four important criteria of high-quality selection of information for data visualizations are mentioned in the literature:
\begin{enumerate}
\item How legible is the selected information?~\cite{weibel1998perspectives}. For example, do the information symbols appear ``cluttered'' or do they overlap? \label{point:legible}
\item How well does the selected information emphasize the relevant information and suppress the unimportant?~\cite{weibel1998perspectives}. For example, are the largest airports and most famous restaurants selected? \label{point:relevant}
\item How well does the selected information represent the input?~\cite{weibel1998perspectives}. For example, is the spatial distribution of the input preserved? \label{point:retain}
\item How well does the selected information fill the available drawing space?~\cite{samet2012duking}. For example, are there empty areas in visualization where data is available? \label{point:fill} 
\end{enumerate}

Since a global selection is by definition both legible (Criterium~\ref{point:legible}) and relevant (Criterium~\ref{point:relevant}) given the user-defined constraints and relevance functions, this section will focus on evaluating the quality of global selection vis-a-vis Criterium~\ref{point:retain} and Criterium~\ref{point:fill}. In a sense, these criteria represent two extreme cases of spatial distribution. At one extreme, the spatial distribution of the selected data equals exactly the spatial distribution of the input (Criterium~\ref{point:retain}). For example, random sampling has this property. At the other extreme, the selected information includes all available information, up to a limit, which essentially ignores the spatial distribution of the input (Criterium~\ref{point:fill}). For example, selecting one item per grid cell, regardless of the number of items that fall within each cell, has this property.

Figure~\ref{fig:glossy:spatial:distribution:evaluation} show that Glossy supports three modes of selection, which represent the extremes of Criterium~\ref{point:retain} and Criterium~\ref{point:fill}, as well as a balanced option in between. In the figure, three different selections have been computed from the same set of airports and each selection contains approximately 2000 of those airports (but not necessarily the same ones): 
\begin{itemize}
\item Image A shows a random sampling of the airports, which (obviously) preserves the spatial distribution of the input data, but does not fill the drawing space as much as possible.
\item Image B shows a global selection with linear scaling, which represents a balanced selection between random sampling and uniform distribution. The selection is achieved by a locus constraint together with the \texttt{SCALE LINEAR} keywords. In this selection, between 1 and 10 airports are selected, with fewer airports selected for sparse areas (e.g. Northern Canada, Northern Siberia, the Sahara Desert, and the Pacific Ocean).
\item Image C shows a global selection without spatial scaling, which results in uniform distribution. For each map tile, the ten most relevant airports are selected, regardless of how many airports exist at the given location in the input. Consequently, more airports are selected for sparsely populated areas (e.g. Greenland, the Andes, and the Pacific Ocean) compared to A and B.
\end{itemize}
%The quality of selections produced by Glossy SQL can be measured in several ways. By definition, the density and proximity of items global selection conforms with the requirements, since these are stated by constraints in the query. In contrast, preservation of spatial distribution is a quality metric that is not explicitely ensured by Glossy SQL. 

%There are two way to preserve spatial distribution in Glossy SQL: random sampling and spatial scaling. First, both Glossy SQL and pure SQL can easily compute random samples of the input by using a randomized predicate in a where clause. Second, a Glossy-specific feature, spatial scaling, allows spatial distribution to be approximately preserved for queries that use a locus constraint. The spatial scaling keywords, SCALE LINEAR and SCALE LOG, ensure that the density of items that are selected per locus has a linear or logarithm relationship to the density items in the input that fall within the locus. For example, say that a locus constraint limits the number of items per locus to $k$. If spatial scaling is used, then fewer than $k$ items may be selected for a locus even if it contains $k$ items. This happens when a locus is relatively sparsely populated compared to the most populated locus.

%In Figure~\ref{fig:glossy:spatial:distribution:evaluation}, spatial scaling has been used to preserve the spatial distribution for a global selection of airports. The figure shows that the normalized spatial distribution of the input and the global selection is the same. The spatial distribution is normalized because the global selection contains orders of magnitude fewer items than the input, namely X and Y airports respectively.
...
\chapter{Tileheat: Framework for Predictive Map Tile Materialization}
...
%\hline
\end{tabular}
\caption{Four heatmaps showing the areas in Denmark most viewed on maps from the Digital Map Supply on four consecutive days (resolution 3.2 meter/pixel). The day in the lower right corner is the day with very high load identified in Figure \ref{fig:anomaly}. Note that hot spots (dark red and black) coincide with urbanized areas in Denmark.}
\label{fig:heatmaps}
\end{figure}
...
We observe that the frequency of tile access is highly skewed; other studies have concluded the same, namely that the spatial distribution of \texttt{GET} requests follows a power law \cite{fisher07hotmap,talagala2000webimage}. We also observe that the skew increases with resolution, i.e., with  levels of the tile pyramid that contain more tiles. While all tiles in the lower scales of the map receive requests, an increasing fraction of tiles receive no requests for increasing scale. % should have a figure that shows this.
...
\end{tabular}
\caption{Result of running the dissipation algorithm on a $1.25\%$ random sample (left) of the requests plotted in Figure~\ref{fig:heatmaps}. The hot spots after dissipation (right) resemble the hot spots seen in Figure~\ref{fig:heatmaps}.}
...
The basic idea of using heat to measure popularity of data items arises naturally in applications with skewed access patterns. For example, Scheuermann et al.~\cite{scheuermann1993diskcooling} propose schemes for data placement and adaptive load balancing that "cool" disks by redistributing file fragments. In spatial services more specifically, many researchers have observed that there is a strong skew in the access frequencies of tiles, and that this skew follows a power law~\cite{fisher07hotmap,li2012prefetching,talagala2000webimage}. Li et al.~\cite{li2012prefetching} exploit skew to create a pre-fetching model of spatial tiles, with focus on predicting short-term user navigation. Their work is based on a substantial body of related short-term prediction approaches~\cite{kang2001prefetching,kim2001prefetching,lee2001prefetching}. These methods are optimized for pre-fetching  tiles seconds before they are requested by a user. The amount of pre-fetching done during a time period is thus proportional to the load. As we have observed, load and latency are proportional, which means that pre-fetching in real time does little to alleviate load peaks. To reduce the high latency caused by load peaks, we instead precompute tiles during periods of low load. To achieve this, we develop methods that do not rely on the input of individual users browsing a map in real time. 
%In contrast, we look at the problem of precomputing a large set of tiles that comprise a production-grade spatial web cache. 

As discussed in Section~\ref{sec:existing:methods}, Quinn and Gahegan~\cite{quinn2010prediction} suggest using certain classes of base objects, such as roads and coastlines, as predictors of where users will look at a map. However, as observed in Fisher's study of Microsoft Hotmap~\cite{fisher07hotmap}, real-world workloads can contradict models of rational user behavior, exclusively focused on a fixed set of rules. An example given by Fisher~\cite{fisher07hotmap} was a banner ad that caused frequent requests for ``empty'' parts of the Pacific Ocean. Based on such observations of real-life events, Fisher develops a multi-scale descriptive model that quantifies web map usage based on a heatmap. Their study supports our observation that anomalous patterns may be transient in time, but partially detectable from a training data set. In contrast to Fisher, however, we exploit this insight to propose multiple strategies to keep hit ratios on a spatial web cache high, while at the same time drastically reducing resource consumption during recomputation of tiles. 
...
The previous chapters have described the contributions of this PhD project: Glossy, CVL, and Tileheat. While the evaluation of these systems show that crucial progress has been made in important areas, the contributions are also limited in some important ways. The first two sections below will discuss some of these limitations as well as future work that could potentially overcome them. The third section presents the conclusions of this thesis.

\section{Glossy and CVL: Discussion and Future Work}
This section discusses some of the limitations as well as future work that have been planned for the Glossy and CVL contributions. The discussion will focus on: the choice of selection over other map generalization operators; the choice of tabular data over other data types; the choice of PostgreSQL over other types of data processing engines; the choice of performance evaluation over usability testing.

\subsection{Aggregation and Projection}
The Glossy and CVL systems support holistic selection of data for multiple scales, which is an important map generalization operator. However, other important operators are not supported by Glossy and CVL, such as holistic aggregation~\cite{haunert2006landcover} and holistic projection~\cite{bader2001energy} (see definitions in Section~\ref{sec:introduction:map:generalization}). There are several reasons for this, which can be reasoned about algorithmically as well as syntactically.

Algorithmically, selection in Glossy and CVL depends on a solver for the set multi-cover problem (SMCP), since the holistic selection problem corresponds to this problem. However, the project has not been able to establish that the same relationship exists between holistic aggregation, holistic projection and SMCP. One approach could be to bridge models for holistic aggregation and holistic projection with SMCP by means of a problem mapping. For example, Wollf and Haunert have developed a modeling for land-cover aggregation, which is based on a coloring problem of an adjencency graph~\cite{haunert2006landcover}. If an efficient mapping from the coloring problem to SMCP was developed, any algorithm for SMCP could be used to solve this holistic aggregation problem. A similar strategy could be used for other specific problems of holistic aggregation and projection. However, it is not clear how general such an approach would be nor how well it would work in practice.

Syntactically, the Glossy SQL and CVL languages were developed for selection. While the project has attempted several times to develop a good syntax for specification of holistic aggregation and projection, these efforts have not been successful. Perhaps the problem is structurally rooted. In selection there are only two outcomes for an object: it is either included or it is excluded. In contrast, for holistic aggregation and holistic projection there are sevaral possible outcomes for an object. For example, in land-cover aggregation an object can be merged with any of its neighbors, while in displacement an object can be displaced to several or even infinitely many positions. In this regard, holistic selection is easier to reason about, which might mean that it is easier to express in a language.

\subsection{Graphs and Semi-structured Data}
The Glossy and CVL systems have focused on processing tabular data. Other interesting data types that can be visualized include graphs, semi-structured, and unstructured data. However, Glossy and CVL are inherently tied to tabular data. As a note, there are many similarities between these different datatypes, when they are rendered as graphics. For example, the effects of scale-reduction -- such as density increase, proximity decrease and size decrease -- apply equally to all types of graphics (see Section~\ref{sec:introduction:map:generalization}).

Graphs can model many important phenomena, such as social networks, communication, and genealogy. Furthermore, it makes sense to visualize big graphs at multiple scales by the same argument that it makes sense to visualize big geographic data at multiple scales. For example, a vast social networks $G$ could meaningfully be visualized at multiple scales by showing communities at lower scales and individuals at higher scales. The transition between scales could be constrained by a graph minor~\cite{lovasz2006graph} requirement, i.e. that at scale $i$, the graph $G_i$ is graph minor of $G_j$ for all $i<j$ (similar to recursive selection in Glossy SQL). Furthermore, nodes could be weighed by a node-specific metric such as pagerank~\cite{brin1998anatomy}. However, these thoughts merely point at a direction for future work.

Semi-structured data, such as XML and JSON, is used in many contexts on the Web. For example, web services typically return data as either XML or JSON. In general, the structure of XML and JSON is nested data and may optionally have a schema. However, by a view mechanism, semi-structured data can be transformed into tabular form. Thus, it can be processed by Glossy or CVL. However, such an approach, which can be categorized as data integration~\cite{halevy2006data}, has not been attempted during this project.

\subsection{Scale-out Processing}
Scale-out distributed systems offer a system architecture where resources are scaled by adding more nodes rather than by substitution of bigger nodes for smaller nodes. Many scale-out systems, e.g. Greenplum~\cite{waas2009beyond}, Dremel~\cite{melnik2011dremel}, and Hive~\cite{thusoo2010hive}, support SQL or an SQL-like query language to process data. In this project, the Glossy SQL extension and CVL domain-specific language have both been implemented by compiling those languages to queries in the PostgreSQL dialect. This limits the systems that can be used to process multi-scale selection to those that accept this SQL dialect. Consequently, Glossy and CVL cannot run on top of Hadoop or most parallel database systems. The PostgreSQL system was chosen as the back-end for Glossy and CVL because it is mature system and because the emphasis in this project was on correctness and intermediate scalability, rather than big scalability. The choice of PostgreSQL over more scalable database systems tailored for analytics has most likely negatively impacted the scalability (data volume) results presented in the Glossy and CVL chapters. 

Greenplum is an example of a parallel database that supports a version of the PostgreSQL dialect. Furthermore, Greenplum is tailored for analytics processing, which would in principle makes it a good fit for Glossy and CVL workloads. Therefore, it would have made sense to implement Glossy and CVL on top of Greenplum. 

Hive is a data warehouse system that runs on top of the popular Hadoop cluster computing system. Hive supports queries in HiveQL, which is a SQL-like query language. Therefore, it would not have been a far reach to target Hive. However, this PhD project has not developed a compiler that targets HiveQL, and therefore Glossy and CVL do not run on Hive. Other SQL-based systems for Hadoop are Apache Drill (ANSI SQL) or Apache Spark (HiveQL), neither of which support the PostgreSQL dialect.

Pigeon~\cite{eldawy2014pigeon} is a spatial data processing dialect of Pig, which is a non-SQL alternative for writting queries in Hadoop. The Pigeon language was implemented for the Spatial Hadoop~\cite{eldawy2014spatialhadoop} spatial data processing system to allow highly scalable geospatial data processing. While this PhD project has not developed a compiler that targets Pigeon, it would make sense to do so. 

In summary, Glossy and CVL will not run on any Hadoop-based SQL engine that we know of, since they do not support the PostgreSQL dialect of SQL. Glossy and CVL might run on Greenplum with some light modification, but this has not been tested.

\subsection{Usability testing}
Usability testing can measure the effectiveness with which real humans utilize a system. The Glossy SQL and CVL languages are designed for non-expert visualization designers, who are familiar with writing queries in SQL. A usability test would have strengthened the argument that Glossy SQL and CVL are time-saving technologies for people who want to create multi-scale, selection-based data visualizations. However, because of time and resource limitations, no formal usability tests has been conducted for Glossy SQL and CVL. Instead, informal testing and implementation of a number of realistic use cases have been shown. The lack of formal qualitative verification is a clear limitation of the results presented in this thesis.

\section{Tileheat: Discussion and Future Work}
This section discusses some of the limitations as well as future work that have been planned for the Tileheat contribution. The discussion will focus on the choice of workload for testing, namely the workload for a popular background map that is provided by the Danish Geodata Agency.

% Tileheat discussion
Tileheat is a system for predicting the sections of a map that will be most requested by the users in an uncoming time period. It works by processing the access log for the $k$ past time periods and applying a smoothing and trend-detecting algorithm. The assumptions are that hot sets change only gradually over time and that most users will request the same content. There are some limitations that follow from this approach, which will be discussed briefly below.

\subsection{Highly Dynamic Workloads}
Tileheat has been tested with the workload of a background map. While this is a realistic workload for an algorithm such as Tileheat, our experiments show that the request pattern in this workload is relatively stable over time. That is, the map sections that are most requested tend to coincide well with densely populated areas (by visual inspection of the heatmaps). Tileheat has not been evaluated for maps that exhibit more dynamic request patterns, i.e. where the focal point of hot spots changes frequently. We expect Tileheat to perform relatively bad in this case. However, this would have to be evaluated experimentally to be sure. Since Tileheat uses exponential smoothing with trend detection, there is a possibility that even a frequent shift in the area that is most requested on a map could be quickly detected, but only if a ``ramp-up'' pattern is present in the log. In summary, Tileheat would need to be tested with a greater variety of workloads in order to determine its suitability for highly dynamic workloads.

\section{Conclusions}

The background for this thesis has been the impact that the Web and cloud computing has made on the time-honored fields of multi-scale map and data visualization. We have argued that new tools are needed, which support the design, processing, and distribution of map and data visualizations in this new situation. The new tools should be able to handle massive amounts of data. Furthermore, a new breed of non-expert designers should be able to operate these tools. Finally, the new tools should ensure low-cost, low-latency, highly-available, and scalable access to maps and data visualizations -- for large numbers of concurrent users.

An important map generalization technique is selection of data (Subject A). This thesis contributes new declarative tools that enable non-expert designers to select data for multi-scale maps and data visualizations over large quantities of data. 

Batch-oriented prediction is an important technique for pre-materializing sections of a map ahead of peak load (Subject B). This thesis contributes a new algorithm and framework for prediction of map tile requests, which bases prediction on the analysis of recent requests. 

Below, we will review the goals for these contributions and summarize the results that were obtained.


\subsection{Selection: Goals and Results}

This thesis makes two contributions on the topic of selection (Subject A): The language contribution (Contribution 1) consists of two languages, Glossy SQL and CVL, which allow users to declaratively express multi-scale selection for geographic maps and data visualizations. The compiler contribution (Contribution 2) consists of two compilers for these declarative languages. The compilers translate Glossy SQL and CVL statements into SQL statements for in-database execution of selection for multi-scale maps and data visualizations.

The goals for the declarative languages (Contribution 1) were to:

\begin{itemize}
\item Formalize a model for holistic, multi-scale selection that subsumes all previously proposed models.
\item Base the semantics of the language on a well-understood optimization problem for which efficient algorithms exist.
\item Design a syntax that allow programmers to concisely express complex selections in this model, including user-defined constraints and objectives.
\end{itemize}

These goals were successfully reached. First, we designed the selection model to support three important modes of selection, i.e. selection with unary, pair, and locus constraints, as well as any combination of these. Second, we based the semantics of the language on the well-known set multi-cover problem; while this problem is NP-hard, efficient approximation algorithms are known. Third, we demonstrated, via a set of realistic use case implementations, that even quite complex selections can be expressed in these languages with no more than 10-20 lines of code.

The goals for the compilers (Contribution 2) were to:

\begin{itemize}
\item Generate SQL code that can correctly select data entirely inside an existing database system and for all queries expressed in the declarative languages (Contribution 1).
\item Generate SQL code that efficiently exploits existing indexes on the data.
\item Be able to process, in a reasonable amount of time, selections for large quantities of data (millions of objects) stored in a relational database system.
\end{itemize}

These goals were successfully or partially reached. First, we implemented a range of realistic and varied use cases in the declarative languages and evaluated them successfully. Second, we showed that the translated queries will successfully exploit any existing indexes on the data. Third, we showed that the approach scales up to databases of half a million objects. While we had hoped for better scalability, we have given directions for future work that can potentially achieve the scalability goal completely.

\subsection{Map Request Predition: Goals and Results}

This thesis makes one contribution on the topic of map request prediction (Subject B): Tileheat (Contribution 3) is an algorithm and framework that predicts map requests. To predict requests, it applies exponential smoothing and a novel dissipation technique to the analysis of request logs. The usefulness of this contribution is to substantially save computation during map rendering.

The goals for Tileheat (Contribution 3) were to:

\begin{itemize}
\item Develop and tune an algorithm that could predict a small hot set of map requests that covers all or nearly all near-future requests; the algorithm should use a log of previous requests as training data.
\item Develop the algorithm to be able to predict previously unseen requests.
\end{itemize}

These goals were successfully reached. First, Tileheat is able to predict 1.5 million requests out of 10 million possible that cover 95\% of near-future requests; this result was obtained for a real-life request log from the Danish Geodata Agency. Second, Tileheat is able to predict requests that have not previously been seen in the log, thanks to a novel heat-dissipation technique; the hypothesis that motivated this technique is that the likelihood that a map section will be requested, is a highly correlated to the likelihood that neighboring map sections will be requested.
