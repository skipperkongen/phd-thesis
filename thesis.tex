...
Data Visualization Management Systems (DVMS) have been proposed as a catch-all solution for data-driven visualizations on the web today~\cite{wu2014case, fekete2012dmvis}. The API of a fully-fledged DVMS must included facilities for both data management, visualization design, and visualization sharing~\cite{foo}. A DVMS must be scalable both in the size of data~\cite{lins2013nanocubes, Im2013visreduce, battle2013scalar, liu2013immens} and audience~\cite{viegas2007manyeyes}. Additionally, it is crucial that designers can craft visualizations using an interface that is both intuitive, efficient (in terms of wall-clock time) and expressive. Examples of systems in production include Google Fusion Tables, which allows data managers, visualization designers and audiences to interface with a unified system for map visualizations~\cite{google2014fusiontables}, and CartoDB, which adds support for spatio-temporal visualizations~\cite{cartodb2014cartodb}.
