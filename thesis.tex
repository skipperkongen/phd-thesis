...
Web and mobile mapping applications typically combine emerging thematic foreground data with the spatial context of existing background map, which is a process informally known as a ``geographic mash-up'' (see Figure~\ref{fig:introduction:background:mashups}}). % Why are they cool? 
Mash-ups are important because they combine new and relevant spatial information with existing high-quality background maps that provide the spatial context in which the new information must be understood~\cite{bereuter2012algorithms} % Why is it hard?
