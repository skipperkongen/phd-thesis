...
% NOTFINISHED
Section~\cite{introduction:problem:statement} presented the case for a declarative langauge that map designers can use to express multi-scale selection for map visualizations. This thesis contributes an extension to SQL which allows map designers to concisely and intuitively express multi-scale selection for maps in a familiar and productive programming style.

During the design process for the SQL language extension, some key design objectives where identified: An extension to SQL for multi-scale selection must (1) allow map designers to express scale-dependent binary and set-based constraints that must be satisfied by a feasible selection, (2) allow map designers to define how record importance should be evaluated for each scale of a map, (3) allow map designers to freely define the notion of a set of scales in a map, (4) allow multi-scale selection in both the star and ladder paradigm~\cite{foerster2010challenges},  (5) allow programs to reference data stored in relational database tables.
