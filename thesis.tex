...
% NOTFINISHED
Section~\cite{introduction:problem:statement} posits the need for declarative languages to express multi-scale selection in map visualizations. This thesis contributes an extension to SQL which addresses this need, not only for map visualizations, but for a substantially larger class of data visualizations, which includes zoomable timelines. Moreover, an SQL extension allows programmers to concisely and intuitively express multi-scale selection in a familiar and productive programming style.

During the design process, some key design objectives where identified: An extension to SQL for multi-scale selection must (1) allow programmers to express scale-dependent binary and set-based constraints that should be satisfied by a valid selection, (2) allow programmers to define how record importance should be evaluated for each scale, (3) allow programmers to freely define a set of tuples that generalize the notion of scales, (4) allow multi-scale selection in both the star and ladder paradigm~\cite{foerster2010challenges},  (5) allow programmers to reference data tables in relational databases.
