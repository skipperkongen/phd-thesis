...
The generalization process can be modelled in many different ways~\cite{harrie2007modelling}. In \emph{constraint-based} modelling, \emph{constraints} act as the descriminator between feasible and infeasible maps. Constraints may define the limits of many different properties of the map, such as symbol size, complexity and density~\cite{weibel1999generalising}. For example, \emph{proximity constraints} define the limit for how close geographic objects symbols may appear on a map in order to be considered \emph{distinct}~\cite{nutanong2012multiresolution}; \emph{topological constraints} restrict which segments of a river network can be pruned such that the river remains connecting to itself and its outlet~\cite{horton1945erosional,shreve1966statistical,strahler1957quantitative}.
