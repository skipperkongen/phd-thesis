...
There are three crucial and orthogonal questions that relate to visualization of data on the web. First, how should we \emph{express} the transformation of data into compelling visual representations? Second, how should we \emph{process} millions of records for scalable data visualization? Third, how should we \emph{serve} data visualizations to millions of demanding online users? Answering each of these questions will require us to investigate areas of computer science beyond the area of pure data visualization. For example, programming languages have addressed the issue of expressing data transformations~\cite{sql}; databases have addressed the issue of processing millions of records~\cite{wow-so-many-to-choose-from}; distributed systems have addressed the issue of serving data to millions of users~\cite{wow-so-many-to-choose-from}. It is crucial that we can pull together the relevant know-how from these diverse fields in order to further the state of data visualization.
...
Data visualizations today must be scalable both in the size of data~\cite{lins2013nanocubes, Im2013visreduce, battle2013scalar, liu2013immens} and audience~\cite{viegas2007manyeyes}. Additionally, it is crucial that designers have access to intuitive and expressive authoring interfaces~\cite{wilkinson2006grammar}. Specifically, authoring interfaces must be effective both in terms of system and human resources~\cite{wu2014case, fekete2012dmvis}.
...
\subsection{Scalable Data Abstraction} 
...
\chapter{State-of-the-Art in Data Table Visualization}
...
Sections (tentatives). For each section, define the field, goals, problems etc:

\begin{itemize}
\item Introduction: principes, terminology (e.g. data abstraction?), general view of process
\item Data Visualization Management Systems
\item Map Generalization
\item Visual Analytics
\item Information Visualization
\end{itemize}

\note{Where does information cartography, data cubes, polaris, etc. fit in?}

How have people reasoned about 
...
\subsection{Multi-scale Visualization}
