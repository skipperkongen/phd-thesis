...
\begin{itemize}
% EXPRESSING
\item \textbf{Expressing Multi-scale Selection}: Designers, specifically those who are not GIS-experts, must be able to express compelling and complex multi-scale map visualizations over large geographical datasets that are stored in a relational database management systems (RDBMS).

% PROCESSING
\item \textbf{Scalable Processing}: Map visualization systems must be able to scalably process a specification for multi-scale selection of geographical data.

% SERVING
\item \textbf{Predictive Map Serving}: Map visualization systems must be able to serve tiled multi-scale maps to large web audiences who expect 24 x 7 availability and acceptable latency~\cite{gaffuri2012toward, pilchin2007dynamo, beaver2010haystack}. In particular, the advantages of scalable distribution systems, such as content delivery networks (CDN), must be cost-efficiently leveraged~\cite{beaver2010haystack, datacentre}. However, since the number of tiles grow expenentially in the number of supported scales, fully materializing a multi-scale map into tiles and storing it in a CDN is costly. 
