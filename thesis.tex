...
Greenplum is an example of a parallel database that supports a version of the PostgreSQL dialect. Furthermore, Greenplum is tailored for analytics processing, which would in principle makes it a good fit for Glossy and CVL workloads. This project could therefore have chosen to evaluate Glossy and CVL on top of Greenplum. However, for technical reasons the PostgreSQL relational database system was used. This choice of database system has probably limited the scalability results presented in the Glossy and CVL chapters. 

Hive is a data warehouse system that supports queries in HiveQL, which is a SQL-like query language. This PhD project has not developed a compiler that targets HiveQL, and therefore Glossy and CVL does not run on Hive. Glossy and CVL also does not run on any other Hadoop-based data processing system, such as Apache Drill (ANSI SQL) or Apache Spark (HiveQL). However, the effort needed to target these platforms via HiveQL or ANSI SQL is probably moderate. 

Pigeon~\cite{eldawy2014pigeon} is a spatial data processing dialect of Pig, which is a non-SQL query language for Hadoop. It has been implemented for the Spatial Hadoop~\cite{eldawy2014spatialhadoop} spatial data processing system. This PhD project has not developed a compiler that targets HiveQL, but doing so might offer superior scalability for map generalization queries.
