...
Successful mapping platforms must ensure high availability and performance, which should be stated in service level agreements (SLA) and monitored continuously. The cost of ensuring these goals grows with the number of concurrent users and data volume, variety, velocity, veracity, validity and volatility (``the Six Vs of Big Data''). 

A complex information processing pipeline is activated in order to materialize a useful map, e.g. on someones mobile device. As a consequence, we need to adopt some sensible, simplifying assumptions in order to scope our analysis of the map problem. We adopt the view that mapping consists of two tasks: \emph{map production} and \emph{map consumption}. As we will see, the line that divides these tasks is fuzzy. However, the semantics of the two tasks is defined broadly. Map production is synonymous with creating good maps -- without implying actual usage. Map consumption is synonymous with materializing maps on computer screens -- without implying that the maps are well designed. 
