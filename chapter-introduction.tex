...
% data/visual abstraction versus model/cartographic generalization

\section{Overview of State-of-the-art Online Maps}
% SWYGS
% Define online maps

% SI
% set maps (VIZ) use case appart from spatial OLAP, OLTP. Argue for end of GIS.
% \cite{stonebraker2005onesizefitsall}
% \cite{stonebraker2007endofanera}

% who defines the state-of-the-art geospatial visualization? big (Google, Apple) and small (OpenScienceMap, OpenStreetMap, MapBox)
% \cite{sarma2012fusiontables}, \cite{nutanong2012multiresolution} be better with Apple cite
% \cite{schmid2013opensciencemap}, \cite{openstreetmap), \cite{mapbox}

\subsection{Automated Generalization}
% describe automated generalization (model and cartographic), cite grundreich
% mention related work and nomenclature (data and visual abstraction)
% mention focus my on multi-scale data abstraction
% talk about preprocessing versus live generalization
% briefly outline some methods used
...
% motivation: client-side rendering, scalable
% challenge: bounded content => content selection/aggregation, i.e. data abstraction.
...
% challenging to keep up to date. may brute force via map reduce cite FriendsOrFoes, LessonsLearned
