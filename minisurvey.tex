...
% generalization
Although much has changed, the fundamental challenges posed by map creation remain largely the same. A geographical map will represent something as big as a planet or a country, on something as small as a sheet of paper or a computer screen. Consequently, every person creating a useful map must choose what information to include and what to leave out. That is, if the map is to remain legible to the human eye. The map creator must eliminate information that is not essential to a map, and reduce the complexity of the visual expression. These are the core tasks of what cartographers call generalization. The other major tasks of cartographer are map design, map projection and map editing.

% challenge today
Today, the amount of spatial data available to map creators dwarfs the amounts that were available to map creators just a few decades ago. Furthermore, the frequency at which new maps must be created is higher than ever before, driven largely by the rise of social media and online news platforms. This has motivated many attempts to automate the generalization process. While research projects have achieved partial success in some areas, a definitive and general solution to automated map generalization has not been found yet. 

% remainder
In the remainder of this chapter we survey the field of automated map generalization in general, and recent contributions in particular.
