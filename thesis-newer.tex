...
\textbf{Why are Graphics are so Powerful?}
In middle of the 19th century, Florence Nightingale worried that her statistical findings, if presented in table form, would put Queen Victoria to sleep rather than inform about why mortality had suddently dropped in the Crimean War. Instead, she presented her statistical findings in the form of a coxcomb diagram ``to affect thro' the eyes what we fail to convey to the public through their word-proof ears''. The key information that Nightingale wanted to convey to the Queen was that people were dying, not from their wounds, but from diseases. By looking at her coxcomb diagram, this important information could be comprehended in an instant.

This work is primarily about geographic maps, not about coxcomb diagrams. However, similar to a coxcomb diagram, geographic maps exploit that the visual sense is keen in humans. The reasons that maps can be so effectivily comprehended by humans was first formally investigated by Jacques Bertin in his seminal work on semiology~\cite{bertin1967semiologie}. While other forms of information, such as data tables, demand that information be held first in human memory before it can be processed by the brain, maps are graphics that allow information to be understood at a glance. The reason that information on maps can be rapidly comprehended is that information is represented in the three dimensions that are most keenly perceived by humans: x, y and color! As such, there are deeply biological reason for why maps are such a powerful means of communication.

%But why are maps fundamentally difficult to produce?  Maps are fundamentally difficult to produce because there is a mismatch between the magnitude of the map medium (e.g. a piece of paper) and the magnitude of the depicted object (e.g. the entire World). In the case of geographic maps, the size of the depicted object is absolute and intrinsically spatial. In the case where information to be displayed is more abstract, the ``magnitude'' may be measured in number of records. However, since all information on a map must be represented by legible symbols, it is not important whether the information is geographic, temporal, financial, or something else. What matters is that only a finite number of legible symbols fit on a map.

%expressed in the three dimensions that humans perceive particularly well, graphics have only three dimensions and the consequences of this limit are important~\cite{bertin1967semiologie}. 

%\minisec{The Fundamental Challenge of Maps}
%To understand the consequences of expressing information in these three dimensions, it is necessary to understand the concept of scale. In the context of maps, scale is the ratio between distance as measured on the map and the corresponding distance on the ground. For example, if a road is one centimeter long on a 1:500.000 map, the road is 500 meters long in reality. The terms ``small scale'' and ``large scale'' are relative to typical scales for a given domain, and are as such quite ill defined. For example, a large scale virus map magnifies reality one hundred thousand times. Conversely, a large scale city map diminishes reality five hundred thousand times, which is however less than the diminishing factor of a world map.

%When trying to fit the entire World onto a piece of paper, the World must necessarily be scaled down. Representing all the information in the World by tiny symbols will not work, as symbols will quickly become imperceptible to the human eye. Conversely, keeping the size of symbols at legible size does not solve the problem either, since an increasing number of symbols will soon result in symbol clutter. Therefore, as the amount of information grows, each symbol must either represent more information (i.e. aggregation), or information and thus symbols must be omitted (i.e. selection).

%While aggregation is in many cases straight-forward and meaningful, it may be the case that a one-to-one correspondance between information objects and symbols in the map is more desirable. For example, on a map that helps people explore individual information objects, such as restaurants, it is suboptimal to aggregate several objects into a single symbol. A better approach is to select the most relevant subset of the information (in this case restaurants) that fits on the map. However, it is not trivial to decide which selection of restaurants is the most relevant and also result in the most legible map.


\note{There many tools to compute aggregation, not many to compute holistic selection, and those that exist are not sufficient because users need to customize selection via constraints and objectives.}

\minisec{The Entangled History of Maps, Politics and Technology}
Humans have created maps since the Stone Age and through to the present day~\cite{friendly2008milestones}. For example, scholars have dated a town map of Catal Hy{\"u}k, near present-day Konya in Turkey, to around 6200 BC. This makes it one of the earliest maps that we know of today. The many surviving maps bear testiment to a huge diversity of use cases for maps. An important use case since ancient times is the registration of land ownership in a cadastre. For example, a Mesopotamian clay tablet, which scholars have dated to around 2400 BC, depicts a land parcel in-between two rivers together with the presumed owner's name, ``Azala''. 

Thematic map, is intimately tied to the rise of statistics (literally ``information about states''). In the 18th and 19th century the combined development of statistics and thematic maps allowed rulers to comprehend statistical information as it pertains to various geographic regions.

The use cases and demand for maps has always been tied to technological innovations. For example, the use of nautical charts in maritime navigation proliferated during the 12th century Song Dynasty, who were probably the first to use a magnetic compass for navigation at sea. Nautical charts came into even greater demand following the invention of the Mercator projection by Gerardus Mercator in 1569.
