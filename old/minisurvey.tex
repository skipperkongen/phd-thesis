...
% challenge today
Today, huge amounts of spatial data is available online. Furthermore, the frequency at which new maps need to be created is higher than ever before. Maps are used all over the Internet, e.g. on social media and online news sites. The growth in these platforms has been explosive in the last decade, and manual cartography cannot keep up with this pace. This has motivated many attempts to automate the generalization process, i.e. develop algorithms for automatically drawing a map from spatial data. Researchers have achieved some degree of success with developing effective and efficient methods, but mostly for specialized domains. The definitive and fully general solution to automated map generalization has yet to be found.
% remainder
The rest of this chapter is a survey of automated map generalization, with an emphasis on recent contributions to the field and the key challenges that remain.
