...
%Digital map visualization studies how the geography of the Earth can be represented on a computer screen. It is fundamentally challenging to represent something as enormous as a planet on something as small as a computer screen~\cite{dillard1974pilgrim}, therefore,  (TODO: find pages quoted by Weibel and Dutton and add to bibtex). While the three dimensions of an  image make the visual perception our most powerful perceptive system, images are finite and have only three dimensions and the consequences of these limits are important~\cite{bertin1973semiologie}.
...
%The funnel that motivates our contributions.
%\begin{itemize}
%\item Maps are really cool
%\item Requirements for web and mobile maps: PoCID, iOS6
%\item Only maps of large data need information abstraction, whether area partitions (haunert, gap) or points (pia, da sarma). in typical map application no need for complex information abstraction, since the hard maps (background maps) already exist.
%\item On-the-fly methods do not cost-effectively scale to massive audiences. ARGUE THIS POINT WELL
%\item scalable batch-oriented data abstraction methods, which sacrifice consistency for query time performance, are the only approach that will cost-effectively scale to massive audiences (e.g. vector tiles); mention content delivery networks, data partionining and replication.
%\item However, current batch-oriented methods either too automatic or too complex for map designers. ARGUE THIS POINT WELL
%\item New paragraph:
%\item From user perspective, only data that is requested needs to be consistent
%\item Reliable prediction can enable us to batch-process only the data that is requested in the future, which will (a) decrease the window of inconsistency, and (b) would be cheaper as well.
%\end{itemize}
...
\note{Generalization encompasses a reduction of the complexity in a map, emphasizing the essential while suppressing the unimportant, maintaining logical and unambiguous relations between map objects, and preserving aesthetic quality. The main objective then is to create maps of high graphical clarity, so that the map image is easily perceived and the message that the map intends to deliver can be readily understood.}

Map generalization has been studied for a long time. However, the web has dramatically transformed the activity of map visualization since Roger Tomlinson kick-started the digital age of maps with the invention of geographic information systems (GIS) in the 1960's~\cite{ucgis2014tomlinson}. Since then the:

\begin{enumerate}
\item map designers have changed: from a few GIS specialists to many data enthusiasts~\cite{hanrahan2012enthusiast};
\item geographic datasets have changed: from moderately-sized and mostly curated geographic datasets inside organizations to big, integrated, and often user-contributed geographic datasets on the web~\cite{gst2014digitalmapsupply, openstreetmap, zooniverse2014oldweather}:
\item map audiences have changed: from a few professional, to millions of people on the web who interact with mobile maps 24 x 7 and on the go;
\item map purposes have changed: from being mostly a professional or educational activity, to being simultaneously a professional, educational, personal and social activity on the web. 
\end{enumerate}



Web and mobile mapping applications typically display thematic foreground data -- for example geocoded events or locations -- on top of an existing background map~\cite{bereuter2012algorithms}. This process is informally known as a \emph{geographic mash-up} (see Figure~\ref{fig:introduction:background:mashups}}). % Why is it cool? 
Geographic mash-ups are important because they communicate spatial narratives over news-worthy or otherwise interesting spatial information sources, which are combined with existing high-quality background maps that provide the familiar geographic context in which the spatial narrative can be understood. % Why is it hard?
While physical and topographic background maps are generally of very high quality, the fruit of time-intensive expert labor, emerging thematic datasets present map designers with an acute and unsatisfied need for data abstraction.  % Why is the state-of-the-art not good enough?
...
State-of-the-art batch-oriented map generalization methods are suboptimal for abstraction of thematic foreground data. % Why? 
Currently, batch-oriented approaches belong to two categories, either fully automatic systems~\cite{sarma2012fusiontables} or very advanced and domain-oriented systems~\cite{lamy1999application, ware2003generalization_sa}. % Yeah, what is wrong with them?

While data integration is a fundamentally hard problem, outstanding work in the field has made the problem easier to address than ever~\cite{marcosthesis}. For example, relational databases~\cite{importantdatabasedataintegrationwork}. As a result, bla bla bla~\cite{citeplaceswheretheyareused}. Relational databases combine a flexible data model with a powerful query language~\cite{codd1970relational}, namely the relational data model and relational algebra. Furthermore, decades of work in query optimization and processing has yielded highly efficient execution of relational algebra queries over large datasets~\cite{graefe1993query,kossmann2000survey}. Logically, recent work in data visualization increasingly propose that visualization pipelines should be integrated with relational databases in order to leverate the performance the performance of the query processing~\cite{wu2014case}. However there is currently a lack of similar database integrated methodologies for map visualization. While there are fundamental differences between data visualization and map visualization, the common ground is that both deal with the visual representation of massive datasets. these differences do not imply that a similar technique would have similar benefits across the two domains. % why is map visualization different?

While distributed systems have progressed ime

% Where does Reactive tree, BLG-tree fit in? wang2014spatialsampling is about estimating aggregates
